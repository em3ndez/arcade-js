...
6. [The decompiler pipeline](06-decompiler-pipeline.md) — the current method for rewriting the proven translation into idiomatic JavaScript, one memory-equivalence-gated routine at a time.
7. [Porting a new game](07-porting-a-new-game.md) — the CPU / board / game layering in practice.

Read them in order; doc 6 is the current rewrite/decompile method (it supersedes the earlier
strict, cycle-exact optimization pass, retained now only as the shipped `optimized/` layer).
