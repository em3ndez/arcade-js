...
import { AddressSpace } from "../../boards/pooyan/memory.js";
import { Io, NotImplemented } from "../../boards/pooyan/io.js";
import {
  SCREEN_W,
  SCREEN_H,
  decodeGraphics,
...
/**
 * Zero, the boundary itself: MAME's screen_vblank fires from vblank_begin, the same function that
 * calls frame_update, so the frame origin IS the vblank point. Derived from the driver, not measured.
 */
export const NMI_CYCLE_IN_FRAME = 0;

/** 50688 / 264 lines, from set_raw(..., 264, ...) -- NOT timeplt's 200 (which is 51200/256). */
export const CYCLES_PER_SCANLINE = 192;

/**
 * From our origin (vpos 240) to the first visible line (vpos 16, wrapping at vtotal 264), so the
 * visible lines end exactly ON the next boundary: 264 - 240 + 16 = 40. NOT vbend, NOT timeplt's 32
 * (which wraps at 256).
 */
...
export class Machine {
  constructor(rom, routines, opts = {}) {
    ...
    // (rom, opts) -- when the 2nd arg is not a routines Map, treat it as opts and build routines
    // here (registry + opts.overrides), so web/worker.js can call `new Machine(rom, {...})`.
