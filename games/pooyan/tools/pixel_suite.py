...
the next untranslated gap (0x6bee) around frame 1447. This gate validates the byte-exact PREFIX_FRAMES
...
# is complete) it runs clean to frame ~1447 and only then reaches the next untranslated gap 0x6bee. So this
...
PREFIX_FRAMES = 170
FULL_BOOT_GAP = 0x6bee         # where the FULL boot now stops (documentation; the prefix gate expects NO gap)
