...
the next untranslated gap (0x15d1) around frame 1357. This gate validates the byte-exact PREFIX_FRAMES
...
# is complete) it runs clean to frame ~1357 and only then reaches the next untranslated gap 0x15d1. So this
...
PREFIX_FRAMES = 170
FULL_BOOT_GAP = 0x15d1         # where the FULL boot now stops (documentation; the prefix gate expects NO gap)
