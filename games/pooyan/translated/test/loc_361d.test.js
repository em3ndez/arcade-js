...
/**
 * Drafter test for translated loc_361d (ROM 0x361d, Pooyan) -- the (ix+0x08) bit0-clear guard.
 * bit 0,(ix+0x08); ret z (bit clear); else tail jp loc_3775. Reached only via jr c from loc_362d,
 * so the guard runs in loc_362d's caller frame: ret z returns to that seated caller, and the tail
 ...
 *
 * Path RET (bit0 clear): bit + ret z, pcSeq [0x3621, CALLER_RET], T=31. Path TAIL (bit0 set):
 * bit + ret-z-not-taken + jp, pcSeq [0x3621, 0x3622, 0x3775], T=35, tail into loc_3775.
 * No push16 in this leaf, so the positive control is the T-state mutation tooth (bit 20T->8T).
 *
 * Run: node --test games/pooyan/translated/test/loc_361d.test.js
 */
...
function makeMachine() {
  ...
  return {
    regs, mem, ram, calls: [], tstates: 0, pc: 0x361d, pcSeq: [],
...
test("loc_361d RET: (ix+0x08) bit0 clear -> ret z to the seated caller", () => {
  const m = makeMachine();
  seatCaller(m);
  m.regs.ix = 0x8b00;
...
test("loc_361d TAIL: (ix+0x08) bit0 set -> jp loc_3775", () => {
  ...
  assert.deepEqual(m.pcSeq, [0x3621, 0x3622, 0x3775]);
  assert.equal(m.pc, 0x3775, "tail jp lands on loc_3775");
  assert.deepEqual(m.calls, [0x3775]);
  assert.equal(m.regs.sp, BASE, "tail callee ret pops the seated caller -> baseline");
});

test("loc_361d MUTATION: bit mis-charged 8T (not 20T) is caught", () => {
  const m = makeMachine();
  const realStep = m.step.bind(m);
  m.step = (n, c) => realStep(n, n === 0x3621 ? 8 : c); // bit b,(ix+d) is 20T, not 8T
  seatCaller(m);
  m.regs.ix = 0x8b00;
