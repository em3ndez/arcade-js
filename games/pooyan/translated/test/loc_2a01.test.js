...
// Equivalence test for loc_2a01. Run: node --test .../loc_2a01.test.js
...
test("loc_2a01 sets flip bit, paints tiles, checksum guard re-enters loc_2c58; 987 T", () => {
  const m = mk();
  loc_2a01(m);
  assert.equal(m.tstates, 987, "T"); assert.deepEqual(m.calls, [0x2c58]);
  assert.equal(m.mem.read8(0x875a), 0xbc, "tile painted");
  assert.equal(m.mem.read8(0x8a90) & 0x80, 0x80, "(ix+0x10) bit7 set");
});
test("loc_2a01 MUTATION: set 7,(ix+0x10) mis-charged 19T (not 23T)", () => {
  const m = mk();
  const r = m.step.bind(m); m.step = (n, c) => r(n, n === 0x2a09 ? 19 : c);
  loc_2a01(m); assert.notEqual(m.tstates, 987);
});