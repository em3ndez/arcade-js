...
// Equivalence test for loc_324d. Run: node --test .../loc_324d.test.js
...
test("loc_324d (ix+0) < 0x40 -> ret c; 37 T", () => {
  const m = mk(); m.regs.ix = 0x8ae0; m.mem.write8(0x8ae0, 0x20);
  loc_324d(m); assert.equal(m.tstates, 37, "T"); assert.deepEqual(m.calls, []);
});
test("loc_324d (ix+0) >= 0x40 -> tick 0x8c page counter; 81 T", () => {
  const m = mk(); m.regs.ix = 0x8ae0; m.mem.write8(0x8ae0, 0x40); m.mem.write8(0x8c45, 0x50); m.mem.write8(0x89e5, 0);
  loc_324d(m); assert.equal(m.tstates, 81, "T"); assert.equal(m.mem.read8(0x8c45), 0x10, "counter -= 0x40");
});
test("loc_324d MUTATION: sub 0x40 mis-charged 4T (not 7T)", () => {
  const m = mk(); m.regs.ix = 0x8ae0; m.mem.write8(0x8ae0, 0x40); m.mem.write8(0x8c45, 0x50);
  const r = m.step.bind(m); m.step = (n, c) => r(n, n === 0x325b ? 4 : c);
  loc_324d(m); assert.notEqual(m.tstates, 81);
});