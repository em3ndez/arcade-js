...
// Equivalence test for loc_24db. Run: node --test .../loc_24db.test.js
...
test("loc_24db expiry -> Y+=4, (ix+6)-=8, tile 0x1a, delay 0x30, advance; 189 T", () => {
  const m = mk(); m.mem.write8(0x8a91, 0x01); m.mem.write8(0x8a84, 0x40); m.mem.write8(0x8a86, 0x30);
  loc_24db(m);
  assert.equal(m.tstates, 189, "T"); assert.equal(m.pc, CR);
  assert.equal(m.mem.read8(0x8a84), 0x44); assert.equal(m.mem.read8(0x8a86), 0x28);
  assert.equal(m.mem.read8(0x8a8f), 0x1a, "tile"); assert.equal(m.mem.read8(0x8a91), 0x30, "long delay");
});
test("loc_24db not expired -> ret nz", () => {
  const m = mk(); m.mem.write8(0x8a91, 0x04); loc_24db(m); assert.equal(m.pc, CR);
});
test("loc_24db MUTATION: ld (ix+0x0f),0x1a mis-charged 13T (not 19T)", () => {
  const m = mk(); m.mem.write8(0x8a91, 0x01);
  const r = m.step.bind(m); m.step = (n, c) => r(n, n === 0x24f3 ? 13 : c);
  loc_24db(m); assert.notEqual(m.tstates, 189);
});