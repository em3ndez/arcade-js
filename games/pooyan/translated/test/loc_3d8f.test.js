...
// Equivalence tests for loc_3d8f. Flat-RAM mock (real Regs); pattern-A/tail delegations use a stub
// that records the target and runs m.ret(). Run: node --test games/pooyan/translated/test/loc_3d8f.test.js
...
test("loc_3d8f timer not elapsed -> ret nz; 61 T", () => {
...
test("loc_3d8f timer elapses -> tail jp 0x3553; 75 T", () => {
  const m = makeMachine(); seat(m); m.mem.write8(0x8c11, 0x01);
  loc_3d8f(m);
  assert.equal(m.tstates, 75); assert.deepEqual(m.calls, [0x4006, 0x3553]); assert.equal(m.pc, CALLER_RET);
});
test("loc_3d8f MUTATION: dec (ix+0x11) mis-charged 19T (not 23T)", () => {
  const m = makeMachine(); seat(m); m.mem.write8(0x8c11, 0x02);
  const r = m.step.bind(m); m.step = (n, c) => r(n, n === 0x3d95 ? 19 : c);
  loc_3d8f(m); assert.equal(61 - m.tstates, 4);
});
