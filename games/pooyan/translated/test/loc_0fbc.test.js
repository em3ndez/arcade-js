...
/**
 * Drafter test for translated loc_0fbc (ROM 0x0fbc, Pooyan) -- enqueue the four tiles
 * 0x28,0x15,0x16,0x17 into the text ring via loc_0ea2 (three CALLs + a tail jp). The entry
 * `jp 0x0fc3` skips the alternate entry 0x0fc1 (A=0x29).
 *
 * The mock's `call` POPS the return address the call site pushed (models loc_0ea2's ret); loc_0fbc
 ...
 * baseline (stack tooth). TEETH: mis-charge a `call` (17 T) as 10 T -> the 99-T golden throws.
 *
 * Run: node --test games/pooyan/translated/test/loc_0fbc.test.js
 */
...
function makeMachine() {
  ...
  return {
    regs, mem, ram, calls: [], tstates: 0, pc: 0x0fbc, pcSeq: [],
    ...
    // loc_0ea2's ret pops the return address loc_0fbc pushed -- model that pop so a missing push16
...
test("loc_0fbc: append 0x28,0x15,0x16,0x17 -- three calls + tail jp into loc_0ea2", () => {
  const m = makeMachine();
  seatCaller(m);

  loc_0fbc(m);

  assert.equal(m.tstates, 99, "T-state total");
  ...
  ], "step boundaries visit the call targets; jp skips 0x0fc1");
...
test("loc_0fbc MUTATION: a `call` mis-charged 10T (not 17T) is caught by the golden", () => {
  const m = makeMachine();
  ...
  assert.equal(m.tstates, 92, "mutation loses 7 T (17 -> 10)");
  assert.throws(
    () => assert.equal(m.tstates, 99, "T-state total"),
    /99/,
    "the 99-T golden must fail on the mutant",
  );
});
