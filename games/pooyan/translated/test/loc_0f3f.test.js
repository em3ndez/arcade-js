...
// Equivalence test for loc_0f3f (ROM 0x0f3f): sound command 0x12 -- `ld a,0x12` then a
...
// Run: node --test games/pooyan/translated/test/loc_0f3f.test.js
...
function makeMachine() {
  const regs = new Regs();
  return {
    regs, calls: [], tstates: 0, pc: 0x0f3f, pcSeq: [],
    step(n, c) { this.pc = n; this.tstates += c; this.pcSeq.push(n); },
    call(a) { this.calls.push(a); return undefined; },
  };
}

test("loc_0f3f: A=0x12 -> tail into loc_0ea2", () => {
  const m = makeMachine();
  loc_0f3f(m);
  assert.equal(m.regs.a, 0x12, "A = 0x12 (sound command) handed to loc_0ea2");
  ...
  assert.deepEqual(m.pcSeq, [0x0f41, 0x0ea2], "step boundaries match the disassembly");
});

test("loc_0f3f MUTATION: `jp 0x0ea2` mis-charged 17T (as a call, not 10T) is caught", () => {
  const m = makeMachine();
