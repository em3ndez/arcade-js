...
// Equivalence test for loc_0f44 (ROM 0x0f44): sound command 0x13 -- `ld a,0x13` then a
...
// Run: node --test games/pooyan/translated/test/loc_0f44.test.js
...
function makeMachine() {
  const regs = new Regs();
  return {
    regs, calls: [], tstates: 0, pc: 0x0f44, pcSeq: [],
    step(n, c) { this.pc = n; this.tstates += c; this.pcSeq.push(n); },
    call(a) { this.calls.push(a); return undefined; },
  };
}

test("loc_0f44: A=0x13 -> tail into loc_0ea2", () => {
  const m = makeMachine();
  loc_0f44(m);
  assert.equal(m.regs.a, 0x13, "A = 0x13 (sound command) handed to loc_0ea2");
  ...
  assert.deepEqual(m.pcSeq, [0x0f46, 0x0ea2], "step boundaries match the disassembly");
});

test("loc_0f44 MUTATION: `jp 0x0ea2` mis-charged 17T (as a call, not 10T) is caught", () => {
  const m = makeMachine();
