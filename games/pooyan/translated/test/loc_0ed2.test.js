...
// Equivalence test for loc_0ed2 (ROM 0x0ed2): sound command 0x01 -- `ld a,0x01` then a
...
// Run: node --test games/pooyan/translated/test/loc_0ed2.test.js
...
function makeMachine() {
  const regs = new Regs();
  return {
    regs, calls: [], tstates: 0, pc: 0x0ed2, pcSeq: [],
    step(n, c) { this.pc = n; this.tstates += c; this.pcSeq.push(n); },
    call(a) { this.calls.push(a); return undefined; },
  };
}

test("loc_0ed2: A=0x01 -> tail into loc_0ea2", () => {
  const m = makeMachine();
  loc_0ed2(m);
  assert.equal(m.regs.a, 0x01, "A = 0x01 (sound command) handed to loc_0ea2");
  ...
  assert.deepEqual(m.pcSeq, [0x0ed4, 0x0ea2], "step boundaries match the disassembly");
});

test("loc_0ed2 MUTATION: `jr 0x0ea2` mis-charged 7T (not 12T) is caught", () => {
  const m = makeMachine();
