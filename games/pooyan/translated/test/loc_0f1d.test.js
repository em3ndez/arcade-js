...
// Equivalence test for loc_0f1d (ROM 0x0f1d): sound command 0x0f -- `ld a,0x0f` then a
...
// Run: node --test games/pooyan/translated/test/loc_0f1d.test.js
...
function makeMachine() {
  const regs = new Regs();
  return {
    regs, calls: [], tstates: 0, pc: 0x0f1d, pcSeq: [],
    step(n, c) { this.pc = n; this.tstates += c; this.pcSeq.push(n); },
    call(a) { this.calls.push(a); return undefined; },
  };
}

test("loc_0f1d: A=0x0f -> tail into loc_0ea2", () => {
  const m = makeMachine();
  loc_0f1d(m);
  assert.equal(m.regs.a, 0x0f, "A = 0x0f (sound command) handed to loc_0ea2");
  ...
  assert.deepEqual(m.pcSeq, [0x0f1f, 0x0ea2], "step boundaries match the disassembly");
});

test("loc_0f1d MUTATION: `jr 0x0ea2` mis-charged 7T (not 12T) is caught", () => {
  const m = makeMachine();
