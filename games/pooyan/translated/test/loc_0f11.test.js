...
// Equivalence test for loc_0f11 (ROM 0x0f11): sound command 0x0c -- `ld a,0x0c` then a
...
// Run: node --test games/pooyan/translated/test/loc_0f11.test.js
...
function makeMachine() {
  const regs = new Regs();
  return {
    regs, calls: [], tstates: 0, pc: 0x0f11, pcSeq: [],
    step(n, c) { this.pc = n; this.tstates += c; this.pcSeq.push(n); },
    call(a) { this.calls.push(a); return undefined; },
  };
}

test("loc_0f11: A=0x0c -> tail into loc_0ea2", () => {
  const m = makeMachine();
  loc_0f11(m);
  assert.equal(m.regs.a, 0x0c, "A = 0x0c (sound command) handed to loc_0ea2");
  ...
  assert.deepEqual(m.pcSeq, [0x0f13, 0x0ea2], "step boundaries match the disassembly");
});

test("loc_0f11 MUTATION: `jr 0x0ea2` mis-charged 7T (not 12T) is caught", () => {
  const m = makeMachine();
