 ...
 *   (spawnEnemyIntoFreeActorSlot) until one of them takes.
 *
 * ROLE IN THE MACHINE
 ...
 *   each record in turn to spawnEnemyIntoFreeActorSlot, which either declines (the record is already occupied) or
 ...
 *
 * LIVE-OUT: memory only — nothing reads a register back. Whatever spawnEnemyIntoFreeActorSlot seats into the chosen
...
const INIT_SEED = 0x04; // the "kind" field handed to spawnEnemyIntoFreeActorSlot, stamped into the record it claims
...
export function seedFirstFreeSpriteBlockInRun(m, base = m.regs.ix, count = m.regs.b, col = m.regs.c) {
  ...
  let remaining = count;
  do {
    // Offer this record to the per-slot spawn body (spawnEnemyIntoFreeActorSlot, ROM 0x572b), carrying the spawn
    ...
    if (spawnEnemyIntoFreeActorSlot(m, cursor, col, INIT_SEED)) return; // seeded a fresh block -> stop the run
