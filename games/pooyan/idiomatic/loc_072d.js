...
/**
 * loc_072d — attract state-0 handler: blank one tilemap row, then finish the board setup.
 *
...
export function loc_072d(m) {
  const { mem8 } = m;
  if (!loc_02ce(m, ROW_FILL_COUNT)) return; // rows remain: wait for the next call
  ...
  loc_0038(m, ATTRACT_SETUP_DISPLAY_CMD_A);
  loc_0038(m, ATTRACT_SETUP_DISPLAY_CMD_B);
  loc_0038(m, ATTRACT_SETUP_DISPLAY_CMD_C);
