...
export function loc_1f40(m, target = m.regs.a, ptr = m.regs.hl, count = m.regs.b, slot = m.regs.c) {
  const { mem8 } = m;
  ...
  if (c === 0) {
    ...
    loc_0010(m, dstEnd, 0x10, 0x03); // clear the trailing cells
  ...
  const labelPtr = loc_0c45(m, c, STAGE_LABEL_PTR_TABLE);
