...
/**
 * loc_6069 — scan head: classify the record under the cursor and route it.
 *
 * WHAT IT IS
 * ----------
 ...
 *      handler (loc_61b4), an even round runs the proximity gate (loc_6080).
...
export function loc_6069(m, hl = m.regs.hl, ix = m.regs.ix, count = m.regs.b, iy = m.regs.iy, ireg = m.regs.i) {
  ...
  // On an odd round the record is handed to the actor-collision handler (loc_61b4); on an even
  // round it goes to the proximity gate (loc_6080). Both routes carry the same pointer/parity
