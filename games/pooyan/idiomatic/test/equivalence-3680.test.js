...
/**
 * Memory-equivalence test for loc_3680 (ROM 0x3680, Pooyan) — "find a free actor slot and spawn".
 *
 ...
 *      frame-counter wrap, and a table-full miss: oracle == loc_3680 in RAM (−stack).
...
function craft(spec = {}) {
  ...
  m.mem8[TEMPLATE + 0x0b] = 0x00; // anim override off -> loc_379d uses the looked-up vector
...
test("EQUAL: crafted scan/spawn cases — loc_3680 == oracle in RAM (−stack)", () => {
...
test("TEETH: a wrong slot / attribute / frame cell is CAUGHT by the RAM diff", () => {
  const spots = [
    ...
    { name: "attribute byte (loc_36de)", craft: () => craft({ t7: 0x00 }), addr: TEMPLATE + 0x08 },
