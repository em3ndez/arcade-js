...
/**
 * Memory-equivalence test for loc_50f1 (Pooyan) — the object-freeze tamper gate that fronts the
 * phase-4 tilemap checksum guard (loc_6ac5).
 *
 * On a good ROM the freeze flag (0x89fb) is 0, so the routine delegates to loc_6ac5. The original
 ...
 * the guard's own gates declining (wave != 2, or the once-latch already set), loc_50f1 must leave
 ...
 *
 * Jobs: 1. EQUAL across the no-op delegation states; 2. WRITE-SET (loc_50f1 writes nothing itself —
...
/** Seat the freeze flag clear and the guard's gates so loc_6ac5 declines without walking/writing. */
...
test("EQUAL: loc_50f1 == oracle in RAM (−stack) across no-op delegation states", () => {
...
test("WRITE-SET: loc_50f1 writes nothing itself; the discarded checksum fold is a no-op", () => {
  ...
  assert.deepEqual(changed, [], `loc_50f1 wrote unexpected cells: ${changed.join(",")}`);
  console.log("  WRITE-SET: 0 own writes (all memory effects belong to the delegate loc_6ac5)");
});
...
test("DELEGATION: the guard is actually entered (its checksum arm fires on a blank tilemap)", () => {
  ...
  // On a blank (all-zero) tilemap the low byte mismatches first, so loc_6ac5 diverts into loc_0929
  // (the ROM's screen-setup arm), which faults on the crafted state; a high-byte-only mismatch would
  // instead throw loc_6ac5's checksum-mismatch guard. Either arm throws, which proves delegation
  // reached loc_6ac5 -- with the freeze flag clear there is no other throw path, so a non-delegating
  // impl would simply return.
  const c = seat({ freeze: 0x00, wave: 0x02, latch: 0x00 });
  const msg = (() => { try { loc_50f1(c); return null; } catch (e) { return String(e && e.message); } })();
  assert.notEqual(msg, null, "loc_50f1 did not delegate into the checksum guard (it returned instead of entering loc_6ac5)");
  assert.doesNotMatch(msg, /object-freeze/, "loc_50f1 threw via its own freeze arm, not the delegated checksum guard");
  console.log("  DELEGATION: guard entered (checksum arm reached via loc_6ac5)");
});
...
test("TEETH: the object-freeze arm traps; a spurious own-write would be caught", () => {
  ...
  assert.throws(() => loc_50f1(t), /0x5119|tamper/i, "the freeze arm failed to trap");
