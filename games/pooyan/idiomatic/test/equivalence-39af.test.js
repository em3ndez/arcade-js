...
/**
 * Memory-equivalence test for loc_39af (ROM 0x39af, Pooyan) — enemy actor state handler (rst-0x28
 * dispatch target) for the record at IX.
 *
 * SEATING: BALANCED — record base in IX (param default m.regs.ix). loc_39af is a void state handler:
 ...
 * branches are crafted: even frame -> the travel handler (loc_3b87); odd frame -> the vertical mover
 * (loc_39ba) and its sub-branches (arm at high<2, sub-state reset at high<4, inert, keep-travelling).
 * The keep-travel arms route through loc_39e0's shared tail, gated inert by the global lane countdown.
 *
 * NOTE (cross-group seam): the odd/state0 case flows loc_39af -> loc_39ba -> loc_3a51. loc_39ba
 * (group g3) and loc_3a51 (group g4) are decompiled concurrently; if their arg ABI is not reconciled
 * this case surfaces it. That is intended coverage, not a defect in loc_39af.
 *
 * Jobs:
...
import { loc_39af } from "../loc_39af.js";
...
function seat(m, { fields = {}, round = 0, gauge = 0x08, wave = 0x0e, lane = 0x01 } = {}) {
  ...
  m.mem.write8(WAVE_PROGRESS, wave); // >= 0x0e routes loc_39e0 into its tail
...
test("EQUAL: loc_39af == oracle in RAM (−stack)", () => {
