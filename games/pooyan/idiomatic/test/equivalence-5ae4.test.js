...
/**
 * Memory-equivalence gate for loc_5ae4 — the master per-frame actor updater, a pure void sequencer
 ...
 * checks oracle == module in RAM (dumpState, minus STACK_SCRATCH). loc_5ae4 has no register live-out
 ...
 * with a near-empty footprint) and a state seated so the ELEVENTH handler (loc_5b2c, end-of-wave
...
const LANE_SPAWN_COUNTDOWN = 0x8d75; // loc_5b2c gate; cleared to 0 on its sweep
const CLEANUP_GATE_8D77 = 0x8d77; //   set -> loc_5b2c takes the sweep path directly
const ACTIVE_LANE_COUNT = 0x8d79; //   loc_5b2c returns early when nonzero
...
/** Seat the eleventh handler (loc_5b2c) onto its sweep-and-clear path. */
function craftEleventh() {
  const m = base();
  m.mem.write8(LANE_SPAWN_COUNTDOWN, 0x01); // nonzero -> loc_5b2c proceeds
  m.mem.write8(ACTIVE_LANE_COUNT, 0x00); // zero -> loc_5b2c proceeds
