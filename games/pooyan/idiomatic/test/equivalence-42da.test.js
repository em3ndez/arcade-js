...
/**
 * Memory-equivalence test for loc_42da (ROM 0x42da, Pooyan) — "object-slot initializer".
 *
 * loc_42da is a CALLER-SKIP for loc_4221's spawn loop. In the oracle a live slot ends with a plain
 ...
 *   false = the caller-skip (slot initialized -> abort loc_4221's loop).
 ...
 * (dumpState) minus STACK_SCRATCH. loc_42da has no register live-out — loc_4221 reloads its own.
 *
 * Jobs:
...
test("TEETH: a wrong boolean is CAUGHT by the boolean check", () => {
  ...
  const brokenTwin = () => true; // a twin that fails to abort loc_4221's loop
