...
/**
 * Memory-equivalence test for loc_0f2b (ROM 0x0f2b) — "enqueue command byte 0x11": force A=0x11
 * and tail into the ring-append helper loc_0ea2, which stashes the byte, and (only while a game is
 ...
 * are NOT compared.
 *
 * The leaf is not reached in a plain attract boot, so every case is CRAFTED: the two append gates
 * (GAME_ACTIVE_FLAG, PLAY_MODE_LATCH) and the ring cursor (SOUND_RING_WRITE_PTR) poked identically
 * on both sides. loc_0f2b supplies its own A, so nothing else is seated.
 *
 * Jobs:
 *   1. EQUAL — over the gate/cursor cases oracle == loc_0f2b in RAM (−stack) AND in A; the module
 ...
 *
 * Run: node --test games/pooyan/idiomatic/test/equivalence-0f2b.test.js
 */
...
import { loc_0f2b as oracle } from "../../translated/loc_0f2b.js";
...
const CMD = 0x11;           // the byte loc_0f2b appends
...
test("WRITE-SET: the append path writes the pending byte, the ring slot, and the advanced cursor", () => {
  const c = craft(1, 0, 0x43);
  loc_0f2b(c);
  assert.equal(c.mem.read8(TEXT_RING_PENDING_BYTE), CMD, "pending-byte cell holds the command");
