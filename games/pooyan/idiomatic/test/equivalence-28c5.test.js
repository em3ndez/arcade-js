...
/**
 * Memory-equivalence test for idleLaunchStateNoop (ROM 0x28c5, Pooyan) — a PHANTOM NO-OP: the frozen routine
 ...
 *
 * Jobs:
 *   1. NO-OP EQUAL — over boot-state clones, oracle == idleLaunchStateNoop in RAM(−stack).
...
test("NO-OP EQUAL: oracle == idleLaunchStateNoop in RAM(−stack) across clones", () => {
  for (let i = 0; i < 4; i++) {
    const o = craft();
    const c = craft();
    oracle(o);
    idleLaunchStateNoop(c);
