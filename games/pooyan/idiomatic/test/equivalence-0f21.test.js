...
/**
 * Memory-equivalence test for loc_0f21 (ROM 0x0f21) — "queue two commands": append 0x95 then 0x10
 ...
 * the ring, so each case uses a FRESH clone per side: oracle on one, loc_0f21 on the other, compared
 ...
 *
 * Jobs:
 *   1. EQUAL (crafted sweep) — over {active, gates-closed, cursor at/near wrap} loc_0f21 == oracle
...
test("WRITE-SET: active from cursor 0x43 writes the two ring slots, the cursor, and the stash", () => {
  ...
  const EXPECTED = new Map([
    ...
    [TEXT_RING_PENDING_BYTE, CMD_SECOND], // last byte stashed
  ]);
