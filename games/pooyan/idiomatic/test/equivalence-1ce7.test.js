...
/**
 * Memory-equivalence test for loc_1ce7 (ROM 0x1ce7) — "stamp a three-cell column, cap first". The
 ...
 * uses a FRESH clone per side. loc_1ce7 takes no register inputs (it loads its own HL) and leaves
 ...
 *
 * Jobs:
 *   1. EQUAL — loc_1ce7 == oracle in RAM (−stack).
...
/** A fresh clone; loc_1ce7 has no register inputs, so only SP (in work RAM) is seated. */
function craft() {
  const m = BASE.clone();
...
test("EQUAL: loc_1ce7 == oracle in RAM (−stack)", () => {
  const o = craft();
  const c = craft();
  oracle(o);
  loc_1ce7(c);
