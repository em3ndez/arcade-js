...
/**
 * Memory-equivalence test for queueSoundCommand0A (Pooyan) — "queue display command 0x0a": load the fixed
 ...
 *
 * Run: node --test games/pooyan/idiomatic/test/equivalence-0f05.test.js
 */
...
import { loc_0f05 as oracle } from "../../translated/loc_0f05.js";
...
test("EQUAL: crafted gate/cursor cases — queueSoundCommand0A == oracle in RAM (−stack) + A", () => {
  for (const { name, active, mode, cursor } of CASES) {
    const o = craft(active, mode, cursor);
    const c = craft(active, mode, cursor);
    oracle(o);
    const ret = queueSoundCommand0A(c);
