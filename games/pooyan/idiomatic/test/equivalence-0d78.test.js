...
/**
 * Memory-equivalence test for loc_0d78 (ROM 0x0d78, Pooyan) — coin/credit post-handler on IN0 bits.
 *
 * Bit 3 (1P-start) of INPUT_PORT0 (0x8810) hands off to loc_0de4. Otherwise, unless bit 4 (2P-start)
 ...
 * continues into loc_0da8. The dissolved tails (loc_0de4, loc_0da8 -> loc_0dab) run their idiomatic
 ...
 *
 * Jobs:
 *   1. EQUAL — bit3 (loc_0de4 0df4 path), bit4-clear return, bit4-set/credits<2 return, and
 *      bit4-set/credits>=2 (checksum + loc_0da8): module == oracle (RAM −stack).
...
function craft({ in0, credits, state = 0x00, ctrl = 0x00 }) {
  ...
  m.mem.write8(PLAY_STATE_INDEX, state & 0xff); // != 0x0e so loc_0de4's 0df4 path sets 0x8805
  m.mem.write8(PLAYER_CTRL, ctrl & 0xff); // bit0 clear -> loc_0dab's shorter (ret nc) path
...
const CASES = {
  "bit3 -> loc_0de4": { in0: 0x08, credits: 0x00, state: 0x00 },
  "bit4 clear -> return": { in0: 0x00, credits: 0x03 },
  "bit4 set, credits<2 -> return": { in0: 0x10, credits: 0x01 },
  "bit4 set, credits>=2 -> loc_0da8": { in0: 0x10, credits: 0x05 },
};

// -- 1. EQUAL -----------------------------------------------------------------

test("EQUAL: loc_0d78 == oracle in RAM (−stack)", () => {
...
test("WRITE-SET: the consume path subtracts 2 credits; the credits<2 path is inert", () => {
  const consume = craft(CASES["bit4 set, credits>=2 -> loc_0da8"]);
  oracle(consume);
...
test("TEETH: a corrupted credit byte is CAUGHT by the RAM diff", () => {
  const o = craft(CASES["bit4 set, credits>=2 -> loc_0da8"]);
  const c = craft(CASES["bit4 set, credits>=2 -> loc_0da8"]);
  oracle(o);
  loc_0d78(c);
