...
/**
 * Memory-equivalence test for advanceAllEnemyActorStates (ROM 0x7621, Pooyan) — a twin entry to the shared
 ...
 * `ret`; advanceAllEnemyActorStates has no `pop af` of its own. The module does no stack ops; the oracle's pushes/pops
...
const SEEDED_COUNT = 14; // the count advanceAllEnemyActorStates seeds
const SP0 = 0x8ff8;
...
test("TEETH: a twin seeding the wrong count (13 under / 15 over) diverges from the oracle", () => {
  ...
  advanceEnemyActorStateWalk(underTwin, 13); // BUG: under-seeded twin leaves record 13 un-stepped
  ...
  advanceEnemyActorStateWalk(overTwin, 15); // BUG: over-seeded twin steps record 14
