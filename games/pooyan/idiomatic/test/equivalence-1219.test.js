 ...
 * register is read back — so the register file is not compared; equivalence is RAM (dumpState)
 * minus STACK_SCRATCH, SP parked in scratch so the dispatch's nested pushes drop out of the diff.
 *
 ...
 *      oracle in RAM (−stack).
 *   2. WRITE-SET — an all-inert sweep leaves RAM untouched; a dispatching record's hold field ticks.
 ...
 *
 * Run: node --test games/pooyan/idiomatic/test/equivalence-1219.test.js
 */
...
const PRE_HOLD = 0x0e; // pre-step field the shared pre-step decrements
...
function dispatchRecord(m, i) {
  ...
  m.mem.write8(rec + PRE_HOLD, 0x05); //   pre-step just decrements (record-local, stays > 0)
...
test("WRITE-SET: an inert sweep is untouched; a dispatching record ticks its hold field", () => {
