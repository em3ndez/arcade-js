...
/**
 * Memory-equivalence test for loc_119a (ROM 0x119a, Pooyan) — a spawned-actor record initialiser.
 ...
 *
 * Jobs:
 *   1. EQUAL — full path (idx 0 and idx 15) and the early-skip path: oracle == loc_119a in RAM, and
...
test("EQUAL: full path (idx 0 / idx 15) + early-skip — loc_119a == oracle in RAM (−stack)", () => {
  for (const round of [0x00, 0x3c, 0x40]) { // idx 0, idx 15, and &0x3f masking (0x40 -> idx 0)
    const o = craft(round);
    const c = craft(round);
    oracle(o);
    const ret = loc_119a(c);
