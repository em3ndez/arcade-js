...
/**
 * Memory-equivalence test for loc_7790 (ROM 0x7790, Pooyan) — the object-draw handler (rst-0x28
 ...
 * falls through to loc_77c8 (clears the record; its ret returns to this handler's caller).
 *
 * IX is an input (param default rec = m.regs.ix). The module dissolves loc_4006, loc_0c45,
 * paintTileBlock2x2Above and the loc_77c8 tail to direct calls;
 * the oracle drives the same helpers. loc_7790 outputs no register — equivalence is RAM
 ...
 * loc_77c8 returns after its clear (no colour-RAM integrity chain). The screen pointer points into
 * writable tile RAM so both 2x2 blits land in RAM/colour RAM.
 *
 * Jobs:
 *   1. EQUAL — draw (timer expiring) and hold (timer running) arms: oracle == loc_7790 (RAM −stack).
...
const SPRITE = REC + 0x13; //   sprite/char index (< 5 -> loc_77c8 returns after its clear)
...
function craft(timer) {
  ...
  m.mem8[SPRITE] = 0x03; //  < 5 -> loc_77c8 returns after clearing the record
...
test("SP-TOOTH: draw (tail dispatch) and hold (omitted ret) are both seam-placeable", () => {
  const drawR = seamPlaceable(withOmittedRet, loc_7790, 0x7790, craft(0x01));
  assert.equal(drawR.placeable, true, `draw tail-dispatch must be seam-placeable; got: ${drawR.error}`);
  const holdR = seamPlaceable(withOmittedRet, loc_7790, 0x7790, craft(0x05));
