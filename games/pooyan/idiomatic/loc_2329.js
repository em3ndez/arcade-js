...
/**
 * loc_2329 — bidirectional position driver for the actor at IX.
 *
...
export function loc_2329(m, actor = m.regs.ix) {
  const { mem8, mem16 } = m;

  if ((mem8[actor + OFF_AIM_FLAGS] & DESCEND_BIT) === 0) {
    return loc_236a(m, actor); // bit 2 clear -> descent handler
  }
  ...
  return loc_23ad(m, STATUS_RENDER_PHASE); // shared render tail
}
