...
/**
 * runVblankNmiService — the vblank NMI service routine, the game's sole per-frame heartbeat.
 *
...
export function runVblankNmiService(m) {
  ...
  if (mem8[PLAY_STATE_INDEX] === 0x04) {
    [a, pos, attr] = copySpriteAttrAndPositionRun(m, SPRITE_DISPLAY_LIST, attr, pos, 0x04);
    [a, pos, attr] = copySpriteAttrAndPositionRun(m, SPRITE_TARGET_SLOTS, attr, pos, 0x03);
    [a, pos, attr] = copySpriteAttrAndPositionRun(m, ENEMY_SCAN_BOX_TABLE, attr, pos, 0x0b);
    [a, pos, attr] = copySpriteAttrAndPositionRun(m, FORMATION_COORD_SLOTS, attr, pos, 0x06);
  } else {
    [a, pos, attr] = copySpriteAttrAndPositionRun(m, SPRITE_DISPLAY_LIST, attr, pos, 0x18);
  }
  ...
  switch (mem8[MAIN_GAME_STATE]) {
    case 0: blankFillRowThenFinishAttractSetup(m); break;
