...
/**
 * verifyFormationGuardChecksum — hunter-formation dispatch state 2: a ROM self-check (anti-tamper guard).
 * ROM 0x3266. [seen]
 *
...
export function verifyFormationGuardChecksum(m) {
  const { mem8 } = m;
  ...
  if (sum !== GUARD_SENTINEL) {
    throw new Error("verifyFormationGuardChecksum: ROM-integrity/sanity trap unreachable with a valid ROM");
  }
}
