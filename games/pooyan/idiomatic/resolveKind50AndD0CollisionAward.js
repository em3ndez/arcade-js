...
/**
 * resolveKind50AndD0CollisionAward — proximity gate and award for the 0x50 / 0xd0 collision dispatch kinds.
 *
 * WHAT IT IS
 * ----------
 ...
 * collision handler (resolveOddRoundCollisionAndAward) finds a target slot whose state byte carries dispatch kind 0x50
...
export function resolveKind50AndD0CollisionAward(m, kind = m.regs.a, hl = m.regs.hl, ix = m.regs.ix, count = m.regs.b, iy = m.regs.iy, ireg = m.regs.i) {
  const { mem8 } = m;
