...
/**
 * loc_7960 — the shared integrity + play-timer nibble-render handler.
 *
...
export function loc_7960(m) {
  ...
  if (sumLo !== mem8[guard] || sumHi !== mem8[guard + 1]
    ...
    throw new Error("loc_7960: entry integrity checksum mismatch (unreachable with intact data)");
  }
  ...
  if (tailLo !== mem8[TAIL_CHECKSUM_GUARD]) {
    throw new Error("loc_7960: tail integrity checksum mismatch (unreachable with intact data)");
  }
