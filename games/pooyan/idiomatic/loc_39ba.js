...
/**
 * loc_39ba — advance the enemy actor's vertical position along its velocity, then branch on state.
 *
 ...
 */
export function loc_39ba(m, rec = m.regs.ix) {
  const { mem8 } = m;
  ...
  if (mem8[rec + 0x07] === 0) return loc_3a51(m, posHigh, rec);
  if (posHigh < 0x04) return loc_3a48(m, rec);
  if (posHigh < 0x10) return;
  return loc_39e0(m, rec);
}
