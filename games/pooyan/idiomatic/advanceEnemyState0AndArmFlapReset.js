...
/**
 * advanceEnemyState0AndArmFlapReset — enemy-actor state-0 handler for the record based at IX. Counts down the state timer
...
export function advanceEnemyState0AndArmFlapReset(m, rec = m.regs.ix) {
  ...
  seatTurnAnimationFromColumnLimit(m, rec); // re-run the turn-select tail (return-slot push dropped)
