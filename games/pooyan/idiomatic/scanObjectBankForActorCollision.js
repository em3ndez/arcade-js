...
/**
 * scanObjectBankForActorCollision — the per-actor proximity/collision scan (ROM 0x6435-0x64bd).
 *
 * WHAT IT IS
...
export function scanObjectBankForActorCollision(m, iy = m.regs.iy, regI = m.regs.i) {
  const { mem8 } = m;
  ...
  // Hand off to the terminator guard verifyTerminatorTableOrCountTamper with its ROM match inputs
  ...
  return verifyTerminatorTableOrCountTamper(m, TERMINATOR_SCAN_SRC, TERMINATOR_MATCH_TABLE); // always false -> abort caller
}
