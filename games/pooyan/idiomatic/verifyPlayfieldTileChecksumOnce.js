...
/**
 * verifyPlayfieldTileChecksumOnce — once-only playfield tile-region tamper checksum and dispatch, ROM 0x68ac-0x68ea. [seen]
 *
...
export function verifyPlayfieldTileChecksumOnce(m) {
  const { mem8 } = m;
  ...
  if (!matched) throw new Error("verifyPlayfieldTileChecksumOnce: tile-region tamper checksum failed (low-byte sum no match)");
  ...
  throw new Error("verifyPlayfieldTileChecksumOnce: tile-region tamper checksum failed (wrap-count no match)");
}
