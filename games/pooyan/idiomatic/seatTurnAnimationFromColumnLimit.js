...
/**
 * seatTurnAnimationFromColumnLimit — the shared turn-select tail (the state-0 handler's fall-through, and its call target).
...
export function seatTurnAnimationFromColumnLimit(m, rec = m.regs.ix) {
  ...
  const [limit] = fetchByteFromTableIndex(m, ANIM_TABLE_3418, index); // rst-0x20 byte-table lookup
