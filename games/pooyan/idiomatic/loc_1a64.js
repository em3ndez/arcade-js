...
/**
 * loc_1a64 — gameplay-state entry. While the play-mode latch is set it tails to the gameplay-state
...
export function loc_1a64(m) {
  ...
  if (mem8[PLAY_MODE_LATCH] !== 0) return loc_1a01(m); // latch set

  queueSoundCommands82And95(m);
  resetBoardRamAndReseedSpawnCounters(m);
  mem8[loc_89e3] = 0x00;

  if (mem8[GAME_ACTIVE_FLAG] === 0) return loc_1d3c(m); // credit gate closed
