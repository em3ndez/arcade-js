...
/**
 * loc_2e5e — rope-cell state 1: spawn a bonus object and draw the segment tile.
 *
...
export function loc_2e5e(m, ix = m.regs.ix) {
  const { mem8 } = m;

  if ((mem8[FRAME_COUNTER] & 0x03) !== 0) return; // act on every fourth frame only

  const [timer, elapsed] = loc_2e45(m, ix & 0xff); // tick this cell's timer; HL := timer cell
