...
/**
 * loc_4221 — per-frame object-state handler for the record at IX (dispatch target). Ticks the
 ...
 *
 * blockC9 calls the slot initializer (loc_4221 is its sole caller); a false return is the
 * caller-skip that aborts the sweep — the boolean that dissolves the pop-af/ret skip.
 */
export function loc_4221(m, ix = m.regs.ix) {
  const { mem8 } = m;
  ...
  const blockC9 = () => { // clear 3 stride-0x18 slots of the spawn-object table
    ...
    let b = 0x03;
    do {
      if (!loc_42da(m, slot, ix)) return; // slot initialized -> caller-skip aborts the sweep
      slot = u16(slot + 0x18);
      b = (b - 1) & 0xff;
    } while (b !== 0);
  };
