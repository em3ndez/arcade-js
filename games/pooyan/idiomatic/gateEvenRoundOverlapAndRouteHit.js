...
/**
 * gateEvenRoundOverlapAndRouteHit — the even-round proximity gate of the object-proximity collision scan.
 *
 * WHAT IT IS
 *   One record's worth of the per-frame collision sweep. The scan head (classifyAndRouteObjectRecordByRound) has already
 ...
 *       collision tag, and control enters the hit handler (resolveShotHitEngageOrSeedRecord) with that tag as the key it
 ...
 *   it; on a hit resolveShotHitEngageOrSeedRecord decides WHAT the overlap means (capture the target, or resolve it
 *   against the enemy pool). Odd rounds take a different handler (resolveOddRoundCollisionAndAward) from the same head.
 *
...
export function gateEvenRoundOverlapAndRouteHit(m, hl = m.regs.hl, ix = m.regs.ix, count = m.regs.b, iy = m.regs.iy, ireg = m.regs.i) {
  const { mem8 } = m;
  ...
  return resolveShotHitEngageOrSeedRecord(m, rec, mem8[rec], ireg); // hit: enter the handler keyed on the record tag
}
