...
/**
 * loc_3473 — interior-entry arm for the shared movement tail. When the anim-armed latch is already
...
export function loc_3473(m, rec = m.regs.ix) {
  const { mem8 } = m;
  ...
  if (phase >= PHASE_CAP) return loc_34b0(m, rec); // capped: straight to the shared tail

  if (phase < PHASE_STEP_CAP) {
    mem8[SPAWN_PHASE_SNAPSHOT] = mem8[SPAWN_PHASE_SNAPSHOT] + 1;
  }
  const [limit] = loc_0020(m, ANIM_TABLE_3418, mem8[SPAWN_PHASE_SNAPSHOT]); // row-table lookup
  ...
  return loc_34b0(m, rec); // fall into the shared despawn/movement tail
}
