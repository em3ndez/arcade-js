...
/**
 * resolveOddRoundCollisionAndAward — the odd-round collision handler of the object-proximity collision scan.
 *
 * WHAT IT IS
 * ----------
 * One record's worth of the per-frame collision sweep, taken on ODD rounds. The scan head
 * (classifyAndRouteObjectRecordByRound) has already confirmed the current record at `hl` is live; because the round counter
 * (ROUND_COUNTER, 0x8907) has bit 0 set, it routes control here instead of to the even-round
 * distance gate (gateEvenRoundOverlapAndRouteHit). This handler decides, for one live record, whether a matching enemy has
 ...
 *     even-round distance gate (gateEvenRoundOverlapAndRouteHit), which re-tests the pair purely by proximity.
 *   - A matching, free slot -> read its STATE byte (+0x16) and dispatch on its high nibble:
 *       nibble 0x00           -> the distance gate (gateEvenRoundOverlapAndRouteHit);
 *       nibble 0x50 or 0xd0   -> the boundary/bounce handler (resolveKind50AndD0CollisionAward);
 *       nibble 0xf0           -> the tight bounding-box handler (testKindF0TightOverlap);
 ...
 * (see mechanisms.md "The object-proximity collision scan"). classifyAndRouteObjectRecordByRound decides a record is worth
 * testing and picks the branch by round parity: the even round runs gateEvenRoundOverlapAndRouteHit, this odd round runs
 * resolveOddRoundCollisionAndAward. Where gateEvenRoundOverlapAndRouteHit is a pure distance gate, this handler adds the tag-keyed enemy lookup and
...
export function resolveOddRoundCollisionAndAward(m, hl = m.regs.hl, ix = m.regs.ix, count = m.regs.b, iy = m.regs.iy, ireg = m.regs.i) {
  const { mem8 } = m;
  ...
  if (stateByte < 0) return gateEvenRoundOverlapAndRouteHit(m, hl, ix, count, iy, ireg); // no usable match
  ...
  if (nibble === 0x00) return gateEvenRoundOverlapAndRouteHit(m, hl, ix, count, iy, ireg);
  // 0x50 / 0xd0: the boundary/bounce handler resolves this kind (ROM `jp z,0x6287`).
  if (nibble === 0x50 || nibble === 0xd0) return resolveKind50AndD0CollisionAward(m, nibble, hl, ix, count, iy, ireg);
  // 0xf0: the tight bounding-box handler resolves this kind (ROM `jp z,0x630f`).
  if (nibble === 0xf0) return testKindF0TightOverlap(m, hl, ix, count, iy, ireg);
  ...
  // the same flip-biased per-axis window as gateEvenRoundOverlapAndRouteHit. A sprite's horizontal hotspot shifts with
