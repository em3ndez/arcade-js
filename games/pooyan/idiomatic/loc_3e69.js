...
export function loc_3e69(m, rec = m.regs.ix) {
  const { mem8 } = m;
  ...
  if (type < TYPE_MIN || type >= TYPE_MAX) {
    return loc_3553(m, rec); // out-of-range descriptor: blank the band and stop
  }
  ...
  return loc_3e9c(m, rec); // fall through into the in-flight mover
}
