...
/**
 * loc_0b32 — attract sub-state 6 handler.
 *
...
export function loc_0b32(m) {
  ...
  for (let i = 0; i < PAIR_COUNT; i++) {
    ...
    if (top !== mem8[row]) return loc_08b3(m); // row mismatch -> re-enter sub-state 0
  }
  ...
  if (sumLow !== mem8[vptr]) return loc_08b3(m); // low-byte miss
  vptr = u16(vptr + 1);
