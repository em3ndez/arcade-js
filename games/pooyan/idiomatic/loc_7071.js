...
/**
 * loc_7071 — anti-tamper attract sub-state-6 handler, entered when the integrity guard finds the
...
export function loc_7071(m) {
  ...
  for (let b = 0x0a; b > 0; b--) { // column integrity: each cell == the one 0x20 above
    const a = mem8[hl];
    hl = u16(hl - 0x20);
    if (a !== mem8[hl]) return loc_08b3(m);
  }
  ...
  if (e !== mem8[check]) return loc_08b3(m);
  check = u16(check + 1);
