...
import { loc_8d45, OBJECT_STATE2_ANIM_TABLE, OBJECT_ANIM_DISPLAY_CMD_BASE } from "./names.js";
/**
 * loc_3d18 — object state-2 handler (dispatched with the object record in IX). Picks a
 ...
 * else 0x20. It then installs the animation from OBJECT_STATE2_ANIM_TABLE, advances the state, and
 * falls through into the state-3 handler (record passed as rec). LIVE-OUT: none — delegates
 * its return to the state-3 chain.
 */
...
export function loc_3d18(m, rec = m.regs.ix) {
  const { mem8 } = m;
  ...
  setActorAnimation(m, rec, loc_0c45(m, index, OBJECT_STATE2_ANIM_TABLE));

  const stateCell = u16(rec + 0x02);
  mem8[stateCell] = mem8[stateCell] + 1; // advance the object state
  return loc_3d5c(m, rec); // fall through into the state-3 handler
}
