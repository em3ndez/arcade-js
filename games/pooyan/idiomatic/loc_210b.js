...
/**
 * loc_210b — one-shot target-slot spawn, gated by a trigger bit and a once latch.
 *
...
export function loc_210b(m) {
  ...
  if (mem8[FORMATION_INIT_LATCH] !== 0) return; // already latched
  mem8[FORMATION_INIT_LATCH]++; // arm the once latch
  ...
  for (let i = 0; i < SLOT_COUNT; i++) {
    if ((mem8[slot] & IN_USE_BIT) === 0) {
      ...
      return loc_22b1(m); // tail to the actor animation stepper
    }
    slot = u16(slot + SLOT_STRIDE);
  }

  // both slots busy: the tamper guard decides
  if (mem8[TAMPER_STRIKES_HUD_GUARD] !== 0) return loc_2157(m); // tail to the (unlifted) tamper re-scan
  return;
}
