 ...
 *   (dispatchEnemyActorAnimTickState). That tick dispatches on the record's state byte (byte +2, masked to 0..3) to one of
...
export function advanceEnemyActorStateWalk(m, count = m.regs.b) {
  ...
  for (let n = count; n > 0; n--) {
    // Hand this record to the per-entry animation tick (dispatchEnemyActorAnimTickState). It reports true to keep sweeping;
    ...
    if (!dispatchEnemyActorAnimTickState(m, ix)) return; // a tick aborted the walk
