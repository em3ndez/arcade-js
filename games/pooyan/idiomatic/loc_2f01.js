...
/**
 * loc_2f01 — rope-cell timer handler gated by the grab test.
 *
...
export function loc_2f01(m, ix = m.regs.ix) {
  const { mem8 } = m;

  if (!loc_305f(m, ix & 0xff)) return; // grab fired: abandon the cell update

  const [timerAddr, reachedZero] = loc_2e45(m, ix & 0xff);
  ...
  const columnBase = loc_2e52(m, ix & 0xff);
