...
/**
 * loc_72cf — dispatch an eagle record's per-frame state handler, record based at IX.
 *
...
export function loc_72cf(m, rec = m.regs.ix) {
  const { mem8 } = m;
  ...
  switch (mem8[rec + STATE_OFFSET]) { // (ix+2) is bounded 0..2 — no other state dispatches
    case 0: return loc_733c(m, rec);
    case 1: return loc_7395(m, rec);
    case 2: return loc_73ce(m, rec);
  }
}
