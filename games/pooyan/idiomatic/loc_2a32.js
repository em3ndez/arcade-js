...
/**
 * loc_2a32 — per-frame handler for an actor in its state 3. Reloads the record's frame delay,
...
export function loc_2a32(m, rec = m.regs.ix) {
  const { mem8 } = m;
  ...
  if (high === MILESTONE_A) { loc_0038(m, DISPLAY_CMD_A); return; }
  if (high === MILESTONE_B) { loc_0038(m, DISPLAY_CMD_B); return; }
