...
/**
 * loc_12af — per-object travel tick for the actor record based at IX.
 *
...
export function loc_12af(m, rec = m.regs.ix) {
  const { mem8 } = m;

  advanceObjectAnimationFrame(m, rec);
  if (mem8[rec + FLAG_FIELD] !== 0) return loc_13fe(m, rec);
  ...
  if (mem8[STAGE_COUNTDOWN] < STAGE_GATE) return loc_1399(m, rec, accum);

  const seqIdx = (mem8[ROUND_COUNTER] & 0x1f) >> 2;
  const tableBase = loc_0c45(m, seqIdx, ANIM_SEQ_TABLE_12FB);
  const [target] = loc_0020(m, tableBase, mem8[ANIM_FRAME_COUNTER] & 0x0f);
