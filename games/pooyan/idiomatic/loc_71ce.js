...
/**
 * loc_71ce — the eagle/arrow approach state machine, run once per frame.
 *
...
function stepGridMarker(m) {
  ...
  if ((mem8[EAGLE_GRID_STEP_TICK] & GRID_STEP_MASK) !== 0) {
    loc_7287(m); // not an eighth-frame boundary: just run the grid-edge guard
    return;
  }
  ...
  const cols = loc_7287(m); // grid-edge guard; hands back the coordinate for the second axis
