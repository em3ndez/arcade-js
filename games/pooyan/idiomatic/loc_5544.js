...
/**
 * loc_5544 — scan an actor-block table and seed the first free slot (spawn scheduler B tail).
 *
...
export function loc_5544(m, base = m.regs.ix, stride = m.regs.de, count = m.regs.b) {
  const { mem8 } = m;
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (!live) {
      ...
      const [kindByte] = loc_0020(m, SPAWN_KIND_TABLE_5647, idx);
