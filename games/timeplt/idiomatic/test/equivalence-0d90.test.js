...
/**
 * paintUnsuppressedDigit — memory-equivalent to the frozen oracle at ROM 0x0D90.
 *
 * WHAT IT IS. One digit painted into one character cell, with its colour beside it. The low four
 * bits of the value select a glyph out of a table through the already-decompiled fetchTableByte,
 * so that transfer is dissolved into a direct call here; the run pointer the caller was walking is
...
function entryState() {
  if (entry === null) gate(paintUnsuppressedDigit);
  return entry;
}
...
function sessions() {
  ...
  sessionCache = SESSIONS.map(([label, factory]) => ({ label, ...replaySession(factory, paintUnsuppressedDigit) }));
  return sessionCache;
}
...
test("EQUAL at the real dispatch: identical outside a fully dirty scratch window", { skip }, () => {
  ...
  // A is not a live-out -- the one caller's `rst 0x20` reloads it before any read. The rewrite
  // matches the oracle's `ld a,c` anyway, because agreeing costs one line and is checkable.
  assert.equal(a.regs.a, b.regs.a, "the value register the oracle leaves behind");
});
...
test("EXHAUSTIVE: every value against every crafted cursor and colour", { skip }, () => {
  for (const [value, cursor, colour] of cross()) {
    const d = unitDiff(paintUnsuppressedDigit, craft(value, cursor, colour));
...
test("WHOLE-MACHINE: a driven session differs only in the scratch window", { skip }, () => {
  const r = wholeRunCells(paintUnsuppressedDigit);
  console.log(
