...
/**
 * presentChecksumForTamperTest — memory-equivalent to the frozen oracle at ROM 0x200C.
 *
...
function entryState() {
  if (entry === null) gate(presentChecksumForTamperTest);
  return entry;
}
...
test("MASKED EQUAL at the real dispatch: only the dead stack bytes moved", { skip: SKIP }, () => {
  gate(presentChecksumForTamperTest);
  assert.notEqual(entry, null, "vacuous: the tape never reached the routine");
  const window = deadWindow(entryState());
  const { a, b } = run(presentChecksumForTamperTest);
...
test("LIVE-OUT at the real dispatch: the walked address and the byte match", { skip: SKIP }, () => {
  const { oracle: o, candidate: c } = run(presentChecksumForTamperTest);
...
test("EXHAUSTIVE over the two byte-wide live-ins: 512 cases match", { skip: SKIP }, () => {
  let cases = 0;
  for (let v = 0; v < 256; v++) {
    const byNarrow = run(presentChecksumForTamperTest, { a: v });
    assert.deepEqual(byNarrow.candidate, byNarrow.oracle, `narrow step ${v} diverged`);
    const byCount = run(presentChecksumForTamperTest, { b: v });
...
test("CRAFTED WRAP: pairs that carry past sixteen bits and past eight", { skip: SKIP }, () => {
  ...
  for (const live of crafted) {
    const { oracle: o, candidate: c } = run(presentChecksumForTamperTest, live);
