...
test("UNREACHED: neither tape dispatches it, and the same apparatus DOES reach a live entry",
  { skip }, () => {
    for (const [label, opts] of [["coin -> start", {}], ["undriven attract", { tape: [] }]]) {
      // POSITIVE CONTROL FIRST. A capture rig that could never reach ANY address throws
      // "never entered" exactly like one that looked and found nothing, so the absence below
      // is evidence only once the same rig, same session, is shown reaching something live.
      // 0x290E is a sibling in this batch, so its oracle and module are already to hand.
      let controlErr = null;
      try {
      ...
      assert.ok(
        !(controlErr && /never entered/.test(controlErr.message)),
        `the ${label} rig reports "never entered" at the live control ${hex4(CONTROL)} too, so ` +
          "a zero at the target says nothing about the ROM",
      );
      ...
      console.log(`  UNREACHED: ${label} — ${hex4(TARGET)} never entered, control ` +
        `${hex4(CONTROL)} entered`);
    }
  });
