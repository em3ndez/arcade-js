...
/**
 * loc_3faf — memory-equivalent to the frozen oracle at ROM 0x3FAF.
 *
...
let cache = null;
const session = () => (cache ??= replaySession(loc_3faf));
...
test("EQUAL at the real dispatch: loc_3faf == oracle outside the scratch window", { skip }, () => {
...
test("EXHAUSTIVE: all 256 headings behave as the oracle", { skip }, () => {
  assert.equal(sweepCaught(loc_3faf), 0, "the rewrite diverged somewhere in the crafted space");
...
test("WHOLE-MACHINE: the pinned session differs only in the dead stack bytes", { skip }, () => {
  const r = wholeRunCells(loc_3faf);
