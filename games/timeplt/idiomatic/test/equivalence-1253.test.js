...
/**
 * postGameOverBanner — memory-equivalent to the frozen oracle at ROM 0x1253.
 *
...
function sessionRun() {
  ...
  let threw = null;
  try {
    unitEquivalence((overrides) => (host = makeMachine(overrides)), TARGET, oracle, postGameOverBanner, {
...
test("CRAFTED: both flags against the ring's free, occupied and wrapping states", { skip }, () => {
  assert.equal(sweepCaught(postGameOverBanner), 0, "the rewrite diverged somewhere in the crafted space");
