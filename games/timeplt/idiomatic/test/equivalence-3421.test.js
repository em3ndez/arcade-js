...
/**
 * drawCaptionFivePastSharedColour — memory-equivalent to the frozen oracle at ROM 0x3421.
 *
...
test("CORPUS: every dispatch of two real sessions replays identically", { skip }, () => {
  ...
  for (const s of corpus()) {
    ...
    for (const state of s.states) {
      const d = unitDiff(drawCaptionFivePastSharedColour, state);
...
test("THE RECORD'S OWN COLOUR IS NOT USED: poking it changes nothing", { skip }, () => {
  ...
  try {
    ...
    assert.equal(unitDiff(drawCaptionFivePastSharedColour, entry), null, "the rewrite diverged under the poke");
...
test("EXHAUSTIVE over the clock: 256 values, with the painted colour read back", { skip }, () => {
  for (let clock = 0; clock < 256; clock++) {
    const d = unitDiff(drawCaptionFivePastSharedColour, craft(clock));
