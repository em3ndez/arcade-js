...
/**
 * rearmHeldControlRepeat — memory-equivalent to the frozen oracle at ROM 0x1980.
 *
...
test("UNREACHED: neither tape dispatches it, so the strict harness throws", { skip: SKIP }, () => {
  for (const [label, opts] of [["coin -> start", {}], ["undriven attract", { tape: [] }]]) {
    assert.throws(
      () => unitEquivalence((ov) => makeMachine(ov, opts), TARGET, oracle, rearmHeldControlRepeat, {
...
test("EXHAUSTIVE: both real cells, every prior, RAM identical", { skip: SKIP }, () => {
  ...
  for (const { addr } of CELLS) {
    for (let prior = 0; prior < 256; prior++) {
      const r = compare(rearmHeldControlRepeat, addr, prior);
...
test("LIVE-OUT: the cell is cleared and a zero is handed back", { skip: SKIP }, () => {
  for (const { addr, saturated } of CELLS) {
    const r = compare(rearmHeldControlRepeat, addr, saturated);
    ...
    assert.equal(rearmHeldControlRepeat(pristine().clone(), addr), 0, "the return value must be zero too");
  }
