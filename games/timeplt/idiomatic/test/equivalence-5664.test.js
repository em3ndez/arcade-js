...
/**
 * requestAttackerSpawnSoundEra0 — memory-equivalent to the frozen oracle at ROM 0x5664.
 *
 ...
 *
 * Run: node --test games/timeplt/idiomatic/test/equivalence-5664.test.js
 */
...
const TARGET = 0x5664;
const SOUND_CODE_CELL = 0x16de;
const EXPECTED_CODE = 0x03;
const WRONG_SOURCE = SOUND_CODE_CELL + 1;
const POKED_CODE = 0x59;
...
test("EQUAL at the real dispatch: requestAttackerSpawnSoundEra0 == oracle on RAM", { skip }, () => {
  const entry = entryState();
  const a = entry.clone();
  const b = entry.clone();
  oracle(a);
  requestAttackerSpawnSoundEra0(b);
...
test("IT READS THE IMAGE: the requested code follows a poked source byte", { skip }, () => {
  ...
  withPokedImage(entry, SOUND_CODE_CELL, POKED_CODE, () => {
    ...
    const d = craftedDiff(requestAttackerSpawnSoundEra0, 0xff, 0x01, 3);
...
test("GATE CROSS: both permission cells swept, including the drop branch", { skip }, () => {
  for (const play of PLAY_VALUES) {
    for (const demo of DEMO_VALUES) {
      const d = craftedDiff(requestAttackerSpawnSoundEra0, play, demo, 3);
...
test("EXHAUSTIVE over the queue length", { skip }, () => {
  for (let length = 0; length < 256; length++) {
    const d = craftedDiff(requestAttackerSpawnSoundEra0, 0xff, 0x01, length);
