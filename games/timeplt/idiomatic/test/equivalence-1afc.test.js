...
/**
 * sampleCellGlyphAndColour — memory-equivalent to the frozen oracle at ROM 0x1AFC.
 *
...
function entryState() {
  if (entry === null) gate(sampleCellGlyphAndColour);
  return entry;
}
...
function sessions() {
  if (!sessionCache) {
    sessionCache = TAPES.map(([label, opts]) => ({ label, ...replaySession(opts, sampleCellGlyphAndColour) }));
  }
  return sessionCache;
}
...
test("EQUAL at the real dispatch: sampleCellGlyphAndColour == oracle on RAM", { skip }, () => {
  const r = gate(sampleCellGlyphAndColour);
...
test("EXHAUSTIVE: every cell of the grid, and destinations that wrap", { skip }, () => {
  assert.equal(sweepCaught(sampleCellGlyphAndColour), 0, "the rewrite diverged somewhere in the crafted space");
