...
/**
 * askForSoundWhileTheGroupIsClear — memory-equivalent to the frozen oracle at ROM 0x40B8.
 *
...
function entryState() {
  if (captured === null) replay(askForSoundWhileTheGroupIsClear);
  return captured;
}
...
test("EQUAL at the real dispatch: every byte identical, the stack scratch included", { skip }, () => {
  ...
  const b = entry.clone();
  oracle(a);
  askForSoundWhileTheGroupIsClear(b);
  assert.deepEqual(
    allDiffs(a, b),
    [],
...
test("CROSS: every era x counter x watched-pattern behaves alike", { skip }, () => {
  eachCrossEntry((era, counter, pattern) => {
    const d = compare(askForSoundWhileTheGroupIsClear, craft(era, counter, pattern));
...
test("CORPUS: every dispatch of a driven session replays identically", { skip }, () => {
  const r = replay(askForSoundWhileTheGroupIsClear);
