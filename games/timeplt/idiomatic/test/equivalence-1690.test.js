 ...
 *      on every crafted entry, and the shortfalls are the routine's shape: the panels that hold
 *      NEITHER start bit are a large minority of them, so every twin that only gets a STARTED game
 *      wrong is silent on those.
 *
...
test("EXCLUDED, deliberately: registers and pc, and nothing else", { skip }, () => {
  ...
  // EXCLUDED is a CEILING, not a set the rewrite must fill: deepEqual against it would go RED on
  // a rewrite that became register-exact, demanding the very divergence it is meant to tolerate.
  assert.deepEqual(
    REG_FIELDS.filter((k) => a.regs[k] !== b.regs[k] && !EXCLUDED.includes(k)),
    [],
