...
/**
 * loc_594e — memory-equivalent to the frozen oracle at ROM 0x594E.
 *
 ...
 *   rung-above, bottom-rung and top-rung hand the same lookup another rung of that ladder — the
 ...
 *   3. TAPE REACH — which tapes reach this entry at all, measured rather than assumed: the shared
 *      coin-then-start tape and undriven attract dispatch it ZERO times, because the caller that
 *      reaches it only runs while the player is turning.
 ...
 *   6. UNIFORM CORPUS — the corpus varies the heading and nothing else. It presents ONE record
 *      base and ONE incoming pointer, and 99 of the 256 headings are reached by neither tape. That
 *      band is not a tape-choice artifact: a second tape walking the compass at a different cadence
 *      reaches a strict SUBSET of the first one's headings, which the test asserts.
 ...
 *  13. WHOLE-MACHINE — driven play with the rewrite wired, diffed every frame, and every twin
 *      caught there too.
 ...
 *
 * Run: node --test games/timeplt/idiomatic/test/equivalence-594e.test.js
 */
...
import { loc_594e as oracle } from "../../translated/loc_594e.js";
import {
  firstStateDiff,
  unitEquivalence,
...
const weavingMachine = (overrides) => makeMachine(overrides, { tape: weaveTape() });
...
const DISPATCHES = { turning: 720, weaving: 730 };
const DISTINCT_HEADINGS = { turning: 157, weaving: 96 };

/** How many of the 256 headings NEITHER tape produces, which the crafted sweep is what covers. */
const NEVER_DRIVEN = 99;
...
function sessions() {
  ...
  sessionCache = SESSIONS.map(([label, factory]) => ({ label, ...replaySession(factory, loc_594e) }));
  return sessionCache;
}
 ...
 */
const TWINS = [
  ["no-op", brokenNoOp, [], true, [720, 730]],
  ["forwards-the-pointer", brokenForwardsPointer, [], true, [720, 730]],
  ["rung-below", (m) => velocityForHeading(m, RUNG_BELOW), [], true, [720, 730]],
  ["rung-above", (m) => velocityForHeading(m, RUNG_ABOVE), [], true, [720, 730]],
  ["bottom-rung", (m) => velocityForHeading(m, LADDER[0]), [], true, [720, 730]],
  ["top-rung", (m) => velocityForHeading(m, LADDER[5]), [], true, [720, 730]],
  ["off-by-one-entry", (m) => velocityForHeading(m, OFF_BY_ONE_ENTRY), [127, 191], true, [717, 723]],
  ["misaligned-by-a-byte", (m) => velocityForHeading(m, MISALIGNED), [], true, [720, 730]],
  ["quarter-forward", brokenQuarterForward, [0, 127, 128, 255], true, [564, 622]],
  ["unwrapped", brokenUnwrapped, [0, 1, ...everyHeading.slice(QUARTER)], false, [71, 136]],
  ["pair-swapped", brokenPairSwapped, [], true, [720, 730]],
  ["second-component-dropped", brokenSecondDropped, [], true, [716, 728]],
];

// ── the gate ────────────────────────────────────────────────────────────────────────────

test("CONTRACT: loc_594e == oracle on RAM and the live-out at the real dispatch", { skip }, () => {
  const r = gate(loc_594e);
  assert.notEqual(entry, null, "vacuous: the tape never reached the routine");
  assert.equal(r.ram, null, `RAM diverged — ${JSON.stringify(r.ram)}`);
  const d = unitDiff(loc_594e, entryState());
...
test("TAPE REACH: only a tape that turns the player reaches this entry", { skip }, () => {
  const counts = [];
  for (const [label, opts] of [["shared", {}], ["attract", { tape: [] }]]) {
    let dispatches = 0;
    ...
    assert.equal(dispatches, 0, `the ${label} tape now reaches this entry — the corpus should be ` +
      "widened to include it rather than leaving it out");
    counts.push(`${label} ${dispatches}`);
  }
  for (const s of sessions()) counts.push(`${s.label} ${s.dispatches}`);
...
test("UNIFORM CORPUS: the corpus varies the heading and nothing else", { skip }, () => {
  ...
  // The band is a property of the game's steering, not of a tape: a completely different cadence
  // reaches a strict SUBSET of the headings, so no tape choice closes it.
  const added = [...weaving.headings].filter((h) => !turning.headings.has(h));
  assert.deepEqual(added, [], "the faster tape now reaches headings the slower one does not, so " +
    "the band IS tape-dependent and the corpus should be widened rather than crafted around");
  ...
  console.log(
    `  UNIFORM CORPUS: ${seen.map((s) => `${s.label} ${s.dispatches}/${s.headings.size}`).join(", ")} ` +
      `(dispatches/headings); ${neither.length} headings no tape reaches; incoming pointer ` +
      `${[...pointers].map(hex4).join(",")}; ${bases.size} record base`,
  );
});
...
test("EXHAUSTIVE: 256 headings crafted off the real entry are identical", { skip }, () => {
  let swept = 0;
  for (const heading of everyHeading) {
    const d = unitDiff(loc_594e, selector(heading));
