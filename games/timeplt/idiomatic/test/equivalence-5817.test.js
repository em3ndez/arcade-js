...
/**
 * requestEnemyWaveSound — memory-equivalent to the frozen oracle at ROM 0x5817.
 *
 ...
 *
 * Run: node --test games/timeplt/idiomatic/test/equivalence-5817.test.js
 */
...
const EXPECTED_CODE = 0x0b;
const WRONG_SOURCE = SOUND_CODE_CELL + 1;
const POKED_CODE = 0x51;
...
test("CRAFTED ENTRY: from a real capture taken at the tail: requestEnemyWaveSound == oracle on RAM", { skip }, () => {
  const entry = entryState();
  const a = entry.clone();
  const b = entry.clone();
  oracle(a);
  requestEnemyWaveSound(b);
...
test("IT READS THE IMAGE: the requested code follows a poked source byte", { skip }, () => {
  ...
  withPokedImage(entry, SOUND_CODE_CELL, POKED_CODE, () => {
    ...
    const d = craftedDiff(requestEnemyWaveSound, 0xff, 0x01, 3);
...
test("GATE CROSS: both permission cells swept, including the drop branch", { skip }, () => {
  for (const play of PLAY_VALUES) {
    for (const demo of DEMO_VALUES) {
      const d = craftedDiff(requestEnemyWaveSound, play, demo, 3);
...
test("EXHAUSTIVE over the queue length", { skip }, () => {
  for (let length = 0; length < 256; length++) {
    const d = craftedDiff(requestEnemyWaveSound, 0xff, 0x01, length);
