...
/**
 * seatCraftSlot0ThenDispatchByEra — memory-equivalent to the frozen oracle at ROM 0x28B7.
 *
 ...
 *      dispatches are INFORMATIVE, meaning the frozen side writes something outside the mask. That
 *      last is not decoration: the arms walk records that are often idle, and a candidate doing
 *      nothing would pass at an inert dispatch.
...
const DISPLAY_ENTRY = 0xaa1a;
/** The three siblings of the same shape, each aiming the same two cursors somewhere else. */
const NEIGHBOURS = [["28c2", 0xa860, 0xaa1c], ["28cd", 0xa870, 0xaa1e], ["28d8", 0xa880, 0xaa20]];
...
const TWINS = [
  ...
  ["record-one-slot-on", aimedAt(CRAFT_RECORD + RECORD_STRIDE, DISPLAY_ENTRY), [0, 1261, 0], false],
  ["entry-one-on", aimedAt(CRAFT_RECORD, DISPLAY_ENTRY + ENTRY_STRIDE), [0, 1009, 0], false],
  ["guarded", brokenGuarded, [0, 393, 0], false],
  ["first-arm-always", brokenFirstArmAlways, [0, 800, 0], false],
  ["neighbour-28c2", aimedAt(0xa860, 0xaa1c), [0, 1261, 0], false],
  ["neighbour-28cd", aimedAt(0xa870, 0xaa1e), [276, 2207, 814], true],
  ["neighbour-28d8", aimedAt(0xa880, 0xaa20), [366, 2505, 857], true],
];
...
const CANDIDATES = [[MODULE, seatCraftSlot0ThenDispatchByEra], [PROBE, probeReproducesTheChain], ...TWINS.map(([l, f]) => [l, f])];
...
test("EQUAL at every kept entry: masked RAM identical", { skip }, () => {
  for (const [label, base] of bases()) {
    const r = diffOf(seatCraftSlot0ThenDispatchByEra, base);
...
test("SCRATCH: the whole raw difference lies below the exit pointer, inside the window", { skip }, () => {
  ...
  for (const [label, base] of bases()) {
    for (let i = 0; i < ARM_COUNT; i++) {
      const r = diffOf(seatCraftSlot0ThenDispatchByEra, craft(base, i));
      for (const d of r.raw) {
...
test("SELECTOR: the five high bits are ignored, over the cell's whole range", { skip }, () => {
  ...
  for (const [label, base] of bases()) {
    for (let v = 0; v < SELECTOR_VALUES; v++) {
      const r = diffOf(seatCraftSlot0ThenDispatchByEra, craft(base, v));
      total++;
      if (r.informative) informative++;
...
test("GUARD: this entry tests no cell before acting, and the instrument can see one that does", { skip }, () => {
  ...
  for (const [label, base] of seen) {
    for (const value of [0, GUARD_SET]) {
      const mine = diffOf(seatCraftSlot0ThenDispatchByEra, craft(base, undefined, value));
...
test("EXCLUDED, deliberately: a CEILING on the registers that may diverge", { skip }, () => {
  ...
  for (const [, base] of bases()) {
    for (let i = 0; i < ARM_COUNT; i++) for (const k of diffOf(seatCraftSlot0ThenDispatchByEra, craft(base, i)).moved) moved.add(k);
  }
...
test("WHOLE RUN: dispatched for a session, cycle-matched, the state is byte-identical", { skip }, () => {
  const matched = wholeRun(seatCraftSlot0ThenDispatchByEra, true);
  ...
  const bare = wholeRun(seatCraftSlot0ThenDispatchByEra, false);
