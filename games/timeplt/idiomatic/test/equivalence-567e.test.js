...
/**
 * requestPlayerShotSound — memory-equivalent to the frozen oracle at ROM 0x567E.
 *
 * WHAT IT IS. Two instructions: load one program-image byte into A, then transfer to the shared
 * request body at 0x5617 (already decompiled -> a direct call with the byte as arg). The content is
 * WHICH code it asks for (fetched at run time) and WHICH permission (game OR the demo-sound switch).
 *
 * ★ NOT REACHED BY THE SHARED TAPE. The coin->start tape never dispatches this entry, so the arms
 *   below run from a real in-play state captured at its tail at 0x5617 -- a surgical entry, not a fabrication.
 *
 ...
 *
 * Run: node --test games/timeplt/idiomatic/test/equivalence-567e.test.js
 */
...
const EXPECTED_CODE = 0x07;
const WRONG_SOURCE = SOUND_CODE_CELL + 1;
const POKED_CODE = 0x5d;
...
test("CRAFTED ENTRY: from a real capture taken at the tail: requestPlayerShotSound == oracle on RAM", { skip }, () => {
  const entry = entryState();
  const a = entry.clone();
  const b = entry.clone();
  oracle(a);
  requestPlayerShotSound(b);
  ...
  console.log(
    `  CRAFTED: entry sp=${hex4(entry.regs.sp)} play=${entry.mem8[PLAY_ACTIVE]} ` +
...
test("IT READS THE IMAGE: the requested code follows a poked source byte", { skip }, () => {
  ...
  withPokedImage(entry, SOUND_CODE_CELL, POKED_CODE, () => {
    ...
    const d = craftedDiff(requestPlayerShotSound, 0xff, 0x01, 3);
...
test("GATE CROSS: both permission cells swept, including the drop branch", { skip }, () => {
  for (const play of PLAY_VALUES) {
    for (const demo of DEMO_VALUES) {
      const d = craftedDiff(requestPlayerShotSound, play, demo, 3);
...
test("EXHAUSTIVE over the queue length", { skip }, () => {
  for (let length = 0; length < 256; length++) {
    const d = craftedDiff(requestPlayerShotSound, 0xff, 0x01, length);
...
const TWINS = [
  ...
  ["misgated-ignores-demo", brokenMisgated],
];
