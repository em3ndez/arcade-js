...
/**
 * setSavedPenFromEra — memory-equivalent to the frozen oracle at ROM 0x339C.
 *
...
function sessionRun() {
  ...
  let threw = null;
  try {
    unitEquivalence((overrides) => (host = makeMachine(overrides)), TARGET, oracle, setSavedPenFromEra, {
...
test("EXHAUSTIVE: three states of the player-up flag against every index", { skip }, () => {
  assert.equal(sweepCaught(setSavedPenFromEra), 0, "the rewrite diverged somewhere in the crafted space");
