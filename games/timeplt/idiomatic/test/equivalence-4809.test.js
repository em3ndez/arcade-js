...
/**
 * showParachutistAward — memory-equivalent to the frozen oracle at ROM 0x4809.
 *
...
let cache = null;
const session = () => (cache ??= replaySession(showParachutistAward));
...
test("EQUAL at the real dispatch: showParachutistAward == oracle outside the scratch window", { skip }, () => {
  const sp = entryState().regs.sp;
  const a = entryState().clone();
  const b = entryState().clone();
  oracle(a);
  showParachutistAward(b);
...
test("EXHAUSTIVE: 256 indices against both states of the sound permission", { skip }, () => {
  assert.equal(sweepCaught(showParachutistAward), 0, "the rewrite diverged somewhere in the crafted space");
...
test("WHOLE-MACHINE: the attract session differs only where the interrupt pushes", { skip }, () => {
  const r = wholeRunCells(showParachutistAward);
