...
function entryState() {
  if (entry === null) gate(hasDriftedOffTheField);
  return entry;
}
...
test("THE SHARED ENTRY BUDGET FALLS SHORT, which is why this file raises it", { skip }, () => {
  assert.throws(
    () => unitEquivalence((ov) => makeMachine(ov, ATTRACT), TARGET, oracle, hasDriftedOffTheField, {
...
test("CORPUS: the undriven session's every dispatch; the driven tape reaches none", { skip }, () => {
  const undriven = replaySession(ATTRACT, hasDriftedOffTheField);
  ...
  const driven = replaySession({}, hasDriftedOffTheField);
...
test("THE REUSED MACHINES ARE SOUND: clone-per-point agrees on a sample", { skip }, () => {
  ...
  for (let i = 0; i < CROSS_CHECK_POINTS; i++) {
    ...
    assert.equal(unitDiff(hasDriftedOffTheField, m), null, `clone-per-point diverged at ${first},${second}`);
    assert.equal(
      answerDiffers(hasDriftedOffTheField, first, second),
      false,
