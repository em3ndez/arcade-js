 ...
 *
 * THE ENTRY BUDGET IS NOT THE SHARED ONE. This file runs the UNDRIVEN ATTRACT tape and caches the
 *   single entry it yields; every machine it builds passes `{ tape: [] }`, and the budget is the
 *   `BUDGET` constant below. The shared coin -> start tape is not run here at all, so this file
 ...
 *      bodies' pushed pairs and the restart address the enqueue pushes. The window is an upper
 *      bound and every arm asserts that nothing escapes it -- that assertion is what this file
 *      enforces, rather than any particular depth reached on either path.
...
test("THE ONE REAL DISPATCH IS REFUSED AND WRITES NOTHING", { skip }, () => {
  ...
  oracle(after);
  // realDiff, not a bare `addr >= sp` filter: the latter masks EVERYTHING below the stack
  // pointer, which is far wider than the [SP-8, SP) window this file declares, and would hide a
  // write the other arms would fail on.
  const moved = realDiff(before, after, entry.regs.sp, SCRATCH_BYTES);
  assert.equal(
    moved,
    null,
    `the refused dispatch now moves a cell outside dead stack, so it is not a pure refusal — ${show(moved)}`,
  );
  assert.equal(
