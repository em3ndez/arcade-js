...
/**
 * requestEnemyLaunchSound — memory-equivalent to the frozen oracle at ROM 0x565F.
 *
 ...
 *
 * Run: node --test games/timeplt/idiomatic/test/equivalence-565f.test.js
 */
...
const TARGET = 0x565f;
const SOUND_CODE_CELL = 0x07a2;
const EXPECTED_CODE = 0x02;
...
test("EQUAL at the real dispatch: requestEnemyLaunchSound == oracle on RAM", { skip }, () => {
  const entry = entryState();
  const a = entry.clone();
  const b = entry.clone();
  oracle(a);
  requestEnemyLaunchSound(b);
...
test("IT READS THE IMAGE: the requested code follows a poked source byte", { skip }, () => {
  ...
  withPokedImage(entry, SOUND_CODE_CELL, POKED_CODE, () => {
    ...
    const d = craftedDiff(requestEnemyLaunchSound, 0xff, 0x01, 3);
...
test("GATE CROSS: both permission cells swept, including the drop branch", { skip }, () => {
  for (const play of PLAY_VALUES) {
    for (const demo of DEMO_VALUES) {
      const d = craftedDiff(requestEnemyLaunchSound, play, demo, 3);
...
test("EXHAUSTIVE over the queue length", { skip }, () => {
  for (let length = 0; length < 256; length++) {
    const d = craftedDiff(requestEnemyLaunchSound, 0xff, 0x01, length);
