...
/**
 * loc_59d1 — memory-equivalent to the frozen oracle at ROM 0x59D1.
 *
 ...
 *   oracle has three call sites and every one of them consumes exactly four registers next: one
 *   banks them into four consecutive bytes of an object record, and the other two move the first
 *   half into the address register and rebuild the second half from the other two, with the
 *   accumulator overwritten before it is read and the pointer register overwritten by the exchange.
 *   Empirically, the LIVE-OUT arm forces a, f and hl hostile AFTER every real dispatch of a whole
 *   session and the run stays bit-identical, while forcing either half of the pair alone moves
 *   hundreds of cells. So the pair is live, the rest is not, and the arm proves both directions.
 *
 * ★ THE HEADING ARRIVES IN A REGISTER, NOT OFF A RECORD, and that is what separates this lookup
 *   from the near-identical one a sibling family fixes tables for. It is worth a tooth of its own:
 *   the heading-off-the-record twin reads the object byte the other family reads, and the sweep
 *   measures exactly where it survives. It also makes the crafted sweep EXHAUSTIVE over the input:
 *   the entry's whole input is that heading byte plus a pointer it discards, and both are swept.
 *
 * ★ THIS ENTRY IS THE THIRD RUNG. Six velocity tables sit in the image whose peak magnitudes climb
 *   in even steps — 0x59D7, 0x5C00, 0x5E00, 0x2530, 0x2E3E, 0x08FA. The twins hand the lookup the
 *   rungs either side, both ends of the ladder, and two pointers that are no table at all; the RUNG
 *   LADDER arm reads the peaks and the steps between them out of memory rather than asserting them
 *   from this comment, and re-derives why a neighbour cannot hide behind a near-zero sample.
 *
 ...
 *   3. TAPE REACH — measured, not assumed: all three sessions reach this entry, and how often.
 ...
 *      containment rather than an exact shape, so a rewrite agreeing on MORE would not turn it red.
 *   7. EXHAUSTIVE — all 256 headings crafted off the real entry, RAM and the pair.
 *   8. POINTER SWEEP — every ladder table and two junk pointers forced in at every heading, so the
 *      claim that the incoming pointer is discarded is swept rather than argued.
 *   9. LIVE-OUT — the pair asserted at every dispatch and over the whole sweep, plus the
 *      whole-session hostile-register instrument described above, with its own positive control.
 ...
 * call that ends in the lookup's own return, so a candidate charging no T-states and not taking
 ...
 * HOLE: the whole-machine arm can only see a twin that changes what the CALLER banks. It is not the
 * arm that holds this entry to its table; the unit arms are.
 *
 * Run: node --test games/timeplt/idiomatic/test/equivalence-59d1.test.js
 */
...
import { loc_59d1 as oracle } from "../../translated/loc_59d1.js";
import {
  firstStateDiff,
  unitEquivalence,
...
const DISPATCHES = { attract: 119, shared: 78, turning: 140 };
/** Distinct headings each session presents. The sweep is what covers the rest of the circle. */
const CORPUS_HEADINGS = { attract: 88, shared: 2, turning: 16 };
...
function sessions() {
  ...
  sessionCache = SESSIONS.map(([label, factory]) => ({ label, ...replaySession(factory, loc_59d1) }));
  return sessionCache;
}
...
const TWINS = [
  ["no-op", brokenNoOp, [], [119, 78, 140]],
  ["forwards-the-pointer", brokenForwardsPointer, [], [119, 78, 140]],
  ["not-doubled", brokenNotDoubled, [], [119, 78, 140]],
  ["quadrupled", brokenQuadrupled, [], [119, 78, 140]],
  ["doubled-first-only", brokenDoubledFirstOnly, DOUBLED_FIRST_ONLY_SURVIVORS, [111, 39, 103]],
  ["rung-below", (m) => doubledVelocityForHeading(m, RUNG_BELOW), [], [119, 78, 140]],
  ["rung-above", (m) => doubledVelocityForHeading(m, RUNG_ABOVE), [], [119, 78, 140]],
  ["bottom-rung", (m) => doubledVelocityForHeading(m, LADDER[0]), [], [119, 78, 140]],
  ["top-rung", (m) => doubledVelocityForHeading(m, LADDER[5]), [], [119, 78, 140]],
  ["off-by-one-entry", brokenOffByOneEntry, OFF_BY_ONE_SURVIVORS, [115, 78, 140]],
  ["misaligned-by-a-byte", brokenMisaligned, [], [119, 78, 140]],
  ["not-perpendicular", brokenNotPerpendicular, [], [119, 78, 140]],
  ["quarter-reversed", brokenQuarterReversed, QUARTER_REVERSED_SURVIVORS, [111, 39, 103]],
  ["pair-swapped", brokenPairSwapped, [], [119, 78, 140]],
  ["heading-off-the-record", brokenHeadingOffTheRecord, RECORD_HEADING_SURVIVORS, [114, 78, 118]],
];

// ── the gate ────────────────────────────────────────────────────────────────────────────

test("CONTRACT: unitEquivalence at the first real dispatch, RAM identical", { skip }, () => {
  const r = gate(loc_59d1);
...
test("TAPE REACH: all three sessions reach this entry", { skip }, () => {
  const seen = sessions();
...
test("EXHAUSTIVE: all 256 headings crafted off the real entry are identical", { skip }, () => {
  for (const heading of everyHeading) {
    const d = unitDiff(loc_59d1, selector(heading));
...
test("POINTER SWEEP: the incoming pointer is discarded at every heading", { skip }, () => {
  ...
  for (const pointer of pointers) {
    for (const heading of everyHeading) {
      const d = unitDiff(loc_59d1, withPointer(heading, pointer));
