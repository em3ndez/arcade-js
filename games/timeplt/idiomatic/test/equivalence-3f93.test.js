...
/**
 * A CAP the moved registers must fall inside, never a set they are required to fill. Derived from
 * the ORACLE's one caller, which resumes with `push ix / push iy / ld d,(iy+0x31)` and reaches its
 * next use of the accumulator at `ld a,(0xad04)` — a WRITE — so neither the accumulator nor the
 * flag byte outlives this entry. `sp` moves because the frozen chain performs the ROM `ret` and no
 * rewrite here does. Written as a cap deliberately: a rewrite that came to AGREE on one of these
 * would be an improvement, and a gate demanding the difference would go red on it.
 */
...
test("EQUAL at the real dispatch: identical outside the scratch window", { skip }, () => {
  ...
  assert.ok(
    ...
    `a register outside the cap moved — ${moved.join(", ")} — and only the capped ones die here`,
  );
