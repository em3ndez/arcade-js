...
/**
 * loc_2207 — the 50m board-object state-machine dispatcher: gate on the 50m board,
 ...
 *        state 0 -> loc_2227  (dwell timer; on a Mario hit stamp the shared flag)
 *        state 1 -> loc_2259  (step the position counter UP, advance state at the top)
 *        state 2 -> loc_2299  (randomised dwell, then advance to the next state)
 *        state 3 -> loc_22a2  (step the counter DOWN, reset the object at the bottom)
 *
 ...
 * that pop into an honest parameter (loc_2259 / loc_2299 / loc_22a2 take the record base as
 * an argument), so they are called directly with it. loc_2227 is still oracle-shaped — it
 * takes its base with a stack pop — so the base is pushed right before it for that pop. That
 * one push is genuine stack-ABI marshalling; it dissolves once loc_2227 takes a parameter.
 *
 ...
 * records drive is not corroborated to the routine-name bar, matching its arms loc_2227 /
 * loc_2259 / loc_2299 / loc_22a2, which all keep loc_ under the same caution.
 *
...
import { loc_2227 } from "./loc_2227.js"; // ROM 0x2227 — state 0 arm (still pops its base off the stack)
import { loc_2259 } from "./loc_2259.js"; // ROM 0x2259 — state 1 arm
import { loc_2299 } from "./loc_2299.js"; // ROM 0x2299 — state 2 arm
import { loc_22a2 } from "./loc_22a2.js"; // ROM 0x22A2 — state 3 arm
...
export function loc_2207(m) {
  ...
  switch (state) {
    case 0:
      // loc_2227 still takes its record base with a stack pop, so hand it the base on the
      // stack — the genuine stack-ABI marshalling that dissolves once loc_2227 takes a param.
      m.push16(recordBase);
      return loc_2227(m);
    case 1:
      return loc_2259(m, recordBase);
    case 2:
      return loc_2299(m, recordBase);
    case 3:
      return loc_22a2(m, recordBase);
    default:
      ...
      throw new NotImplemented(
        `loc_2207: object-state dispatch on unexpected state ${state}`,
      );
  }
}
