...
/**
 * loc_2284 — step Mario down one pixel, held in the climb-down pose.  ROM 0x2284.
 *
 * The Y-descend tail of loc_2259: when that routine finds Mario's screen Y below 0x68
 ...
 *     that sprite record's Y field — both done by loc_3fc0.
 ...
 *
 * NAME: kept the neutral loc_ — the mechanism (drop Mario a pixel, hold the climb pose)
 * is pinned to the oracle and grounded in named cells, but which scripted descent this
 * serves is not yet corroborated to the routine-name bar. Promote once it is.
 *
 * Memory-equivalent to the frozen oracle — equivalence-2284.test.js.
 * GATE:     crafted-entry — 0x2284 is reached only through loc_2259's still-translated
 ...
 * LIVE-OUT: memory-only. This routine is tail-reached from loc_2259, itself tail-reached
 *           from sub_2207's dispatcher, which returns into loc_197a at its next call site
 ...
 *           0x694D) and Y byte (MARIO_SPRITE_RECORD+SPRITE_Y, 0x694F) are inside loc_3fc0.
 */

import { MARIO_Y } from "./ram.js";
import { loc_3fc0 } from "./loc_3fc0.js"; // ROM 0x3FC0 — pin the climb pose, return the sprite Y pointer

export function loc_2284(m) {
  const { mem } = m;
