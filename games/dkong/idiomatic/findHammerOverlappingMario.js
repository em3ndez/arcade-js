 ...
 * LIVE-OUT: registers as the search leaves them — a hit/miss byte, and on a hit the record count
 * minus the matched index. Returns [touching, matched], the same two values, for its caller.
 */
...
export function findHammerOverlappingMario(m) {
  // Seat the register-bridge inputs (they ride the return per the ABI-seam idiom), run the search
