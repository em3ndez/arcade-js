...
/**
 * hold50mObjectParked — one arm of the dispatch50mObjectState board-object state machine: tick this object's
 ...
 *
 * dispatch50mObjectState walks an 8-byte object record (its base is pushed on the stack and this arm
 ...
 *
 * ORACLE BOUNDARY: the record base arrives on the stack, pushed by dispatch50mObjectState — which is
 * still the frozen lift — so it is received with a stack pop here, exactly as the lift
 * takes it. When dispatch50mObjectState is brought across, this dissolves into a parameter. Likewise
 ...
 *           (dispatch50mObjectState's board gate skips its whole body there), so realistic states are
 *           captured at the reachable ancestor dispatch50mObjectState (ROM 0x2207) and a valid record
...
export function hold50mObjectParked(m) {
  ...
  // The record base was pushed by dispatch50mObjectState (still the frozen lift) — receive it here.
  const base = m.pop16();
