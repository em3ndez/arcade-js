...
/**
 * loc_0478 — shift the sprite-object block's X column by a board-specific delta, then
 * run the per-frame colour-cycle repaint.  ROM 0x0478.
 *
 * The even-board arm of the per-frame colour cascade: the dispatcher (loc_0450) routes
 * here when the current board's low bit is clear — the 50m and 100m boards. It nudges
 * the whole sprite-object row horizontally by an amount that depends on which of those
 * boards is active, then falls into the colour-cycle repaint:
 *
 *   - 50m (board bit 1 SET)   -> the shift comes from a RAM byte staged for this board.
 ...
 *           delta from 0x63b7) and BOARD == 4 (bit-1-clear arm, default +0x44), each with
 *           the incoming board-rotated byte set to match the oracle's register read, plus
 *           a 0x63b7-varied entry proving the RAM delta is really read. Whole contract
 ...
 * NAMES:    SPRITE_OBJ_BLOCK (0x6908), BOARD (0x6227) from ram.js. The shift-delta source
 *           0x63b7 is UNNAMED in ram.js (a thin, board-scoped scratch byte) — kept a local
 *           hex const for the lead to name later. The default shift and the board-bit mask
 *           are immediates.
 */

import { SPRITE_OBJ_BLOCK, BOARD } from "./ram.js";
...
const SHIFT_DELTA_SOURCE = 0x63b7; // 50m's staged X-shift delta (unnamed in ram.js — kept hex)
...
export function loc_0478(m) {
  ...
  const shiftX = (board & BOARD_BIT1) !== 0 ? mem.read8(SHIFT_DELTA_SOURCE) : DEFAULT_SHIFT;
