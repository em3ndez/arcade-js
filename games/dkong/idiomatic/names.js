...
/** [seen] 10 sprite records (stride 4) inside SPRITE_BUFFER (0x6980-0x69A7); entry_2e04 mirrors the
...
/** [seen] Effect sprite record (4 bytes at 0x6A2C inside SPRITE_BUFFER, immediately before
 ...
 *  transitions, tied to EFFECT_SEQ_STATE) — reach 0x6A2D as EFFECT_SPRITE + SPRITE_CODE. [seen] because
...
/** [seen] Score-popup sprite record inside SPRITE_BUFFER; awardScorePopup writes +0=MarioX, +1=glyph,
 ...
 *  spawnRequestedFireAndRecolorLiveFires — plus 5 board-init writes at pc 0x0F69/0x1230. The hammer A/B is the causal control. */
export const OBJ_SPRITE_ATTR = 0x08;
/** [seen] Object-record field (+0x18): spawnRequestedFireAndRecolorLiveFires's pending-insert marker. Grounded live: takes {0,1}, held
...
/** [seen] The two-record hammer pair's IN-PLAY flag (+0x01 of OBJ_PAIR_6680 only — 0x6681 / 0x6691).
...
/** [seen] Object-record field: per-object collision half-extent on the X axis (+0x09) — the extra span
...
/** [seen] Saved iterator pointer (word) walking the 0x6400 stride-0x20 array; entry_31b1 seeds/advances
 ...
 *  616 / 328); in the tight A/B the first frame that differs carries a single blob at cols 101..114,
 *  rows 231..245 — this array's logged record position to the pixel; and boxes drawn at the logged
 *  positions land on a fireball and nothing else on every board (2 records for all 2176 gameplay
 *  frames on 75m, 5 on 100m spawned one per 128 frames). The record activates at (X=39, Y=232) every
 ...
 *  its probes read records 0..4 and nothing above. [seen] from the lift, not from MAME: the board-4
...
/** [seen] Object pair, stride 0x10, 2 records (0x6680 / 0x6690); seedSpriteObjectPair, gathered to 0x6A18. */
...
/** [seen] THE BARRELS. Object array, stride 0x20, 10 records spanning page 0x68 (0x6700-0x6840);
 *  sub_2880 / entry_2c8f / sub_1f72. Board 1 only (ROM 0x1F72 returns unless BOARD == 1); during
 *  gameplay it is identically zero on 50m, 75m and 100m. IDENTITY GROUNDED on the real ROM under
 *  MAME 0.288, natural zero-poke 25m (scratchpad/grounding-object-arrays.md): forcing the ten
 *  records' +0 to 0 collapses barrel motion 328 -> 29 and produces no new barrel, while the fireball
 *  is untouched (34/40 frames vs baseline 35/40); pinning the records' +3 X every frame parks EVERY
 *  barrel at the commanded column (baseline spans cols 0..173; pinned to X=0x28 it spans 0..62, to
 *  X=0xA0 it clusters at ~145) and nothing else on screen moves — a working positive control, which
 *  the fire array does not have. Live sprite codes are six barrel frames x the 0x80 flip bit.
 *  Records 0-8 grounded; records 6..9 never activated together in any run.
 *  Field-level: record-0's fields all took live values (full-playfield barrel) in 25m attract. */
...
/** [seen] Active-player index; value-lockstep mirror of CURRENT_PLAYER (loc_141e/144f/13aa/13bb write
...
/** [seen] (own byte = constant 2 -- the colour/attr field -- whenever Mario active; the {0,2} 7 transitions are just board-boundary clears, NOT attr variation; no live attr value observed, LOW confidence. RUN-2P) Colour/attribute byte of Mario's sprite record (byte +2), copied to 0x694E by entry_1da6
 ...
 *  0x1D28/0x1D2E). On 25m 0x621B was the smaller (top). CAUTION: the two writer paths (armMarioClimbAtLadderEnd vs
...
// ── Score & bonus ────────────────────────────────────────────────────────────
/** [seen] (base = the low BCD pair, structurally ALWAYS 0x00 -- DK scores are x100; write-tap 0x60B2 wrote only 0x00. The live score is the NEIGHBOR middle byte 0x60B3 {0x00,0x01,0x37}=100/3700 pts, PROBE-SR -- so the base's own byte was never seen taking a meaningful live value) Player 1 score: 3-byte LITTLE-endian packed BCD (0x60B4 = most-significant pair). Award opcode 0
...
/** [seen] 1 = a prize/item was just collected; set at pickup (edge / airborne collision / rivet),
...
/** [seen] Base of the 50m board-object records. initBoardState (ROM 0x0F56) copies the template
...
/** [seen] Spawn-cadence timer; at 0 sub_27da claims a free 0x6600 slot, seeds it, reloads 0x34; always
 *  decrements. */
export const SPAWN_TIMER = 0x62a7;
/** [seen] Base of the per-board type-0 object-init table (stride 5); loadBoardObjectRecords de-interleaves
 *  type-0 records here, sub_2441/sub_236e consume it. Exact record layout inferred. */
export const OBJ_PARAM_TABLE0 = 0x6300;
/** [seen] Base of the per-board type-1 object-init table (parallel to TABLE0, IY-indexed). */
export const OBJ_PARAM_TABLE1 = 0x6310;
/** [seen] Spawn request; sub_2fcb stores 3 each bonus period, loc_2ea7 tests bit0 -> activates the
 *  object (+0:=1, position/appearance seeded) and clears it to 0. */
export const SPAWN_REQUEST = 0x6396;
/** [seen] One-shot "Mario Y just repositioned" flag; sub_29af sets 1 right after writing MARIO_Y, read
 ...
 *  raise it := 1 — raisePeriodicObjectSpawnRequests's difficulty-scaled periodic trigger (50m/100m) and animateFixedHazardAndReleaseFire's
 *  ARM_COUNTER-underflow re-arm (animateFixedHazardAndReleaseFire does not read it back — one reader only). Grounded
...
/** [seen] Per-scan tally of the live (non-empty) records spawnRequestedFireAndRecolorLiveFires counts while sweeping OBJ_ARRAY_64; the
 *  scan returns a caller-skip when it lands on zero. Grounded live: the SOLE writer in the entire ROM is
 *  ROM 0x313C (verified by grep over the frozen translated/ oracle). The own byte took {0,1,2,3,4,5} across
...
/** [seen] Source char-string pointer (word); walked to a 0x7F terminator, stored back each step. */
...
/** [seen] Destination pointer (word); the renderer writes the 4-byte record here (a slot inside
 *  SPRITE_BUFFER, 0x6980+(10-B)*4). */
export const RENDER_DST_PTR = 0x62ac;

// ── Effect subsystem ─────────────────────────────────────────────────────────
/** [seen] Effect state / 4-way rst-0x28 router (sub_1dbd); pickup/hit sites raise it to 1. */
...
/** [seen] Effect param pointer (word); indirect base of the hit record, deref'd by stageAwardPopupAtHitObject. */
export const EFFECT_PARAM_PTR = 0x6343;
/** [seen] Effect-sequence state / 3-way rst-0x28 router (sub_1e96); re-arms EFFECT_STATE on completion. */
export const EFFECT_SEQ_STATE = 0x6345;
/** [seen] Effect-sequence INNER countdown; decremented first each tick, steps the outer when it drains. */
export const EFFECT_SEQ_INNER = 0x6346;
/** [seen] Effect-sequence OUTER countdown; decremented when the inner hits 0, advances EFFECT_SEQ_STATE on 0. */
...
/** P1_SCORE_TEMPLATE_ROM (0x01ba) — ROM 9-byte initial score-display template, copied into P1_SCORE by powerOnInit. [seen] */
...
/** IN_GAME_SUBSTATE_TABLE (0x0702) — ROM base of the 29-entry in-game sub-state rst-0x28 jump table, indexed base + (2*substate & 0xff) by dispatchInGameSubstate. [seen] */
...
/** RIVET_TILE_DEST_TABLE (0x0d17) — ROM address of the 8-entry little-endian pointer table of VRAM destination cells stamped with the fixed rivet tile pair (0xB8 then 0xB7) during 100m r… [seen] */
...
/** BOARD_CLEARED_STEP_TABLE_50M (0x1637) — ROM 5-entry table of little-endian target addresses [16a3,16bb,1732,1757,178e] for the 50m (BOARD bit1) board-cleared interlude; indexed by BOARD_ADVA… [seen] */
export const BOARD_CLEARED_STEP_TABLE_50M = 0x1637;
/** RIVET_INTERLUDE_STEP_TABLE (0x1648) — ROM 6-entry inline jump table of little-endian targets for the rivet/100m board interlude sequence; indexed by BOARD_ADVANCE_STEP (0x6388) doubled wit… [seen] */
export const RIVET_INTERLUDE_STEP_TABLE = 0x1648;
/** AIRBORNE_PROBE_RETURN (0x1c08) — ROM CODE-continuation address (not a data cell): return address pushed by loc_1c05 before call 0x2b1c (descent probe) so the callee unwinds back into … [seen] */
export const AIRBORNE_PROBE_RETURN = 0x1c08;
/** AIRBORNE_OVERLAP_SEARCH_RETURN (0x1c23) — ROM CODE-continuation address: return address pushed by loc_1c05 before searchPlayerObjectOverlap on the fall-height trigger frame, so the callee retu… [seen] */
export const AIRBORNE_OVERLAP_SEARCH_RETURN = 0x1c23;
/** BARREL_ROLL_GATE_RETURN (0x2017) — ROM CODE-continuation address: return address pushed by advanceRollingBarrel before call 0x24b4 (bottom-of-playfield gate); consumed/discarded by the … [seen] */
export const BARREL_ROLL_GATE_RETURN = 0x2017;
/** HAMMER_HIT_HANDLER_RETURN (0x283e) — ROM CODE-continuation address: return address pushed by recordHammerHitOnObject before dispatchBoardCollision; the board collision handler unwinds by … [seen] */
export const HAMMER_HIT_HANDLER_RETURN = 0x283e;
/** BOARD_COLLISION_TABLE (0x2874) — ROM 6-entry jump table of little-endian collision-handler addresses, indexed by BOARD (0x6227: 1=25m,2=50m,3=75m,4=100m; 0/5 null guards), doubled wit… [seen] */
export const BOARD_COLLISION_TABLE = 0x2874;
/** LIVE_FIRE_ADVANCE_RETURN (0x31d0) — ROM CODE-continuation address: return address pushed by advanceLiveFires before call 0x3202 (per-object fire advance), which pops it. Manual-stack sea… [seen] */
export const LIVE_FIRE_ADVANCE_RETURN = 0x31d0;
/** FIRE_STATE_MACHINE_RETURN (0x3233) — ROM CODE-continuation address: return bracket pushed by advanceFire before call 0x333d (driveFireLadderClimb heading/collision state machine); two cal… [seen] */
export const FIRE_STATE_MACHINE_RETURN = 0x3233;
/** OPTION_TABLE_ROM (0x3565) — ROM source of the fixed 0xAA (170)-byte option/attract table copied verbatim into work RAM at 0x6100 by decodeDipSwitches at power-on. [seen] */
export const OPTION_TABLE_ROM = 0x3565;
/** BONUS_ITEM_POSITION_TABLE (0x360f) — ROM table of (X,Y) position pairs for the on-board bonus-item sprite; positionBonusItemSprite indexes it by 2*C, entry supplies X then Y stamped into … [seen] */
export const BONUS_ITEM_POSITION_TABLE = 0x360f;
/** STRING_DESCRIPTOR_PTR_TABLE (0x364b) — ROM table of little-endian pointers to string descriptors; drawStringVertical indexes it by the doubled payload (bit7 masked off) to reach a descripto… [seen] */
export const STRING_DESCRIPTOR_PTR_TABLE = 0x364b;
/** INTRO_SETUP_LAYOUT_TABLE (0x380d) — ROM board-layout segment table passed (in DE) to drawBoardLayout by setupIntroCutsceneStep to draw the intro cutscene's step-0 static playfield. Disti… [seen] */
...
/** INTRO_WALK_TABLE_A_ROM (0x38b4) — ROM intro-walk table A (Kong-climb cutscene); setupIntroCutsceneStep seeds it into INTRO_WALK_PTR_A. A-side sibling of INTRO_WALK_TABLE_B_ROM. [seen] */
export const INTRO_WALK_TABLE_A_ROM = 0x38b4;
/** SPRITE_BASE_FIGURE_ROM (0x385c) — ROM ten-record "base figure" sprite-object template; stamped into SPRITE_OBJ_BLOCK then X-shifted +0x44 by the recapture/animation routines. [seen] */
export const SPRITE_BASE_FIGURE_ROM = 0x385c;
/** SPRITE_OBJECT_BLOCK_TEMPLATE (0x388c) — ROM base of the fixed 40-byte (0x28 = ten 4-byte records) sprite-object block template reloaded into SPRITE_OBJ_BLOCK via loadSpriteObjectBlock; same … [seen] */
export const SPRITE_OBJECT_BLOCK_TEMPLATE = 0x388c;
/** INTRO_WALK_TABLE_B_ROM (0x38cb) — ROM intro-walk table B (per-step signed-Y-delta list); setupIntroCutsceneStep seeds it into INTRO_WALK_PTR_B and loc_0b68 walks it. [seen] */
...
/** SPRITE_OBJ_ANIM_FRAME_TABLE (0x3932) — ROM base of a table of 40-byte sprite-object animation-frame records (ten 4-byte sprite records each). advanceBarrelRelease reads a sub-counter and co… [seen] */
...
/** SPRITE_ANIM_FIGURE_A_ROM (0x39cf) — ROM sprite animation figure A, one of the two figures the rate-limited animation sequence alternates (twin of SPRITE_ANIM_FIGURE_B_ROM). [seen] */
export const SPRITE_ANIM_FIGURE_A_ROM = 0x39cf;
/** SPRITE_ANIM_FIGURE_B_ROM (0x39f7) — ROM sprite animation figure B, the alternate figure of SPRITE_ANIM_FIGURE_A_ROM. [seen] */
export const SPRITE_ANIM_FIGURE_B_ROM = 0x39f7;
/** BOARD_ADVANCE_INTERLUDE_SPRITE_FRAME (0x3a1f) — ROM base of this board-advance interlude step's ten-record (40-byte) sprite-object frame, copied over SPRITE_OBJ_BLOCK by loc_186f when SUBSTATE_TIMER… [seen] */
export const BOARD_ADVANCE_INTERLUDE_SPRITE_FRAME = 0x3a1f;
/** HOWHIGH_GIRDER_BLOCK0_SEG_ROM (0x3a47) — ROM girder/ladder layout-segment table for how-high tile block 0 (paired with HOWHIGH_GIRDER_BLOCK0_VRAM). [seen] */
export const HOWHIGH_GIRDER_BLOCK0_SEG_ROM = 0x3a47;
/** HOWHIGH_TILE_BLOCK2_SEG_ROM (0x3a4d) — ROM layout-segment table for how-high tile block 2. [seen] */
export const HOWHIGH_TILE_BLOCK2_SEG_ROM = 0x3a4d;
/** HOWHIGH_TILE_BLOCK3_SEG_ROM (0x3a53) — ROM layout-segment table for how-high tile block 3. [seen] */
export const HOWHIGH_TILE_BLOCK3_SEG_ROM = 0x3a53;
/** HOWHIGH_TILE_BLOCK4_SEG_ROM (0x3a59) — ROM layout-segment table for how-high tile block 4. [seen] */
export const HOWHIGH_TILE_BLOCK4_SEG_ROM = 0x3a59;
/** INTERLUDE_LAYOUT_SEGMENT_TABLE (0x3a5f) — ROM board-layout line-segment table for the between-boards interlude scene; loc_1880 hands it to drawBoardLayout (via regs.de) on the single frame the… [seen] */
export const INTERLUDE_LAYOUT_SEGMENT_TABLE = 0x3a5f;
/** BOARD_SEQ_RESTART_L5_ROM (0x3a73) — ROM head of the L5+ board-order group; on the 0x7F terminator BOARD_SEQ_PTR reloads here so levels 5+ repeat forever. [seen] */
...
/** WALK_PATH_TABLE_3478 (0x3aac) — ROM scripted position-walk path table for the loc_3478 object walker (twin of loc_342c but its OWN table); a fresh walk aims here and picks direction … [seen] */
...
/** BOARD_LAYOUT_TABLE_50M (0x3b5d) — ROM board-2 (50m conveyors) layout/object-record data table; drawn via loc_0cc6 (setup50mConveyorBoard sets regs.de=0x3b5d inline) and read as object-… [seen] */
export const BOARD_LAYOUT_TABLE_50M = 0x3b5d;
/** BOARD_LAYOUT_TABLE_75M (0x3be5) — ROM board-3 (75m elevators) layout/object-record data table; drawn via loc_0cc6 (setUp75mBoard sets regs.de=0x3be5 inline) and read as object-init rec… [seen] */
export const BOARD_LAYOUT_TABLE_75M = 0x3be5;
/** BOARD_LAYOUT_TABLE_RIVET (0x3c8b) — ROM 100m-rivet layout table (board 4 and any BOARD past the first three / board 0); buildBoard's rivet arm hands it to loc_0cc6, and loadBoardObjectRe… [seen] */
export const BOARD_LAYOUT_TABLE_RIVET = 0x3c8b;
/** HOW_HIGH_CLIMB_FIGURE_TABLE (0x3cf0) — ROM table of 4-byte climbing-figure sprite records (3 bytes read, 1 skipped) painted onto the 'HOW HIGH CAN YOU GET?' girder stack by buildHowHighScre… [seen] */
...
/** BOARD_OBJ_SCRATCH_TEMPLATE_ROM (0x3d9c) — ROM 0x40-byte source of the per-board object-scratch initializer, copied into BOARD_OBJ_SCRATCH (0x6280) by initBoardState. [seen] */
...
/** BONUS_ITEM_POS_DIVIDER (0x6030) — [seen] Frame divider between grid-position steps in runBonusItemValueDisplay: reloaded to 0x0a, decremented each frame a lateral input (P1_INPUT & 0x0… [code] */
export const BONUS_ITEM_POS_DIVIDER = 0x6030;
/** BONUS_ITEM_SPRITE_TOGGLE (0x6031) — [seen] Two-state sprite-source toggle for the bonus-item animation: flipped 0<->1 each ANIM_TIMER expiry to select source = SLOT_PTR+3 (live slot spri… [code] */
export const BONUS_ITEM_SPRITE_TOGGLE = 0x6031;
/** BONUS_ITEM_ANIM_TIMER (0x6032) — [seen] Bonus-item sprite-animation countdown in runBonusItemValueDisplay: armed 0x10, decremented per frame; on expiry it flips SPRITE_TOGGLE, re-rend… [code] */
export const BONUS_ITEM_ANIM_TIMER = 0x6032;
/** BONUS_ITEM_VALUE (0x6033) — [seen] Decrementing point value shown by runBonusItemValueDisplay: initialised 0x1e, decremented one on each DISPLAY_TIMER expiry; at 0 the item is to… [code] */
export const BONUS_ITEM_VALUE = 0x6033;
/** BONUS_ITEM_DISPLAY_TIMER (0x6034) — [seen] Display-rate countdown gating the value step in runBonusItemValueDisplay: armed 0x3e, decremented per frame; on 0 it reloads 0x3e and decrement… [code] */
export const BONUS_ITEM_DISPLAY_TIMER = 0x6034;
/** BONUS_ITEM_POS_INDEX (0x6035) — [seen] Grid-position index (0..0x1d) of the bonus item in runBonusItemValueDisplay: stepped by held left/right input via POS_DIVIDER, fed to positionB… [code] */
export const BONUS_ITEM_POS_INDEX = 0x6035;
/** BONUS_ITEM_VIDEO_PTR (0x6036) — [seen] 16-bit video-RAM write pointer for the bonus-item value column in runBonusItemValueDisplay: init VIDEO_BASE 0x75e8, walked +/-0x20 as the colum… [code] */
export const BONUS_ITEM_VIDEO_PTR = 0x6036;
/** BONUS_ITEM_SLOT_PTR (0x6038) — [seen] 16-bit pointer to the current player's slot record found by the 0x22-stride scan (key = rotl8(ACTIVE_PLAYER_INDEX)+1) over PLAYER_SLOT_RECORDS … [code] */
export const BONUS_ITEM_SLOT_PTR = 0x6038;
/** BONUS_ITEM_SLOT_COL_PTR (0x603a) — [seen] 16-bit destination pointer (SLOT_PTR-0x0d) in runBonusItemValueDisplay: on teardown the 0x0c-byte video value column (walked from VIDEO_BASE by… [code] */
export const BONUS_ITEM_SLOT_COL_PTR = 0x603a;
/** SND_TRIGGER_EFFECT (0x6085) — [seen] Slot 5 of the 8-entry SND_TRIGGER latch array (base 0x6080, ls259.6h): the score-popup/item effect sound. stampScorePopupSprite stores 3 to ass… [code] */
...
/** SCORE_SORT_TABLE_KEY (0x61a5) — [seen] Compare anchor (HL start) for loc_13ca's descending high-score bubble sort: the 3-byte little-endian score key of the record one 0x22-stride sl… [code] */
export const SCORE_SORT_TABLE_KEY = 0x61a5;
/** SCORE_SORT_DIGITS (0x61b1) — [seen] Base of the 21-byte display field of loc_13ca's staging high-score record: 6 unpacked BCD digits (MS first) + 14 blank tiles (0x10) + a 0x3f te… [code] */
export const SCORE_SORT_DIGITS = 0x61b1;
/** SCORE_SORT_TAG (0x61c6) — [seen] Player-tag byte of loc_13ca's staging record: the A-register tag stored here first (before the game-active guard); sits between the display dig… [code] */
export const SCORE_SORT_TAG = 0x61c6;
/** SCORE_SORT_STAGING_KEY (0x61c7) — [seen] 3-byte packed-BCD score key of the NEW staging record in loc_13ca: ldir-copied from the live score pointer (HL), then used as the DE compare/sw… [code] */
export const SCORE_SORT_STAGING_KEY = 0x61c7;
/** MARIO_CLIMB_TOGGLE (0x6219) — [seen] Mario climb-state byte in the climb region (adjacent to CLIMB_FLAG 0x621a, MARIO_CLIMB_LIMIT_A/B 0x621b/c): cleared to 0 by endClimbAtLadderLim… [code] */
export const MARIO_CLIMB_TOGGLE = 0x6219;
/** CLIMB_CENTERING_PHASE (0x6222) — [seen] Shared two-phase ladder-centering toggle: advanceClimbStep XORs it 0<->1 each climb step to gate the centering arm vs the frame-select/limit ar… [code] */
export const CLIMB_CENTERING_PHASE = 0x6222;
/** FIXED_HAZARD_PRESCALER (0x62b8) — 4-frame prescaler of the fixed-hazard/fire machine (animateFixedHazardAndReleaseFire): decremented each qualifying pass, returns until underflow, then… [seen] */
export const FIXED_HAZARD_PRESCALER = 0x62b8;
/** FIXED_HAZARD_ARM_COUNTER (0x62ba) — Bit1-arm down-counter of the fixed-hazard/fire machine (animateFixedHazardAndReleaseFire): decremented on the bit1-set arm; on underflow it resets PHA… [seen] */
export const FIXED_HAZARD_ARM_COUNTER = 0x62ba;
/** BARREL_DIFFICULTY_LATCH (0x6348) — One-way barrel behavior/difficulty-mode latch (0=simple/default, nonzero=difficulty-graded). retireBarrelIntoOilDrum sets it to 1 one-way on the first… [seen] */
export const BARREL_DIFFICULTY_LATCH = 0x6348;
/** PALETTE_ANIM_TIMER (0x638a) — Countdown timer of loc_07cb's timed palette-pattern animation sub-state, paired with 0x638b. 0 arms a fresh 0x60-frame run; otherwise ticks down; on r… [seen] */
export const PALETTE_ANIM_TIMER = 0x638a;
/** PALETTE_ANIM_PATTERN (0x638b) — Rotating pattern byte paired with PALETTE_ANIM_TIMER (loc_07cb): each active frame its top two bits are streamed into the write-only palette latches 0… [seen] */
export const PALETTE_ANIM_PATTERN = 0x638b;
/** BARREL_RELEASE_ARMED (0x6392) — 25m barrel-release 'a release is armed this pass' flag (bit0). armBarrelRelease raises it to 1 on entry; driveBarrelRelease reads bit0 to decide wheth… [seen] */
export const BARREL_RELEASE_ARMED = 0x6392;
/** FIRE_SWEEP_INDEX (0x63a2) — Loop-counter cell for advanceLiveFires' per-frame fire-array sweep: zeroed at sweep start, re-read from memory and incremented once per iteration (OBJ… [seen] */
export const FIRE_SWEEP_INDEX = 0x63a2;
/** CLIMB_FIGURE_INDEX (0x63a7) — Index into the ROM climb-figure record table (0x3cf0, 4-byte records, <<2) for the 'HOW HIGH' screen (buildHowHighScreen): reset to 0, incremented onc… [seen] */
export const CLIMB_FIGURE_INDEX = 0x63a7;
/** CLIMB_FIGURE_WALK_PTR (0x63a8) — 16-bit VRAM write pointer (word cell 0x63a8/0x63a9) for the climbing figures on the 'HOW HIGH' screen (buildHowHighScreen): seeded to 0x76dc, each row… [seen] */
export const CLIMB_FIGURE_WALK_PTR = 0x63a8;
/** SPRITE_OBJ_REC4_CODE (0x6919) — Byte at SPRITE_OBJ_BLOCK(0x6908)+0x11 = sprite-object block record 4's code byte (test equivalence-1880 pins REC4_CODE=SPRITE_OBJ_BLOCK+0x11==0x6919).… [seen] */
export const SPRITE_OBJ_REC4_CODE = 0x6919;
/** MARIO_SPRITE_Y (0x694f) — Y byte of Mario's sprite record = MARIO_SPRITE_RECORD(0x694c) + SPRITE_Y(0x03) (already an export at 0x694c; test equivalence-2259 treats 0x694f as MA… [seen] */
export const MARIO_SPRITE_Y = 0x694f;
/** BONUS_COUNTDOWN_SPRITES (0x69a8) — Base of the 25m bonus-countdown sprite group: four 4-byte sprite records (0x10 bytes), block-copied from ROM 0x3ddc at board build (seed25mBoardObject… [seen] */
export const BONUS_COUNTDOWN_SPRITES = 0x69a8;
/** M50_OBJ2_SPRITE_PAIR (0x69ec) — Base of 50m object-2's mirrored sprite-code record (loc_264c): HL=this base is passed to loc_26a6 every 32nd frame to advance the pair; low cell base+… [seen] */
export const M50_OBJ2_SPRITE_PAIR = 0x69ec;
/** M50_OBJ2_SPRITE_PAIR_LOW (0x69ed) — Low cell of 50m object-2's mirrored sprite-code pair (M50_OBJ2_SPRITE_PAIR base+1): on the 32nd-frame arm loc_264c re-stamps it from the high cell wit… [seen] */
export const M50_OBJ2_SPRITE_PAIR_LOW = 0x69ed;
/** M50_OBJ3_SPRITE_PAIR_BASE (0x69f4) — [seen] 50m board: base/anchor for object-3's mirrored sprite tile-code counter PAIR inside SPRITE_BUFFER. loc_268d loads HL=0x69f4 and calls loc_26a6,… [code] */
export const M50_OBJ3_SPRITE_PAIR_BASE = 0x69f4;
/** HAMMER_OBJ2_SPRITE_RECORD (0x6a1c) — [seen] 4-byte sprite-shadow record (X,code,attr,Y) in SPRITE_BUFFER for the SECOND hammer object (OBJ_PAIR_6680's second record, base 0x6690). driveHa… [code] */
export const HAMMER_OBJ2_SPRITE_RECORD = 0x6a1c;
/** CUTSCENE_SPRITE_RECORD (0x6a20) — [seen] 4-byte sprite-shadow record (X,code,attr,Y) in SPRITE_BUFFER reused by between-board cutscene tableaus. spawnInterludeHeart seeds it 80/76/09/2… [code] */
export const CUTSCENE_SPRITE_RECORD = 0x6a20;
/** CUTSCENE_SPRITE_RECORD_2 (0x6a24) — [seen] Second 4-byte cutscene sprite-shadow record in SPRITE_BUFFER (adjacent to CUTSCENE_SPRITE_RECORD 0x6a20). loc_1880 (the 'how high' board-advanc… [code] */
export const CUTSCENE_SPRITE_RECORD_2 = 0x6a24;
/** CUTSCENE_SPRITE_2_CODE (0x6a25) — [seen] The +1 (sprite-code) field of CUTSCENE_SPRITE_RECORD_2 (0x6a24). loc_1880 writes it (=0x39) when staging the 7F/39/01/D8 record; runRivetBoardF… [code] */
export const CUTSCENE_SPRITE_2_CODE = 0x6a25;
/** OBJ_66A0_SPRITE_RECORD (0x6a28) — [seen] 4-byte sprite-shadow record in SPRITE_BUFFER mirroring OBJ_RECORD_66A0's first 4 bytes (X/code/attr/Y). loc_11fa scatters a 6-byte source into … [code] */
export const OBJ_66A0_SPRITE_RECORD = 0x6a28;
/** OBJ_66A0_SPRITE_CODE (0x6a29) — [seen] The +1 (sprite-code) field of OBJ_66A0_SPRITE_RECORD (0x6a28). animateFixedHazardAndReleaseFire stamps the fixed-hazard sprite byte here (0x40 … [code] */
export const OBJ_66A0_SPRITE_CODE = 0x6a29;
/** TILEMAP_BASE (0x7400) — [seen] Base of the 32x32 tilemap video RAM (0x7400-0x77FF; 0x400 cells, row stride 0x20). tileAddrForPixel computes 0x7400 + row*32 + col; clearTilema… [code] */
export const TILEMAP_BASE = 0x7400;
/** PLAYFIELD_TOP (0x7404) — [seen] Top-left cell of the central 28-column tilemap playfield interior (TILEMAP_BASE + 4). clearPlayfieldAndSprites blanks 32 rows x 28 cols startin… [code] */
export const PLAYFIELD_TOP = 0x7404;
/** BONUS_READOUT_TILE_BASE (0x7465) — [seen] First video-RAM cell of the on-screen bonus readout's tile frame. loc_062a stamps 18 bytes from ROM table 0x384a here (6 columns x 3 cells, str… [code] */
export const BONUS_READOUT_TILE_BASE = 0x7465;
/** LEVEL_UNITS_CELL (0x74a3) — [seen] Tilemap cell holding the units digit of the on-screen level number. drawLivesAndLevel decimal-splits LEVEL and writes the units tile here (tens… [code] */
export const LEVEL_UNITS_CELL = 0x74a3;
/** CREDIT_DIGITS_CELL (0x74bf) — [seen] Tilemap cursor for the 'CREDIT nn' count: high digit lands here, low digit one tilemap row UP (0x74bf-0x20, DE step 0xffe0). drawCreditDisplay … [code] */
export const CREDIT_DIGITS_CELL = 0x74bf;
/** LEVEL_TENS_CELL (0x74c3) — [seen] Tilemap cell holding the tens digit of the on-screen level number (paired with LEVEL_UNITS_CELL 0x74a3). drawLivesAndLevel writes the tens tile… [code] */
export const LEVEL_TENS_CELL = 0x74c3;
/** BONUS_READOUT_LOW_DIGIT_CELL (0x74c6) — [seen] Low (units) digit cell of the two-digit bonus readout field (inside the BONUS_READOUT_TILE_BASE frame; high digit one screen column later at 0x… [code] */
export const BONUS_READOUT_LOW_DIGIT_CELL = 0x74c6;
/** P2_INDICATOR_COLUMN_BASE (0x74e0) — [seen] Video-RAM column base of player 2's on-screen '2UP' indicator column. draw2UpLabel stamps '2'/'U'/'P' at 0x74e0, -0x20, -0x40 (ascending); sele… [code] */
export const P2_INDICATOR_COLUMN_BASE = 0x74e0;
/** TWO_DIGIT_FIELD_HIGH_CELL (0x74e6) — VRAM tilemap cell that stampTwoDigitField writes with the HIGH (tens) digit tile of its two-digit on-screen field; the low-digit partner is one screen… [seen] */
export const TWO_DIGIT_FIELD_HIGH_CELL = 0x74e6;
/** SCORE_VRAM_COLUMN_P2 (0x7521) — Destination VRAM column loc_056b selects (via IX) when its zero/nonzero selector is nonzero — player 2's score/counter column; the zero selector picks… [seen] */
...
/** INTRO_CUTSCENE_TILE_C (0x75aa) — Third of three fixed VRAM tiles stamped by setupIntroCutsceneStep in the opening Kong-climb intro cutscene setup; stamped with tile 0xD4 (A and B get … [seen] */
export const INTRO_CUTSCENE_TILE_C = 0x75aa;
/** HOW_HIGH_GIRDER_VRAM_BASE (0x75bc) — Base VRAM cell where buildHowHighScreen begins the descending girder-stack tile fill for the 'HOW HIGH CAN YOU GET?' screen. [seen] */
...
/** BONUS_ITEM_VALUE_COL_CEILING_SENTINEL (0x7608) — Ceiling sentinel of the bonus-item value's video-column walk (column top 0x75e8 + 0x20): when the upward advance (cur+0x20) reaches this address the p… [seen] */
export const BONUS_ITEM_VALUE_COL_CEILING_SENTINEL = 0x7608;
/** COLOR_COLUMN_A_TOP (0x7623) — Top cell of decorative colour column A (paired with column B at 0x7583) in the descending colour-cycle fill; painted by runRivetColorCycleBlink (base … [seen] */
export const COLOR_COLUMN_A_TOP = 0x7623;
/** INTRO_CUTSCENE_TILE_B (0x7663) — Second of three fixed VRAM tiles stamped by setupIntroCutsceneStep (value 0x10) in the intro cutscene setup. [seen] */
export const INTRO_CUTSCENE_TILE_B = 0x7663;
/** INTRO_CUTSCENE_TILE_A (0x76a3) — First of three fixed VRAM tiles stamped by setupIntroCutsceneStep (value 0x10) in the intro cutscene setup. [seen] */
export const INTRO_CUTSCENE_TILE_A = 0x76a3;
/** INTERLUDE_TILE_BLOCK_TOPLEFT (0x76c6) — Top-left VRAM start cell of the 70-tile (5x14) descending tile fill loc_1880 hands to loc_1826 when building the next between-boards interlude arrival… [seen] */
export const INTERLUDE_TILE_BLOCK_TOPLEFT = 0x76c6;
/** GAMEOVER_BANNER_TOPLEFT_2P (0x76d3) — Top-left VRAM cell of the game-over banner 5x14 tile block at its one-column-left (2-player) position: used directly by loc_1344 (the this-player/othe… [seen] */
export const GAMEOVER_BANNER_TOPLEFT_2P = 0x76d3;
/** GAMEOVER_BANNER_TOPLEFT_1P (0x76d4) — Top-left VRAM cell of the game-over banner 5x14 tile block at its default 1-player position; losePlayer1Life fills 70 tiles from here (shifting one co… [seen] */
export const GAMEOVER_BANNER_TOPLEFT_1P = 0x76d4;
/** HOW_HIGH_CLIMB_FIGURE_VRAM_START (0x76dc) — Initial VRAM walk-pointer value seeded into CLIMB_FIGURE_WALK_PTR (0x63a8) for the small climbing figure drawn on each girder of the 'HOW HIGH' screen… [seen] */
export const HOW_HIGH_CLIMB_FIGURE_VRAM_START = 0x76dc;
/** P1_INDICATOR_COLUMN_BASE (0x7740) — Video-RAM tilemap (0x7400-0x77ff) column-base cell of player-1's top-of-screen indicator/score column. selectPlayerIndicatorColumnBase returns it for … [seen] */
export const P1_INDICATOR_COLUMN_BASE = 0x7740;
/** P1_SCORE_COLUMN_BASE (0x7781) — Video-RAM tilemap column base for player-1's packed-BCD counter/score render, chosen by loc_056b when the selector is zero (nonzero/P2 twin 0x7521). D… [seen] */
export const P1_SCORE_COLUMN_BASE = 0x7781;
/** RESERVE_LIVES_MARKER_BASE (0x7783) — Bottom video-RAM tilemap cell of the reserve-lives marker column. drawLivesAndLevel blanks six slots upward from here (TILE_BLANK 0x10) then stamps on… [seen] */
export const RESERVE_LIVES_MARKER_BASE = 0x7783;
/** DMA_CH0_ADDR (0x7800) — i8257 DMA channel-0 memory-address register (board output, not work RAM); channel 0 = sprite-shadow-buffer source. Written twice (lo then hi) per the … [seen] */
export const DMA_CH0_ADDR = 0x7800;
/** DMA_CH0_COUNT (0x7801) — i8257 DMA channel-0 terminal-count register (board output); written lo-then-hi. Paired with the channel-0 address for the sprite-buffer source. [seen] */
export const DMA_CH0_COUNT = 0x7801;
/** DMA_CH1_ADDR (0x7802) — i8257 DMA channel-1 memory-address register (board output); channel 1 = sprite-RAM destination. Written lo-then-hi. [seen] */
export const DMA_CH1_ADDR = 0x7802;
/** DMA_CH1_COUNT (0x7803) — i8257 DMA channel-1 terminal-count register (board output); written lo-then-hi. Paired with the channel-1 address for the sprite-RAM destination. [seen] */
...
/** IN1_PORT (0x7c80) — Hardware input port IN1 (read): player-2 joystick/button port, read only in cocktail mode when ACTIVE_PLAYER_INDEX is nonzero. Sibling of IN0=0x7c00 (… [seen] */
export const IN1_PORT = 0x7c80;
/** IN0_PORT (0x7c00) — Hardware input port IN0 (read): player-1 joystick + jump button. Read device at the same address as the write-side SOUND_TUNE_LATCH (read and write are different devices). [seen] */
export const IN0_PORT = 0x7c00;
/** IN2_PORT (0x7d00) — Hardware input port IN2 (read): coin (bit7) + START1/START2 (bits2/3); the read also kicks the watchdog (dkong_in2_r). Read device at the SOUND_TRIGGER_LATCH write address. [seen] */
export const IN2_PORT = 0x7d00;
/** DSW0_PORT (0x7d80) — Hardware DIP-switch bank DSW0 (read); the driver map comment mislabels it DSW1 but the MAME port tag is DSW0. Read device at the SOUND_IRQ write address. [seen] */
...
/** DMA_DRQ (0x7d85) — i8257 DRQ request latch (board output): pulsed 0->1->0 in blitSpritesViaDma; the rising edge synchronously blits the 385-byte sprite shadow buffer to … [seen] */
export const DMA_DRQ = 0x7d85;
/** PALETTE_BANK_BIT0 (0x7d86) — LS259 palette/gfx-bank output latch, low bit (bit 0) of the 2-bit palette-bank select (write-only, unreadable). Cleared to 0 at power-on. Widely refer… [seen] */
...
/** FIRE_RECORDS_100M_CODE (0x64a7) — OBJ_SPRITE_CODE (+7) of the first 100m extra fire record (FIRE_RECORDS_100M+7); replicateGroupStrided seeds the appearance group into records 5-6. [seen] */
...
/** SPRITE_OBJ_REC2_X (0x6910) — X byte of sprite-object block record 2 (SPRITE_OBJ_BLOCK 0x6908 + 0x08); dispatchKongWalkFrame routes the Kong-walk sprite by this X vs the rail regio [seen] */
...
/** loc_6944 (0x6944) — Dest of an 8-byte (2 sprite-record) fixed-table block copy into SPRITE_BUFFER during 50m board object setup; specific per-record role unclear. [seen] (placeholder; names-debt) */
...
/** P2_SAVED_LEVEL (0x6049) — Player 2's saved LEVEL field (offset +1 of P2_CONTEXT 0x6048's 8-byte block: LIVES,LEVEL,SEQPTR_lo,SEQPTR_hi,...). ldir-restored to the live LEVEL 0x6229 at ROM [seen] */
...
/** CLIMB_FIGURE_WALK_PTR_HI (0x63a9) — High byte of the 16-bit CLIMB_FIGURE_WALK_PTR word (0x63a8): VRAM write pointer for the climbing figures on the 'HOW HIGH' bonus screen (buildHowHighScreen), se [seen] */
...
/** FIRE1_ACTIVE (0x6420) — OBJ_ACTIVE field (+0) of fire slot 1 in OBJ_ARRAY_64 (record-1 base = 0x6400+0x20): bit0 = active/live slot; scanned by spawn/collision sweeps [seen] */
export const FIRE1_ACTIVE = 0x6420;
/** HAMMER0_IN_PLAY (0x6681) — In-play flag (OBJ_PAIR_6680 + HAMMER_IN_PLAY, +0x01) of the FIRST hammer object record; marks that Mario has grabbed hammer 0. Set by latchHammerTouch (ROM 0x29 [seen] */
...
/** BLINK_SPRITE0_CODE (0x6901) — Code byte of sprite-shadow record 0 (SPRITE_BUFFER 0x6900 +1) — the first of the rivet board's decorative blink sprite pair; blinkSpritePairOn/Off set/clear bit [seen] */
...
/** LEVEL_HUD_FURNITURE_A (0x74e3) — Fixed furniture tile (0x34) in the lives/level HUD column, drawn by drawLivesAndLevel below the level digits (same column as LEVEL_UNITS_CELL 0x74a3 / LEVEL_TEN [seen] */
...
/** PLAYFIELD_SIDE_COL_BASE (0x7522) — Top cell of the 14-cell left tilemap side-column strip blanked during playfield clear (steps +0x20 for 0x0e cells); pairs with 0x7523. Cleared to BLANK_TILE 0x1 [seen] */
...
/** HOW_HIGH_TILE_BLOCK_TOPLEFT_3 (0x76d0) — Top-left VRAM cell of the 3rd of four 5x14 (70-tile) blank-tile blocks loc_17b6 stamps with tile 0x10 (fillTileBlock) then overdraws with girder/ladder segment [seen] */
...
/** HOW_HIGH_TILE_BLOCK_4_ANCHOR (0x76cb) — Role is correct and code-readable: 0x76cb is the fillTileBlock destination anchor of the 4th of four 5x14 (70-tile) blank-tile blocks in loc [seen] */
...
/** BAND_TABLE (0x38dc) — ROM table of per-band values read by loc_0b68. [seen] */
export const BAND_TABLE = 0x38dc;
/** ATTRACT_SCRIPT_TABLE (0x21d1) — ROM attract-demo input script table. [seen] */
export const ATTRACT_SCRIPT_TABLE = 0x21d1;
/** ANIM_STRING_START (0x39cc) — ROM base of the animation string emitted by loc_2d83. [seen] */
export const ANIM_STRING_START = 0x39cc;
/** ANIM_STRING_RESTART (0x39c3) — ROM anim-string restart pointer. [seen] */
export const ANIM_STRING_RESTART = 0x39c3;
/** SCORE_ADDEND_TABLE (0x3529) — ROM table of per-event score addends read by addToScoreTask. [seen] */
...
/** BONUS_COUNTDOWN_SPRITES_TEMPLATE (0x3ddc) — 25m source block-copied into BONUS_COUNTDOWN_SPRITES. [seen] */
export const BONUS_COUNTDOWN_SPRITES_TEMPLATE = 0x3ddc;
/** OBJ_ARRAY_64_TEMPLATE (0x3dec) — shared 25m/50m/75m appearance template broadcast into OBJ_ARRAY_64. [seen] */
export const OBJ_ARRAY_64_TEMPLATE = 0x3dec;
/** OBJ_ARRAY_64_TEMPLATE_100M (0x3df0) — 100m appearance template broadcast into OBJ_ARRAY_64. [seen] */
export const OBJ_ARRAY_64_TEMPLATE_100M = 0x3df0;
/** OBJ_RECORD_66A0_TEMPLATE_25M (0x3df4) — 25m source record scattered into OBJ_RECORD_66A0. [seen] */
export const OBJ_RECORD_66A0_TEMPLATE_25M = 0x3df4;
/** OBJ_RECORD_66A0_TEMPLATE_50M (0x3dfa) — 50m source record scattered into OBJ_RECORD_66A0. [seen] */
export const OBJ_RECORD_66A0_TEMPLATE_50M = 0x3dfa;
/** SPRITE_69FC_TEMPLATE_25M (0x3e00) — 25m template block-copied into loc_69fc. [seen] */
export const SPRITE_69FC_TEMPLATE_25M = 0x3e00;
/** SPRITE_69FC_TEMPLATE_50M (0x3e04) — 50m template block-copied into loc_69fc. [seen] */
export const SPRITE_69FC_TEMPLATE_50M = 0x3e04;
/** OBJ_APPEARANCE_TEMPLATE_ROM (0x3e08) — ROM 0x0c-byte shared sprite code/attribute appearance template stamped into both object records by seedSpriteObjectPair. [seen] */
export const OBJ_APPEARANCE_TEMPLATE_ROM = 0x3e08;
/** OBJ_PAIR_6680_POSITION_TABLE_25M (0x3e0c) — 25m X/Y table scattered into OBJ_PAIR_6680. [seen] */
export const OBJ_PAIR_6680_POSITION_TABLE_25M = 0x3e0c;
/** OBJ_PAIR_6680_POSITION_TABLE_50M (0x3e10) — 50m X/Y table scattered into OBJ_PAIR_6680. [seen] */
export const OBJ_PAIR_6680_POSITION_TABLE_50M = 0x3e10;
/** OBJ_PAIR_6680_POSITION_TABLE_100M (0x3e14) — 100m X/Y table scattered into OBJ_PAIR_6680. [seen] */
export const OBJ_PAIR_6680_POSITION_TABLE_100M = 0x3e14;
/** OBJ_ARRAY_65A0_TEMPLATE (0x3e18) — 50m appearance template broadcast into OBJ_ARRAY_65A0. [seen] */
export const OBJ_ARRAY_65A0_TEMPLATE = 0x3e18;
/** SPRITE_6944_TEMPLATE_50M (0x3e1c) — 50m template block-copied into loc_6944. [seen] */
...
/** COLLISION_SPRITES_TEMPLATE_100M (0x3e54) — 100m template block-copied into OBJECT_COLLISION_SPRITES. [seen] */
export const COLLISION_SPRITES_TEMPLATE_100M = 0x3e54;
/** OBJ_ARRAY_66_TEMPLATE (0x3e60) — 75m appearance template broadcast into OBJ_ARRAY_66. [seen] */
export const OBJ_ARRAY_66_TEMPLATE = 0x3e60;
/** OBJ_ARRAY_66_POSITION_TABLE (0x3e64) — 75m X/Y table scattered into OBJ_ARRAY_66. [seen] */
export const OBJ_ARRAY_66_POSITION_TABLE = 0x3e64;
/** OBJ_ARRAY_67_TEMPLATE (0x101b) — 25m appearance template broadcast into OBJ_ARRAY_67 (and its page-68 tail). [seen] */
export const OBJ_ARRAY_67_TEMPLATE = 0x101b;
/** SPRITE_6970_TEMPLATE_75M (0x1121) — 75m template block-copied into loc_6970. [seen] */
export const SPRITE_6970_TEMPLATE_75M = 0x1121;
/** OBJ_ARRAY_64_TEMPLATE_100M_EXTRA (0x117e) — 100m appearance template for the extra fire records in OBJ_ARRAY_64. [seen] */
export const OBJ_ARRAY_64_TEMPLATE_100M_EXTRA = 0x117e;
/** OBJ_ARRAY_64_POSITION_TABLE_100M_EXTRA (0x1182) — 100m X/Y table for the extra fire records in OBJ_ARRAY_64. [seen] */
export const OBJ_ARRAY_64_POSITION_TABLE_100M_EXTRA = 0x1182;
/** OBJ_ARRAY_65_TEMPLATE (0x11a2) — 50m/75m appearance template broadcast into OBJ_ARRAY_65. [seen] */
...
export const ROUTINES = {
  ...
  0x0028: { name: "dispatchInlineJumpTable", role: "the `rst 0x28` inline-jump-table trampoline", cert: "seen" },
  0x0030: { name: "boardBitGate", role: "the `rst 0x30` vector: a per-board skip gate", cert: "seen" },
  0x0038: { name: "addToSpriteObjectColumn", role: "the `rst 0x38` vector: add the delta C into one field across all ten sprite-object records", cert: "seen" },
  ...
  0x0347: { name: "selectPlayerIndicatorColumnBase", entry: "selectPlayerIndicatorColumnBaseFromRegisters", role: "pick one of two video-RAM column-base addresses from a player selector", cert: "seen" },
  ...
  0x03f2: { name: "loc_03f2", role: "store a byte at a caller-given address, then bump-and-restore it on a spin coin-flip", cert: "seen" },
  0x03fb: { name: "slide50mSpriteRowAndServiceColorCycle", role: "the per-frame colour-cycle driver entry, with a 50m-only preamble that slides the whole sprite-object row's X by the published step and publishes the resulting row shift", cert: "seen" },
  ...
  0x0426: { name: "advanceColorCycleSweep", role: "advance the colour-cycle sweep counter one step per frame and dispatch this frame's colour work", cert: "seen" },
  0x0450: { name: "dispatchColorCascadeByBoard", role: "the per-frame colour-cascade dispatcher: route by the current board into one of the colour-cycle arms", cert: "seen" },
  ...
  0x055f: { name: "selectCurrentPlayerScoreCounter", role: "select the score-counter address for the player currently up", cert: "seen" },
  0x056b: { name: "loc_056b", role: "pick one of two destination columns from a zero/nonzero selector, then render a 3-byte packed-BCD counter up that column", cert: "seen" },
  0x0578: { name: "renderBcdColumnFixedCell", role: "draw a packed 3-byte BCD counter as six digits up a fixed video column", cert: "seen" },
  0x057c: { name: "renderBcdColumn", role: "draw a packed 3-byte BCD value as six digits up a video column", cert: "seen" },
  0x0583: { name: "expandBcdDigits", role: "unpack a run of packed BCD/hex bytes into two digit cells each", cert: "seen" },
  ...
  0x05c6: { name: "drawScoreTask", role: "the score-counter draw task: repaint one of the three on-screen score readouts, chosen by the task payload", cert: "seen" },
  0x05da: { name: "drawHighScore", role: "repaint the on-screen high-score readout from the HIGH_SCORE counter", cert: "seen" },
  0x05e9: { name: "drawStringVertical", role: "draw a doubly-indirected string down a tilemap column", cert: "seen" },
  0x0611: { name: "drawCreditLineInAttract", role: "repaint the 'CREDIT nn' line, but only while no credited game is in progress (attract)", cert: "seen" },
  0x0616: { name: "drawCreditDisplay", role: "paint the 'CREDIT nn' line: the label plus the credit count", cert: "seen" },
  0x066a: { name: "renderBonusDisplay", role: "render a packed two-digit BCD byte into its on-screen field, suppressing a leading zero", cert: "seen" },
  0x0689: { name: "stampTwoDigitField", role: "stamp a two-digit number's tile pair into its on-screen field: the high-digit tile into one cell, the low-digit tile into the cell one column over", cert: "seen" },
  ...
  0x06a8: { name: "stepBonusDisplayDown", role: "decrement the packed two-digit BCD counter by one, latch a 'reached zero' marker when it rolls from 01 to 00, store it back, and render it", cert: "seen" },
  0x06b8: { name: "drawLivesAndLevel", role: "redraw the reserve-lives indicator and the level-number digits", cert: "seen" },
  0x06fe: { name: "dispatchInGameSubstate", role: "vector the credited game to its current sub-state handler", cert: "seen" },
  0x073c: { name: "runAttractState", role: "service the attract game-state (GAME_STATE == 1) once per NMI", cert: "seen" },
  0x0763: { name: "restartAttractDemoAt25m", role: "on the timed sub-state advance, reset the live player context to a fresh 25m / level-1 / single-life start and (re)build the board -- the ATTRACT demo's own round, unreachable for a credited game (DIP_LIVES is 3..6 at every DSW setting)", cert: "seen" },
  ...
  0x08b2: { name: "dispatchCreditedSubstate", role: "vector the credited game (game-state 2) to its sub-state handler", cert: "seen" },
  ...
  0x08d5: { name: "readStartButtonSelector", role: "read which allowed start button is pressed on the credit screen, and once every 8 frames redraw the start prompt", cert: "seen" },
  ...
  0x0965: { name: "enqueueTaskBatch", role: "post a fixed, hard-coded batch of messages onto the task ring", cert: "seen" },
  ...
  0x09d6: { name: "armTwoPlayerBoardSetup", role: "the 2-player arm of the board-setup sub-state step: clear two board control latches, post two draw tasks, advance the game sub-state, then paint the shared 3-cell column", cert: "seen" },
  0x09ee: { name: "draw2UpLabel", role: "stamp the three fixed video-RAM cells of player 2's '2UP' score marker", cert: "seen" },
  0x09fe: { name: "restorePlayer2Context", role: "reinstate Player 2's saved game context and arm the start-of-turn wait", cert: "seen" },
  0x0a1b: { name: "loc_0a1b", role: "one step of the two-player board-setup chain", cert: "seen" },
  ...
  0x0a76: { name: "dispatchIntroCutsceneStep", role: "vector the opening Kong-climb cutscene to its current step handler", cert: "seen" },
  ...
  0x0bda: { name: "buildHowHighScreen", role: "build the 'HOW HIGH CAN YOU GET?' interlude screen, then step the in-game sub-state forward", cert: "seen" },
  0x0c91: { name: "buildBoardWhenTimerExpires", role: "gated board (re)build: tick the sub-state countdown and build the board only on the frame the countdown expires", cert: "seen" },
  0x0c92: { name: "buildBoard", role: "build a board: wipe the playfield, arm the palette bank and the opening task, then dispatch to the per-board setup arm selected by BOARD", cert: "seen" },
  0x0cc6: { name: "loc_0cc6", role: "the shared tail every board-setup dispatch arm converges on", cert: "seen" },
  0x0cd4: { name: "setup25mGirderBoard", role: "the 25m (board 1) board-setup arm: select the 25m girder layout table, queue the 25m background tune, then hand off to the shared board-setup tail", cert: "seen" },
  0x0cdf: { name: "setup50mConveyorBoard", role: "the 50m (board 2, conveyors) board-setup arm", cert: "seen" },
  0x0cf2: { name: "setUp75mBoard", role: "the 75m (board 3, elevators) board-setup arm", cert: "seen" },
  0x0d00: { name: "stampRivetBoardTiles", role: "stamp a fixed 2-tile motif into eight video-RAM cells during 100m-rivet (board 4) setup", cert: "seen" },
  0x0d27: { name: "stamp75mBoardTiles", role: "during 75m (board 3, elevators) setup, stamp two fixed two-row tile motifs into the background tilemap", cert: "seen" },
  0x0d30: { name: "fillTileRowPair", role: "stamp a fixed two-row tile motif into the tilemap from HL: 17 cells of 0xFD along one row, then 17 cells of 0xFC on the row directly below", cert: "seen" },
  0x0d43: { name: "stampRivetBoardBands", role: "stamp the two-band tile motif into two fixed tilemap rows during 100m-rivet (board 4) setup", cert: "seen" },
  0x0d4c: { name: "stampTwoTileBands", role: "stamp two 4-cell tile bands (0xFD then 0xFC) into a video-RAM row, given the row-base pointer", cert: "seen" },
  0x0d5f: { name: "loc_0d5f", role: "board-setup continuation: run the common per-board init, scatter the object records, arm the setup dwell timer (SUBSTATE_TIMER = 0x40) and advance GAME_SUBSTATE, stage the sprite-object block from the ROM template at 0x385C (plus the 8-byte continuation into SPRITE_BUFFER), then apply the per-board sprite offset selected by BOARD -- 100m an X-column shift of +0x44 and two rivet field nudges, 50m/75m none, 25m a Y-column shift of -4", cert: "seen" },
  ...
  0x0ee8: { name: "drawCappedTileColumn", role: "stamp a capped vertical tile run (top cap, body, bottom cap) down the tilemap for a kind-3 board-layout record", cert: "seen" },
  ...
  0x0f56: { name: "initBoardState", role: "reset the per-board work RAM, compute the board's bonus/timer values, seed the shared top sprites, then dispatch to the board's object setup", cert: "seen" },
  0x0fd7: { name: "seed25mBoardObjects", role: "build the 25m board's initial object records and their sprite shadows from ROM templates", cert: "seen" },
  ...
  0x11fa: { name: "loc_11fa", role: "scatter a 6-byte source record into a fixed IX record + a 4-byte array", cert: "seen" },
  0x122a: { name: "replicateGroupStrided", role: "copy ONE 4-byte source group into B strided destination slots", cert: "seen" },
  0x123c: { name: "seedMarioActorRecord", role: "spawn Mario's actor record at a board-dependent start, advance the sub-state, and post the follow-up task", cert: "seen" },
  ...
  0x12de: { name: "loc_12de", role: "on sub-state-timer expiry, tear down the finished sub-state's sprite scratch, advance GAME_SUBSTATE, and re-arm the timer to fire immediately", cert: "seen" },
  0x12f2: { name: "losePlayer1Life", role: "spend one of player 1's lives, snapshot their context, then route to the resume interlude or the game-over sequence", cert: "seen" },
  0x1344: { name: "loc_1344", role: "idx-15 in-game sub-state handler: decrement the current player's lives, save the live context to player 2's slot, then branch on lives-left", cert: "seen" },
  0x138f: { name: "loc_138f", role: "timed in-game sub-state transition, branched on player 2's saved context", cert: "seen" },
  0x13a1: { name: "loc_13a1", role: "timer-gated 0x0702 sub-state handler (table idx 0x17)", cert: "seen" },
  0x13aa: { name: "loc_13aa", role: "small in-game state reset: mirror the cabinet DIP into the flip-screen latch, clear the sub-state, and set the player-context word to 1", cert: "seen" },
  ...
  0x15fa: { name: "positionBonusItemSprite", role: "place the on-board bonus-item sprite at its current grid cell", cert: "seen" },
  0x1615: { name: "dispatchBoardClearedInterlude", role: "top dispatcher for the board-advance state, keyed on the board type", cert: "seen" },
  0x1641: { name: "runRivetBoardInterludeFrame", role: "run the effect-sprite state machine, then dispatch the board-render sequence step", cert: "seen" },
  0x1644: { name: "dispatchRivetBoardInterludeStep", role: "vector the board-advance render sequence to its current-step handler", cert: "seen" },
  0x1654: { name: "beginKongRecaptureInterlude", role: "step 0 of the board-advance render sequence: run the interlude's opening tableau (spawnInterludeHeart) -- NOT a board-load or intro-cutscene spawn, stage the first sprite-object animation frame, arm the pose-hold timer, then the shared tail", cert: "seen" },
  0x1662: { name: "advanceInterludeStepAndLiftKongFigure", role: "bump the board-advance sequence step, then (only on the 25m board) subtract 4 from field 3 of every sprite-object record", cert: "seen" },
  0x1670: { name: "stageNextKongPoseWhenHoldExpires", role: "one timer-gated step of the board-advance render sequence", cert: "seen" },
  0x168a: { name: "stageKongClimbPose", role: "one timer-gated step of the board-advance render sequence: re-init the sprite-object block from a ROM template, then tail into the shared advance tail", cert: "seen" },
  0x16a3: { name: "begin50mKongRecaptureInterlude", role: "sequence step 0: spawn init, stamp the fixed ten-record figure over the sprite-object block re-anchored to its previous X, then advance the 0x6388 step", cert: "seen" },
  0x16bb: { name: "dispatchKongWalkFrame", role: "every frame, clear object #1's reversal flag, then route the moving sprite group to bounce/slide/hand-off by its position and travel direction", cert: "seen" },
  ...
  0x176c: { name: "cullSpriteObjectsAtTop", role: "clear the X of any sprite-object that has risen to the top of the screen", cert: "seen" },
  0x1783: { name: "allSlotsClear", entry: "allSlotsClearFromRegisters", role: "is a strided table of ten object slots fully cleared?", cert: "seen" },
  0x178e: { name: "advanceToNextBoard", role: "step the board-order pointer to the next board and enter the 'HOW HIGH CAN YOU GET?' interlude", cert: "seen" },
  0x17b6: { name: "loc_17b6", role: "idx 0 of the BOARD_ADVANCE_STEP render sequence: build the initial how-high screen -- silence the sound, set the priority tune (SND_PRIORITY 0x0E for SND_PRIORITY_FRAMES 3), lay two 3-cell descending tile runs, draw four board-layout items (a 5x14 tile block plus a ROM segment table each), then stamp the ten-record sprite-object figure from ROM 0x385C and shift its X column by +0x44 -- then arm the sub-state gate, increment BOARD_ADVANCE_STEP itself (ROM 0x1821) and point SEQ_ADVANCE_PTR at it so the gated advancer drives the rest of the sequence", cert: "seen" },
  0x1826: { name: "fillTileBlock", role: "stamp a fixed 5-wide × 14-tall block of tile 0x10 into the tilemap at the caller's address", cert: "seen" },
  0x1839: { name: "stepSpriteAnimationSequence", role: "advance one step of the BOARD_ADVANCE_STEP-driven sprite animation sequence: on every 8th call it restamps the sprite-object block from one of two ROM figures (0x39CF / 0x39F7, chosen by bit 3 of the sub-counter) and re-applies the +0x44 X shift -- a two-frame alternation at 1/8 rate -- and on the sub-counter's 8-bit wrap it instead stamps the base figure at 0x385C, re-arms the hold timer and hands off to the next sequence step", cert: "seen" },
  0x186f: { name: "loc_186f", role: "one timer-gated step of the board-advance render sequence: stage a sprite-object frame, pulse a sound latch, advance the step", cert: "seen" },
  0x1880: { name: "loc_1880", role: "one step of the board-advance / 'how high' interlude render sequence: descend the sprite-object block, then on arrival build the next scene and advance", cert: "seen" },
  0x18c6: { name: "runRivetBoardFinaleThenAdvanceLevel", role: "runs the rivet board's end-of-board finale and then increments LEVEL -- the rivet-board twin of the ordinary board advance, which does not", cert: "seen", why: "the finale's length is a hard count: the counter it decrements is seeded to 0 and stepped once per dispatch, so a full 8-bit wrap is exactly 256 dispatches, and the progression tape measured exactly 256 on all three 100m boards. It is refutable rather than circular -- that counter has nine other write sites and over two thousand writes per run, including an increment elsewhere, any of which would have broken the exact 256" },
  0x196b: { name: "clearScreenAndSelectSubstate", role: "wipe the whole display, then jump the in-game sub-state index to a computed target", cert: "seen" },
  ...
  0x1ae6: { name: "walkRightWhileHeld", role: "the shared setup of the ground-movement dispatch plus its RIGHT arm: call the horizontal position gate, load P1_INPUT once, then walk right only when the right-limit verdict is clear AND the Right bit is held; everything else falls through to the Left arm", cert: "seen" },
  0x1af5: { name: "walkLeftWhileHeld", role: "the LEFT arm of the ground-movement dispatch: walk Mario one frame left iff the left-limit verdict is clear and the Left bit is held, else fall through to the ladder/climb handler. Consumes the verdict and control word its Right twin loaded", cert: "seen" },
  0x1afe: { name: "armMarioClimbAtLadderEnd", role: "at a ladder end, stamps the ladder-standing pose and this frame's climb-limit pair, arming a climb the movement code may then perform; the hammer state only FORBIDS it here", cert: "seen", why: "the verb is measured, not assumed: in attract the routine stamps the pose and limits roughly 46 times for every climb that actually starts, so it arms rather than starts -- the ratio is the claim, because a total is run-length dependent and this one was measured on a single attract run. The stamped sprite code is the ladder-STANDING frame -- the one written when a climb ends -- not a climbing frame, which refutes the reading that this routine drives the climb" },
  0x1b38: { name: "climbDownWhileHeld", role: "the Down half of the ladder-climb input dispatch: drive Mario's downward climb when Down is held, otherwise hand the frame to the up-climb path", cert: "seen" },
  0x1b45: { name: "climbUpWhileHeld", role: "when the player is holding UP, drive Mario's upward climb this frame", cert: "seen" },
  0x1b4e: { name: "loc_1b4e", role: "commit this frame's ladder-extent limits, then drive the Up-climb", cert: "seen" },
  0x1b55: { name: "tickPostLandingFreeze", role: "count down Mario's post-landing freeze; unfreeze on expiry", cert: "seen" },
  0x1b6e: { name: "initMarioJump", role: "begin Mario's jump: flag him airborne and pick the horizontal launch velocity from the held direction, then commit the arc", cert: "seen" },
  0x1b8a: { name: "launchMarioJump", role: "commit Mario's ballistic jump: write the airborne motion record, set the jump pose, snapshot the take-off height, fire the jump sound", cert: "seen" },
  0x1bb2: { name: "advanceMarioAirborneFrame", role: "airborne frame head: snapshot the pre-motion position, step the ballistic arc one frame, then steer on the horizontal position gate's two-flag verdict", cert: "seen" },
  0x1bd8: { name: "reverseMarioVerticalArc", role: "airborne PLAYFIELD-LIMIT reflection (NOT merely a screen edge -- limitMarioHorizontalTravel's left verdict also fires for an INTERIOR wall: odd BOARD, Y<0x58, X<0x6C, the left end of the top platform on 25m/75m), VERTICAL half: re-base the ballistic arc in place (velocity := 16*frames - velocity, frame count restarted) so the same parabola continues with its vertical step negated; a fall already latched lethal skips the re-base and keeps falling", cert: "seen" },
  0x1bec: { name: "loc_1bec", role: "airborne join point: advance the actor one ballistic frame, then hand the stepped frame to the airborne handler; reached only from the PLAYFIELD-LIMIT clamp block, not from ordinary airborne frames", cert: "seen" },
  0x1bf2: { name: "loc_1bf2", role: "airborne PLAYFIELD-LIMIT reflection, HORIZONTAL half: on the right-limit verdict stamp a leftward half-pixel-per-frame drift and clear the sprite facing bit, then continue into the vertical re-base", cert: "seen" },
  ...
  0x1c8f: { name: "walkMarioRight", role: "one frame of Mario's RIGHTWARD ground walk: slide one pixel while the step pacer runs; on expiry advance the walk-cycle index through the packed table and begin a new step with the facing bit set", cert: "seen" },
  0x1cab: { name: "walkMarioLeft", role: "one frame of Mario's LEFTWARD ground walk: the mirror of the rightward arm, differing only in the step delta, the walk-cycle table key and the cleared facing bit", cert: "seen" },
  0x1cc2: { name: "beginWalkStep", role: "start a new walk-animation step for Mario", cert: "seen" },
  0x1cd2: { name: "advanceMarioWalkX", role: "advance Mario one pixel along a horizontal walk step", cert: "seen" },
  0x1ceb: { name: "continueWalkStep", role: "carry an in-progress walk step one frame further", cert: "seen" },
  0x1cf2: { name: "climbMarioDown", role: "per-frame driver for Mario's DOWNWARD ladder climb", cert: "seen" },
  0x1d03: { name: "climbMarioUp", role: "drive Mario's upward climb one animation step per frame, paced by the move-step timer", cert: "seen" },
  0x1d11: { name: "advanceClimbStep", role: "advance one climb-animation step for Mario", cert: "seen" },
  0x1d3f: { name: "setClimbSpriteFrame", role: "stamp Mario's climb-animation sprite for one climb step, then flag him on the ladder and refresh his sprite record", cert: "seen" },
  0x1d49: { name: "markOnLadderAndCommitSprite", role: "flag Mario as on a ladder, then refresh his sprite record", cert: "seen" },
  0x1d51: { name: "centerMarioAndCommitClimbStep", role: "the ladder-centering phase of a climb step: snap Mario onto the ladder column, tick the alternating climb footstep, then commit his sprite", cert: "seen" },
  0x1d67: { name: "endClimbAtLadderLimit", role: "finish a ladder climb that has reached a ladder end", cert: "seen" },
  0x1d76: { name: "loc_1d76", role: "the 'sub-step timer still running' branch of the walk/climb animation stepper; conditionally ticks the timer down", cert: "seen" },
  0x1d8a: { name: "tickMoveStepTimer", role: "decrement the player's walk/climb sub-step timer", cert: "seen" },
  0x1d8f: { name: "triggerWalkSound", role: "request Mario's footstep ('walk') sound for 3 frames", cert: "seen" },
  0x1d95: { name: "loc_1d95", role: "commit A into the 0x6225 collection flag, then (off 25m) queue a 3-frame priority sound", cert: "seen" },
  0x1da6: { name: "writeMarioSpriteRecord", role: "refresh Mario's 4-byte hardware sprite record from his live position/sprite state", cert: "seen" },
  0x1dbd: { name: "dispatchEffectState", role: "the router for the effect-sprite state machine held in EFFECT_STATE (0x6340)", cert: "seen" },
  0x1dc9: { name: "armScorePopupAndSelectAward", role: "dispatchEffectState's state-1 handler: unconditionally arm the state-2 countdown (EFFECT_TIMER = 0x40) and advance EFFECT_STATE 1 -> 2, then tail-jump to the effect-sprite setter picked by the FIRST set bit of EFFECT_SELECT's low three bits (bit0 -> pickAwardTierByObjectCount, bit1 -> stageAward300Popup, bit2 -> pickRandomAwardTier) or, if none are set, cue the effect sound and pick by LEVEL (1 -> 300, 2 -> 500, else 800)", cert: "seen" },
  ...
  0x1e49: { name: "effectStateIdle", role: "the idle (do-nothing) arm of sub_1dbd's EFFECT_STATE (0x6340) router", cert: "seen" },
  0x1e4a: { name: "tickDispatcherCountdown", role: "tick sub_1dbd's state-2 hold timer; reset the dispatcher on expiry", cert: "seen" },
  0x1e57: { name: "checkBoardWonByType", role: "Mario's per-frame board-won position check: decide whether the current board has been won, dispatched by board type, and hand off to the arm that completes it", cert: "seen" },
  0x1e6d: { name: "loc_1e6d", role: "stamp Mario's sprite facing on the board-won path, then commit the board-advance and unwind out of the movement cascade", cert: "seen" },
  0x1e7a: { name: "completeBoardWhenMarioReachesRescueRow", role: "the rescue-row test inside Mario's per-frame position check (sub_1e57): on the ODD boards -- 25m AND 75m, selected by ROM 0x1E5F's `rra` on BOARD bit 0 -- a Mario Y above the rescue row completes the board", cert: "seen" },
  0x1e80: { name: "completeRivetBoardWhenCleared", role: "on a rivet (100m) board, complete the board the frame its last rivet is gone", cert: "seen" },
  0x1e85: { name: "enterBoardAdvanceAndUnwind", role: "commit 'this board is complete': set the board-advance sub-state, then unwind out of the movement cascade", cert: "seen" },
  0x1e8c: { name: "runHitEffectInsteadOfPlay", role: "the effect-latch frame gate: with 0x6350 clear, let the caller run its ordinary frame; with it set, run ONE effect beat and abandon the rest of the gameplay update. The latch suspends play for the duration of an effect", cert: "seen" },
  0x1e94: { name: "loc_1e94", role: "unconditional caller-skip: make the call return past its caller", cert: "seen" },
  0x1e96: { name: "dispatchEffectSequenceStep", role: "effect-sequence router: hand the frame to one of three effect step handlers keyed on EFFECT_SEQ_STATE; the slot index doubles at 8 bits, so states 128/129/130 alias steps 0/1/2", cert: "seen" },
  0x1ea0: { name: "buildEffectSprite", role: "effect-sequence step 0: spawn the hit effect sprite from the collided object's record, then arm the effect countdown and its priority sound", cert: "seen" },
  ...
  0x1f23: { name: "animateEffectSpriteThenRearmEffect", role: "effect-sequence step 2: a two-stage rate divider -- EFFECT_SEQ_INNER ticks every dispatch and reloads to 12 on each beat, EFFECT_SEQ_OUTER ticks once per beat and is never reloaded here -- that increments the effect sprite's code byte on every beat but the last, and on the last resets EFFECT_SEQ_STATE to 0, clears the shared scratch byte and re-arms the parent effect state machine (EFFECT_STATE = 1, EFFECT_PARAM_PTR back at EFFECT_SPRITE)", cert: "seen" },
  0x1f46: { name: "beginMarioFall", role: "when the 'ground went away' trigger is armed, drop Mario into a fresh falling state and remember the height he fell from", cert: "seen" },
  0x1f72: { name: "update25mBarrels", role: "the 25m barrel engine's head: walks the ten barrel slots once per frame, servicing each live one and publishing its sprite; returns immediately off board 1", cert: "seen", why: "the board gate is in the routine's own first test and the array it walks is empty on 50m/75m/100m, so a name naming any other board would be refuted by the array being identically zero there" },
  0x1f83: { name: "serviceBarrelSlotIfLive", role: "per-slot gate of the 25m barrel loop: reads the record's active flag and hands a live slot to advanceBarrelMotion, else skips the slot's four buffer bytes and advances", cert: "seen", why: "it writes no memory at all, which refutes the rival reading that it updates the slot; and it has zero absolute references anywhere in the ROM, so it is reached only as the loop's own head" },
  0x1f93: { name: "advanceBarrelMotion", role: "dispatches a live barrel record to its motion arm on the record's +1/+2 state bytes; all five arms are tail-jumps, so the motion it selects is its own continuation", cert: "seen", why: "named for the effect rather than the dispatch because every arm is a tail-jp and none returns here, so the routine's live-out IS the moved barrel; the alternative 'dispatch' reading names the method, which the pipeline's naming rule forbids" },
  0x1fce: { name: "advanceBarrelTileAnimation", role: "advances one barrel's tile animation as it rolls", cert: "seen", why: "board leakage would refute it: a shared object handler would have shown 50m and 75m hits on a tape that spent 1462 fire updates there, and it shows none -- it is confined to the board the barrel engine serves" },
  0x1fe5: { name: "stepBarrelRight", role: "the +X motion arm: BC = 0x0100, increments the record's X, then joins the shared roll tail", cert: "seen", why: "the direction is measured, not inferred: pinning the barrel records' X on the real ROM parks every barrel at the commanded column, and display_col tracks OBJ_X - 15, so the sign of this arm's delta is observable on screen" },
  0x1fef: { name: "stepBarrelLeft", role: "the -X motion arm: BC = 0xff04, decrements the record's X, then falls into the shared roll tail", cert: "seen", why: "same measured basis as stepBarrelRight, with the opposite sign; the two arms' entry counts sum exactly to the shared tail's in both a 150s attract run and a full-progression run" },
  0x1ff6: { name: "advanceRollingBarrel", role: "the shared tail of the two roll arms: carries the barrel one step along its current run and routes it to the ladder detour or an edge arm when the position gates fire", cert: "seen", why: "it has exactly two entries, both roll arms, which is why it is named for the roll and not for the girder; the rival 'along girder' reading is refuted by measurement -- the ladder detour alone is 12.7% of its entries, plus two edge arms, so a girder-only name would misdescribe an eighth of its work (the proportion is quoted rather than the two totals it came from, which are run-length dependent)" },
  0x20ec: { name: "advanceFallingBarrel", role: "advances a barrel that is falling between girders, restarting the gravity ramp at each landing", cert: "seen", why: "a rise on this arm would refute 'falling': the shared ballistic stepper only rises when the launch-speed pair is non-zero, and that pair is traced zero for the record's whole life on this arm (zeroed at release, never restored -- the contact arm discards the value and the other writer touches different fields). NOT CLAIMED: the sibling arm at ROM 0x2053 steps the same arc with a non-zero launch pair and does bounce; it is a different behaviour and must be named separately" },
  0x2104: { name: "retireBarrelAtEndOfRange", role: "retires a barrel that has run out its travel range", cert: "seen", why: "a third entry or a cosmetic-only write would refute it: a byte scan finds exactly two entries, and the arm accounting closes exactly against the other retirement paths with no residue" },
  ...
  0x2407: { name: "loc_2407", role: "spread a packed nibble-pair into a fixed-point value, then subtract a 16-bit operand; returns the difference", cert: "seen" },
  0x241f: { name: "limitMarioHorizontalTravel", role: "horizontal position gate: classify Mario's X into the two-flag (D,E) verdict all three consumers turn into a physical restraint on his X", cert: "seen" },
  0x2441: { name: "loadBoardObjectRecords", role: "scatter this board's ROM object-init records into two parallel work-RAM attribute arrays", cert: "seen" },
  0x24b4: { name: "retireBarrelIntoOilDrum", role: "retires a barrel that has reached the bottom band into the oil drum, and for an alternate-kind barrel also writes the shared cell that ARMS a fire release (the fire itself is spawned later, subject to a live-count cap)", cert: "seen", why: "grounded by A/B on the real ROM: forcing the barrel kind byte to 0 yields 0 fires across three environments against 21/23/29 baseline and 30/35/35 forced to 1, pin verified 195/195 at the consuming instruction; a cleaner 4-arm pin on 0x62B9 (single reader in the ROM) gives 21/0/0/114 with the routine still retiring 19-22 barrels in the zero arms, which separates the retirement from the ignition; captured frames show the object labelled OIL and the drum dark in the zero arm" },
  0x24ea: { name: "update50mMovingObjects", role: "the 50m moving-object subsystem tick", cert: "seen" },
  0x2523: { name: "service50mObjectSpawnRequest", role: "service the 50m moving-object spawn request, paced by a cooldown timer", cert: "seen" },
  0x2591: { name: "advance50mObjectRow", role: "advance and edge-cull the 50m moving-object row", cert: "seen" },
  0x25f2: { name: "update50mConveyorObjects", role: "the 50m board's per-frame object update: gate on the 50m board, then run the three conveyor-object step drivers and carry Mario along his conveyor row", cert: "seen" },
  0x2602: { name: "loc_2602", role: "per-frame driver for the first of sub_25f2's three timed sprite objects", cert: "seen" },
  0x262f: { name: "loc_262f", role: "per-frame driver for the SECOND of sub_25f2's three timed sprite objects", cert: "seen" },
  0x264c: { name: "loc_264c", role: "publish object-2's ±1 step in both polarities and, every 32nd frame, advance its mirrored sprite pair", cert: "seen" },
  0x266f: { name: "loc_266f", role: "ensure object-2's step-direction latch is negative-going, then run the shared publish/animate tail", cert: "seen" },
  0x2679: { name: "loc_2679", role: "on a timer, reverse object-3's step direction; then run the shared tail", cert: "seen" },
  0x268d: { name: "loc_268d", role: "publish object-3's step and, every 32nd frame, advance its sprite pair", cert: "seen" },
  0x26a6: { name: "loc_26a6", role: "step a mirrored pair of animation counters one frame, opposite ways", cert: "seen" },
  0x26de: { name: "reverseStepDirection", role: "flip the sign of a signed direction-step byte at (HL)", cert: "seen" },
  0x26e9: { name: "signStepHalfRate", role: "collapse a direction byte to a ±1 step, every other frame", cert: "seen" },
  0x26fa: { name: "service75mBoard", role: "the 75m per-frame service router: the bottom-of-screen death, then a LEVEL-scaled cadence over the board's object service and its vertical-reposition machine", cert: "seen" },
  0x271e: { name: "loc_271e", role: "thin wrapper: run the vertical-reposition machine, then return", cert: "seen" },
  0x2722: { name: "serviceBoardObjects", role: "service the six board objects for one pass, then publish their positions to the sprite buffer", cert: "seen" },
  0x2745: { name: "dispatchElevatorRideByColumn", role: "the lift-ride machine: gate on the lift flag, then dispatch by Mario's X into the up-mover (band 44..66), the down-mover (108..130), or loc_2766 (start Mario falling, clear the lift flag) -- never observed executing, so what TAKING that arm means is unclaimed", cert: "seen" },
  0x2766: { name: "loc_2766", role: "start Mario falling and clear the edge-reposition flag", cert: "seen" },
  0x276f: { name: "carryMarioUpWithLift", role: "carry Mario UP with the lift (MARIO_Y - 1; larger Y is lower on screen), or kill him if MARIO_Y is already below the 0x71 limit", cert: "seen" },
  0x277f: { name: "killMarioAtEndOfLiftTravel", role: "kill Mario: clear MARIO_ACTIVE and the lift flag 0x6398. All three callers trigger on MARIO_Y -- the two lift arms at their limits, and service75mBoard at MARIO_Y >= 0xF0 with no X-band test, so the caller need not be on a lift", cert: "seen" },
  0x2787: { name: "carryMarioDownWithLift", role: "carry Mario DOWN with the lift (MARIO_Y + 1), or kill him once MARIO_Y reaches the 0xE8 limit", cert: "seen" },
  0x2797: { name: "advanceBoardObjectTravel", role: "advance the six board objects in the 0x6600 array: each active object drifts one pixel vertically toward its limit, then lands or deactivates on arrival", cert: "seen" },
  0x27da: { name: "spawnBoardObject", role: "on the spawn cadence, claim a free object slot and seed a new board object; always tick the cadence timer down", cert: "seen" },
  0x2806: { name: "decrementByteAt", role: "decrement the byte at the given address by one", cert: "seen" },
  0x2808: { name: "killMarioOnObjectCollision", role: "kill Mario when a board object overlaps his hitbox", cert: "seen" },
  0x281d: { name: "recordHammerHitOnObject", role: "test the active special-object record against the board's hazards and, on an overlap, record where it was found", cert: "seen" },
  0x2853: { name: "searchPlayerObjectOverlap", role: "run the current board's object-overlap search for the player and hand its severity code back to the caller", cert: "seen" },
  0x286f: { name: "dispatchBoardCollision", role: "vector a collision test to the current board's handler", cert: "seen" },
  ...
  0x2901: { name: "search100mObjectOverlap", role: "run one bounding-box collision sweep over the 0x6400 object array", cert: "seen" },
  0x2913: { name: "findCollidingObject", role: "scan an object list for the first record whose bounding box overlaps a reference point on both axes; stop and report a hit, or report the list exhausted", cert: "seen" },
  0x2954: { name: "latchHammerTouch", role: "hammer-touch latch: test Mario against the two-record hammer pair, publish the overlap flag into MARIO_HAMMER_PENDING, drive the pickup sound (a miss clears what a touch set), and mark the matched hammer record", cert: "seen" },
  0x2974: { name: "findHammerOverlappingMario", role: "test whether Mario overlaps either of the two objects in the 0x6680 pair, and report which one", cert: "seen" },
  0x298c: { name: "turnFireAtGroundEdge", role: "reports that a fire has reached the edge of its ground, which is the sole determinant its caller uses to turn the fire around; the probe samples the tile 12 px BELOW the object, not ahead of it", cert: "seen", why: "if the tile test tripped on something common -- a ladder column, the right half of a tile pair -- the result would not be an edge; measured 23 turns in 864 probes, and 0 in 1448 probes in attract, so it fires rarely and only where a ground actually ends. The 12-px-below offset is confirmed three independent ways and is the same axis and offset Mario's own ground test uses" },
  ...
  0x2a2f: { name: "loc_2a2f", role: "probe the tile 4 px below a moving object and, if the object has reached the sloped girder there, snap its OBJ_Y UP onto the girder surface and report the contact", cert: "seen" },
  0x2a85: { name: "startMarioFallWhenGroundGivesWay", role: "while Mario is in plain grounded contact, look at the tile under his foot and, if the girder there is not level, defer to the slope-footing fall check", cert: "seen" },
  0x2ab4: { name: "decideSlopeGirderFooting", role: "decide whether Mario keeps his footing on an angled girder or the ground has run out and he starts to fall", cert: "seen" },
  0x2acd: { name: "triggerMarioFall", role: "request that Mario begin falling because the ground under him went away", cert: "seen" },
  0x2ad3: { name: "carryMarioOnConveyorRow", role: "carry Mario along whichever 50m conveyor (moving-platform) row he is standing on", cert: "seen" },
  0x2af6: { name: "selectConveyorStepAndMoveMario", role: "pick the drift step for this platform row by Mario's X, then move him", cert: "seen" },
  0x2b02: { name: "moveMarioX", role: "advance Mario's X by the current velocity, then hold it inside the horizontal limits", cert: "seen" },
  ...
  0x2b51: { name: "loc_2b51", role: "a reject exit of the player-vs-tilemap probe cascade; forces the two-level caller-skip so control unwinds past entry_2b1c", cert: "seen" },
  0x2b53: { name: "loc_2b53", role: "the non-25m arm of the player-vs-tilemap descent probe", cert: "seen" },
  0x2b74: { name: "loc_2b74", role: "the reject arm of the tile-probe cascade: hand back a zeroed result and unwind out of the probe and its caller", cert: "seen" },
  ...
  0x2c49: { name: "loc_2c49", role: "one entry of the bonus-event slot-claim cluster (0x2C41): stash mode byte 1, then hand off to the shared slot-claim entry with the caller's bonus value", cert: "seen" },
  ...
  0x2ddb: { name: "raisePeriodicObjectSpawnRequests", role: "raise two periodic event requests, on 50m/100m, while Mario is alive", cert: "seen" },
  0x2e04: { name: "update75mActorObjects", role: "the actor-object scan loop: on 75m, while Mario is alive, update all ten records of the actor object array", cert: "seen" },
  ...
  0x2ed4: { name: "driveHammerSprite", role: "per-frame hammer sprite / background-tune dispatcher", cert: "seen" },
  ...
  0x2f7c: { name: "commitSpriteRecordAtMarioOffset", role: "commit an object's on-screen sprite record, positioned at a fixed offset from Mario, and mirror that position back into the object record", cert: "seen" },
  0x2f97: { name: "buildPendingHammerSprite", role: "one build arm of the hammer/object sprite updater: when a hammer is pending, stamp the object's state and appearance, then commit its sprite record", cert: "seen" },
  0x2fb7: { name: "selectHammerSpriteBlinkByTimer", role: "pick which object-sprite build path lays down this frame's record, based on how far the hammer's duration counter has run", cert: "seen" },
  0x2fbe: { name: "blinkHammerSpriteOnFramePhase", role: "choose the object sprite's attribute for this frame's blink phase, then commit the record", cert: "seen" },
  ...
  0x30bd: { name: "clearSpriteColumns", role: "zero the X byte of four fixed groups of sprite records", cert: "seen" },
  0x30db: { name: "loc_30db", role: "zero the X byte of Mario's sprite record, then a stride-4 run of six more sprite-shadow records", cert: "seen" },
  0x30e4: { name: "clearStridedBytes", role: "zero B bytes at stride 4, walking the LOW address byte only", cert: "seen" },
  0x30ed: { name: "updateFires", role: "the per-frame fire service: runs the difficulty gate, the state scan, the census and the sprite publish in sequence", cert: "seen", why: "its four difficulty guards partition every dispatch exactly (the four arm counts sum to the entry count in both a 150s attract run and a full-progression run), which rules out a second caller and confirms the sequence is the whole of its work" },
  0x30fa: { name: "gateFireUpdateByDifficulty", role: "select which frame-gate arm paces the object update, by DIFFICULTY: 0-1 -> 1/2, 2 -> 5/8, 3-4 -> 3/4, 5+ -> 7/8, with the table clamping anything >= 6 to the last slot", cert: "seen", why: "its four difficulty guards partition every dispatch of the fire service exactly -- the four arm counts sum to the service's entry count in both a 150-second attract run and a full-progression run -- so the thing this selector paces is the fire service and nothing else" },
  0x3110: { name: "loc_3110", role: "frame-phase caller-skip guard: proceed on the odd frames (one of every two)", cert: "seen" },
  0x311b: { name: "loc_311b", role: "frame-phase caller-skip guard: proceed on 5 of every 8 frames", cert: "seen" },
  0x3126: { name: "loc_3126", role: "caller-skip frame-throttle: proceed on three of every four frames", cert: "seen" },
  0x3131: { name: "loc_3131", role: "let the caller proceed on seven of every eight frames; skip it on the eighth", cert: "seen" },
  0x313c: { name: "spawnRequestedFireAndRecolorLiveFires", role: "sweep the five OBJ_ARRAY_64 fire records: tally the live ones into OBJ_LIVE_COUNT, set each live record's OBJ_SPRITE_ATTR (cleared while Mario's hammer is active), service one pending spawn request into a free slot, then return a caller-skip boolean (splice on zero count). It is the only routine that SPAWNS one of these objects during play -- records are also activated outright at board build (seed75mBoardObjects, ROM 0x1087, stamps records 0 and 1 active)", cert: "seen" },
  0x31b1: { name: "advanceLiveFires", role: "walks the live fire records once per frame and advances each", cert: "seen", why: "named for the level it sits at rather than duplicating its caller's name: the chain is the per-frame fire service above it, this walker, and the per-object advance below. Board confinement is the check that could have failed -- on board 1 of the full-progression tape the object scan runs 923 times while this walker runs 0, and it is the ZERO that carries the claim, since the other total moves with the run" },
  0x31dd: { name: "armAlternateFireModeAtHighDifficulty", role: "arm mode 2 in field +0x19 of fire records 1 and 3 (0x6439 / 0x6479) when DIFFICULTY is 3 or more AND a rare entropy draw comes up; the mode itself is named by neither reader and is not asserted here", cert: "seen" },
  0x31f6: { name: "loc_31f6", role: "pick a byte from the two timing-entropy cells: the low two bits of the random accumulator, or the frame counter in the single case those bits are 1", cert: "seen" },
  0x3202: { name: "advanceFire", role: "advances one fire: steps it along its current heading and rerolls the heading when its timer expires", cert: "seen", why: "it is the per-object body beneath the fire service rather than a second service of its own -- the entry-count ratio against the service is a clean integer on 25m and 100m but non-integer on 50m, which is what a per-object body under a varying live count looks like and what a second service would not" },
  0x32bd: { name: "loc_32bd", role: "a three-way object-walker dispatch keyed on the current board", cert: "seen" },
  0x32d6: { name: "loc_32d6", role: "an object's interval down-counter with position-gated reload, then a periodic-timer tick", cert: "seen" },
  0x330f: { name: "tickFireTimerAndRerollDirection", role: "tick one fire's periodic timer; on expiry reload it to 43 and re-roll the object's travel direction on a random bit", cert: "seen" },
  0x333d: { name: "driveFireLadderClimb", role: "drives a fire's climb up or down a ladder", cert: "seen", why: "the climb states not being movement states would refute it: the vertical mover, which is reachable only from those states, tracks them exactly" },
  0x33a1: { name: "loc_33a1", role: "the guard at the head of the movement path, with TWO different skips: a closed board gate returns ordinarily, while an object risen above the line (record +0x0f < 89) skips the caller entirely", cert: "seen" },
  0x33ad: { name: "walkFireOneStep", role: "steps a fire one position along its current heading", cert: "seen", why: "swapped direction arms would refute it -- the caller writes one heading at the left screen bound and the other at the right, and reading the arms backwards would drive fires off the screen at both edges instead of turning them" },
  0x33c3: { name: "settleFireOnGirderSlope", role: "settles a fire's height onto the slope of the girder it is standing on", cert: "seen", why: "a third caller would refute the reading that this is the turn path's tail: a byte scan finds exactly one call site, on the turn path, which is what the entry-count gap over the walk step accounts for" },
  0x33e7: { name: "loc_33e7", role: "advance an object's sprite animation, then nudge its step counter up or down according to the object's state", cert: "seen" },
  ...
  0x342c: { name: "loc_342c", role: "start or resume one object's scripted position walk, advance its X one step, then hand to the shared table-walk tail", cert: "seen" },
  0x3445: { name: "loc_3445", role: "advance one object's table-driven position walk, or finalize it at the end of the table", cert: "seen" },
...
// ── Jump-table dispatch-target entry addresses (loc_00ca whitelist) ───────────
export const POWER_ON_INIT_ADDR = 0x01c3; // powerOnInit
export const RUN_ATTRACT_STATE_ADDR = 0x073c; // runAttractState
export const DISPATCH_CREDITED_SUBSTATE_ADDR = 0x08b2; // dispatchCreditedSubstate
export const DISPATCH_IN_GAME_SUBSTATE_ADDR = 0x06fe; // dispatchInGameSubstate
export const CONFIGURE_FLIP_SCREEN_AND_SELECT_SUBSTATE_ADDR = 0x0986; // configureFlipScreenAndSelectSubstate
export const RESTORE_PLAYER_1_CONTEXT_ADDR = 0x09ab; // restorePlayer1Context
export const ARM_TWO_PLAYER_BOARD_SETUP_ADDR = 0x09d6; // armTwoPlayerBoardSetup
export const RESTORE_PLAYER_2_CONTEXT_ADDR = 0x09fe; // restorePlayer2Context
export const LOC_0A1B_ADDR = 0x0a1b; // loc_0a1b
export const COMPOSE_SCREEN_AND_ADVANCE_SUBSTATE_ADDR = 0x0a37; // composeScreenAndAdvanceSubstate
export const CLEAR_SCREEN_AND_SELECT_INTRO_ADDR = 0x0a63; // clearScreenAndSelectIntro
export const DISPATCH_INTRO_CUTSCENE_STEP_ADDR = 0x0a76; // dispatchIntroCutsceneStep
export const BUILD_HOW_HIGH_SCREEN_ADDR = 0x0bda; // buildHowHighScreen
export const BUILD_BOARD_WHEN_TIMER_EXPIRES_ADDR = 0x0c91; // buildBoardWhenTimerExpires
export const SEED_MARIO_ACTOR_RECORD_ADDR = 0x123c; // seedMarioActorRecord
export const RUN_DEATH_ANIMATION_SUBSTATE_ADDR = 0x127c; // runDeathAnimationSubstate
export const LOSE_PLAYER_1_LIFE_ADDR = 0x12f2; // losePlayer1Life
export const LOC_1344_ADDR = 0x1344; // loc_1344
export const LOC_138F_ADDR = 0x138f; // loc_138f
export const LOC_13A1_ADDR = 0x13a1; // loc_13a1
export const LOC_13AA_ADDR = 0x13aa; // loc_13aa
export const SELECT_PLAYER_1_CONTEXT_ADDR = 0x13bb; // selectPlayer1Context
export const SELECT_PLAYER_SCREEN_OR_ATTRACT_ADDR = 0x141e; // selectPlayerScreenOrAttract
export const RUN_BONUS_ITEM_VALUE_DISPLAY_ADDR = 0x1486; // runBonusItemValueDisplay
export const DISPATCH_BOARD_CLEARED_INTERLUDE_ADDR = 0x1615; // dispatchBoardClearedInterlude
export const CLEAR_SCREEN_AND_SELECT_SUBSTATE_ADDR = 0x196b; // clearScreenAndSelectSubstate
export const RUN_GAMEPLAY_FRAME_ADDR = 0x197a; // runGameplayFrame
export const RESTART_ATTRACT_DEMO_AT_25_M_ADDR = 0x0763; // restartAttractDemoAt25m
export const COMPOSE_ATTRACT_TITLE_SCREEN_ADDR = 0x0779; // composeAttractTitleScreen
export const CLEAR_SCREEN_AND_ADVANCE_SUBSTATE_ADDR = 0x07c3; // clearScreenAndAdvanceSubstate
export const LOC_07CB_ADDR = 0x07cb; // loc_07cb
export const CLEAR_SUBSTATE_WHEN_TIMER_EXPIRES_ADDR = 0x084b; // clearSubstateWhenTimerExpires
export const RUN_ATTRACT_DEMO_FRAME_ADDR = 0x1977; // runAttractDemoFrame
export const ENTER_CREDIT_SCREEN_ADDR = 0x08ba; // enterCreditScreen
export const COMMIT_GAME_START_ADDR = 0x08f8; // commitGameStart
export const SETUP_INTRO_CUTSCENE_STEP_ADDR = 0x0a8a; // setupIntroCutsceneStep
export const RUN_INTRO_CLIMB_STEP_ADDR = 0x0abf; // runIntroClimbStep
export const ANIMATE_INTRO_CLIMB_STEP_ADDR = 0x0ae8; // animateIntroClimbStep
export const LOC_0B06_ADDR = 0x0b06; // loc_0b06
export const LOC_0B68_ADDR = 0x0b68; // loc_0b68
export const RUN_INTRO_ROAR_STEP_ADDR = 0x0bb3; // runIntroRoarStep
export const ADVANCE_SEQUENCE_STEP_WHEN_TIMER_EXPIRES_ADDR = 0x3069; // advanceSequenceStepWhenTimerExpires
export const BEGIN_MARIO_DEATH_ANIMATION_ADDR = 0x128b; // beginMarioDeathAnimation
export const STEP_MARIO_DEATH_ANIMATION_ADDR = 0x12ac; // stepMarioDeathAnimation
export const LOC_12DE_ADDR = 0x12de; // loc_12de
export const BEGIN_KONG_RECAPTURE_INTERLUDE_ADDR = 0x1654; // beginKongRecaptureInterlude
export const STAGE_NEXT_KONG_POSE_WHEN_HOLD_EXPIRES_ADDR = 0x1670; // stageNextKongPoseWhenHoldExpires
export const STAGE_KONG_CLIMB_POSE_ADDR = 0x168a; // stageKongClimbPose
export const BEGIN_50_M_KONG_RECAPTURE_INTERLUDE_ADDR = 0x16a3; // begin50mKongRecaptureInterlude
export const DISPATCH_KONG_WALK_FRAME_ADDR = 0x16bb; // dispatchKongWalkFrame
export const CLIMB_KONG_FIGURE_AND_BREAK_HEART_ADDR = 0x1732; // climbKongFigureAndBreakHeart
export const ADVANCE_BOARD_STEP_WHEN_SPRITES_CLEARED_ADDR = 0x1757; // advanceBoardStepWhenSpritesCleared
export const ADVANCE_TO_NEXT_BOARD_ADDR = 0x178e; // advanceToNextBoard
export const LOC_17B6_ADDR = 0x17b6; // loc_17b6
export const STEP_SPRITE_ANIMATION_SEQUENCE_ADDR = 0x1839; // stepSpriteAnimationSequence
export const LOC_186F_ADDR = 0x186f; // loc_186f
export const LOC_1880_ADDR = 0x1880; // loc_1880
export const RUN_RIVET_BOARD_FINALE_THEN_ADVANCE_LEVEL_ADDR = 0x18c6; // runRivetBoardFinaleThenAdvanceLevel
export const SEARCH_25_M_OBJECT_OVERLAP_ADDR = 0x2880; // search25mObjectOverlap
export const SEARCH_50_M_OBJECT_OVERLAP_ADDR = 0x28b0; // search50mObjectOverlap
export const SEARCH_75_M_OBJECT_OVERLAP_ADDR = 0x28e0; // search75mObjectOverlap
export const SEARCH_100_M_OBJECT_OVERLAP_ADDR = 0x2901; // search100mObjectOverlap
export const LOC_3110_ADDR = 0x3110; // loc_3110
export const LOC_311B_ADDR = 0x311b; // loc_311b
export const LOC_3126_ADDR = 0x3126; // loc_3126
export const LOC_3131_ADDR = 0x3131; // loc_3131
export const LOC_3E99_ADDR = 0x3e99; // loc_3e99
