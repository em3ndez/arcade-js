...
export const ROUTINES = {
  ...
  0x1ac3: { name: "dispatchMarioMovement", role: "the movement machine's router: five tests in a fixed priority order (airborne, freeze, hammer, ladder, jump-press, ground). The ORDER is the mechanic -- the hammer arm sits ABOVE both the ladder and jump tests (ROM 0x1AD1 before 0x1AD8 and 0x1ADF) and routes to the ground walk, so those two are never reached and a hammer-carrying Mario can only walk", cert: "code" },
  0x1ae6: { name: "walkRightWhileHeld", role: "the shared setup of the ground-movement dispatch plus its RIGHT arm: call the horizontal position gate, load P1_INPUT once, then walk right only when the right-limit verdict is clear AND the Right bit is held; everything else falls through to the Left arm", cert: "code" },
  0x1af5: { name: "walkLeftWhileHeld", role: "the LEFT arm of the ground-movement dispatch: walk Mario one frame left iff the left-limit verdict is clear and the Left bit is held, else fall through to the ladder/climb handler. Consumes the verdict and control word its Right twin loaded", cert: "code" },
  ...
  0x1e7a: { name: "completeBoardWhenMarioReachesRescueRow", role: "the rescue-row test inside Mario's per-frame position check (sub_1e57): on the ODD boards -- 25m AND 75m, selected by ROM 0x1E5F's `rra` on BOARD bit 0 -- a Mario Y above the rescue row completes the board", cert: "code" },
  ...
  0x1f72: { name: "update25mBarrels", role: "the 25m barrel engine's head: walks the ten barrel slots once per frame, servicing each live one and publishing its sprite; returns immediately off board 1", cert: "code", why: "the board gate is in the routine's own first test and the array it walks is empty on 50m/75m/100m, so a name naming any other board would be refuted by the array being identically zero there" },
  0x1f83: { name: "serviceBarrelSlotIfLive", role: "per-slot gate of the 25m barrel loop: reads the record's active flag and hands a live slot to advanceBarrelMotion, else skips the slot's four buffer bytes and advances", cert: "code", why: "it writes no memory at all, which refutes the rival reading that it updates the slot; and it has zero absolute references anywhere in the ROM, so it is reached only as the loop's own head" },
  0x1f93: { name: "advanceBarrelMotion", role: "dispatches a live barrel record to its motion arm on the record's +1/+2 state bytes; all five arms are tail-jumps, so the motion it selects is its own continuation", cert: "code", why: "named for the effect rather than the dispatch because every arm is a tail-jp and none returns here, so the routine's live-out IS the moved barrel; the alternative 'dispatch' reading names the method, which the pipeline's naming rule forbids" },
  0x1fce: { name: "advanceBarrelTileAnimation", role: "advances one barrel's tile animation as it rolls", cert: "code", why: "board leakage would refute it: a shared object handler would have shown 50m and 75m hits on a tape that spent 1462 fire updates there, and it shows none -- it is confined to the board the barrel engine serves" },
  ...
  0x1ff6: { name: "advanceRollingBarrel", role: "the shared tail of the two roll arms: carries the barrel one step along its current run and routes it to the ladder detour or an edge arm when the position gates fire", cert: "code", why: "it has exactly two entries, both roll arms, which is why it is named for the roll and not for the girder; the rival 'along girder' reading is refuted by measurement -- the ladder detour alone is 12.7% of its entries, plus two edge arms, so a girder-only name would misdescribe an eighth of its work (the proportion is quoted rather than the two totals it came from, which are run-length dependent)" },
  ...
  0x241f: { name: "limitMarioHorizontalTravel", role: "horizontal position gate: classify Mario's X into the two-flag (D,E) verdict all three consumers turn into a physical restraint on his X", cert: "code" },
  ...
  0x26fa: { name: "service75mBoard", role: "the 75m per-frame service router: the bottom-of-screen death, then a LEVEL-scaled cadence over the board's object service and its vertical-reposition machine", cert: "code" },
  ...
  0x277f: { name: "killMarioAtEndOfLiftTravel", role: "kill Mario: clear MARIO_ACTIVE and the lift flag 0x6398. All three callers trigger on MARIO_Y -- the two lift arms at their limits, and service75mBoard at MARIO_Y >= 0xF0 with no X-band test, so the caller need not be on a lift", cert: "code" },
  ...
  0x2a2f: { name: "loc_2a2f", role: "probe the tile 4 px below a moving object and, if the object has reached the sloped girder there, snap its OBJ_Y UP onto the girder surface and report the contact", cert: "code" },
  0x2a85: { name: "startMarioFallWhenGroundGivesWay", role: "while Mario is in plain grounded contact, look at the tile under his foot and, if the girder there is not level, defer to the slope-footing fall check", cert: "code" },
  ...
  0x31b1: { name: "advanceLiveFires", role: "walks the live fire records once per frame and advances each", cert: "code", why: "named for the level it sits at rather than duplicating its caller's name: the chain is the per-frame fire service above it, this walker, and the per-object advance below. Board confinement is the check that could have failed -- on board 1 of the full-progression tape the object scan runs 923 times while this walker runs 0, and it is the ZERO that carries the claim, since the other total moves with the run" },
  0x31dd: { name: "armAlternateFireModeAtHighDifficulty", role: "arm mode 2 in field +0x19 of fire records 1 and 3 (0x6439 / 0x6479) when DIFFICULTY is 3 or more AND a rare entropy draw comes up; the mode itself is named by neither reader and is not asserted here", cert: "code" },
  ...
  0x3202: { name: "advanceFire", role: "advances one fire: steps it along its current heading and rerolls the heading when its timer expires", cert: "code", why: "it is the per-object body beneath the fire service rather than a second service of its own -- the entry-count ratio against the service is a clean integer on 25m and 100m but non-integer on 50m, which is what a per-object body under a varying live count looks like and what a second service would not" },
  ...
  0x330f: { name: "tickFireTimerAndRerollDirection", role: "tick one fire's periodic timer; on expiry reload it to 43 and re-roll the object's travel direction on a random bit", cert: "code" },
  0x333d: { name: "driveFireLadderClimb", role: "drives a fire's climb up or down a ladder", cert: "code", why: "the climb states not being movement states would refute it: the vertical mover, which is reachable only from those states, tracks them exactly" },
  0x33a1: { name: "loc_33a1", role: "the guard at the head of the movement path, with TWO different skips: a closed board gate returns ordinarily, while an object risen above the line (record +0x0f < 89) skips the caller entirely", cert: "code" },
  0x33ad: { name: "walkFireOneStep", role: "steps a fire one position along its current heading", cert: "code", why: "swapped direction arms would refute it -- the caller writes one heading at the left screen bound and the other at the right, and reading the arms backwards would drive fires off the screen at both edges instead of turning them" },
  0x33c3: { name: "settleFireOnGirderSlope", role: "settles a fire's height onto the slope of the girder it is standing on", cert: "code", why: "a third caller would refute the reading that this is the turn path's tail: a byte scan finds exactly one call site, on the turn path, which is what the entry-count gap over the walk step accounts for" },
