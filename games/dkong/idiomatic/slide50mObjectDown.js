...
/**
 * slide50mObjectDown — one arm of the dispatch50mObjectState board-object state machine: tick this object's
 ...
 *
 * dispatch50mObjectState selects one of two 8-byte board-object records by frame parity, pushes the
 * record base, and dispatches on the object's state byte to one of a few arms; this is
 * one such arm. It is the UP mirror of the sibling arm raise50mObjectAndPark: where raise50mObjectAndPark steps the
 ...
 *      one, and mirror the new counter into the object's on-screen sprite cell (publish50mObjectYToSprite,
 ...
 *
 * ORACLE BOUNDARY: the record base arrives on the stack, pushed by dispatch50mObjectState — still the
 ...
 * `call` site does); both dissolve fully once dispatch50mObjectState is brought across.
 *
 * NAME: kept the neutral loc_ — the timer/counter/hit-test/climb-settle mechanics are
 * pinned to the oracle and it is the confirmed UP mirror of raise50mObjectAndPark, but which board
 * object this arm services (and what its sprite markers and the 0x6222 toggle mean) is not
 * corroborated to the routine-name bar, matching its siblings raise50mObjectAndPark / hold50mObjectParked.
 *
 * Memory-equivalent to the frozen oracle — equivalence-2259.test.js.
 * GATE:     crafted-entry, factored. dispatch50mObjectState's board gate is closed in attract, so this
 ...
 *                            record — 0x6280 or 0x6288), pushed by the still-frozen dispatch50mObjectState.
 * @returns {void}
 */
