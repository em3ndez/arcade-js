 ...
 * loc_3e70 / loc_1e00 / loc_1e08 / loc_1e10, the RNG sub-dispatcher loc_1df5, and the
 * shared tail loc_1e15 / loc_1e36 — all stayed neutral for the same reason; naming the
...
export function loc_1dc9(m) {
  ...
  if (select & 0x02) return loc_1e00(m); // bit 1
  if (select & 0x04) return loc_1df5(m); // bit 2 (RNG-dispatched)
  ...
  if (level === 1) return loc_1e00(m);
  if (level === 2) return loc_1e08(m);
  return loc_1e10(m); // LEVEL not in {1, 2} (level >= 3, or 0)
}
