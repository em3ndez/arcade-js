 ...
 * loc_2c72); if all five are occupied it does nothing further.
 *
 ...
 * barrel record loc_2cf6 later stamps lives in OBJ_ARRAY_67.
 *
 ...
 *           BARREL_CLAIM_MODE (0x6382) is named in ram.js and imported inside loc_2c72, which is
 *           where this routine's only write to it happens.
 */

import { BONUS_EVENT_MARK, OBJ_ARRAY_64 } from "./ram.js";
import { loc_2c72 } from "./loc_2c72.js"; // ROM 0x2C72 — raise bit 7 of BARREL_CLAIM_MODE
...
export function loc_2c4f(m, scratchValue, bonus) {
  ...
  for (let i = 0; i < RECORDS; i++) {
    if (mem.read8(OBJ_ARRAY_64 + i * STRIDE) === 0) {
      loc_2c72(m); // raise bit 7 (the barrel-kind select) on BARREL_CLAIM_MODE
      return;
    }
  }
