 ...
 *
 * Callees: blitSpritesViaDma is the idiomatic sprite blit; readControls (ROM 0x0087)
 * and perFrame (ROM 0x00B5, the loc_00b5 tail + epilogue) are still the frozen oracle
 * and are called directly — perFrame owns steps 5 and the register/NMI-mask restore.
 *
 * The oracle opens by pushing AF/BC/DE/HL/IX/IY (12 bytes) and perFrame's epilogue
 ...
 * register save — the registers are dead to the memory gate — but perFrame is still
 ...
 * RESERVE the 12-byte frame (advance SP by 12) before calling perFrame, exactly where
 * the six pushes would have left SP. That keeps perFrame's pops in mapped RAM (without
 ...
 *           covered by real captured NMI entries; the game-in-play arm (readControls,
 ...
 *           perFrame's epilogue restore them), but the test does not rely on that and
...
export function serviceVblankNmi(m) {
  ...
  // 1. Acknowledge the NMI (and lock out re-entry until perFrame re-enables it).
  ...
  //    perFrame is the oracle: its epilogue pops the 12-byte register-save frame the
  ...
  //    where the six pushes would leave it — so perFrame's pops stay in mapped RAM and
