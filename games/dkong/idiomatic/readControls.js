 ...
 * control word the movement code reads.
 *
 * Runs once per vblank NMI, but only for a credited game: the NMI skips it entirely while
 * the attract flag is set, so it is never reached in plain attract. It picks one 8-bit input
 * port and folds it into the two control-word bytes:
 *
 *   - PORT SELECT. An upright cabinet (DIP_UPRIGHT != 0) always reads IN0. A cocktail
 *     cabinet reads IN1 when the active-player select is non-zero (player 2's side of the
 *     shared panel), otherwise IN0.
 ...
 *   - Finally, raw bit 6 set means a service / soft-reset input. That arm is unexercised, so
 *     it throws — but only AFTER the store, so the two output bytes are already written when
 *     it throws.
 *
 * Reads only memory and no live-in register, so it is a pure function of memory state:
 * DIP_UPRIGHT, the cocktail player select, the selected port, and P1_INPUT_RAW.
 *
...
export function readControls(m) {
  ...
  mem.write8(P1_INPUT, cooked); // low byte of the one 16-bit control-word store
  mem.write8(P1_INPUT_RAW, raw); // high byte

  // Raw bit 6 set is a service / soft-reset input. That path is unexercised and throws —
  // but only after the store above, so both output bytes are already written.
  if (raw & 0x40) {
