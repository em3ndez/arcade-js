 ...
 *
 * A board/cutscene setup step that seeds TWO object records — based at 0x6680 and
 * 0x6690 (stride 0x10) — from ROM templates, marks both active, and gathers each
 ...
 * LIVE-OUT: memory-only — the object records (0x6680–0x669a) and the two sprite records
 ...
 * NAMES:    none imported — the two object records at 0x6680/0x6690 are unnamed work
 *           RAM, kept hex; the sprite destination 0x6A18 falls inside SPRITE_BUFFER
 *           (0x6900–0x6A7F) but stays hex, matching gatherSpriteRecords.
 */
...
export function seedSpriteObjectPair(m) {
  ...
  regs.de = 0x6683; // destination: record0.+3
  ...
  regs.de = 0x6687; // destination: record0.+7
  regs.bc = 0x020c; // B = 2 groups, C = 0x0c (record size C+4 = 0x10)
  replicateGroupStrided(m);

  // Step 3 — mark both records active: offset +0 = 1 at 0x6680 and 0x6690.
  regs.ix = 0x6680;
