...
/**
 * dispatchElevatorRideByColumn — the vertical-reposition machine: gate on the reposition flag, then
 ...
 * run a vertical mover arm (carryMarioUpWithLift and carryMarioDownWithLift, which step Mario's Y and mirror
 ...
 * routine-name bar — the whole arm family (carryMarioUpWithLift/carryMarioDownWithLift/killMarioAtEndOfLiftTravel/loc_2766)
 ...
 *           (0x6203) from ram.js; loc_2766 (ROM 0x2766), carryMarioUpWithLift (ROM 0x276F),
 *           carryMarioDownWithLift (ROM 0x2787) direct-called with no register inputs.
 */
...
import { carryMarioUpWithLift } from "./carryMarioUpWithLift.js"; // ROM 0x276F — vertical mover arm
import { carryMarioDownWithLift } from "./carryMarioDownWithLift.js"; // ROM 0x2787 — vertical mover arm
...
const MOVER_276F_BAND_LO = 44;   // [44, 67)   -> carryMarioUpWithLift
const MOVER_276F_BAND_HI = 67;
const MOVER_2787_BAND_LO = 108;  // [108, 131) -> carryMarioDownWithLift
...
export function dispatchElevatorRideByColumn(m) {
  ...
  if (x < MOVER_276F_BAND_HI) { carryMarioUpWithLift(m); return; } // [44, 67)
  if (x < MOVER_2787_BAND_LO) { loc_2766(m); return; } // gap band between the movers
  if (x < MOVER_2787_BAND_HI) { carryMarioDownWithLift(m); return; } // [108, 131)
  loc_2766(m);                                          // [131, 255]
}
