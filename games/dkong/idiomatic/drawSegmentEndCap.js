 ...
 *
 * The endpoint write pointer is the second point's tilemap address, held at 0x63AD
 * (a video-RAM cell in every real dispatch). Three stamps, guarded:
 *   - ALWAYS: the endpoint body tile = (0x63B0 span-remainder) + 0xD0, at the ptr.
 *   - IF the record kind (0x63B3) == 1 (a single-cell segment): the closing tile
 *     0xC0, one cell BACK (ptr's column − 1, wrapping within the tilemap page).
 *   - IF the span-remainder (0x63B0) != 0 (a sub-tile overhang): a partial-cell
 ...
 * 0x63B0 is not written between the two reads, so a single read feeds both tiles.
 *
 ...
 * NAMES:    none — 0x63B0/0x63B3/0x63AD are board-render scratch, unnamed in ram.js.
 */

export function drawSegmentEndCap(m) {
  const { regs, mem } = m;

  const remainder = mem.read8(0x63b0); // sub-tile span remainder (feeds both tiles)
  const ptr = mem.read16(0x63ad); // second point's tilemap (VRAM) address
