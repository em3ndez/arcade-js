...
export function countObjectOverlaps(m, { objectBase, probeBase, count, probeA, stride, threshA, threshB }) {
  ...
  do {
    ...
    if (mem8[base] & 0x01) {
      const ref = mem8[(base + 0x05) & 0xffff];
      let d = (a - ref) & 0xff;
      ...
      if (d < tA) {
        ...
        const rem = (d - tA) & 0xff;
        overlapsAxis1 = rem < mem8[(base + OBJ_HIT_EXTENT_Y) & 0xffff];
      }

      if (overlapsAxis1) {
        const ref2 = mem8[(base + 0x03) & 0xffff];
        ...
        if (e < tB) {
          ...
          overlapsAxis2 = rem2 < mem8[(base + OBJ_HIT_EXTENT_X) & 0xffff];
        }
