 ...
 * tile-code (0x63B5) ALREADY staged by that arm's setup. It:
 *
 ...
 *     (0x63B1) down 8 px — one tile — per row; lay at least one tile and keep going
 *     while the height has not run out. 0x63B5 is loop-invariant (the loop writes only
 *     the tilemap and 0x63B1), so its read is hoisted out.
 ...
 *           (0x63AB tilemap ptr, 0x63B1 column extent, DE record ptr), then the cursor
 *           HL and fill tile (0x63B5) are staged exactly as fillTileColumn would, on
 ...
 *           (DE live-out), and a dropped tail walk (the walk's 0x63B3 write missing).
 * LIVE-OUT: memory + DE. Memory = 0x63B1 (extent), the tilemap VRAM, plus whatever the
 *           resumed walk draws. DE = the record pointer the walk leaves (a walk-integrity
 ...
 * NAMES:    none from ram.js — 0x63AB/0x63B1/0x63B5 are board-render scratch (tilemap
 *           ptr / column extent / fill tile), not named memory.
 */
