 ...
 *
 * loc_2c86 CLEARS the slot-claim mode byte BARREL_CLAIM_MODE to zero, then tails into armBarrelRelease with the
 ...
 * the stack. The candidate models no stack (plain JS return + a direct armBarrelRelease call), so the
 ...
 *        BONUS-SWEEP (exhaustive) — all 256 bonus values with a PRE-DIRTIED BARREL_CLAIM_MODE and a
 *          fixed mark, so the gate is closed for every bonus but one (bonus == mark, all-occupied);
 *          isolates the up-front clear of BARREL_CLAIM_MODE + the 0x638F/0x6392 writes across the whole space.
 *        SLOT sweep (crafted) — the gate OPEN (mark == bonus) over every first-free-slot position
 *          (records 0..4) and the all-occupied case, with a pre-dirtied BARREL_CLAIM_MODE, proving the byte is
 ...
 *        (a) skip-clear — omits the BARREL_CLAIM_MODE clear; caught there on the pre-dirtied gate-closed
 *            sweep (oracle clears to 0, twin leaves the dirty value).
 *        (b) wrong mode byte — passes 0x02 not 0x03; caught at 0x638F (armBarrelRelease always stashes it).
 *
...
const TARGET = 0x2c86;
const SCRATCH_MODE = 0x638f; // armBarrelRelease stashes the mode byte here (this entry always passes 0x03)
const SCRATCH_FLAG = 0x6392; // armBarrelRelease raises this to 1 on every entry
const MODE_BYTE = 0x03; // the mode byte this entry hands armBarrelRelease
const STRIDE = 32; // OBJ_ARRAY_64 record stride
const RECORDS = 5;
const EVENT_STEP = 8;
const DIRTY = 0x55; // a non-zero BARREL_CLAIM_MODE seeded in, to prove the up-front clear
 ...
 * OBJ_ARRAY_64 active bytes, and a pre-dirtied BARREL_CLAIM_MODE / mode / entry scratch, plus a safe
...
function makeEntry(base, { c, mark, records = OCCUPIED, req = DIRTY, aNoise = 0xa5 }) {
  ...
  e.mem.write8(SCRATCH_MODE, 0xee); // noise: prove armBarrelRelease rewrites it
  e.mem.write8(SCRATCH_FLAG, 0xee); // noise: prove armBarrelRelease rewrites it
  e.regs.sp = SAFE_SP;
  e.nextNmi = Infinity;
...
test("EQUAL: loc_2c86 == oracle across the bonus-sweep and every gate-open slot case", () => {
  ...
  // (i) gate closed, BARREL_CLAIM_MODE pre-dirtied -> cleared to 0, both scratch writes, mark kept.
  {
    ...
    loc_2c86(b);
    for (const mm of [a, b]) {
      assert.equal(mm.mem.read8(BARREL_CLAIM_MODE), 0x00, "BARREL_CLAIM_MODE must be cleared to 0");
  ...
  // (ii) gate open, slot found -> flag cleared then top bit set; mark stepped by 8.
  {
    ...
    loc_2c86(b);
    for (const mm of [a, b]) {
      assert.equal(mm.mem.read8(BARREL_CLAIM_MODE), 0x80, "cleared then top bit set -> exactly 0x80 (dirty low bits gone)");
      assert.equal(mm.mem.read8(BONUS_EVENT_MARK), 0x08 - EVENT_STEP, "mark must step down by 8");
    }
  }
  // (iii) gate open, all occupied -> mark stepped, BARREL_CLAIM_MODE stays cleared (no claim).
  {
    ...
    loc_2c86(b);
    for (const mm of [a, b]) {
      assert.equal(mm.mem.read8(BARREL_CLAIM_MODE), 0x00, "no free slot -> flag stays cleared");
...
/** BUG (a): omits the up-front clear of BARREL_CLAIM_MODE — caught on the pre-dirtied gate-closed sweep. */
function brokenSkipClear(m) {
  // BUG: no `mem.write8(BARREL_CLAIM_MODE, 0)` here.
  armBarrelRelease(m, MODE_BYTE, m.regs.c);
}

/** BUG (b): hands armBarrelRelease the wrong mode byte (0x02) — caught at 0x638F, which it always stashes. */
function brokenWrongMode(m) {
  m.mem.write8(BARREL_CLAIM_MODE, 0);
  armBarrelRelease(m, 0x02, m.regs.c); // BUG: 0x02, not 0x03
}
...
test("TEETH: the skip-clear twin is CAUGHT (0x6382 diverges)", () => {
  ...
  assert.equal(mismatch.ram.addr, BARREL_CLAIM_MODE, "the skip-clear twin must diverge on 0x6382");
