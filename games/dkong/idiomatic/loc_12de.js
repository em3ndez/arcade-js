 ...
 *
 * Reached as arm 2 of the 0x639D rst-0x28 dispatch (entry_127f). It is polled every
 ...
 *      the leading (X) byte of Mario's sprite record (0x694C) and six shadow records
 *      (0x6958/5C/60/64/68/6C);
 *   2. advances GAME_SUBSTATE (0x600A) — by ONE normally, by TWO when 0x600E != 0 (the
 *      extra `inc (hl)`); and
 ...
 * NAMES:    GAME_SUBSTATE (0x600A), SUBSTATE_TIMER (0x6009) — ram.js. 0x600E kept hex —
 *           the 1P/2P index is unnamed in ram.js (a wrong name is worse than hex).
...
const PLAYER_INDEX = 0x600e; // 1P/2P index; unnamed in ram.js -> hex. Non-zero adds one extra inc.
...
export function loc_12de(m) {
  ...
  // Advance GAME_SUBSTATE: +1 for a 1-player game, +2 when 0x600E != 0 (the extra
  // `inc (hl)`). Two 8-bit `inc (hl)` in a row equal one (+2 mod 256), so a single
