...
/**
 * carryMarioUpWithLift — step Mario's Y toward the top of its travel, or edge-reset once it arrives.
 * ROM 0x276F.
 *
 * One arm of the per-frame vertical-reposition machine dispatchElevatorRideByColumn, chosen while Mario's X
 ...
 *     reached the top of this arm's travel), it hands off to the edge reset killMarioAtEndOfLiftTravel, which
 ...
 *
 * The 0x71 limit and the -1 step are this arm's; the sibling arm carryMarioDownWithLift runs the same
 * shape in the opposite direction, and both fall into the same killMarioAtEndOfLiftTravel edge reset.
 *
 ...
 * confirmed to the routine-name bar — the sibling killMarioAtEndOfLiftTravel/carryMarioDownWithLift stay loc_ for the same
 ...
 *           killMarioAtEndOfLiftTravel). The caller (dispatchElevatorRideByColumn, itself a discarded per-frame tail) consumes no
 ...
 *           ram.js; killMarioAtEndOfLiftTravel (ROM 0x277F) direct-called for the edge reset.
 */

import { MARIO_Y, MARIO_SPRITE_RECORD, SPRITE_Y } from "./ram.js";
import { killMarioAtEndOfLiftTravel } from "./killMarioAtEndOfLiftTravel.js"; // ROM 0x277F — the vertical-mover edge reset

// MARIO_Y below this = it has reached the top of this arm's travel (paired with the band
// limits 0x2C/0x43/0x6C/0x83 the dispatcher dispatchElevatorRideByColumn keys the arm off).
