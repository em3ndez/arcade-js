...
/**
 * stepBarrelLeft — the -X motion arm of the barrel walk: stage the two direction constants the
 * shared roll tail consumes, and decrement this barrel's OBJ_X.
...
export function stepBarrelLeft(m, cur, objBase = m.regs.ix) {
  const { mem8, regs } = m;

  mem8[objBase + OBJ_X] = mem8[objBase + OBJ_X] - 1;

  // The b/c writes ride the return so the roll tail reads the slope selector and direction code.
  return (regs.b = SLOPE_STEP_SELECTOR, regs.c = ORIENTATION_DIRECTION, advanceRollingBarrel(m, cur));
}
