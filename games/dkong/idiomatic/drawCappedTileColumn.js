 ...
 * records walked by sub_0da7; each record's kind byte is stashed at 0x63B3 and
 * dispatched down the chain — kind 2 -> loc_0e4f (the ladder drawer), kind 3 ->
 ...
 * VRAM address (0x63AB, the tile the record's corner fell in), sized by the column
 * extent at 0x63B1:
 ...
 *
 * The extent counter LIVES IN 0x63B1 — reloaded, decremented, and stored every body
 * row by the oracle, not a hidden local — so its per-row intermediate values are
 * observable in work RAM; `extent` below mirrors that byte (nothing aliases it
 ...
 *           render-scratch this routine consumes (0x63AB tilemap ptr, 0x63B1 extent,
 *           0x63B3 kind, DE record ptr), and the kind + extent are nudged to reach
 ...
 * LIVE-OUT: memory + DE. Memory = 0x63B1 (extent) and the tilemap VRAM cells (top /
 *           body / bottom tiles). DE = the record pointer advanced one past this
 ...
 * NAMES:    none from ram.js — 0x63AB/0x63B1/0x63B3 are board-render scratch (tilemap
 *           ptr / column extent / record kind), not named memory.
 */
...
export function drawCappedTileColumn(m) {
  ...
  for (;;) {
    ...
    mem.write8(0x63b1, extent);           // store the decremented extent (observable)
