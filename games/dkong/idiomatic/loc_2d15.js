...
/**
 * loc_2d15 — the frame-gated step of the intro string/sprite renderer.  ROM 0x2D15.
 *
 * Reached by fall-through from the 0x2C-cluster cutscene setup (entry_2cf6) and by a
 * couple of conditional tail-jumps from its neighbours. Runs once per renderer tick but
 * only DOES anything every 0x18 frames: a down-counter at FRAME_GATE (0x62AF) is
 * decremented every entry and the routine returns until it underflows.
 *
 ...
 *     ANIM_TABLE (0x3932). bit0 of BRANCH_PARITY (0x6382) chooses whether the record
 ...
 *
 * NAME: kept loc_ — the mechanics (frame gate, table select, sub-counter step, branch)
 * are pinned to the oracle, but which cutscene element this animates is not corroborated
 * to the routine-name bar. Promote once grounded.
 *
 ...
 * NAMES:    FRAME_GATE (0x62AF), ANIM_COUNTER (0x638F) and BRANCH_PARITY (0x6382) were each
 *           examined and left UNNAMED in ram.js (thin/shared engine scratch), so each stays
 *           a local hex const here; ANIM_TABLE (0x3932) is a ROM address (the table data),
 *           kept as a hex const. loadSpriteObjectBlock fills SPRITE_OBJ_BLOCK (0x6908).
 */
...
const BRANCH_PARITY = 0x6382;  // bit0 selects the ±1 record adjust and the underflow branch
...
export function loc_2d15(m) {
  ...
  // Select the animation-table record. bit0 of the parity byte decides whether the
  // record index is the sub-counter itself or one less; index*40 is taken 8-bit.
  let index = counter;
  if ((mem.read8(BRANCH_PARITY) & 0x01) === 0) {
    index = (index - 1) & 0xff;
  }
  ...
  // Sub-counter reached zero: shorten the gate to one frame, then branch on the parity
  // bit — set restarts the fixed source string, clear renders the next character.
