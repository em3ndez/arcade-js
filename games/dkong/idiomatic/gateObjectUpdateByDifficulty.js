...
/**
 * gateObjectUpdateByDifficulty — pick this difficulty's frame gate and hand its proceed/skip decision to the
 * caller.  ROM 0x30FA.
 *
 ...
 * out than the gate itself: the caller of gateObjectUpdateByDifficulty is the one that gets skipped, and ITS
 * caller resumes. So the decision is simply returned unchanged, and a caller consumes it as
 * an early return: `if (!gateObjectUpdateByDifficulty(m)) return;`. true = proceed, false = return at once.
 *
 ...
 */
export function gateObjectUpdateByDifficulty(m) {
