...
/**
 * carryMarioDownWithLift — advance Mario's vertical position by one, or hand off to the edge
 ...
 * it stops moving and hands off to killMarioAtEndOfLiftTravel, the edge reset — which switches the
 ...
 * open (its sibling edge reset killMarioAtEndOfLiftTravel is held for the same reason). Promote
 * once corroborated.
 *
 * A NEAR-LEAF: reads MARIO_Y, writes MARIO_Y and the sprite-record Y; on the
 * reset arm it direct-calls killMarioAtEndOfLiftTravel. Returns nothing a caller consumes.
 *
 ...
 *           oracle models its tail-jump to killMarioAtEndOfLiftTravel as a call and ends every path
 ...
 *           via killMarioAtEndOfLiftTravel on the reset arm, MARIO_ACTIVE and EDGE_REPOSITION_FLAG).
 ...
 *           ram.js; killMarioAtEndOfLiftTravel (ROM 0x277F) direct-called with no register inputs.
...
/**
 * @param {object} m  the machine (uses m.mem, and calls killMarioAtEndOfLiftTravel on the reset arm).
 * @returns {void}
 */
