 ...
 * reads BOARD (0x6227) and, for 0x6227 == 2, tail-jumps here — the same table whose
 * 0x6227 == 4 arm loc_0d5f documents as "the 100m RIVETS board setup", which fixes
 ...
 *      records at 0x6407, record stride C+4 = 0x20.
 *   2. seedObjectBlockSprites (0x1186): seed the 0x6500 object block's shared sprite
 *      field from a ROM template, then gather its 10 hardware sprite records to 0x6980.
 *   3. replicateGroupStrided (0x122a): stamp the 4-byte ROM group at 0x3E18 into 6
 *      records at 0x65A7, record stride C+4 = 0x10.
 *   4. gatherSpriteRecords (0x11d3): gather 6 sprite records (fields +3/+7/+8/+5) from
 *      the object array at IX = 0x65A0 (stride 0x10) into 0x69B8.  C = 0x0C carries
 *      over from step 3 (the gather ignores it), matching the oracle's `ld b,0x06`.
 *   5. loc_11fa (0x11fa): scatter the 6-byte ROM record at 0x3DFA into the 0x66A0
 *      field record + the 0x6A28 4-byte array.
 ...
 *   9. seedSpriteObjectPair (0x11a6): seed the object pair at 0x6680/0x6690 from the
 *      ROM position table at 0x3E10 and emit their two sprite records at 0x6A18.
 *   10. ldir 0x3E3C->0x6A0C (0x0C), then set the 0x62B9 marker.
 *
 ...
 * NAMES:    none imported — 0x62B9 is board-object bookkeeping (ram.js groups it but does
 *           not name it individually), kept hex; every other operand is a fixed immediate
 *           (ROM template pointers, object-record and SPRITE_BUFFER (0x6900-0x6A7F)
 *           destinations), all ROM-hardcoded engine scratch, kept hex to match the callees.
 */
...
export function seed50mBoardObjects(m) {
  ...
  // 1. Stamp the 4-byte ROM group at 0x3DEC into 5 records at 0x6407 (stride C+4 = 0x20).
  regs.hl = 0x3dec;
  regs.de = 0x6407;
  ...
  // 3. Stamp the 4-byte ROM group at 0x3E18 into 6 records at 0x65A7 (stride C+4 = 0x10).
  regs.hl = 0x3e18;
  regs.de = 0x65a7;
  regs.bc = 0x060c; // B = 6 records, C = 0x0c
  replicateGroupStrided(m);

  // 4. Gather 6 sprite records from the object array at IX = 0x65A0 (stride 0x10) into
  ...
  // 10. Copy the last ROM table, then mark the board's objects as set up.
  copyBlock(mem, 0x3e3c, 0x6a0c, 0x000c);
