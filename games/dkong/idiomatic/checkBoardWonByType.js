 ...
 *   • Girder (25m) board — the win is positional at the rescue row near Pauline, tested
 *     by loc_1e7a against Mario's screen Y.
 *   • The remaining boards — won once Mario has climbed above the 0x51 line (screen Y
 ...
 *
 * REGISTER-ABI MARSHALLING (dissolves once the arms take honest args): loc_1e7a and
 * loc_1e6d are idiomatic but still take their inputs in registers, so this routine loads
 * exactly what the oracle's jump sites load — Mario's screen Y before loc_1e7a (it reads
 ...
 *           0x51 is a screen-Y threshold with no ram.js name (mirroring loc_1e7a's 0x31), so
...
export function checkBoardWonByType(m) {
  ...
  // The other boards test Mario's position. Load his screen Y — loc_1e7a reads it as a
  // register live-in (its ABI is not yet promoted), and the climb test below uses it too.
  const marioY = mem.read8(MARIO_Y);
  regs.a = marioY;

  // Girder (25m) board: the rescue-row test near Pauline decides the win.
