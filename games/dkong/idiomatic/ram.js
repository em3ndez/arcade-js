...
// ───────────────────────────────────────────────────────────────────────────────────
// STACK SCRATCH  (the dead stack region, excluded from the convergence compare)
// ───────────────────────────────────────────────────────────────────────────────────
// The relaxed convergence gate (core/equivalence.js → convergentEquivalence, wired in
// optimized/test/convergent.js) compares ALL work RAM byte-for-byte EXCEPT this region.
// The stack scratch is read+written but DEAD: a mistimed vblank NMI pushes a coarse PC
// here that is overwritten before it is ever read, so a transient diff here is benign and
// excluded outright. Bounds measured from SP's low-water-mark by tools/footprint.mjs.
//
// (The full per-address read/write FOOTPRINT table that used to live here has been removed
// — it was never read by any gate; the compare is all-RAM-minus-STACK_SCRATCH, not a
// footprint-scoped list. tools/footprint.mjs still regenerates that table on demand as a
// reference dump when address-level coverage detail is wanted.)
