 ...
 * only the resulting SIGN is consumed downstream — sub_2523 reads 0x62A3/0x62A6 and
 * takes bit 7 (via `rlca`) to pick a +7 vs -8 sprite movement field, and
 * loc_264c/tail_268d publish the byte on to 0x63A5/0x63A6. So the observable effect
 * is a direction reversal on the object this byte steers.
 *
 ...
 * on-timer-expiry direction flip: sub_2602 (HL=0x62A1), sub_262f (HL=0x62A3), and
 * sub_2679 (HL=0x62A6) each set HL, reload a paired countdown, invoke this, and
 ...
 *           (0x62A1/0x62A3/0x62A6) on a real captured attract machine, plus crafted
 ...
 * NAMES:    none — HL is a live-in pointer supplied by the caller, so the routine
 *           references no fixed RAM address. (The real callers point HL at
 *           0x62A1/0x62A3/0x62A6 — unnamed engine object-direction scratch, named
 *           only in this prose.)
 */
