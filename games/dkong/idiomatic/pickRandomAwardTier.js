...
/**
 * pickRandomAwardTier — pick one of three effect-sprite setters from two bits of RANDOM.  ROM 0x1df5.
 *
 * The RNG tail of armScorePopupAndSelectAward (sub_1dbd's 0x6340==1 entry): armScorePopupAndSelectAward tail-jumps here when
 * 0x6342 bit2 is set. pickRandomAwardTier reads the PRNG accumulator RANDOM (0x6018) and dispatches
 ...
 * shared feeder stageAwardPopupAtHitObject:
 *
 *   RANDOM bit0 set             -> stageAward500Popup (B=0x7E, DE=0x0005)
 *   bit0 clear, bit1 set        -> stageAward800Popup (B=0x7F, DE=0x0008)
 *   bit0 clear, bit1 clear      -> stageAward300Popup (B=0x7D, DE=0x0003)   [fall-through]
 *
 ...
 * fall-through into stageAward300Popup (no `jp`), modelled here as the else arm. Each arm's `jp` is
 * a TAIL JUMP — a direct JS call in this layer, so the Z80 stack becomes the JS call
 * stack. pickRandomAwardTier WRITES no memory of its own and READS only RANDOM; the rotated A it
 * leaves is dead (the arm chains overwrite it before any use, and armScorePopupAndSelectAward's caller reads
 ...
 * game-identity the whole family (stageAward300Popup/1e08/1e10/1e15/1e36, and the sibling dispatcher
 * armScorePopupAndSelectAward) declined to claim to the routine-name evidence bar. Promote the family
 * together once the effect is corroborated.
 *
 * Memory-equivalent to the frozen oracle — equivalence-1df5.test.js.
 * GATE:     exhaustive (dispatch) — pickRandomAwardTier's only input is RANDOM's low two bits, so on
 *           real captured 0x1df5 bases (attract reaches this via armScorePopupAndSelectAward's 0x6342-bit2
 ...
 * LIVE-OUT: memory-only — nothing here; everything is written by the CHOSEN arm's stageAwardPopupAtHitObject
...
export function pickRandomAwardTier(m) {
  ...
  // fall-through into stageAward300Popup (both bits clear).
