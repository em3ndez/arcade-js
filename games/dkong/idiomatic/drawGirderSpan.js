 ...
 * pointer HL (the first point's tilemap cell) and the span counter 0x63B2 (the run's
 * pixel extent). This routine is the BODY FILL: it walks HL across the segment,
 ...
 *
 *   - The counter LIVES IN MEMORY at 0x63B2. Each step reloads it, subtracts 8 (one
 *     tile), and stores it back; when that `sub 0x08` borrows — the span has dropped
 ...
 *     than a JS local reproduces every value the ROM leaves at 0x63B2.
 ...
 * LIVE-OUT: memory (VRAM tile cells + the 0x63B2 counter) + DE, advanced past the record
 ...
 * NAMES:    none — 0x63B2 is board-render scratch (the span counter), unnamed in ram.js;
 *           0xC0 is the girder body tile code.
 */
...
export function drawGirderSpan(m) {
  ...
  for (;;) {
    const span = mem.read8(0x63b2);
    mem.write8(0x63b2, (span - 0x08) & 0xff); // store back every step, borrow or not
