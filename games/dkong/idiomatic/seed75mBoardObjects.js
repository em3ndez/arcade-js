 ...
 * work RAM (at 0x6400, 0x6500, 0x6600), builds their hardware sprite records inside
 * SPRITE_BUFFER, and copies two fixed sprite/data templates from ROM. All work is
 * memory writes; nothing it computes is read back as a register.
 *
 ...
 *      the +7 field of 5 records in the 0x6400 block (dest 0x6407, B=5, C=0x1C so the
 *      record stride is C+4 = 0x20): 0x6407/0x6427/0x6447/0x6467/0x6487.
 *   2. seedObjectBlockSprites (0x1186): seed the 0x6500 block's shared sprite field
 *      from the ROM template at 0x11A2 and build its 10 sprite records at 0x6980.
 *   3. Fill 6 cells of the 0x6600 block, stride 0x10, with 0x01
 *      (0x6600/0x6610/.../0x6650) — an object-active/flag column.
 *   4. Fill 3 cells at 0x660D, stride 0x10, with 0x08 (0x660D/0x661D/0x662D). The ROM
 *      runs this inner fill twice (outer C=2) over the SAME three cells with HL reset
 ...
 *      ROM table at 0x3E64 into records at 0x6603, stride C+2 = 0x10 (offsets +0/+2).
 *   6. replicateGroupStrided (0x122a): broadcast the 4-byte ROM group at 0x3E60 into 6
 *      records at 0x6607, stride C+4 = 0x10.
 *   7. gatherSpriteRecords (0x11d3): build 6 hardware sprite records at 0x6958 (inside
 *      SPRITE_BUFFER) by gathering fields +3/+7/+8/+5 from the 0x6600 block (stride 0x10).
 *   8. Block-copy 12 bytes of the ROM sprite template at 0x3E48 to 0x6A0C (inside
 *      SPRITE_BUFFER) — a fixed sprite record.
 *   9. Write the 0x6400 block's two lead records (base 0x6400 and 0x6420, stride 0x20)
 ...
 *      routine's code) to 0x6970 (inside SPRITE_BUFFER).
 *
 * The precise identity of the three object blocks (which 75m actors they are —
 * elevators/springs/fireballs) is not independently established; 0x6400/0x6500/0x6600
 * are unnamed work RAM. The name describes the MECHANISM and the board, exactly at the
 ...
 * LIVE-OUT: memory-only — the object-record blocks at 0x6400/0x6500/0x6600 and their
 *           sprite records / template copies inside SPRITE_BUFFER (0x6958.., 0x6980..,
 *           0x69A0.., 0x6970.., 0x6A0C..). The board-setup caller reads game RAM next
 ...
 * NAMES:    SPRITE_BUFFER (0x6900) from ram.js — every sprite destination lands in it.
 *           The object blocks 0x6400/0x6500/0x6600 and the ix-block field offsets stay
 *           hex (unnamed work RAM / record slots, per the callee convention).
 */
...
export function seed75mBoardObjects(m) {
  ...
  // 1. Seed field +7 of the 5 records in the 0x6400 block from the ROM group at 0x3DEC.
  regs.hl = 0x3dec; // 4-byte source group (re-read every record — a broadcast)
  regs.de = 0x6407; // dest: +7 of the first 0x6400-block record
  regs.bc = 0x051c; // B=5 records, C=0x1C -> record stride C+4 = 0x20
  replicateGroupStrided(m);

  // 2. Seed the 0x6500 object block's sprite field and build its 10 sprite records
  //    at 0x6980 (fixed immediates — this coordinator takes no register inputs).
  seedObjectBlockSprites(m);

  // 3. Fill 6 cells of the 0x6600 block, stride 0x10, with 0x01.
  for (let i = 0; i < 6; i++) mem.write8((0x6600 + i * 0x10) & 0xffff, 0x01);

  // 4. Fill the 3 cells at 0x660D, stride 0x10, with 0x08. The ROM's outer C=2 loop
  ...
  // 5. Scatter 6 source byte-pairs from the ROM table at 0x3E64 into 0x6603, stride 0x10.
  regs.hl = 0x3e64; // contiguous source (walks 2*B = 12 bytes)
  regs.de = 0x6603; // dest base (offsets +0/+2 of each record)
  regs.bc = 0x060e; // B=6 pairs, C=0x0E -> record stride C+2 = 0x10
  copyBytePairsStrided(m);

  // 6. Broadcast the 4-byte ROM group at 0x3E60 into 6 records at 0x6607, stride 0x10.
  regs.hl = 0x3e60;
  regs.de = 0x6607;
  regs.bc = 0x060c; // B=6 records, C=0x0C -> record stride C+4 = 0x10
  replicateGroupStrided(m);

  // 7. Build 6 hardware sprite records at 0x6958 (in SPRITE_BUFFER) from the 0x6600
  //    block, gathering fields +3/+7/+8/+5 (X<-+3, code<-+7, attr<-+8, Y<-+5).
  regs.ix = 0x6600; // object-record base
  regs.hl = 0x6958; // dest inside SPRITE_BUFFER
  regs.b = 0x06; // record count
  regs.de = 0x0010; // per-record source stride
  gatherSpriteRecords(m);

  // 8. Copy the fixed 12-byte ROM sprite template at 0x3E48 to 0x6A0C (in SPRITE_BUFFER).
  blockCopy(mem, 0x6a0c, 0x3e48, 0x0c);

  // 9. Write the 0x6400 block's two lead records field-by-field (base 0x6400, stride 0x20).
