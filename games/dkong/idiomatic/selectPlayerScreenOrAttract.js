 ...
 * screen, clears the two-byte player index, then scans the five 0x611C player-context
 * records (stride 0x22) to decide the next screen:
 *
 ...
 * and 0x611C is unnamed scratch in ram.js (kept hex).
 *
 ...
 *           reaches are forced by a single-byte upstream nudge — poke one 0x611C record —
 *           plus SUBSTATE_TIMER=1 to force expiry, identically on both sides. Fresh clone
 ...
 * NAMES:    CURRENT_PLAYER (0x600D) from ram.js. Its player-index companion 0x600E and the
 *           scanned record array 0x611C are unnamed in ram.js, so local hex constants.
...
// two-byte "player index", clearing both up front; the record==3 arm rewrites both.
// Unnamed in ram.js, so a local hex constant (matching selectPlayer2AndComposeScreen).
const PLAYER_INDEX_COMPANION = 0x600e;

// The five stride-0x22 player-context records this handler scans (0x611C, 0x613E,
// 0x6160, 0x6182, 0x61A4). Engine scratch, unnamed in ram.js — kept hex.
const PLAYER_RECORDS = 0x611c;
