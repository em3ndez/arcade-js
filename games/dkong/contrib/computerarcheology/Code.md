...
gameActiveGuard:
0008: 3A 07 60        LD      A,($6007)           ; {ram.attract} read the attract flag
...
marioActiveGuard:
0010: 3A 00 62        LD      A,($6200)           ; {ram.marioActive} read the player-alive flag
0013: 0F              RRCA                        
...
stirRandomSeed:
0057: 3A 18 60        LD      A,($6018)           ; {ram.random} read the pseudo-random seed
...
0062: 32 18 60        LD      ($6018),A           ; {ram.random} store the stirred seed
0065: C9              RET                         
...
0080: 3A 07 60        LD      A,($6007)           ; {ram.attract} attract skips the control read entirely
0083: A7              AND     A                   
...
; is set.
readControls:
0087: 3A 26 60        LD      A,($6026)           ; {ram.dipUpright} an upright cabinet always reads IN0
008A: A7              AND     A                   
008B: C2 98 00        JP      NZ,$0098            ; {code.loc_0098} upright: read IN0
008E: 3A 0E 60        LD      A,($600E)           ; {ram.activePlayerIndex} cocktail: is player 2 the one up?
0091: A7              AND     A                   
...
009E: 4F              LD      C,A                 
009F: 3A 11 60        LD      A,($6011)           ; {ram.p1InputRaw} last frame's raw port, for the edge test
00A2: 2F              CPL                         
...
00AA: 60              LD      H,B                 
00AB: 6F              LD      L,A                 
00AC: 22 10 60        LD      ($6010),HL          ; {ram.p1Input} store the cooked word and the raw port
00AF: 78              LD      A,B                 
...
00C5: E5              PUSH    HL                  
00C6: 3A 05 60        LD      A,($6005)           ; {ram.gameState} 0 power-on, 1 attract, 2 credited, 3 in-game
...
00E6: 3A 07 60        LD      A,($6007)           ; {ram.attract} the driver is silent during attract
00E9: A7              AND     A                   
...
loc_010b:
010B: 32 00 7C        LD      ($7C00),A           ; {hard.tuneLatch} hand the tune index to the sound CPU
...
loc_0118:
0118: 32 80 7D        LD      ($7D80),A           ; {hard.soundIrq} drive the sound-CPU interrupt line
011B: C9              RET                         
...
0131: 32 80 7D        LD      ($7D80),A           ; {hard.soundIrq} drop the sound-CPU interrupt line
0134: 32 00 7C        LD      ($7C00),A           ; {hard.tuneLatch} and silence the tune latch
0137: C9              RET                         
...
018C: E5              PUSH    HL                  
018D: 3A 05 60        LD      A,($6005)           ; {ram.gameState} a game in play keeps its own audio
0190: FE 03           CP      $03                 
...
01D1: 3E 01           LD      A,$01               
01D3: 32 07 60        LD      ($6007),A           ; {ram.attract} start in attract
01D6: 32 29 62        LD      ($6229),A           ; {ram.level} level 1
01D9: 32 28 62        LD      ($6228),A           ; {ram.lives} one life, for the indicator
...
01E7: 32 05 60        LD      ($6005),A           ; {ram.gameState} advance to state 1, attract
01EA: 32 27 62        LD      ($6227),A           ; {ram.board} select the 25m board
01ED: AF              XOR     A                   
01EE: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} clear the in-game sub-state selector
...
029C: 3E C0           LD      A,$C0               
029E: 32 B0 60        LD      ($60B0),A           ; {ram.taskTail} park the enqueue pointer at the ring base
02A1: 32 B1 60        LD      ($60B1),A           ; {ram.taskHead} and the dequeue pointer: an empty queue
02A4: AF              XOR     A                   
...
02BF: 3A B1 60        LD      A,($60B1)           ; {ram.taskHead} the dequeue pointer
02C2: 6F              LD      L,A                 
...
02D4: 3A 1A 60        LD      A,($601A)           ; {ram.frame} has the vblank moved the frame counter on?
02D7: BE              CP      (HL)                
...
loc_02f6:
02F6: 32 B1 60        LD      ($60B1),A           ; {ram.taskHead} store the advanced dequeue pointer
...
0315: 3A 1A 60        LD      A,($601A)           ; {ram.frame} read the frame counter
...
031D: 3A 0D 60        LD      A,($600D)           ; {ram.currentPlayer} which player is up
...
032F: 19              ADD     HL,DE               
0330: 77              LD      (HL),A              
0331: 3A 0F 60        LD      A,($600F)           ; {ram.twoPlayerGame} a lone player's marker just blinks
0334: A7              AND     A                   
0335: C8              RET     Z                   
0336: 3A 0D 60        LD      A,($600D)           ; {ram.currentPlayer} two players: show the other one instead
...
0350: 3A 2D 62        LD      A,($622D)           ; {ram.bonusLifeAwarded} already granted this player?
0353: A7              AND     A                   
...
0358: 3A 0D 60        LD      A,($600D)           ; {ram.currentPlayer} the player up owns the score
035B: A7              AND     A                   
...
0373: 3E 01           LD      A,$01               
0375: 32 2D 62        LD      ($622D),A           ; {ram.bonusLifeAwarded} latch it so it happens only once
...
0393: 47              LD      B,A                 
0394: 3A 29 62        LD      A,($6229)           ; {ram.level} difficulty rises with the level too
0397: 80              ADD     A,B                 
0398: FE 05           CP      $05                 
...
loc_039e:
039E: 32 80 63        LD      ($6380),A           ; {ram.difficulty} the barrel and fire behaviour read this
03A1: C9              RET                         
...
03DB: 32 A0 63        LD      ($63A0),A           ; {ram.eventReq313c} ask for a new fire to be released
...
03F3: 3A 19 60        LD      A,($6019)           ; {ram.spinCount} jitter it on the spin counter's low bit
03F6: 0F              RRCA                        
03F7: D8              RET     C                   
...
03FB: 3A 27 62        LD      A,($6227)           ; {ram.board} the 50m conveyor board has a preamble
03FE: FE 02           CP      $02                 
...
0406: 3A A3 63        LD      A,($63A3)           ; {ram.m50Obj1Step} the 50m object's signed step this frame
0409: 4F              LD      C,A                 
...
0410: 32 B7 63        LD      ($63B7),A           ; {ram.m50ObjRowShift} the 50m column painter adds this back
...
serviceColorCycle:
0413: 3A 91 63        LD      A,($6391)           ; {ram.colourCycleActive} is a sweep already running?
0416: A7              AND     A                   
0417: C2 26 04        JP      NZ,$0426            ; {code.advanceColorCycleSweep} step it on
041A: 3A 1A 60        LD      A,($601A)           ; {ram.frame} a sweep starts at the frame-counter wrap
041D: A7              AND     A                   
041E: C2 86 04        JP      NZ,$0486            ; {code.dispatchColorCyclePaint} not yet: repaint only
0421: 3E 01           LD      A,$01               
0423: 32 91 63        LD      ($6391),A           ; {ram.colourCycleActive} arm a fresh sweep
...
0450: 3A 27 62        LD      A,($6227)           ; {ram.board} route the cascade by board
0453: 0F              RRCA                        
...
0481: 3A B7 63        LD      A,($63B7)           ; {ram.m50ObjRowShift} 50m uses its own staged delta
0484: 4F              LD      C,A                 
...
048D: 3A 27 62        LD      A,($6227)           ; {ram.board} the rivet board has its own painter
0490: FE 04           CP      $04                 
...
04D1: 3A 03 62        LD      A,($6203)           ; {ram.marioX} which half of the screen is Mario in?
04D4: FE 80           CP      $80                 
...
blinkSpritePairByX:
0509: 3A 03 62        LD      A,($6203)           ; {ram.marioX} Mario's position picks the blink phase
...
0538: 3A 0D 60        LD      A,($600D)           ; {ram.currentPlayer} the player's own score column
...
0562: 3A 0D 60        LD      A,($600D)           ; {ram.currentPlayer} which player is up
0565: A7              AND     A                   
0566: C8              RET     Z                   
...
0611: 3A 07 60        LD      A,($6007)           ; {ram.attract} non-zero while no credited game is running
0614: 0F              RRCA                        
...
062E: 3A 8C 63        LD      A,($638C)           ; {ram.bonusDisplay} how much is showing on the readout
0631: A7              AND     A                   
0632: C2 A8 06        JP      NZ,$06A8            ; {code.stepBonusDisplayDown} still counting: take one notch off it
0635: 3A B8 63        LD      A,($63B8)           ; {ram.bonusDisplayZeroed} already bottomed out, so nothing to do
0638: A7              AND     A                   
0639: C0              RET     NZ                  
063A: 3A B0 62        LD      A,($62B0)           ; {ram.bonusStart} the board's starting bonus
...
0648: 07              RLCA                        
0649: 07              RLCA                        
064A: 32 8C 63        LD      ($638C),A           ; {ram.bonusDisplay} seed the readout with it
...
0667: 3A 8C 63        LD      A,($638C)           ; {ram.bonusDisplay} render the freshly seeded value
...
0678: 3E 03           LD      A,$03               
067A: 32 89 60        LD      ($6089),A           ; {ram.sndBgm} latch the background-music command
...
0691: 3A 8C 63        LD      A,($638C)           ; {ram.bonusDisplay} two digits, one per nibble
0694: 47              LD      B,A                 
...
06E8: 3E 63           LD      A,$63               
06EA: 32 29 62        LD      ($6229),A           ; {ram.level} write the clamp back
...
06FE: 3A 0A 60        LD      A,($600A)           ; {ram.gameSubstate} which step of the credited game this frame runs
...
073F: 3A 01 60        LD      A,($6001)           ; {ram.credits} an accepted coin shows up as a credit
0742: A7              AND     A                   
...
0768: 32 A0 63        LD      ($63A0),A           ; {ram.eventReq313c} clear the object-insert request
076B: 3E 01           LD      A,$01               
076D: 32 27 62        LD      ($6227),A           ; {ram.board} board 1, the 25m girders
0770: 32 29 62        LD      ($6229),A           ; {ram.level} level 1
0773: 32 28 62        LD      ($6228),A           ; {ram.lives} one life, which no dip setting can produce
...
079B: 3A 0F 60        LD      A,($600F)           ; {ram.twoPlayerGame} 2UP is drawn only in a two-player game
079E: FE 01           CP      $01                 
07A0: CC EE 09        CALL    Z,$09EE             ; {code.draw2UpLabel}
07A3: ED 5B 22 60     LD      DE,($6022)          ; {ram.dipCoinsFor1P} both coinage settings, read as one word
...
08B2: 3A 0A 60        LD      A,($600A)           ; {ram.gameSubstate} credited step 0 sets up, step 1 waits for start
...
08BD: AF              XOR     A                   
08BE: 32 07 60        LD      ($6007),A           ; {ram.attract} clearing this is what leaves attract mode
...
0922: 3A 20 60        LD      A,($6020)           ; {ram.dipLives} byte 0 is the starting life count
0925: 12              LD      (DE),A              
0926: 1C              INC     E                   
...
loc_0938:
0938: 22 0E 60        LD      ($600E),HL          ; {ram.activePlayerIndex} active player and 1P/2P flag, one store
...
0954: AF              XOR     A                   
0955: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} the credited sub-state machine is done
0958: 3E 03           LD      A,$03               
095A: 32 05 60        LD      ($6005),A           ; {ram.gameState} state 3 is gameplay
095D: C9              RET                         
...
0995: 3A 0E 60        LD      A,($600E)           ; {ram.activePlayerIndex} zero for a one-player start
0998: A7              AND     A                   
...
loc_099f:
099F: 3A 26 60        LD      A,($6026)           ; {ram.dipUpright} an upright cabinet keeps the flip
09A2: 3D              DEC     A                   
...
09B4: ED B0           LDIR                        
09B6: 2A 2A 62        LD      HL,($622A)          ; {ram.boardSeqPtr} the pointer just restored, not the old one
...
09E9: 3E 05           LD      A,$05               
09EB: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} advance the board setup, 2 to 5
...
0A07: ED B0           LDIR                        
0A09: 2A 2A 62        LD      HL,($622A)          ; {ram.boardSeqPtr} player 2's own place in the board order
0A0C: 7E              LD      A,(HL)              
0A0D: 32 27 62        LD      ($6227),A           ; {ram.board} cache the board it names
0A10: 3E 78           LD      A,$78               
0A12: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} hold 120 frames before player 2's turn
0A15: 3E 04           LD      A,$04               
0A17: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} then sub-state 4
0A1A: C9              RET                         
...
0A31: 3E 05           LD      A,$05               
0A33: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} chain the setup cascade on to sub-state 5
0A36: C9              RET                         
...
0A76: 3A 85 63        LD      A,($6385)           ; {ram.introStep} which beat of the cutscene this frame runs
...
0AAC: 22 C2 63        LD      ($63C2),HL          ; {ram.introWalkPtrA} seed the first walk pointer
0AAF: 21 CB 38        LD      HL,$38CB            
0AB2: 22 C4 63        LD      ($63C4),HL          ; {ram.introWalkPtrB} and the second
0AB5: 3E 40           LD      A,$40               
0AB7: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} arm the 64-frame gate the next beat waits on
...
0AD1: FF              RST     $38                 
0AD2: 3E 1F           LD      A,$1F               
0AD4: 32 8E 63        LD      ($638E),A           ; {ram.introScrollIndex} seed the climb's scroll index
0AD7: AF              XOR     A                   
...
0AF9: 3E 20           LD      A,$20               
0AFB: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} topped out: arm a 32-frame pause
0AFE: 21 85 63        LD      HL,$6385            
0B01: 34              INC     (HL)                ; step the cutscene on
0B02: 22 C0 63        LD      ($63C0),HL          ; {ram.seqAdvancePtr} let the shared gated tick advance it next
0B05: C9              RET                         
...
0B0B: 2A C2 63        LD      HL,($63C2)          ; {ram.introWalkPtrA} the walk cursor into the delta table
0B0E: 7E              LD      A,(HL)              
0B0F: FE 7F           CP      $7F                 ; the sentinel that ends the walk
0B11: CA 1E 0B        JP      Z,$0B1E             ; {code.loc_0b1e}
0B14: 23              INC     HL                  
0B15: 22 C2 63        LD      ($63C2),HL          ; {ram.introWalkPtrA} advance the cursor one byte
...
0B56: 3E 05           LD      A,$05               
0B58: 32 8D 63        LD      ($638D),A           ; {ram.cutsceneBandCount} five bands still to stamp
0B5B: 3E 20           LD      A,$20               
0B5D: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} arm the 32-frame pause
0B60: 21 85 63        LD      HL,$6385            
0B63: 34              INC     (HL)                ; step the cutscene on
0B64: 22 C0 63        LD      ($63C0),HL          ; {ram.seqAdvancePtr} let the gated tick advance it
0B67: C9              RET                         
...
0B6D: 2A C4 63        LD      HL,($63C4)          ; {ram.introWalkPtrB} the walk cursor into the delta table
0B70: 7E              LD      A,(HL)              
0B71: FE 7F           CP      $7F                 ; the sentinel means the scroll path has wrapped
0B73: CA 86 0B        JP      Z,$0B86             ; {code.loc_0b86}
0B76: 23              INC     HL                  
0B77: 22 C4 63        LD      ($63C4),HL          ; {ram.introWalkPtrB} advance the cursor one byte
...
0B89: 22 C4 63        LD      ($63C4),HL          ; {ram.introWalkPtrB} rewind the cursor to the table start
0B8C: 3E 03           LD      A,$03               
...
0BA9: 3E B0           LD      A,$B0               
0BAB: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} all bands placed: hold 176 frames
...
0BB6: 3A 09 60        LD      A,($6009)           ; {ram.substateTimer} the phase countdown as the frame begins
...
0BD2: AF              XOR     A                   
0BD3: 32 85 63        LD      ($6385),A           ; {ram.introStep} the cutscene is over: wrap the step sequence
...
0BE3: 3A 00 62        LD      A,($6200)           ; {ram.marioActive} passed as the count of lives in play
0BE6: 5F              LD      E,A                 
...
0C0C: 3E 05           LD      A,$05               
0C0E: 32 2E 62        LD      ($622E),A           ; {ram.howHighIndex} write the clamp back

loc_0c11:
0C11: 3A 2F 62        LD      A,($622F)           ; {ram.howHighLastSeq} the board-order pointer as it was last time
0C14: 47              LD      B,A                 
0C15: 3A 2A 62        LD      A,($622A)           ; {ram.boardSeqPtr} and where it is now
0C18: B8              CP      B                   
...
loc_0c1f:
0C1F: 32 2F 62        LD      ($622F),A           ; {ram.howHighLastSeq} remember it for next time
...
0C95: AF              XOR     A                   
0C96: 32 8C 63        LD      ($638C),A           ; {ram.bonusDisplay} start the bonus readout at zero
...
0CA7: 3A 27 62        LD      A,($6227)           ; {ram.board} which board is being built
0CAA: 3D              DEC     A                   
...
0CBE: 3E 0B           LD      A,$0B               
0CC0: 32 89 60        LD      ($6089),A           ; {ram.sndBgm} start the rivet-board background tune
...
0CD7: 3E 08           LD      A,$08               
0CD9: 32 89 60        LD      ($6089),A           ; {ram.sndBgm} start the 25m background tune
...
0CEA: 3E 09           LD      A,$09               
0CEC: 32 89 60        LD      ($6089),A           ; {ram.sndBgm} the 50m tune, the middle of the four slots
...
0CF5: 3E 0A           LD      A,$0A               
0CF7: 32 89 60        LD      ($6089),A           ; {ram.sndBgm} start the 75m background tune
...
0DA8: 32 B3 63        LD      ($63B3),A           ; {ram.segKind} stash it for the drawers to dispatch on
...
0DBA: D1              POP     DE                  
0DBB: 22 AB 63        LD      ($63AB),HL          ; {ram.segAddr1} the cell the first point falls in
0DBE: 78              LD      A,B                 
...
0DE7: D1              POP     DE                  
0DE8: 22 AD 63        LD      ($63AD),HL          ; {ram.segAddr2} the cell the far end falls in
...
0DF3: 3A B2 63        LD      A,($63B2)           ; {ram.segRun} kinds 0 and 1: the ladders
0DF6: D6 10           SUB     $10                 
0DF8: 47              LD      B,A                 
...
0E15: AF              XOR     A                   
0E16: 32 B2 63        LD      ($63B2),A           ; {ram.segRun} kind 1 is caps only: no body run
...
0E5C: 32 B5 63        LD      ($63B5),A           ; {ram.segTile} the live tile code lives in this cell
0E5F: 2A AB 63        LD      HL,($63AB)          ; {ram.segAddr1} start at the girder's first cell
...
0EF0: 2A AB 63        LD      HL,($63AB)          ; {ram.segAddr1} start at the cell the record falls in
0EF3: 3E B3           LD      A,$B3               
...
0F32: 2A AB 63        LD      HL,($63AB)          ; {ram.segAddr1} start at the cell the record falls in
...
0F7A: 3A 29 62        LD      A,($6229)           ; {ram.level} the bonus scales with the level
0F7D: 47              LD      B,A                 
...
127F: 3A 9D 63        LD      A,($639D)           ; {ram.deathAnimPhase} which phase of the death animation
...
1299: 3E 0D           LD      A,$0D               
129B: 32 9E 63        LD      ($639E),A           ; {ram.deathAnimTicksLeft} thirteen ticks to run
129E: 3E 08           LD      A,$08               
12A0: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} the eight-frame tick cadence
12A3: CD BD 30        CALL    $30BD               ; {code.clearSpriteColumns}
12A6: 3E 03           LD      A,$03               
12A8: 32 88 60        LD      ($6088),A           ; {ram.sndIrqTrigger} fire the death sound, held three frames
12AB: C9              RET                         
...
12AD: 3E 08           LD      A,$08               
12AF: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} re-arm the eight-frame tick
...
12D8: 3E 80           LD      A,$80               
12DA: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} a long pause before the hand-off
12DD: C9              RET                         
...
12E5: 3A 0E 60        LD      A,($600E)           ; {ram.activePlayerIndex} which player just died
12E8: A7              AND     A                   
...
12F5: AF              XOR     A                   
12F6: 32 2C 62        LD      ($622C),A           ; {ram.playIntro} the next life skips the opening cutscene
...
loc_133f:
133F: 79              LD      A,C                 
1340: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} that is the next sub-state
1343: C9              RET                         

loc_1344:
1344: CD 1C 01        CALL    $011C               ; {code.silenceSound} everything goes quiet as the life ends
1347: AF              XOR     A                   
1348: 32 2C 62        LD      ($622C),A           ; {ram.playIntro} the next life skips the opening cutscene
...
1381: 3A 40 60        LD      A,($6040)           ; {ram.p1Context} player 1's saved life count
1384: A7              AND     A                   
...
loc_138a:
138A: 79              LD      A,C                 
138B: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} that is the next sub-state
138E: C9              RET                         
...
1392: 3A 48 60        LD      A,($6048)           ; {ram.p2Context} read player 2's saved life count
...
loc_139c:
139C: 79              LD      A,C                 
139D: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} publish the chosen follow-on sub-state
13A0: C9              RET                         
...
13A4: 3A 40 60        LD      A,($6040)           ; {ram.p1Context} read player 1's saved life count
13A7: C3 95 13        JP      $1395               ; {code.loc_1395} share the twin handler's re-arm and choice

loc_13aa:
13AA: 3A 26 60        LD      A,($6026)           ; {ram.dipUpright} read the cabinet orientation switch
13AD: 32 82 7D        LD      ($7D82),A           ; {hard.flipScreen} mirror it into the flip-screen latch
13B0: AF              XOR     A                   
13B1: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} restart the in-state sub-sequence
13B4: 21 01 01        LD      HL,$0101            ; 1 into both halves of the player-index pair
13B7: 22 0D 60        LD      ($600D),HL          ; {ram.currentPlayer} currentPlayer and activePlayerIndex together
13BA: C9              RET                         
...
13BB: AF              XOR     A                   
13BC: 32 0D 60        LD      ($600D),A           ; {ram.currentPlayer} player 1 is up
13BF: 32 0E 60        LD      ($600E),A           ; {ram.activePlayerIndex} player 1 is the active player
13C2: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} restart the in-game sub-state sequence
13C5: 3C              INC     A                   
...
1425: 3E 00           LD      A,$00               
1427: 32 0E 60        LD      ($600E),A           ; {ram.activePlayerIndex} clear both bytes of the player index
...
144F: 3E 01           LD      A,$01               
1451: 32 0E 60        LD      ($600E),A           ; {ram.activePlayerIndex} player 2 is the active player
1454: 32 0D 60        LD      ($600D),A           ; {ram.currentPlayer} player 2 is up; scoring keys off this
...
1460: 3E 00           LD      A,$00               
1462: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} no wait: the new sub-state runs next frame
...
147A: 32 05 60        LD      ($6005),A           ; {ram.gameState} state 1 is attract, dispatched next vblank
147D: 32 07 60        LD      ($6007),A           ; {ram.attract} no credited game in progress
1480: 3E 00           LD      A,$00               
1482: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} clear the sub-dispatch index
1485: C9              RET                         
...
1502: 3A 10 60        LD      A,($6010)           ; {ram.p1Input} the item's walk is driven by the control word
1505: CB 7F           BIT     7,A                 
...
1618: 3A 27 62        LD      A,($6227)           ; {ram.board} route on the board just cleared
161B: 0F              RRCA                        ; bit 0 marks 25m and 75m
161C: D2 2F 16        JP      NC,$162F            ; {code.loc_162f}
161F: 3A 88 63        LD      A,($6388)           ; {ram.boardAdvanceStep} which step of the interlude this frame is
...
1633: 3A 88 63        LD      A,($6388)           ; {ram.boardAdvanceStep} which step of the interlude this frame is
...
1644: 3A 88 63        LD      A,($6388)           ; {ram.boardAdvanceStep} which step of the rivet finale runs now
...
165D: 3E 20           LD      A,$20               
165F: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} hold the pose 32 frames
...
1677: 3E 20           LD      A,$20               
1679: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} hold the new pose 32 frames
...
16BB: AF              XOR     A                   
16BC: 32 A0 62        LD      ($62A0),A           ; {ram.m50Obj1ReverseTimer} clear the reversal flag: keep going
16BF: 3A A3 63        LD      A,($63A3)           ; {ram.m50Obj1Step} the published signed per-frame step
16C2: 4F              LD      C,A                 
...
loc_16d0:
16D0: 3E 01           LD      A,$01               
16D2: 32 A0 62        LD      ($62A0),A           ; {ram.m50Obj1ReverseTimer} schedule a reversal on the next tick
...
16D8: 3A A3 63        LD      A,($63A3)           ; {ram.m50Obj1Step} take the freshly published step
16DB: 4F              LD      C,A                 
...
173B: AF              XOR     A                   
173C: 32 00 69        LD      ($6900),A           ; {ram.spriteBuffer} park a sprite X at the left edge
...
1762: 3E 40           LD      A,$40               
1764: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} dwell 64 frames before the next sub-state
...
178F: 2A 2A 62        LD      HL,($622A)          ; {ram.boardSeqPtr} the board-order pointer
...
loc_179d:
179D: 22 2A 62        LD      ($622A),HL          ; {ram.boardSeqPtr} store the advanced pointer
17A0: 32 27 62        LD      ($6227),A           ; {ram.board} publish the board coming up
17A3: 11 00 05        LD      DE,$0500            
17A6: CD 9F 30        CALL    $309F               ; {code.enqueueTask} post its deferred task
17A9: AF              XOR     A                   
17AA: 32 88 63        LD      ($6388),A           ; {ram.boardAdvanceStep} the next cleared-board run starts at 0
...
1814: 3E 20           LD      A,$20               
1816: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} hold the tableau 32 frames
1819: 3E 80           LD      A,$80               
...
1822: 22 C0 63        LD      ($63C0),HL          ; {ram.seqAdvancePtr} aim the shared timed advancer at that byte
1825: C9              RET                         
...
1865: 3E 20           LD      A,$20               
1867: 32 09 60        LD      ($6009),A           ; {ram.substateTimer} hold it 32 frames
...
18F8: 3E 9F           LD      A,$9F               
18FA: 32 4C 69        LD      ($694C),A           ; {ram.marioSpriteRecord} at a fixed X to one side of mid-screen
18FD: 3A 03 62        LD      A,($6203)           ; {ram.marioX} unless he finished on the other side
...
190A: 3E 5F           LD      A,$5F               
190C: 32 4C 69        LD      ($694C),A           ; {ram.marioSpriteRecord} and the mirror-image X
...
1918: 3A 29 62        LD      A,($6229)           ; {ram.level} which of the two depends on the level's low bit
191B: 0F              RRCA                        
...
1931: 3A 03 62        LD      A,($6203)           ; {ram.marioX} which side he finished on, once more
1934: FE 80           CP      $80                 
1936: D0              RET     NC                  
1937: 3E 6F           LD      A,$6F               
...
loc_193d:
193D: 2A 2A 62        LD      HL,($622A)          ; {ram.boardSeqPtr} the board-order pointer
...
loc_194b:
194B: 22 2A 62        LD      ($622A),HL          ; {ram.boardSeqPtr} store the advanced pointer
194E: 32 27 62        LD      ($6227),A           ; {ram.board} publish the board coming up
...
195B: AF              XOR     A                   
195C: 32 2E 62        LD      ($622E),A           ; {ram.howHighIndex} the how-high height starts over
195F: 32 88 63        LD      ($6388),A           ; {ram.boardAdvanceStep} the next cleared-board run starts at 0
...
196E: 3A 0E 60        LD      A,($600E)           ; {ram.activePlayerIndex} the player who is up
1971: C6 12           ADD     A,$12               ; select that player's later phase group
1973: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} hand the sub-state machine on
1976: C9              RET                         
...
19C3: 00              NOP                         
19C4: 00              NOP                         
19C5: 3A 00 62        LD      A,($6200)           ; {ram.marioActive} the death hand-off: did he survive the frame?
19C8: A7              AND     A                   
...
scanObjectsAtMarioX:
19DA: 3A 03 62        LD      A,($6203)           ; {ram.marioX} Mario's X, the key for the broad phase
...
confirmObjectHit:
19ED: 3A 05 62        LD      A,($6205)           ; {ram.marioY} the Y half of the test
...
19FB: 22 43 63        LD      ($6343),HL          ; {ram.effectParamPtr} hand the record to the effect machine
19FE: AF              XOR     A                   
19FF: 32 42 63        LD      ($6342),A           ; {ram.effectSelect} effect kind 0
1A02: 3C              INC     A                   
1A03: 32 40 63        LD      ($6340),A           ; {ram.effectState} arm the effect for later frames
1A06: C9              RET                         
...
1A07: 3A 86 63        LD      A,($6386)           ; {ram.bonusExpiredStep} which step of the sequence is current
...
1A15: AF              XOR     A                   
1A16: 32 87 63        LD      ($6387),A           ; {ram.bonusExpiredDelay} clear it; the first tick wraps it to 255
1A19: 3E 02           LD      A,$02               
1A1B: 32 86 63        LD      ($6386),A           ; {ram.bonusExpiredStep} on to the counting step
...
1A24: 3E 03           LD      A,$03               
1A26: 32 86 63        LD      ($6386),A           ; {ram.bonusExpiredStep} on to the wait-for-landing step
1A29: C9              RET                         
...
1A2A: 3A 16 62        LD      A,($6216)           ; {ram.marioAirborne} wait until Mario has landed
1A2D: A7              AND     A                   
...
1A43: 3A 91 62        LD      A,($6291)           ; {ram.edgeRivetArmed} was he on an edge last frame?
1A46: 3D              DEC     A                   
...
armEdgeRivetPickup:
1A4B: 3E 01           LD      A,$01               
1A4D: 32 91 62        LD      ($6291),A           ; {ram.edgeRivetArmed} arm; nothing is collected this frame
1A50: C9              RET                         

loc_1a51:
1A51: 32 91 62        LD      ($6291),A           ; {ram.edgeRivetArmed} disarm the latch
...
1AAA: 3E 01           LD      A,$01               
1AAC: 32 40 63        LD      ($6340),A           ; {ram.effectState} arm the pickup effect
1AAF: 32 42 63        LD      ($6342),A           ; {ram.effectSelect} the pickup effect kind
1AB2: 32 25 62        LD      ($6225),A           ; {ram.itemCollected} flag the collection
...
1AE9: 3A 10 60        LD      A,($6010)           ; {ram.p1Input} read once for every arm below
...
1B5D: 32 17 62        LD      ($6217),A           ; {ram.marioHammerActive} commit a hammer touched while airborne
1B60: 21 07 62        LD      HL,$6207            ; Mario's sprite code
1B63: 7E              LD      A,(HL)              
1B64: E6 80           AND     $80                 ; keep the facing bit, drop the pose bits
1B66: 77              LD      (HL),A              
1B67: AF              XOR     A                   
1B68: 32 02 62        LD      ($6202),A           ; {ram.marioWalkAnim} restart the walk cycle
...
initMarioJump:
1B6E: 3E 01           LD      A,$01               
1B70: 32 16 62        LD      ($6216),A           ; {ram.marioAirborne} route him through the ballistic handler
...
1B96: 32 04 62        LD      ($6204),A           ; {ram.marioXFrac} clear the accumulated horizontal remainder
1B99: 32 06 62        LD      ($6206),A           ; {ram.marioYFrac} and the vertical one
...
1BA3: 32 07 62        LD      ($6207),A           ; {ram.marioSpriteCode} commit the jump sprite
1BA6: 3A 05 62        LD      A,($6205)           ; {ram.marioY}
1BA9: 32 0E 62        LD      ($620E),A           ; {ram.marioAirStartY} the take-off height the fall test measures
...
1BD8: 3A 20 62        LD      A,($6220)           ; {ram.marioFatalFall} is this fall already condemned?
1BDB: 3D              DEC     A                   
...
1C1B: 3E 01           LD      A,$01               
1C1D: 32 1F 62        LD      ($621F),A           ; {ram.marioAirLandcheck} arm the fall-height test for this arc
...
1C27: 32 42 63        LD      ($6342),A           ; {ram.effectSelect} the count picks the score award
1C2A: 3E 01           LD      A,$01               
1C2C: 32 40 63        LD      ($6340),A           ; {ram.effectState} raise the effect that pays it out
1C2F: 32 25 62        LD      ($6225),A           ; {ram.itemCollected} and flag it; the landing consumes it
1C32: 00              NOP                         
...
1C3E: 3C              INC     A                   
1C3F: 32 1F 62        LD      ($621F),A           ; {ram.marioAirLandcheck} arm the land-check phase
1C42: AF              XOR     A                   
...
1C4F: 32 16 62        LD      ($6216),A           ; {ram.marioAirborne} stop the airborne handler driving him
1C52: 3A 20 62        LD      A,($6220)           ; {ram.marioFatalFall} read back whether the fall was condemned
1C55: EE 01           XOR     $01                 ; invert it: a clean landing leaves him alive
1C57: 32 00 62        LD      ($6200),A           ; {ram.marioActive} a lethal one leaves him inert for the death
...
1C62: 77              LD      (HL),A              
1C63: 3E 04           LD      A,$04               
1C65: 32 1E 62        LD      ($621E),A           ; {ram.marioFreezeTimer} hold him still four frames on touchdown
1C68: AF              XOR     A                   
1C69: 32 1F 62        LD      ($621F),A           ; {ram.marioAirLandcheck} the fall-height test is finished with
1C6C: 3A 25 62        LD      A,($6225)           ; {ram.itemCollected} was a pickup latched during the jump?
1C6F: 3D              DEC     A                   
...
1C82: 3E 01           LD      A,$01               
1C84: 32 20 62        LD      ($6220),A           ; {ram.marioFatalFall} latch the fall lethal; the landing kills him
...
1C91: 3A 0F 62        LD      A,($620F)           ; {ram.marioMoveStepTimer} read the walk pacer
1C94: A7              AND     A                   
...
loc_1ca1:
1CA1: 32 02 62        LD      ($6202),A           ; {ram.marioWalkAnim} store the new walk-cycle index
...
1CAD: 3A 0F 62        LD      A,($620F)           ; {ram.marioMoveStepTimer} read the walk pacer
1CB0: A7              AND     A                   
...
loc_1cbd:
1CBD: 32 02 62        LD      ($6202),A           ; {ram.marioWalkAnim} store the new walk-cycle index
...
1CCA: 3E 02           LD      A,$02               
1CCC: 32 0F 62        LD      ($620F),A           ; {ram.marioMoveStepTimer} re-arm the pacer for two 1px slide frames
...
loc_1ce7:
1CE7: 7D              LD      A,L                 
1CE8: 32 05 62        LD      ($6205),A           ; {ram.marioY} commit the slope-corrected row
...
climbMarioDown:
1CF2: 3A 0F 62        LD      A,($620F)           ; {ram.marioMoveStepTimer} read the climb pacer
1CF5: A7              AND     A                   
1CF6: C2 8A 1D        JP      NZ,$1D8A            ; {code.tickMoveStepTimer} mid sub-step: hold the pose this frame
1CF9: 3E 03           LD      A,$03               
1CFB: 32 0F 62        LD      ($620F),A           ; {ram.marioMoveStepTimer} down-climb pace: three frames a sub-step
...
; stepper +2.
climbMarioUp:
1D03: 3A 0F 62        LD      A,($620F)           ; {ram.marioMoveStepTimer} read the climb pacer
1D06: A7              AND     A                   
1D07: C2 76 1D        JP      NZ,$1D76            ; {code.loc_1d76} mid sub-step: decide whether to tick the pacer
1D0A: 3E 04           LD      A,$04               
1D0C: 32 0F 62        LD      ($620F),A           ; {ram.marioMoveStepTimer} up-climb pace: four frames a sub-step
...
1D49: 3E 01           LD      A,$01               
1D4B: 32 15 62        LD      ($6215),A           ; {ram.marioOnLadder} on a ladder; only the dismount clears this
...
1D70: 32 15 62        LD      ($6215),A           ; {ram.marioOnLadder} he is off the ladder now
...
triggerWalkSound:
1D8F: 3E 03           LD      A,$03               
1D91: 32 80 60        LD      ($6080),A           ; {ram.sndTrigger} hold the footstep latch three frames
1D94: C9              RET                         

loc_1d95:
1D95: 32 25 62        LD      ($6225),A           ; {ram.itemCollected} commit the collection flag; callers pass 0
...
dispatchEffectState:
1DBD: 3A 40 63        LD      A,($6340)           ; {ram.effectState} read the effect machine's state
...
1DC9: 3E 40           LD      A,$40               
1DCB: 32 41 63        LD      ($6341),A           ; {ram.effectTimer} arm the 64-frame hold of the display state
1DCE: 3E 02           LD      A,$02               
1DD0: 32 40 63        LD      ($6340),A           ; {ram.effectState} step the machine on to that display state
1DD3: 3A 42 63        LD      A,($6342)           ; {ram.effectSelect} read the award selector
1DD6: 1F              RRA                         
...
1DE7: 3A 29 62        LD      A,($6229)           ; {ram.level} and take the tier from the level instead
1DEA: 3D              DEC     A                   
...
pickRandomAwardTier:
1DF5: 3A 18 60        LD      A,($6018)           ; {ram.random} read the rolling random byte
1DF8: 1F              RRA                         
...
1E18: 2A 43 63        LD      HL,($6343)          ; {ram.effectParamPtr} point at the effect's parameter block
...
1E30: 4F              LD      C,A                 
1E31: 3A 03 62        LD      A,($6203)           ; {ram.marioX} and in his own column
1E34: 00              NOP                         
1E35: 00              NOP                         
...
1E4F: AF              XOR     A                   
1E50: 32 30 6A        LD      ($6A30),A           ; {ram.popupSprite} blank the popup sprite
1E53: 32 40 63        LD      ($6340),A           ; {ram.effectState} and return the machine to idle
1E56: C9              RET                         
...
checkBoardWonByType:
1E57: 3A 27 62        LD      A,($6227)           ; {ram.board} read the board being played
...
1E69: 3A 03 62        LD      A,($6203)           ; {ram.marioX} his X's high bit picks the facing he is left in
1E6C: 17              RLA                         
...
1E80: 3A 90 62        LD      A,($6290)           ; {ram.rivetsLeft} how many rivets are still standing?
1E83: A7              AND     A                   
...
1E85: 3E 16           LD      A,$16               
1E87: 32 0A 60        LD      ($600A),A           ; {ram.gameSubstate} enter the board-cleared sub-state
...
1E96: 3A 45 63        LD      A,($6345)           ; {ram.effectSeqState} read the effect sequence's step
...
loc_1eb4:
1EB4: DD 2A 51 63     LD      IX,($6351)          ; {ram.collidedObjectBase} the hit object's array base
1EB8: 16 00           LD      D,$00               
1EBA: 3A 53 63        LD      A,($6353)           ; {ram.collidedObjectStride} that array's record stride
1EBD: 5F              LD      E,A                 
1EBE: 01 04 00        LD      BC,$0004            ; sprite records are a fixed four bytes
1EC1: 3A 54 63        LD      A,($6354)           ; {ram.collidedObjectIndex} the hit record's index within the array
1EC4: A7              AND     A                   
...
loc_1ede:
1EDE: 32 42 63        LD      ($6342),A           ; {ram.effectSelect} stage the variant for the award setter
...
1F3B: 3C              INC     A                   
1F3C: 32 40 63        LD      ($6340),A           ; {ram.effectState} re-arm the parent effect machine
1F3F: 21 2C 6A        LD      HL,$6A2C            
1F42: 22 43 63        LD      ($6343),HL          ; {ram.effectParamPtr} point it back at the effect sprite
1F45: C9              RET                         
...
beginMarioFall:
1F46: 3A 21 62        LD      A,($6221)           ; {ram.marioStartFall} read the fall trigger
1F49: A7              AND     A                   
1F4A: C8              RET     Z                   ; no ground went away this frame
1F4B: AF              XOR     A                   
1F4C: 32 04 62        LD      ($6204),A           ; {ram.marioXFrac} the fall starts from a whole pixel
1F4F: 32 06 62        LD      ($6206),A           ; {ram.marioYFrac}
1F52: 32 21 62        LD      ($6221),A           ; {ram.marioStartFall} consume the one-shot trigger
1F55: 32 10 62        LD      ($6210),A           ; {ram.marioAirVxHi} no horizontal speed: he drops straight down
1F58: 32 11 62        LD      ($6211),A           ; {ram.marioAirVxLo}
1F5B: 32 12 62        LD      ($6212),A           ; {ram.marioAirVyHi} and no vertical speed: gravity alone
1F5E: 32 13 62        LD      ($6213),A           ; {ram.marioAirVyLo}
1F61: 32 14 62        LD      ($6214),A           ; {ram.marioAirFrames} arc frame count back to zero
1F64: 3C              INC     A                   
1F65: 32 16 62        LD      ($6216),A           ; {ram.marioAirborne} the mover drives him airborne from next frame
1F68: 32 1F 62        LD      ($621F),A           ; {ram.marioAirLandcheck} evaluate the drop from the first frame
1F6B: 3A 05 62        LD      A,($6205)           ; {ram.marioY}
1F6E: 32 0E 62        LD      ($620E),A           ; {ram.marioAirStartY} remember the height he fell from
1F71: C9              RET                         
...
update25mBarrels:
1F72: 3A 27 62        LD      A,($6227)           ; {ram.board} read the board being played
1F75: 3D              DEC     A                   
...
2186: 3A 80 63        LD      A,($6380)           ; {ram.difficulty} weight the throttle by difficulty
2189: 1F              RRA                         
218A: 3C              INC     A                   
218B: 47              LD      B,A                 ; half the difficulty, plus one
218C: 3A 18 60        LD      A,($6018)           ; {ram.random} read the rolling random byte
218F: 4F              LD      C,A                 
2190: E6 03           AND     $03                 
...
21F9: 32 10 60        LD      ($6010),A           ; {ram.p1Input} overwrite the control word: the demo plays itself
21FC: 2C              INC     L                   
...
220A: 3A 1A 60        LD      A,($601A)           ; {ram.frame} frame parity picks which record is serviced
220D: 1F              RRA                         
...
2243: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read Mario's row
...
224B: 3A 16 62        LD      A,($6216)           ; {ram.marioAirborne} is he mid jump or fall?
224E: A7              AND     A                   
...
2279: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read Mario's screen row
...
229A: 3A 18 60        LD      A,($6018)           ; {ram.random} sample the random accumulator
...
22D2: 3A 80 63        LD      A,($6380)           ; {ram.difficulty} otherwise the difficulty picks the source
...
loc_22e1:
22E1: 3A 29 62        LD      A,($6229)           ; {ram.level} read the level
22E4: 47              LD      B,A                 
22E5: 05              DEC     B                   
...
loc_22f6:
22F6: 3A 18 60        LD      A,($6018)           ; {ram.random} difficulty 1-2: magnitude straight from chance
...
loc_2303:
2303: 3A 18 60        LD      A,($6018)           ; {ram.random} difficulty 3-4: a random step magnitude
2306: DD 77 11        LD      (IX+$11),A          
2309: 3A 03 62        LD      A,($6203)           ; {ram.marioX} steer the step toward Mario
...
loc_231a:
231A: 3A 03 62        LD      A,($6203)           ; {ram.marioX} difficulty 5: derive both fields from the offset
...
2422: 3A 03 62        LD      A,($6203)           ; {ram.marioX} read Mario's column
...
242E: 3A 27 62        LD      A,($6227)           ; {ram.board} the gate never opens on 50m or 100m
2431: 0F              RRCA                        
2432: D0              RET     NC                  
2433: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read his screen row
2436: FE 58           CP      $58                 ; below the top platform -- no restraint
2438: D0              RET     NC                  
2439: 3A 03 62        LD      A,($6203)           ; {ram.marioX} read his column again
...
loc_2456:
2456: 3A 27 62        LD      A,($6227)           ; {ram.board} pick the record table for this board
2459: 3D              DEC     A                   
...
252B: 3A 9A 63        LD      A,($639A)           ; {ram.objSpawnReq} is a spawn actually being asked for?
252E: A7              AND     A                   
...
2551: 3A A3 62        LD      A,($62A3)           ; {ram.m50Obj2StepDir} otherwise object 2's heading decides
2554: 3D              DEC     A                   
2555: C2 6E 25        JP      NZ,$256E            ; {code.loc_256e} not that way -- re-roll, just for the side

loc_2558:
2558: DD 36 05 CC     LD      (IX+$05),$CC        ; the lower row
255C: 3A A6 62        LD      A,($62A6)           ; {ram.m50Obj3StepDir} object 3's heading picks the side
255F: 07              RLCA                        
...
2586: 3E 7C           LD      A,$7C               
2588: 32 9B 63        LD      ($639B),A           ; {ram.objSpawnTimer} no other may spawn for 124 frames
258B: AF              XOR     A                   
258C: 32 9A 63        LD      ($639A),A           ; {ram.objSpawnReq} the request is consumed
...
25B4: 3A A6 63        LD      A,($63A6)           ; {ram.m50Obj3Step} everything else rides object 3's step
25B7: 84              ADD     A,H                 
...
25C6: 3A A5 63        LD      A,($63A5)           ; {ram.m50Obj2StepPos} past the middle, object 2's step
25C9: D2 CF 25        JP      NC,$25CF            ; {code.loc_25cf}
25CC: 3A A4 63        LD      A,($63A4)           ; {ram.m50Obj2StepNeg} short of it, the mirror of that step
...
loc_2602:
2602: 3A 1A 60        LD      A,($601A)           ; {ram.frame} read the frame counter
2605: 0F              RRCA                        
...
261C: 32 A3 63        LD      ($63A3),A           ; {ram.m50Obj1Step} publish the step the mover reads
...
2652: 32 A5 63        LD      ($63A5),A           ; {ram.m50Obj2StepPos} publish the positive-going step
2655: ED 44           NEG                         ; negate it
2657: 32 A4 63        LD      ($63A4),A           ; {ram.m50Obj2StepNeg} publish the other polarity too
265A: 3A 1A 60        LD      A,($601A)           ; {ram.frame} read the frame counter
265D: E6 1F           AND     $1F                 
...
loc_2679:
2679: 3A 1A 60        LD      A,($601A)           ; {ram.frame} read the frame counter
267C: 0F              RRCA                        
...
2693: 32 A6 63        LD      ($63A6),A           ; {ram.m50Obj3Step} publish object 3's step
2696: 3A 1A 60        LD      A,($601A)           ; {ram.frame} read the frame counter
2699: E6 1F           AND     $1F                 
...
signStepHalfRate:
26E9: 3A 1A 60        LD      A,($601A)           ; {ram.frame} read the frame counter
26EC: E6 01           AND     $01                 
...
26FD: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read Mario's screen row
...
2705: 3A 29 62        LD      A,($6229)           ; {ram.level} read the level
2708: 3D              DEC     A                   
2709: 3A 1A 60        LD      A,($601A)           ; {ram.frame} read the frame counter
...
2745: 3A 98 63        LD      A,($6398)           ; {ram.edgeRepositionFlag} the lift flag: is he riding one?
2748: A7              AND     A                   
2749: C8              RET     Z                   ; no -- nothing to carry
274A: 3A 16 62        LD      A,($6216)           ; {ram.marioAirborne} is he airborne?
274D: A7              AND     A                   
274E: C0              RET     NZ                  ; yes -- carry him only on a grounded frame
274F: 3A 03 62        LD      A,($6203)           ; {ram.marioX} read his column
...
loc_2766:
2766: AF              XOR     A                   
2767: 32 98 63        LD      ($6398),A           ; {ram.edgeRepositionFlag} clear the lift flag
276A: 3C              INC     A                   
276B: 32 21 62        LD      ($6221),A           ; {ram.marioStartFall} and start him falling
276E: C9              RET                         
...
276F: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read Mario's screen row
...
277F: AF              XOR     A                   
2780: 32 00 62        LD      ($6200),A           ; {ram.marioActive} clear the alive flag -- he dies here
2783: 32 98 63        LD      ($6398),A           ; {ram.edgeRepositionFlag} and the lift flag with it
2786: C9              RET                         
...
2787: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read Mario's screen row
...
280C: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read his screen row
280F: 4F              LD      C,A                 
...
2819: 32 00 62        LD      ($6200),A           ; {ram.marioActive} clear the alive flag -- he dies
281C: C9              RET                         
...
2843: 3A B9 63        LD      A,($63B9)           ; {ram.objSearchCount} the count the sweep started with
2846: 90              SUB     B                   ; less the ones still to go: which record was struck
2847: 32 54 63        LD      ($6354),A           ; {ram.collidedObjectIndex}
284A: 7B              LD      A,E                 ; the stride the board's sweep was walking
284B: 32 53 63        LD      ($6353),A           ; {ram.collidedObjectStride}
284E: DD 22 51 63     LD      ($6351),IX          ; {ram.collidedObjectBase} and the array it came from
2852: C9              RET                         
...
2857: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read his screen row
285A: C6 0C           ADD     A,$0C               ; probe twelve pixels below him
285C: 4F              LD      C,A                 
285D: 3A 10 60        LD      A,($6010)           ; {ram.p1Input} read the control word
...
286F: 3A 27 62        LD      A,($6227)           ; {ram.board} read the current board
...
2883: 78              LD      A,B                 
2884: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp the count, for index recovery
...
2893: 78              LD      A,B                 
2894: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp this sweep's count
2897: 1E 20           LD      E,$20               
...
28A2: 78              LD      A,B                 
28A3: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp this sweep's count
28A6: 1E 00           LD      E,$00               
...
28B3: 78              LD      A,B                 
28B4: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp the count, for index recovery
...
28C3: 78              LD      A,B                 
28C4: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp this sweep's count
28C7: 1E 10           LD      E,$10               
...
28D2: 78              LD      A,B                 
28D3: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp this sweep's count
28D6: 1E 00           LD      E,$00               
...
28E3: 78              LD      A,B                 
28E4: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp the count, for index recovery
...
28F3: 78              LD      A,B                 
28F4: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp this sweep's count
28F7: 1E 10           LD      E,$10               
...
2904: 78              LD      A,B                 
2905: 32 B9 63        LD      ($63B9),A           ; {ram.objSearchCount} stamp the count, for index recovery
...
295A: 32 18 62        LD      ($6218),A           ; {ram.marioHammerPending} publish the touch; a miss clears it
...
2978: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read his screen row
297B: 4F              LD      C,A                 
...
298C: 2A C8 63        LD      HL,($63C8)          ; {ram.objIterPtr} the record the fire driver is walking
...
29B6: 3A 05 62        LD      A,($6205)           ; {ram.marioY} read his screen row
29B9: 4F              LD      C,A                 
...
29D5: 57              LD      D,A                 
29D6: 3A 0C 62        LD      A,($620C)           ; {ram.marioAirPrevY} where he was before this frame's motion
29D9: C6 05           ADD     A,$05               
29DB: BA              CP      D                   
...
29E0: D6 08           SUB     $08                 
29E2: 32 05 62        LD      ($6205),A           ; {ram.marioY} place him there
29E5: 3E 01           LD      A,$01               
29E7: 47              LD      B,A                 
29E8: 32 98 63        LD      ($6398),A           ; {ram.edgeRepositionFlag} raise the just-repositioned one-shot
29EB: 33              INC     SP                  ; skip the caller -- the handler above resumes
29EC: 33              INC     SP                  
29ED: C9              RET                         

loc_29ee:
29EE: 3A 0C 62        LD      A,($620C)           ; {ram.marioAirPrevY} where Mario was before this frame's motion
...
29F7: 3A 10 62        LD      A,($6210)           ; {ram.marioAirVxHi} route selector: airborne horizontal speed
29FA: A7              AND     A                   
29FB: 3A 03 62        LD      A,($6203)           ; {ram.marioX} his X, the value both routes snap
...
loc_2a0e:
2A0E: 32 03 62        LD      ($6203),A           ; {ram.marioX} side-on contact: put him in the cell middle
2A11: 32 4C 69        LD      ($694C),A           ; {ram.marioSpriteRecord} so the sprite follows him
...
loc_2a1b:
2A1B: AF              XOR     A                   
2A1C: 32 00 62        LD      ($6200),A           ; {ram.marioActive} struck from below -- Mario dies
2A1F: C9              RET                         
...
2A85: 3A 15 62        LD      A,($6215)           ; {ram.marioOnLadder} footing is not in question while on a ladder
2A88: A7              AND     A                   
2A89: C0              RET     NZ                  
2A8A: 3A 16 62        LD      A,($6216)           ; {ram.marioAirborne} nor while he is already in the air
2A8D: A7              AND     A                   
2A8E: C0              RET     NZ                  
2A8F: 3A 98 63        LD      A,($6398)           ; {ram.edgeRepositionFlag} nor while an edge reposition is running
2A92: FE 01           CP      $01                 
2A94: C8              RET     Z                   
2A95: 3A 03 62        LD      A,($6203)           ; {ram.marioX} foot probe: 3 pixels back along his X
2A98: D6 03           SUB     $03                 
2A9A: 67              LD      H,A                 
2A9B: 3A 05 62        LD      A,($6205)           ; {ram.marioY} and 12 along his Y
2A9E: C6 0C           ADD     A,$0C               
2AA0: 6F              LD      L,A                 
...
triggerMarioFall:
2ACD: 3E 01           LD      A,$01               
2ACF: 32 21 62        LD      ($6221),A           ; {ram.marioStartFall} raise the one-shot that launches the fall
2AD2: C9              RET                         
...
2AD3: 3A 03 62        LD      A,($6203)           ; {ram.marioX} his X, the value each mover adds its step to
2AD6: 47              LD      B,A                 
2AD7: 3A 05 62        LD      A,($6205)           ; {ram.marioY} which platform row is he standing on?
...
loc_2aea:
2AEA: 3A A3 63        LD      A,($63A3)           ; {ram.m50Obj1Step} that row's published drift step
2AED: C3 02 2B        JP      $2B02               ; {code.moveMarioX} carry him by it

loc_2af0:
2AF0: 3A A6 63        LD      A,($63A6)           ; {ram.m50Obj3Step} that row's published drift step
...
2AF7: FE 80           CP      $80                 
2AF9: 3A A5 63        LD      A,($63A5)           ; {ram.m50Obj2StepPos} the positive-drift shadow
2AFC: D2 02 2B        JP      NC,$2B02            ; {code.moveMarioX} far-right half of the range: drift positive
2AFF: 3A A4 63        LD      A,($63A4)           ; {ram.m50Obj2StepNeg} otherwise the negative-drift shadow
...
2B03: 32 03 62        LD      ($6203),A           ; {ram.marioX} move Mario
2B06: 32 4C 69        LD      ($694C),A           ; {ram.marioSpriteRecord} and the sprite record that draws him
...
2B29: 3A 27 62        LD      A,($6227)           ; {ram.board} 25m takes the single-point probe below
2B2C: 3D              DEC     A                   
2B2D: C2 53 2B        JP      NZ,$2B53            ; {code.loc_2b53} every other board uses the two-point form
2B30: 3A 03 62        LD      A,($6203)           ; {ram.marioX} probe point: straight under Mario
2B33: 67              LD      H,A                 
2B34: 3A 05 62        LD      A,($6205)           ; {ram.marioY} seven pixels below his feet
2B37: C6 07           ADD     A,$07               
2B39: 6F              LD      L,A                 
...
2B4B: 32 05 62        LD      ($6205),A           ; {ram.marioY} land Mario
2B4E: 3E 01           LD      A,$01               
...
loc_2b53:
2B53: 3A 03 62        LD      A,($6203)           ; {ram.marioX} first probe: 3 pixels back along his X
2B56: D6 03           SUB     $03                 
2B58: 67              LD      H,A                 
2B59: 3A 05 62        LD      A,($6205)           ; {ram.marioY} and seven below his feet
2B5C: C6 07           ADD     A,$07               
2B5E: 6F              LD      L,A                 
...
loc_2b7a:
2B7A: 3A 10 62        LD      A,($6210)           ; {ram.marioAirVxHi} route selector: airborne horizontal speed
2B7D: A7              AND     A                   
2B7E: 3A 03 62        LD      A,($6203)           ; {ram.marioX} the X both routes snap
...
loc_2b91:
2B91: 32 03 62        LD      ($6203),A           ; {ram.marioX} put Mario on the snapped column
2B94: 32 4C 69        LD      ($694C),A           ; {ram.marioSpriteRecord} so the sprite jumps there this frame
...
2BE1: 3A 0C 62        LD      A,($620C)           ; {ram.marioAirPrevY} where he was before this frame's motion
...
2BF2: 32 05 62        LD      ($6205),A           ; {ram.marioY} land Mario on this tile
...
2C0C: 3A B1 62        LD      A,($62B1)           ; {ram.bonus} the live bonus is what paces the throws
2C0F: A7              AND     A                   
2C10: C8              RET     Z                   ; no bonus left: nothing to schedule
2C11: 4F              LD      C,A                 
2C12: 3A B0 62        LD      A,($62B0)           ; {ram.bonusStart} the board's starting bonus
...
2C1B: 3A 82 63        LD      A,($6382)           ; {ram.barrelClaimMode} the slot-claim mode byte
2C1E: CB 4F           BIT     1,A                 
2C20: C2 86 2C        JP      NZ,$2C86            ; {code.loc_2c86} bit 1 set: the clear-then-mode-3 entry
2C23: 3A 80 63        LD      A,($6380)           ; {ram.difficulty} the countdown's length is the difficulty
2C26: 47              LD      B,A                 
2C27: 3A 1A 60        LD      A,($601A)           ; {ram.frame} phase test on the frame counter
2C2A: E6 1F           AND     $1F                 
...
loc_2c33:
2C33: 3A B0 62        LD      A,($62B0)           ; {ram.bonusStart} half the starting bonus
2C36: CB 3F           SRL     A                   
2C38: B9              CP      C                   
2C39: DA 41 2C        JP      C,$2C41             ; {code.loc_2c41} past halfway: fire on every matching frame
2C3C: 3A 19 60        LD      A,($6019)           ; {ram.spinCount} earlier, fire only on odd spin-counter frames
2C3F: 0F              RRCA                        
2C40: D0              RET     NC                  
...
loc_2c4b:
2C4B: 32 82 63        LD      ($6382),A           ; {ram.barrelClaimMode} record the claim mode
...
2C57: 3A B2 62        LD      A,($62B2)           ; {ram.bonusEventMark} the bonus value this event is scheduled at
2C5A: B9              CP      C                   
...
2C72: 3A 82 63        LD      A,($6382)           ; {ram.barrelClaimMode} the slot-claim mode byte
2C75: F6 80           OR      $80                 ; bit 7 selects the alternate barrel kind
2C77: 32 82 63        LD      ($6382),A           ; {ram.barrelClaimMode} the mode value in the low bits survives
2C7A: C9              RET                         
...
loc_2c86:
2C86: AF              XOR     A                   
2C87: 32 82 63        LD      ($6382),A           ; {ram.barrelClaimMode} wipe it, so nothing stale carries over
...
2CB8: DD 22 AA 62     LD      ($62AA),IX          ; {ram.renderObjPtr} publish the claimed record
...
2CCB: 19              ADD     HL,DE               
2CCC: 22 AC 62        LD      ($62AC),HL          ; {ram.renderDstPtr} barrel k renders into sprite slot k
2CCF: 3E 01           LD      A,$01               
...
2CE1: 3E 01           LD      A,$01               
2CE3: 32 86 63        LD      ($6386),A           ; {ram.bonusExpiredStep} bonus exhausted: start the expiry sequence
...
2D02: 3A 82 63        LD      A,($6382)           ; {ram.barrelClaimMode} bit 7 selects the barrel kind
2D05: 07              RLCA                        
...
2D27: 3A 82 63        LD      A,($6382)           ; {ram.barrelClaimMode} bit 0 picks which record index
2D2A: 0F              RRCA                        
...
2D4A: 3A 82 63        LD      A,($6382)           ; {ram.barrelClaimMode} the same bit 0 picks the ending
2D4D: 0F              RRCA                        
2D4E: DA 83 2D        JP      C,$2D83             ; {code.loc_2d83} set: restart the one-waypoint table

loc_2d51:
2D51: 2A A8 62        LD      HL,($62A8)          ; {ram.renderStrPtr} reload the path cursor
...
2D55: DD 2A AA 62     LD      IX,($62AA)          ; {ram.renderObjPtr} the barrel being moved
2D59: ED 5B AC 62     LD      DE,($62AC)          ; {ram.renderDstPtr} the sprite slot it publishes into
...
2D7D: 12              LD      (DE),A              
2D7E: 23              INC     HL                  
2D7F: 22 A8 62        LD      ($62A8),HL          ; {ram.renderStrPtr} advance the cursor past this waypoint
2D82: C9              RET                         

loc_2d83:
2D83: 21 CC 39        LD      HL,$39CC            ; the one-waypoint path
2D86: 22 A8 62        LD      ($62A8),HL          ; {ram.renderStrPtr} restart the walk at its head
...
2D96: 3A 82 63        LD      A,($6382)           ; {ram.barrelClaimMode} bit 0 chose which path was walked
2D99: 0F              RRCA                        
...
2DDF: 3A 80 63        LD      A,($6380)           ; {ram.difficulty} the period narrows as difficulty rises
2DE2: 3C              INC     A                   
2DE3: A7              AND     A                   ; clear carry so the halving shifts in a zero
2DE4: 1F              RRA                         ; steps = (difficulty + 1) / 2
2DE5: 47              LD      B,A                 
2DE6: 3A 27 62        LD      A,($6227)           ; {ram.board} 50m gets one extra step, so it fires twice as often
2DE9: FE 02           CP      $02                 
...
2DF5: 47              LD      B,A                 
2DF6: 3A 1A 60        LD      A,($601A)           ; {ram.frame} fire on the frames that land zero under the mask
2DF9: A0              AND     B                   
2DFA: C0              RET     NZ                  
2DFB: 3E 01           LD      A,$01               
2DFD: 32 A0 63        LD      ($63A0),A           ; {ram.eventReq313c} raise the insert request
2E00: 32 9A 63        LD      ($639A),A           ; {ram.objSpawnReq} and the request that moves with it
2E03: C9              RET                         
...
2E19: 3A 1A 60        LD      A,($601A)           ; {ram.frame} once every sixteen frames...
2E1C: E6 0F           AND     $0F                 
...
2EA7: 3A 96 63        LD      A,($6396)           ; {ram.spawnRequest} is a spawn pending?
2EAA: 0F              RRCA                        
2EAB: D2 78 2E        JP      NC,$2E78            ; {code.advanceToNextObject} no: leave the slot empty
2EAE: AF              XOR     A                   
2EAF: 32 96 63        LD      ($6396),A           ; {ram.spawnRequest} consume it, so only one slot fills this pass
...
2EF5: 3A 17 62        LD      A,($6217)           ; {ram.marioHammerActive} is a hammer in his hands?
2EF8: 0F              RRCA                        
2EF9: D2 97 2F        JP      NC,$2F97            ; {code.buildPendingHammerSprite}
2EFC: AF              XOR     A                   
2EFD: 32 18 62        LD      ($6218),A           ; {ram.marioHammerPending} in hand now, so no longer merely pending
...
2F0F: 3A 07 62        LD      A,($6207)           ; {ram.marioSpriteCode} the hammer faces the way Mario faces
2F12: CB 27           SLA     A                   
...
2F1D: 4F              LD      C,A                 
2F1E: 3A 94 63        LD      A,($6394)           ; {ram.hammerTimerLo} bit 3 alternates the two swing poses
2F21: CB 5F           BIT     3,A                 
...
2F5A: AF              XOR     A                   
2F5B: 32 95 63        LD      ($6395),A           ; {ram.hammerTimerHi} the hammer's time is up
2F5E: 32 17 62        LD      ($6217),A           ; {ram.marioHammerActive} it leaves his hands
...
2F6C: 3A 07 62        LD      A,($6207)           ; {ram.marioSpriteCode} give Mario his ordinary pose back
2F6F: 32 4D 69        LD      ($694D),A           ; {hard.workRam+94D}
2F72: DD 36 00 00     LD      (IX+$00),$00        ; and deactivate the hammer object
2F76: 3A 89 63        LD      A,($6389)           ; {ram.hammerSavedBgm} restore the tune the hammer interrupted
...
2F7C: EB              EX      DE,HL               
2F7D: 3A 03 62        LD      A,($6203)           ; {ram.marioX} the sprite sits at a fixed offset from Mario
...
2F8B: 23              INC     HL                  
2F8C: 3A 05 62        LD      A,($6205)           ; {ram.marioY} the other axis, likewise offset
...
2F97: 3A 18 62        LD      A,($6218)           ; {ram.marioHammerPending} is a hammer waiting to be picked up?
2F9A: 0F              RRCA                        
...
2FA4: 3A 07 62        LD      A,($6207)           ; {ram.marioSpriteCode} take Mario's facing bit
2FA7: 07              RLCA                        
2FA8: 3E 3C           LD      A,$3C               
2FAA: 1F              RRA                         ; so the waiting hammer faces the way he does
2FAB: 47              LD      B,A                 
2FAC: 0E 07           LD      C,$07               ; its attribute
2FAE: 3A 89 60        LD      A,($6089)           ; {ram.sndBgm} remember the current tune...
2FB1: 32 89 63        LD      ($6389),A           ; {ram.hammerSavedBgm} ...so it can come back when the hammer ends
...
2FB7: 3A 95 63        LD      A,($6395)           ; {ram.hammerTimerHi} how far the hammer's life has run
2FBA: A7              AND     A                   
...
2FBE: 3A 1A 60        LD      A,($601A)           ; {ram.frame} the flash runs off the frame counter
...
2FD8: 32 96 63        LD      ($6396),A           ; {ram.spawnRequest} ...and request a board object
2FDB: 11 01 05        LD      DE,$0501            ; task: take one notch off the bonus readout
2FDE: CD 9F 30        CALL    $309F               ; {code.enqueueTask}
2FE1: 3A B3 62        LD      A,($62B3)           ; {ram.bonusPeriod} reload the tick for the next period
2FE4: 77              LD      (HL),A              
...
2FEA: 3E 01           LD      A,$01               
2FEC: 32 86 63        LD      ($6386),A           ; {ram.bonusExpiredStep} bonus exhausted: start the expiry sequence
2FEF: C9              RET                         
...
304D: 3A 8E 63        LD      A,($638E)           ; {ram.introScrollIndex} which cell of the strip moves this call
3050: 4F              LD      C,A                 
3051: 06 00           LD      B,$00               
...
306A: 2A C0 63        LD      HL,($63C0)          ; {ram.seqAdvancePtr} the word names whichever step is armed
...
30A3: 3A B0 60        LD      A,($60B0)           ; {ram.taskTail} low byte of the next slot to write
...
loc_30b8:
30B8: 32 B0 60        LD      ($60B0),A           ; {ram.taskTail} publish the advanced tail
...
30FA: 3A 80 63        LD      A,($6380)           ; {ram.difficulty} which frame gate paces the fires
...
loc_3110:
3110: 3A 1A 60        LD      A,($601A)           ; {ram.frame} the frame counter drives the duty cycle
...
3140: AF              XOR     A                   
3141: 32 A1 63        LD      ($63A1),A           ; {ram.objLiveCount} start the live tally at zero
...
3151: 3A A1 63        LD      A,($63A1)           ; {ram.objLiveCount} live record: the running tally
3154: 3C              INC     A                   
3155: 32 A1 63        LD      ($63A1),A           ; {ram.objLiveCount} count it
3158: 3E 01           LD      A,$01               
315A: DD 77 08        LD      (IX+$08),A          ; flag the fire's sprite attribute on
315D: 3A 17 62        LD      A,($6217)           ; {ram.marioHammerActive} is Mario swinging a hammer?
3160: FE 01           CP      $01                 
...
3173: 3A A1 63        LD      A,($63A1)           ; {ram.objLiveCount} the finished tally
3176: FE 00           CP      $00                 
...
loc_317c:
317C: 3A A1 63        LD      A,($63A1)           ; {ram.objLiveCount} empty slot: how many are live so far
317F: FE 05           CP      $05                 
3181: CA 6A 31        JP      Z,$316A             ; {code.loc_316a} array already full -- nothing can go in
3184: 3A 27 62        LD      A,($6227)           ; {ram.board} 50m is the only board that caps spawns here
3187: FE 02           CP      $02                 
...
loc_3195:
3195: 3A A0 63        LD      A,($63A0)           ; {ram.eventReq313c} is an insert being asked for?
3198: FE 01           CP      $01                 
...
31A3: AF              XOR     A                   
31A4: 32 A0 63        LD      ($63A0),A           ; {ram.eventReq313c} consume the request -- one insert per sweep
31A7: 3A A1 63        LD      A,($63A1)           ; {ram.objLiveCount}
31AA: 3C              INC     A                   
31AB: 32 A1 63        LD      ($63A1),A           ; {ram.objLiveCount} count the new fire
...
31BB: 22 C8 63        LD      ($63C8),HL          ; {ram.objIterPtr} where the per-fire body finds its record

loc_31be:
31BE: 2A C8 63        LD      HL,($63C8)          ; {ram.objIterPtr} re-read it -- a callee may have moved it
31C1: 01 20 00        LD      BC,$0020            
31C4: 09              ADD     HL,BC               ; step on to the next record
31C5: 22 C8 63        LD      ($63C8),HL          ; {ram.objIterPtr} publish it before the body is entered
...
31DD: 3A 80 63        LD      A,($6380)           ; {ram.difficulty} difficulty gates this arming pass
31E0: FE 03           CP      $03                 
...
loc_31f6:
31F6: 3A 18 60        LD      A,($6018)           ; {ram.random} the entropy draw's first source
31F9: E6 03           AND     $03                 
31FB: FE 01           CP      $01                 
31FD: C0              RET     NZ                  ; 0, 2 or 3 is the draw itself
31FE: 3A 1A 60        LD      A,($601A)           ; {ram.frame} exactly 1 substitutes the frame counter
3201: C9              RET                         
...
advanceFire:
3202: DD 2A C8 63     LD      IX,($63C8)          ; {ram.objIterPtr} the record the walk is pointing at
...
loc_3221:
3221: 3A 18 60        LD      A,($6018)           ; {ram.random} keep the rerolls off a fixed cadence
3224: E6 03           AND     $03                 
...
3246: DD 2A C8 63     LD      IX,($63C8)          ; {ram.objIterPtr} re-read the record; the probe may have moved it
...
loc_3297:
3297: DD 2A C8 63     LD      IX,($63C8)          ; {ram.objIterPtr} re-read the record after the ground probe
...
loc_32bd:
32BD: 3A 27 62        LD      A,($6227)           ; {ram.board} which board decides the walker
32C0: FE 01           CP      $01                 
...
32EA: 3A 05 62        LD      A,($6205)           ; {ram.marioY} where Mario's row is now
...
331F: 3A 18 60        LD      A,($6018)           ; {ram.random} a coin flip decides the new heading
3322: 0F              RRCA                        
...
3363: 3A 05 62        LD      A,($6205)           ; {ram.marioY} Mario's row
3366: 47              LD      B,A                 
...
33C3: 3A 27 62        LD      A,($6227)           ; {ram.board} only the girder board has a slope to settle onto
33C6: FE 01           CP      $01                 
...
348A: 3A 03 62        LD      A,($6203)           ; {ram.marioX} the context byte the direction comes from
348D: CB 7F           BIT     7,A                 
...
34BF: 3A 03 62        LD      A,($6203)           ; {ram.marioX} Mario's screen half picks the template
34C2: CB 7F           BIT     7,A                 
...
loc_34ca:
34CA: 06 00           LD      B,$00               
34CC: 3A 19 60        LD      A,($6019)           ; {ram.spinCount} the spin counter picks one of its four entries
34CF: E6 06           AND     $06                 
34D1: 4F              LD      C,A                 
...
3E88: 3A 27 62        LD      A,($6227)           ; {ram.board} the board picks the collision arm
...
3E9A: AF              XOR     A                   
3E9B: 32 60 60        LD      ($6060),A           ; {ram.overlapCount} both scans share one tally
...
3EB3: 3A 60 60        LD      A,($6060)           ; {ram.overlapCount} the total across both arrays
3EB6: A7              AND     A                   
...
3EF6: 3C              INC     A                   
3EF7: 32 60 60        LD      ($6060),A           ; {ram.overlapCount} both axes overlapped -- tally this object
