...
/**
 * Equivalence tests for loc_1cab -- the LEFTWARD walk-step handler (ld b,0xff ;
 * ld a,(0x620f) ; and a ; jp nz,0x1cd2 ; ld a,(0x6202) ; ld b,a ; ld a,0x01 ;
 * call 0x3009 ; ld (0x6202),a ; and 0x03 -> fall through into loc_1cc2). It is the
 * mirror twin of loc_1c8f: same shape, but it does NOT set the facing-right bit (no
 * `or 0x80`) and it FALLS THROUGH into loc_1cc2 instead of jumping there. COLLAPSED to
 * one m.step per basic block with the mid-body `call 0x3009` kept verbatim as a boundary:
 * arm A (timer running) 34 t -> 0x1CD2; arm B pre-call 58 t -> PC 0x1CBA, call 17 t,
 * post-call 20 t -> fall through to 0x1CC2 (the twin's post-call is 37 t; the 17 t less here
 * is exactly the dropped `or 0x80` + `jp`). See optimized/loc_1cab.js for the fold and flags.
 *
 * REACHABILITY + ATOMICITY (measured here, not assumed -- see the ATOMICITY test below).
 * loc_1cab is tail-called from loc_1ae6's move arm (input Left), inside the per-frame movement
 * cascade under loc_197a (entry_1ac3 -> loc_1ae6 -> loc_1cab). The leftward walk enters the
 * attract demo later than the rightward twin (first dispatch ~f900-1500); over 1600 frames it
 * is reached ~153x (arm A ~103, arm B ~50) with both arms exercised. It is ATOMIC: every entry occurs with io.nmiMask
 * CLEARED (inside the vblank NMI, mask zeroed so it cannot re-enter), and the NMI's pushed PC
 * never lands in [0x1CAB,0x1CC2) (0 landings). So the STRICT byte-exact whole-machine gate is
 * the right license, NOT the convergent gate.
 *
 ...
 * AND each arm's exact cycle TOTAL, callees stubbed to isolate loc_1cab's OWN cycles); and
...
import { loc_1cab as translated_1cab } from "../../translated/state0.js";
import { loc_1cab as optimized_1cab } from "../loc_1cab.js";
...
const TARGET = 0x1cab;
const RANGE = [0x1cab, 0x1cc2]; // [lo, hi) the routine's own ROM span, for NMI-landing checks
const FRAMES_WHOLE = 1600; // leftward walk enters the demo ~f900-1500; 1600 reaches both arms (~103/50)
const FRAMES_UNIT = 1600; // the unit host must run past the first leftward dispatch
...
test("ATOMICITY: every loc_1cab dispatch runs mask-cleared and no NMI lands inside it", () => {
  ...
  assert.ok(masks.length >= 1, "loc_1cab was never dispatched -- widen the window");
  assert.ok(masks.every((v) => v === 0), `io.nmiMask must be 0 at every dispatch (saw ${[...new Set(masks)]})`);
  assert.equal(pushedInRange.length, 0, `an NMI pushed a PC inside [0x1cab,0x1cc2): ${pushedInRange.map((p) => "0x" + p.toString(16))}`);
...
test("STRICT (whole-machine): collapsed loc_1cab is byte-exact EQUAL vs translated over attract", () => {
  const r = wholeMachineEquivalence(ROM, {}, FRAMES_WHOLE, new Map([[TARGET, optimized_1cab]]));
...
test("STRICT-TEETH (cycles): a wrong branch total forks the trajectory and is CAUGHT", () => {
  ...
  const cyclebroken = (m) => {
    ...
    regs.b = regs.a;
    regs.a = 0x01;
    m.step(0x1cba, 53); // 58 -> 53
    m.push16(0x1cbd); m.step(0x3009, 17); m.call(0x3009);
    mem.write8(WALK_ANIM, regs.a);
    regs.and(0x03);
    m.step(0x1cc2, 15); // 20 -> 15
...
/** Capture the pristine machine the instant loc_1cab is first entered (via m.call, deep
...
test("EQUAL (unit): idiomatic loc_1cab matches translated in RAM + full register file + pc", () => {
  const r = runBoth(NATURAL_ENTRY, optimized_1cab);
...
test("TEETH (unit behavioural): routing the natural arm to the WRONG handler is CAUGHT", () => {
  ...
  const broken_wrongTarget = (m) => {
    ...
    mem.write8(WALK_ANIM, regs.a);
    regs.and(0x03);
    m.step(0x1cd2, 20); return m.call(0x1cd2); // BUG: arm B -> loc_1cd2, not loc_1cc2
  };
 ...
 * stubs, so each arm's cycle TOTAL is loc_1cab's OWN charge alone and the stub captures
 * which tail was taken. entry_3009 leaves A as-is (0x01, the mode arg), so the arm-B
 * post-call path is deterministic and identical on both sides.
 */
function seed(timer, anim) {
  const captured = { who: null };
  const m = new Machine(ROM);
  m.regs.sp = 0x6c00;
  m.push16(RET_ADDR); // loc_1cab's caller frame (the tail callee's ret lands here)
  ...
  m.regs.b = 0x77; // arbitrary -- overwritten by ld b,0xff
  m.routines.set(0x1cd2, () => { captured.who = 0x1cd2; }); // arm-A tail: no-op, 0 t
  m.routines.set(0x3009, () => { captured.who = 0x3009; }); // mid-body: no-op, leaves A=0x01, 0 t
...
test("BRANCH (unit): arm A -- timer!=0, jp nz taken -> loc_1cd2 (7+13+4+10 = 34 t)", () => {
  ...
  assert.equal(m.regs.b, 0xff, "arm A leaves B = -1 (0xFF, the X delta loc_1cd2 consumes)");
});

test("BRANCH (unit): arm B -- timer==0, new step -> call 0x3009 -> fall into loc_1cc2 (58+17+20 = 95 t)", () => {
  assertArm("armB", 0x00, 0x02, 95);
  const { m, captured } = seed(0x00, 0x02);
  optimized_1cab(m);
  assert.equal(m.pc, 0x1cc2, "arm B falls through into the shared move tail 0x1CC2");
  assert.equal(captured.who, 0x1cc2, "arm B tails loc_1cc2");
  assert.equal(m.mem.read8(WALK_ANIM), 0x01, "arm B stores entry_3009's result (stub leaves A=0x01, the mode arg) to 0x6202");
  assert.equal(m.regs.a, 0x01, "arm B leaves A = (0x01 & 0x03) = 0x01 -- NO or 0x80, so bit 7 clear (facing LEFT)");
});

test("BRANCH-TEETH (cycles): a dropped charge yields a wrong total and is CAUGHT", () => {
  // Arm B post-call block charged 15 instead of 20 -> total no longer matches the oracle.
  const dropped = (m) => {
    const { regs, mem } = m;
    regs.b = 0xff;
    regs.a = mem.read8(MOVE_TIMER);
    ...
    mem.write8(WALK_ANIM, regs.a);
    regs.and(0x03);
    m.step(0x1cc2, 15); return m.call(0x1cc2); // DROPPED: 20 -> 15
  };
