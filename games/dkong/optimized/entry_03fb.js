 ...
 *
 * ATOMIC? NO -- reached from loc_197a, the interruptible per-frame main-loop
 * cascade (NMI mask ENABLED); the vblank NMI CAN land inside this routine. But
 * "atomic" is a property of the scenario exercised, not the routine (a strict
 * gate pass on one scenario is not proof for another), so per the fleet-wide
 * collapse rule this is COLLAPSED anyway (one m.step per basic block, folding a
 * branch's "not taken" charge forward into whatever unconditional code follows it,
 * same convention as sub_0350/handler_01c3) and its whole-machine test is gated
 * with the CONVERGENT license unconditionally (see test/equivalence-03fb.test.js).
 * Every branch TOTAL sums to the oracle's, EXACTLY (355 t / 868 t, asserted by the
 * BRANCH unit tests, which are cycle-exact and unaffected by the collapse grain).
 */
export function entry_03fb(m) {
  const { regs, mem } = m;

  // ld a,(BOARD)[13]+cp 0x02[7] -- board dispatch.  20 t
  ...
  // jp nz not-taken[10]+ld hl,0x6908[10]+ld a,(63a3)[13]+ld c,a[4] -- BOARD==2 (COLD) prologue.  37 t
  ...
  // ld a,(6910)[13]+sub 0x3b[7]+ld(63b7),a[13] -- derive + stash the index.  33 t
