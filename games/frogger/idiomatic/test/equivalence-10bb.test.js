 ...
 * loads its row-advance / row-count / column-index from the lane block (0x827C..0x827E), stashes the
...
test("EQUAL (crafted): renderFrogAnimArm4 == oracle on the arm-4 render", { skip }, () => {
  assert.equal(diff(seeded()), null, "arm-4 render diverged");
  assert.ok(diff(seeded(), () => {}), "vacuous: oracle wrote nothing on the sample entry");
  console.log("  EQUAL: renderFrogAnimArm4 == oracle on the real render");
});

test("TEETH: broken twins are caught", { skip }, () => {
  const noOp = () => {};
  const skipRowAdv = (m) => { m.regs.a = m.mem8[BLOCK]; m.mem16[SRC_PTR] = 0x145f; }; // omits mem8[0x81b1]=A + the render
