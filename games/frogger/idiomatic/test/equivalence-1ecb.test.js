...
/**
 * awardHomeBay5Goal — memory-equivalent to the frozen oracle loc_1ecb (ROM 0x1ECB), the bay-5 home
...
import { awardHomeBay5Goal as cand } from "../awardHomeBayGoal.js";
import { loc_1ecb as oracle } from "../../translated/loc_1e7a.js";

const skip = romsPresent() ? false : "ROM images are gitignored; none assembled";
const GATE_P1 = 0x8262, GATE_P2 = 0x8267, COUNT_P1 = 0x825c;
...
const awardMatch = () => craft((mem) => { base(mem); mem[KEY] = 0x05; });
const collision = () => craft((mem) => { base(mem); mem[COLLISION] = 1; });
const player2 = () => craft((mem) => { base(mem); mem[PLAYER] = 2; });

test("EQUAL (crafted): awardHomeBay5Goal == oracle over every branch", { skip }, () => {
  ...
  assert.equal(a.mem8[GATE_P1], 1, "positive control: award did not set the bay-5 gate");
  console.log("  EQUAL: done/reject/miss/match/collision/player2; bay-5 gate 0->1 non-vacuous");
});
