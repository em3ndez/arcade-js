...
/**
 * equivalence-1138 — crafted-entry gate: renderFrogAnimArm8 vs the frozen oracle at ROM 0x1138. Arm 8
 * loads its row-advance / row-count / column-index from the lane block (0x8288..0x828A), points the
 ...
 * the RAM diff isolates arm 8's setup writes. Teeth: a no-op and a twin that skips the 0x81B1 stash
...
import { renderFrogAnimArm8 as cand } from "../renderFrogAnimArm8.js";
import { loc_1138 as oracle } from "../../translated/loc_10f8.js";

const skip = romsPresent() ? false : "ROM images are gitignored; none assembled";
const STACK_LO = 0x87e0, STACK_HI = 0x8800;
const RENDER_LOOP = 0x0ff1;
const BLOCK = 0x8288, ROWADV = 0x81b1, SRC_PTR = 0x8001;
...
test("EQUAL (crafted): renderFrogAnimArm8 == oracle on the arm-8 setup", { skip }, () => {
  assert.equal(diff(seeded()), null, "arm-8 setup diverged");
  ...
  assert.equal(a.mem16[SRC_PTR], 0x14ab, "vacuous: tile source never stashed");
  console.log(`  EQUAL: rowadv ${beforeAdv}->${a.mem8[ROWADV]}, src 0x${beforeSrc.toString(16)}->0x${a.mem16[SRC_PTR].toString(16)}`);
});

test("TEETH: broken twins are caught", { skip }, () => {
  const noOp = () => {};
  const skipRowAdv = (m) => { m.regs.a = m.mem8[BLOCK]; m.mem16[SRC_PTR] = 0x14ab; }; // omits mem8[0x81b1]=A
