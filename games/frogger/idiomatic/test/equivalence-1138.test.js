 ...
 * loads its row-advance / row-count / column-index from the lane block (0x8288..0x828A), stashes the
...
test("EQUAL (crafted): renderFrogAnimArm8 == oracle on the arm-8 render", { skip }, () => {
  assert.equal(diff(seeded()), null, "arm-8 render diverged");
  assert.ok(diff(seeded(), () => {}), "vacuous: oracle wrote nothing on the sample entry");
  console.log("  EQUAL: renderFrogAnimArm8 == oracle on the real render");
});

test("TEETH: broken twins are caught", { skip }, () => {
  const noOp = () => {};
  const skipRowAdv = (m) => { m.regs.a = m.mem8[BLOCK]; m.mem16[SRC_PTR] = 0x14ab; }; // omits mem8[0x81b1]=A + the render
