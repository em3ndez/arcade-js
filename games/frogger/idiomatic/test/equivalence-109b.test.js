 ...
 * loads its row-advance / row-count / column-index from the lane block (0x8279..0x827B), stashes the
...
test("EQUAL (crafted): renderFrogAnimArm3 == oracle on the arm-3 render", { skip }, () => {
  assert.equal(diff(seeded()), null, "arm-3 render diverged");
  assert.ok(diff(seeded(), () => {}), "vacuous: oracle wrote nothing on the sample entry");
  console.log("  EQUAL: renderFrogAnimArm3 == oracle on the real render");
});

test("TEETH: broken twins are caught", { skip }, () => {
  const noOp = () => {};
  const skipRowAdv = (m) => { m.regs.a = m.mem8[BLOCK]; m.mem16[SRC_PTR] = 0x1453; }; // omits mem8[0x81b1]=A + the render
