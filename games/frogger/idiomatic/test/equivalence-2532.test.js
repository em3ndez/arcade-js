...
/**
 * stampHomeBayGatorFull — memory-equivalent to the frozen oracle at ROM 0x2532.
...
import { loc_2532 as oracle } from "../../translated/loc_2532.js";
...
const SLOT_CURSOR = 0x8120;
const MIRROR = 0x8121;
const COUNT_CELL = 0x83fd;
const TILES = [208, 209, 210, 211];
...
function brokenWrongTile(m) { // right structure, wrong bottom-right tile
  const { mem8 } = m;
  ...
  stampAt(mem8, i, [TILES[0], TILES[1], TILES[2], TILES[3] + 1]);
}
...
test("CRAFTED: over real attract states x every path, oracle == rewrite", { skip }, () => {
  ...
  for (const st of c) for (const s of scen) assert.equal(ramDiff(stampHomeBayGatorFull, crafted(st, s)), null, `diverged: slot=${s.slot} count=${s.count}`);
