...
/**
 * loc_2532 — memory-equivalent to the frozen oracle at ROM 0x2532.
...
test("CRAFTED: over real attract states x every path, oracle == rewrite", { skip }, () => {
  ...
  for (const st of c) for (const s of scen) assert.equal(ramDiff(loc_2532, crafted(st, s)), null, `diverged: lane=${s.lane} count=${s.count}`);
