...
/**
 * activateFrogObject — memory-equivalent to the frozen oracle at ROM 0x0804.
...
test("REAL: oracle == rewrite on every captured neighbour state", { skip }, () => {
  ...
  for (const e of entries) assert.equal(ramDiff(activateFrogObject, e), null, "a captured machine diverged");
...
test("CRAFTED: oracle == rewrite on the two-player timer arm", { skip }, () => {
  ...
  assert.equal(ramDiff(activateFrogObject, two), null, "diverged on the two-player timer arm");
