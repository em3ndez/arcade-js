...
/**
 * awardHomeBay2Goal — memory-equivalent to the frozen oracle loc_1dd8 (ROM 0x1DD8), the bay-2 home
 * goal handler. GATE: crafted-entry. The candidate dissolves the reject tail into the input scan and
 * the goal-sprite arm into armHomeGoalSprite, and keeps the m.call into the still-translated award
 * helper (0x2673) and the shared goal fill (0x1f1c). Covered: gate-already-set early return; frog
 * not-reached reject (scan locked identical); award with the points key missed and matched (0x8120=0
 * so no caller-skip); a latched collision arming the sprite; and player-2 routing. RAM compared,
 * stack masked. Teeth: no-op, unmarked gate, mis-counted; positive control the gate flips 0->1.
 */
import test from "node:test";
import assert from "node:assert/strict";

import { romsPresent, craft, ramDiff, FROG_Y } from "./_frogHop.js";
import { awardHomeBay2Goal as cand } from "../awardHomeBayGoal.js";
import { loc_1dd8 as oracle } from "../../translated/loc_1dd8.js";

const skip = romsPresent() ? false : "ROM images are gitignored; none assembled";
const GATE_P1 = 0x825f, GATE_P2 = 0x8264, COUNT_P1 = 0x825c;
...
const awardMatch = () => craft((mem) => { base(mem); mem[KEY] = 0x02; });
const collision = () => craft((mem) => { base(mem); mem[COLLISION] = 1; });
const player2 = () => craft((mem) => { base(mem); mem[PLAYER] = 2; });

test("EQUAL (crafted): awardHomeBay2Goal == oracle over every branch", { skip }, () => {
  ...
  assert.equal(a.mem8[GATE_P1], 1, "positive control: award did not set the bay-2 gate");
  console.log("  EQUAL: done/reject/miss/match/collision/player2; bay-2 gate 0->1 non-vacuous");
});
