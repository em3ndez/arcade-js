...
/**
 * moveSpriteObjectArmA — memory-equivalent to the frozen oracle at ROM 0x29F9.
...
test("EQUAL (crafted): moveSpriteObjectArmA == oracle on every path", { skip }, () => {
  const entries = [
  ...
  for (const e of entries) assert.equal(ramDiff(moveSpriteObjectArmA, e), null, "a crafted entry diverged");
  assert.ok(ramDiff(brokenNoOp, entry(TURN)), "vacuous: oracle wrote nothing on the turn path");
  console.log(`  EQUAL: ${entries.length} crafted paths, moveSpriteObjectArmA == oracle`);
});
