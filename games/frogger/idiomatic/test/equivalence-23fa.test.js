...
/**
 * stampHomeBayFly — memory-equivalent to the frozen oracle at ROM 0x23FA.
...
test("CRAFTED: over real attract states x every path, oracle == rewrite", { skip }, () => {
  ...
  for (const st of c) for (const s of scen) assert.equal(ramDiff(stampHomeBayFly, crafted(st, s)), null, `diverged: slot=${s.slot} count=${s.count}`);
