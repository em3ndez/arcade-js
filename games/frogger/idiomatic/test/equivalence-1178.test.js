 ...
 * loads its row-advance / row-count / column-index from the lane block (0x828e..0x8290), points the
 ...
 * the RAM diff isolates arm 10's setup writes. Teeth: a no-op and a twin that skips the 0x81B1 stash
...
test("EQUAL (crafted): renderFrogAnimArm10 == oracle on the arm-10 setup", { skip }, () => {
  assert.equal(diff(seeded()), null, "arm-10 setup diverged");
  ...
  assert.equal(a.mem16[SRC_PTR], 0x14b3, "vacuous: tile source never stashed");
  console.log(`  EQUAL: rowadv ${beforeAdv}->${a.mem8[ROWADV]}, src 0x${beforeSrc.toString(16)}->0x${a.mem16[SRC_PTR].toString(16)}`);
});

test("TEETH: broken twins are caught", { skip }, () => {
  const noOp = () => {};
  const skipRowAdv = (m) => { m.regs.a = m.mem8[BLOCK]; m.mem16[SRC_PTR] = 0x14b3; }; // omits mem8[0x81b1]=A
