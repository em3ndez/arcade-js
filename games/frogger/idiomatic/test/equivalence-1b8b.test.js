...
/**
 * beginRiverLane0Ride — memory-equivalent to the frozen oracle at ROM 0x1B8B.
 * GATE: crafted-entry. The lane-0 begin guards on frog Y, and on a fresh ride emits the ride sound + stamps
 * the ride sprite, primes the ride counter from its reload length, then falls into the lane-0 commit. From
 * a captured post-boot state the lane cells are poked to drive: position guard, fresh ride (sprite unset),
 * fresh ride when the sprite is already the ride code (re-prime, no counter bump), ride-in-progress, and
...
import { romsPresent, craft, ramDiff, FROG_Y, FROG_SPRITE, PLAY_FLAG, ARRIVAL, COUNTER, RELOAD, VDELTA } from "./_riverRide.js";
import { beginRiverLane0Ride as cand } from "../rideRiverLaneAndCommitArrival.js";
import { loc_1b8b as oracle } from "../../translated/loc_1b8b.js";

const skip = romsPresent() ? false : "ROM images are gitignored; none assembled";
const RIDE_CODE = 0xde;
...
const freshSpriteSet = () => craft((mem) => { base(mem); mem[COUNTER[0]] = 0; mem[FROG_SPRITE] = RIDE_CODE; });
...
test("EQUAL (crafted): beginRiverLane0Ride == oracle on guard/fresh/re-prime/in-progress/wrap", { skip }, () => {
