...
/**
 * loc_12e4 — memory-equivalent to the frozen oracle at ROM 0x12E4.
...
test("EQUAL (broad): loc_12e4 == oracle on every frog X 0..255", { skip }, () => {
  let n = 0;
  for (let x = 0; x <= 255; x++) {
    const e = seedMachine().clone(); e.mem8[HOLD] = 0; e.mem8[FROG_X] = x;
    assert.equal(ramDiff(loc_12e4, e), null, `broad frog X=${x} diverged`); n++;
  }
  console.log(`  EQUAL: ${n} frog-X sweep values, loc_12e4 == oracle`);
});
...
test("EQUAL (crafted): guard / block / kill / wrap paths", { skip }, () => {
  ...
  assert.equal(ramDiff(loc_12e4, g), null, "guard path diverged");
  ...
  assert.equal(ramDiff(loc_12e4, be), null, "block path diverged");
  ...
  assert.equal(ramDiff(loc_12e4, ke), null, "kill path diverged");
  ...
  assert.equal(ramDiff(loc_12e4, ee), null, "width-edge path diverged");
  ...
  assert.equal(ramDiff(loc_12e4, we), null, "wrap path diverged");
  const wchk = we.clone(); oracle(wchk);
  assert.equal(wchk.mem8[HOLD], 1, "wrap path did not see the object through the band wrap");

  console.log(`  EQUAL: guard, block (X=${bx}), kill (X=${kx}), width-edge, wrap, loc_12e4 == oracle`);
});
