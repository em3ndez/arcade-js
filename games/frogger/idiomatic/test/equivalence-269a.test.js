...
/**
 * clearFourByteCounterBlock — memory-equivalent to the frozen oracle at ROM 0x269A.
 ...
 * zero-writes are observable. clearFourByteCounterBlock takes no register live-in and its live-out is memory-only,
...
test("EQUAL (crafted): clearFourByteCounterBlock == oracle", { skip }, () => {
  const e = dirtyEntry();
  assert.equal(ramDiff(clearFourByteCounterBlock, e), null, "the crafted entry diverged");
  console.log("  EQUAL: clearFourByteCounterBlock == oracle on the pre-dirtied block");
});
