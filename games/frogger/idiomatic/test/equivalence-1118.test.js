...
/**
 * equivalence-1118 — crafted-entry gate: renderFrogAnimArm7 vs the frozen oracle at ROM 0x1118. Arm 7
 * loads its row-advance / row-count / column-index from the lane block (0x8285..0x8287), points the
 ...
 * the RAM diff isolates arm 7's setup writes. Teeth: a no-op and a twin that skips the 0x81B1 stash
...
import { renderFrogAnimArm7 as cand } from "../renderFrogAnimArm7.js";
import { loc_1118 as oracle } from "../../translated/loc_10f8.js";

const skip = romsPresent() ? false : "ROM images are gitignored; none assembled";
const STACK_LO = 0x87e0, STACK_HI = 0x8800;
const RENDER_LOOP = 0x0ff1;
const BLOCK = 0x8285, ROWADV = 0x81b1, SRC_PTR = 0x8001;
...
test("EQUAL (crafted): renderFrogAnimArm7 == oracle on the arm-7 setup", { skip }, () => {
  assert.equal(diff(seeded()), null, "arm-7 setup diverged");
  ...
  assert.equal(a.mem16[SRC_PTR], 0x14a7, "vacuous: tile source never stashed");
  console.log(`  EQUAL: rowadv ${beforeAdv}->${a.mem8[ROWADV]}, src 0x${beforeSrc.toString(16)}->0x${a.mem16[SRC_PTR].toString(16)}`);
});

test("TEETH: broken twins are caught", { skip }, () => {
  const noOp = () => {};
  const skipRowAdv = (m) => { m.regs.a = m.mem8[BLOCK]; m.mem16[SRC_PTR] = 0x14a7; }; // omits mem8[0x81b1]=A
