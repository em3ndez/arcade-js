...
/**
 * awardHomeBay1Goal — memory-equivalent to the frozen oracle loc_1d87 (ROM 0x1D87), the bay-1 home
...
import { awardHomeBay1Goal as cand } from "../awardHomeBayGoal.js";
import { loc_1d87 as oracle } from "../../translated/loc_1d77.js";

const skip = romsPresent() ? false : "ROM images are gitignored; none assembled";
const GATE_P1 = 0x825e, GATE_P2 = 0x8263, COUNT_P1 = 0x825c;
...
const awardMatch = () => craft((mem) => { base(mem); mem[KEY] = 0x01; });
const collision = () => craft((mem) => { base(mem); mem[COLLISION] = 1; });
const player2 = () => craft((mem) => { base(mem); mem[PLAYER] = 2; });

test("EQUAL (crafted): awardHomeBay1Goal == oracle over every branch", { skip }, () => {
  ...
  assert.equal(a.mem8[GATE_P1], 1, "positive control: award did not set the bay-1 gate");
  console.log("  EQUAL: done/reject/miss/match/collision/player2; bay-1 gate 0->1 non-vacuous");
});
