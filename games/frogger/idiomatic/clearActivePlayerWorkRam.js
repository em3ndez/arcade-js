 ...
 *   current frog/scratch state should be blanked before the machine reseeds a player, and — when it
 *   should — hands off to the unconditional clear. On a one-player game it does nothing; on a two-player
 *   game or in attract it wipes the 32-byte frog object block (FROG_X 0x8044 .. 0x8063) and a 12-byte
 *   player-work scratch block (PLAYER_WORK_SCRATCH_8420 0x8420 .. 0x842b).
 *
 * WHERE IT SITS
...
export function clearActivePlayerWorkRam(m) {
  ...
  // that many players. Exactly one player (== 1) means the wipe is skipped and the frog/scratch block is
  // left intact — a single player carries its frog and scratch state straight across this reset. A
  ...
  // zeroes the 32-byte frog object block (FROG_X 0x8044 .. 0x8063) and a 12-byte player-work scratch block
  // (PLAYER_WORK_SCRATCH_8420 0x8420 .. 0x842b). In the frozen ROM the guard above simply falls into that code;
