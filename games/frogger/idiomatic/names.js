...
export const RIVER_LANE1_DIR = 0x8249; // [code] river lane-1 direction flag; loc_23b7 tails to the lane-1 commit handler when set, else clears the lane-1 mirror RIVER_LANE1_ARRIVAL
...
export const RIVER_LANE3_DIR = 0x824b; // [code] river lane-3 direction flag; loc_23b7 tails to the lane-3 commit handler when set, else clears the lane-3 mirror RIVER_LANE3_ARRIVAL
...
export const RIVER_LANE1_ARRIVAL = 0x824d; // [code] river lane-1 arrival mirror flag; loc_23b7 clears it when the lane-1 direction flag RIVER_LANE1_DIR is clear
...
export const RIVER_LANE3_ARRIVAL = 0x824f; // [seen] river lane-3 arrival mirror flag; loc_23b7 clears it when the lane-3 direction flag RIVER_LANE3_DIR is clear
export const RIVER_LANE0_RIDE_COUNTER = 0x8250; // [code] river lane-0 ride counter; the lane-0 begin handler primes it from RIVER_LANE0_RIDE_RELOAD, the lane-0 commit handler decrements it each frame and marks arrival (RIVER_LANE0_ARRIVAL) when it reaches 0
export const RIVER_LANE1_RIDE_COUNTER = 0x8251; // [code] river lane-1 ride counter; the lane-1 begin handler primes it from RIVER_LANE1_RIDE_RELOAD, the lane-1 commit handler decrements it each frame and marks arrival (RIVER_LANE1_ARRIVAL) when it reaches 0
export const RIVER_LANE2_RIDE_COUNTER = 0x8252; // [code] river lane-2 ride counter; the lane-2 begin handler primes it from RIVER_LANE2_RIDE_RELOAD, the lane-2 commit handler decrements it each frame and marks arrival (RIVER_LANE2_ARRIVAL) when it reaches 0
export const RIVER_LANE3_RIDE_COUNTER = 0x8253; // [code] river lane-3 ride counter; the lane-3 begin handler primes it from RIVER_LANE3_RIDE_RELOAD, the lane-3 commit handler decrements it each frame and marks arrival (RIVER_LANE3_ARRIVAL) when it reaches 0
export const RIVER_VERTICAL_RIDE_DELTA = 0x8254; // [code] vertical river-ride carry delta; added to FROG_Y by the lane-0 commit handler and subtracted by the lane-1 commit handler while carrying a riding frog
export const RIVER_HORIZONTAL_RIDE_DELTA = 0x8255; // [code] horizontal river-ride carry delta; added to FROG_X by the lane-2 commit handler and subtracted by the lane-3 commit handler while carrying a riding frog
export const RIVER_LANE0_RIDE_RELOAD = 0x8256; // [code] river lane-0 ride-length reload; the lane-0 begin handler copies it into RIVER_LANE0_RIDE_COUNTER to prime the ride
export const RIVER_LANE1_RIDE_RELOAD = 0x8257; // [code] river lane-1 ride-length reload; the lane-1 begin handler copies it into RIVER_LANE1_RIDE_COUNTER to prime the ride
export const RIVER_LANE2_RIDE_RELOAD = 0x8258; // [code] river lane-2 ride-length reload; the lane-2 begin handler copies it into RIVER_LANE2_RIDE_COUNTER to prime the ride
export const RIVER_LANE3_RIDE_RELOAD = 0x8259; // [code] river lane-3 ride-length reload; the lane-3 begin handler copies it into RIVER_LANE3_RIDE_COUNTER to prime the ride
...
export const ROUTINES = {
  ...
  // ── frog-movement cluster: the four river ride lanes (begin + commit handlers) + row-progress score ──
  // Multi-entry unit: all eight handlers live in rideRiverLaneAndCommitArrival.js; the score routine in
  // scoreFrogRowProgress.js. Reached with the frog X/Y cursors armed by commitRiverLaneArrivals (0x23b7,
  // commit halves each vblank), the frog input scan (0x1acb) and the hop-across driver (0x236d).
  0x1b8b: { name: "rideRiverLaneAndCommitArrival", entry: "beginRiverLane0Ride", role: "[code] begin a river lane-0 (vertical) ride: guard frog Y<0xF0, on a fresh ride emit the ride sound + stamp ride sprite 0xDE onto the frog, prime RIVER_LANE0_RIDE_COUNTER from RIVER_LANE0_RIDE_RELOAD, then fall into the lane-0 commit handler. Reached from 0x1acb/0x236d. Memory-only live-out. MAME-grounding pending", cert: "code" },
  0x1bba: { name: "rideRiverLaneAndCommitArrival", entry: "commitRiverLane0Arrival", role: "[code] commit a river lane-0 (vertical) ride one frame: return if already arrived (RIVER_LANE0_ARRIVAL), else raise RIVER_LANE0_DIR and tick RIVER_LANE0_RIDE_COUNTER down; on drain mark arrived + stamp home sprite 0xDE, else carry the frog FROG_Y += RIVER_VERTICAL_RIDE_DELTA + stamp moving sprite 0xDC. Reached from 0x23b7/0x1acb + lane-0 begin fall-through. Memory-only live-out. MAME-grounding pending", cert: "code" },
  0x1be4: { name: "rideRiverLaneAndCommitArrival", entry: "beginRiverLane1Ride", role: "[code] begin a river lane-1 (vertical) ride: no position guard, fresh ride emits the ride sound + stamps ride sprite 0x1E, primes RIVER_LANE1_RIDE_COUNTER from RIVER_LANE1_RIDE_RELOAD, then falls into the lane-1 commit handler. Reached from 0x1acb/0x236d. Memory-only live-out. MAME-grounding pending", cert: "code" },
  0x1c0d: { name: "rideRiverLaneAndCommitArrival", entry: "commitRiverLane1Arrival", role: "[code] commit a river lane-1 (vertical) ride one frame: first steps the home-bay slot cursor (loc_23eb, A discarded); return if arrived (RIVER_LANE1_ARRIVAL), else raise RIVER_LANE1_DIR and tick RIVER_LANE1_RIDE_COUNTER down; on drain mark arrived + stamp sprite 0x1E + score row progress (scoreFrogRowProgress), else carry the frog FROG_Y -= RIVER_VERTICAL_RIDE_DELTA + stamp moving sprite 0x1C. Reached from 0x23b7/0x1acb + lane-1 begin fall-through. Memory-only live-out. MAME-grounding pending", cert: "code" },
  0x1c41: { name: "rideRiverLaneAndCommitArrival", entry: "beginRiverLane2Ride", role: "[code] begin a river lane-2 (horizontal) ride: guard frog Y>=0x30 and frog X<0xE0, fresh ride emits the ride sound + stamps ride sprite 0xA1, primes RIVER_LANE2_RIDE_COUNTER from RIVER_LANE2_RIDE_RELOAD, then falls into the lane-2 commit handler. Reached from 0x1acb/0x236d. Memory-only live-out. MAME-grounding pending", cert: "code" },
  0x1c76: { name: "rideRiverLaneAndCommitArrival", entry: "commitRiverLane2Arrival", role: "[code] commit a river lane-2 (horizontal) ride one frame: return if arrived (RIVER_LANE2_ARRIVAL), else raise RIVER_LANE2_DIR and tick RIVER_LANE2_RIDE_COUNTER down; on drain mark arrived + stamp sprite 0xA1, else carry the frog FROG_X += RIVER_HORIZONTAL_RIDE_DELTA + stamp moving sprite 0x9F. Reached from 0x23b7/0x1acb + lane-2 begin fall-through. Memory-only live-out. MAME-grounding pending", cert: "code" },
  0x1ca0: { name: "rideRiverLaneAndCommitArrival", entry: "beginRiverLane3Ride", role: "[code] begin a river lane-3 (horizontal) ride: guard frog Y>=0x30 and frog X>=0x20, fresh ride emits the ride sound + stamps ride sprite 0x21, primes RIVER_LANE3_RIDE_COUNTER from RIVER_LANE3_RIDE_RELOAD, then falls into the lane-3 commit handler. Reached from 0x1acb/0x236d. Memory-only live-out. MAME-grounding pending", cert: "code" },
  0x1cd5: { name: "rideRiverLaneAndCommitArrival", entry: "commitRiverLane3Arrival", role: "[code] commit a river lane-3 (horizontal) ride one frame: return if arrived (RIVER_LANE3_ARRIVAL), else raise RIVER_LANE3_DIR and tick RIVER_LANE3_RIDE_COUNTER down; on drain mark arrived + stamp sprite 0x21, else carry the frog FROG_X -= RIVER_HORIZONTAL_RIDE_DELTA + stamp moving sprite 0x1F. Reached from 0x23b7/0x1acb + lane-3 begin fall-through. Memory-only live-out. MAME-grounding pending", cert: "code" },
  0x1fd6: { name: "scoreFrogRowProgress", role: "[code] award a progress point when the frog reaches a new furthest row: range-check FROG_Y to [0x30,0xd0]; the 0xd0 edge seeds the high-water mark (FROG_FURTHEST_ROW) above the band on the first crossing; a row nearer the top than the recorded furthest updates it and awards the BCD delta 1 via the score routine 0x08e0 (kept m.call, unlifted), except the mid row 0x80. Reached only from lane-1 commit. Memory-only live-out. MAME-grounding pending", cert: "code" },
};
