...
export const FROG_SPRITE_CODE = 0x8045; // [code] frog sprite/tile code; activateFrogObject clears it on activate, the river-ride handlers stamp the ride/move/home tile codes into it
...
export const LANE_OBJECT_INDEX = 0x80ff; // [seen] lane-object walk index (0..10); moveLaneObjectsAndCarryFrog reads/increments it per object, wraps to 0 after object 10. Also the base of the 183-byte per-player object work page swapIn/swapOutActivePlayerPages bank from/to (this index byte leads that page). Walk role grounded by consequence (11 objects each step per frame); vblank snapshots read the post-wrap 0, not 1..10.
...
export const FROG_FURTHEST_ROW = 0x8269; // [code] frog row-progress high-water mark; scoreFrogRowProgress seeds/updates it as the furthest (nearest-top) row the frog has reached, resetFrogObject clears it on frog reset
...
export const FLY_SPRITE_X = 0x8040; // [seen,poked] fly sprite X position / base of the four-cell block armHomeGoalSprite arms; driveFlyPatrol writes path base loc_811c + path-table offset, armHomeGoalSprite arms it with the lead byte + fixed tail 25,3,16 (sibling loc_27de zeroes 0x8040-0x8043)
...
export const loc_811c = 0x811c; // [code] fly path X base; driveFlyPatrol adds it to the path-table offset to form the sprite X at FLY_SPRITE_X
...
export const LANE_OBJECT_PHASE_TABLE = 0x81a6; // [seen] 11-byte per-object phase-countdown table (0x81a6+i for lane object i); moveLaneObjectsAndCarryFrog holds object i while its countdown drains, moving one pixel and clearing it when it reaches 1. Grounded (golden_broad gameplay f263+): each cell counts down 1/frame, reloads to its lane control-nibble minus one, its lead sprite stepping one pixel on the frame the countdown reads 0
...
export const loc_800f = 0x800f; // [seen] demo scroll register; loc_0de0 writes 3 here (paired with OBJECT_ANIM_STATE_800D) each dwell tick
...
export const FROG_HOP_DOWN_ACTIVE = 0x8248; // [code] down-hop active flag; advanceActiveFrogHops tails to advanceFrogHopDown when set, else clears the down-hop mirror FROG_HOP_DOWN_ARRIVAL. Not exercised in golden_hop/golden_hop2 (only UP+RIGHT hopped); shares the grounded beginHop/advanceHop body
export const FROG_HOP_UP_ACTIVE = 0x8249; // [seen] up-hop active flag; advanceActiveFrogHops tails to advanceFrogHopUp when set, else clears FROG_HOP_UP_ARRIVAL. golden_hop: =1 across frames 302-309 during the live UP hop, cleared to 0 at frame 310 on arrival
export const FROG_HOP_RIGHT_ACTIVE = 0x824a; // [seen] right-hop active flag; advanceActiveFrogHops tails to advanceFrogHopRight when set, else clears FROG_HOP_RIGHT_ARRIVAL. golden_hop2: =1 across frames 342-348 during a RIGHT hop
export const FROG_HOP_LEFT_ACTIVE = 0x824b; // [code] left-hop active flag; advanceActiveFrogHops tails to advanceFrogHopLeft when set, else clears FROG_HOP_LEFT_ARRIVAL. Not exercised in either golden; shares the grounded beginHop/advanceHop body
export const FROG_HOP_DOWN_ARRIVAL = 0x824c; // [code] down-hop arrival mirror flag; advanceActiveFrogHops clears it when FROG_HOP_DOWN_ACTIVE is clear, advanceFrogHopDown sets it on drain. No down hop in the goldens
export const FROG_HOP_UP_ARRIVAL = 0x824d; // [seen] up-hop arrival mirror flag; advanceActiveFrogHops clears it when FROG_HOP_UP_ACTIVE is clear, advanceFrogHopUp sets it on drain. golden_hop: flips 0->1 at frame 310 as the UP hop's counter drains
export const FROG_HOP_RIGHT_ARRIVAL = 0x824e; // [code] right-hop arrival mirror flag; advanceActiveFrogHops clears it when FROG_HOP_RIGHT_ACTIVE is clear, advanceFrogHopRight sets it on drain. golden_hop2's RIGHT hop was cut short by a road collision (0x8004=1 at frame 348) before drain, so the flip was not observed
export const FROG_HOP_LEFT_ARRIVAL = 0x824f; // [code] left-hop arrival mirror flag; advanceActiveFrogHops clears it when FROG_HOP_LEFT_ACTIVE is clear. No left hop in the goldens
export const FROG_HOP_DOWN_ANIM_COUNTER = 0x8250; // [code] down-hop animation counter; beginFrogHopDown primes it from FROG_HOP_DOWN_ANIM_RELOAD, advanceFrogHopDown decrements it each frame and marks arrival (FROG_HOP_DOWN_ARRIVAL) at 0. Not exercised in the goldens
export const FROG_HOP_UP_ANIM_COUNTER = 0x8251; // [seen] up-hop animation counter; beginFrogHopUp primes it from FROG_HOP_UP_ANIM_RELOAD, advanceFrogHopUp decrements it each frame and marks arrival (FROG_HOP_UP_ARRIVAL) at 0. golden_hop: 8->0 across frames 302-310, -1/frame
export const FROG_HOP_RIGHT_ANIM_COUNTER = 0x8252; // [seen] right-hop animation counter; beginFrogHopRight primes it from FROG_HOP_RIGHT_ANIM_RELOAD, advanceFrogHopRight decrements it each frame and marks arrival (FROG_HOP_RIGHT_ARRIVAL) at 0. golden_hop2: 8->2 across frames 342-348, -1/frame (hop cut short by a collision)
export const FROG_HOP_LEFT_ANIM_COUNTER = 0x8253; // [code] left-hop animation counter; beginFrogHopLeft primes it from FROG_HOP_LEFT_ANIM_RELOAD, advanceFrogHopLeft decrements it each frame and marks arrival (FROG_HOP_LEFT_ARRIVAL) at 0. Not exercised in the goldens
export const FROG_HOP_VERTICAL_DELTA = 0x8254; // [seen] vertical hop step delta; added to FROG_Y by advanceFrogHopDown and subtracted by advanceFrogHopUp each frame while a hop steps. golden_hop: constant 2; FROG_Y steps -2/frame (0xe0->0xd0 = one 16px cell over 8 frames)
export const FROG_HOP_HORIZONTAL_DELTA = 0x8255; // [seen] horizontal hop step delta; added to FROG_X by advanceFrogHopRight and subtracted by advanceFrogHopLeft each frame while a hop steps. golden_hop2: constant 2; FROG_X steps +2/frame (0x80->0x8e)
export const FROG_HOP_DOWN_ANIM_RELOAD = 0x8256; // [code] down-hop animation-length reload; beginFrogHopDown copies it into FROG_HOP_DOWN_ANIM_COUNTER to prime the hop. Not observed in the goldens
...
export const FROG_ANIM_ARM6_SPRITE_CODE = 0x8282; // [seen] frog-anim arm-6 sprite triple code byte; loc_10f8 reads it into A and stashes it at loc_81b1 for the render loop
...
export const loc_1423 = 0x1423; // [code] ROM tile source; loc_2005 passes it in DE as the phase-16 scroll-grid source (object A), loc_1058 loads it as the frog-anim tile source (DE) and stashes it at SCROLL_COPY_SRC_PTR
...
export const loc_149f = 0x149f; // [code] frog-anim arm-6 tile source base (ROM); loc_10f8 sets DE to it and stashes it at SCROLL_COPY_SRC_PTR for the render loop's row copy
...
export const loc_2f6e = 0x2f6e; // [code] ROM 4-tile strip source; loc_0aba blits it up the LAYOUT_SETUP_STRIP_VRAM column (rst 0x28), loc_085b's first blit copies it up the loc_aa51 column
...
export const FROG_RENDER_VRAM_COL_G1 = 0xa843; // [seen] frog-render VRAM column base (group 1); loc_1952 copies four tiles from loc_19f6 down it, 5 columns +0x40 apart
...
export const ROUTINES = {
  ...
  0x20cc: { name: "blitScrollTileGrid", role: "scroll-copy engine: stamp a source block into VRAM as a grid of two-byte column pairs -- save source/row-count to scratch (SCROLL_COPY_SRC_PTR/loc_8003), then for C columns copy B rows of a pair from the source (restarted each column) down the destination at a 32-byte row pitch, advancing by the column stride loc_81b1 between columns; count 0 runs 256; live-in DE source/B rows/C columns, memory-only live-out; code-level, MAME-grounding pending", cert: "seen" },
  0x20fb: { name: "stampScrollRevealColumn", role: "scroll-reveal column stamp: build a VRAM address from the scroll object's row/column/row-count fields (loc_8273 +0/+1/+2), then dispatch on the scroll-phase byte SCROLL_STAMP_PHASE -- phases 80/208 use table loc_2190, 128/176 use loc_2194 and clear edge flag loc_8107, 160 uses loc_2198 and sets loc_8107, any other stamps nothing -- and always write the row-count-minus-one mirror SCROLL_STAMP_ROWCOUNT; no register live-in, memory-only live-out; code-level, MAME-grounding pending", cert: "seen" },
  ...
  0x23b7: { name: "advanceActiveFrogHops", role: "[seen] per-vblank hop continuation: for each of four hop directions, when its active flag 0x8248-0x824b is set tail-calls that direction's advance handler to step the hop one frame; else clears that direction's arrival mirror 0x824c-0x824f. golden_hop: continues the live UP hop (0x8249=1, counter 0x8251 counting 8->0). Live-out memory.", cert: "seen" },
  ...
  0x1c0d: { name: "animateFrogHop", entry: "advanceFrogHopUp", role: "[seen] advance an UP hop one frame: first steps the home-bay slot cursor (loc_23eb, A discarded); return if arrived (FROG_HOP_UP_ARRIVAL), else raise FROG_HOP_UP_ACTIVE and tick FROG_HOP_UP_ANIM_COUNTER down; on drain mark arrived + stamp sprite 0x1E + score row progress (scoreFrogRowProgress), else step the frog FROG_Y -= FROG_HOP_VERTICAL_DELTA + stamp moving sprite 0x1C. Reached from 0x23b7/0x1acb + the up begin fall-through. Memory-only live-out. golden_hop: FROG_Y 0xe0->0xd0 at -2/frame over frames 302-309, counter 8->0, arrival set at frame 310", cert: "seen" },
  0x1c41: { name: "animateFrogHop", entry: "beginFrogHopRight", role: "[seen] begin a RIGHT hop: guard frog Y>=0x30 (field top) and frog X<0xE0 (right edge), fresh hop emits the hop sound + stamps rest sprite 0xA1, primes FROG_HOP_RIGHT_ANIM_COUNTER from FROG_HOP_RIGHT_ANIM_RELOAD (=9), then falls into advanceFrogHopRight. Reached from 0x1acb/0x236d. Memory-only live-out. golden_hop2: the RIGHT hop primes near frame 342", cert: "seen" },
  0x1c76: { name: "animateFrogHop", entry: "advanceFrogHopRight", role: "[seen] advance a RIGHT hop one frame: return if arrived (FROG_HOP_RIGHT_ARRIVAL), else raise FROG_HOP_RIGHT_ACTIVE and tick FROG_HOP_RIGHT_ANIM_COUNTER down; on drain mark arrived + stamp sprite 0xA1, else step the frog FROG_X += FROG_HOP_HORIZONTAL_DELTA + stamp moving sprite 0x9F. Reached from 0x23b7/0x1acb + the right begin fall-through. Memory-only live-out. golden_hop2: FROG_X 0x80->0x8e at +2/frame over frames 342-348, counter 8->2 (hop cut short by a road collision at frame 348, 0x8004=1)", cert: "seen" },
  0x1ca0: { name: "animateFrogHop", entry: "beginFrogHopLeft", role: "[code] begin a LEFT hop: guard frog Y>=0x30 (field top) and frog X>=0x20 (left edge), fresh hop emits the hop sound + stamps rest sprite 0x21, primes FROG_HOP_LEFT_ANIM_COUNTER from FROG_HOP_LEFT_ANIM_RELOAD, then falls into advanceFrogHopLeft. Reached from 0x1acb/0x236d. Memory-only live-out. LEFT not hopped in the goldens; shares the beginHop body grounded via UP/RIGHT", cert: "code" },
  0x1cd5: { name: "animateFrogHop", entry: "advanceFrogHopLeft", role: "[code] advance a LEFT hop one frame: return if arrived (FROG_HOP_LEFT_ARRIVAL), else raise FROG_HOP_LEFT_ACTIVE and tick FROG_HOP_LEFT_ANIM_COUNTER down; on drain mark arrived + stamp sprite 0x21, else step the frog FROG_X -= FROG_HOP_HORIZONTAL_DELTA + stamp moving sprite 0x1F. Reached from 0x23b7/0x1acb + the left begin fall-through. Memory-only live-out. LEFT not hopped in the goldens; shares the advanceHop body grounded via UP/RIGHT", cert: "code" },
  0x1fd6: { name: "scoreFrogRowProgress", role: "[code] award a progress point when the frog reaches a new furthest row: range-check FROG_Y to [0x30,0xd0]; the 0xd0 edge seeds the high-water mark (FROG_FURTHEST_ROW) above the band on the first crossing; a row nearer the top than the recorded furthest updates it and awards the BCD delta 1 via the score routine 0x08e0 (kept m.call, unlifted), except the mid row 0x80. Reached only from the UP-hop advance (advanceFrogHopUp). Memory-only live-out. MAME-grounding pending", cert: "code" },
};
