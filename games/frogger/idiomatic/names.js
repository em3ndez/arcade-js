...
export const ROUTINES = {
  ...
  0x07d9: { name: "clearSoundQueue", role: "[code] game-start reset of the sound-command queue: zeroes the 48-byte region (pending count 0x8300 + the command slots above it)", cert: "code" },
  0x0b0a: { name: "initNewGameScoreAndTimers", role: "[code] new-game start reset: zeros the extra-life-awarded flag word (0x83e7/e8), the player-2 score word (0x83eb) and the high-score word (0x83ed), then copies the start-time byte (0x83e4) into both time-remaining bytes (0x83e5/e6) so both time bars start full; pure leaf, memory-only live-out. Caller loc_0341 (new-game setup)", cert: "code" },
  0x0b1f: { name: "renderScoreHeader", role: "[code] redraw the score header each frame -- player-1/high/player-2 score readouts and their label strips; skips the player-2 side in one-player mode (player-count cell == 1)", cert: "code" },
  0x0b67: { name: "renderCreditLine", role: "[code] draws the credit line -- first-call-only column clear + init-flag latch, then blits the credit label, raises the display flag, and prints the packed-BCD credit count", cert: "code" },
};
