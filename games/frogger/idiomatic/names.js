...
export const loc_803f = 0x803f; // [seen] page-swap-done flag; swapOutActivePlayerPages sets it to 1 after banking the active player's pages
...
export const loc_83eb = 0x83eb; // [seen,poked] player-2 score word (16-bit); loc_0f69 reads it as one of the two words to rank and pack
export const loc_83ed = 0x83ed; // [seen,poked] high score word (16-bit); loc_0f69 reads it as the other word to rank and pack
...
export const ROUTINES = {
  ...
  0x06ee: { name: "swapInActivePlayerPages", role: "[seen] For player 1 (0x83fd==1): banks the live object page (43B) to 0x85c0 and the live work page (183B) to 0x8600, restores this player's object page from 0x86c0 into 0x800c and work page from 0x8500 into 0x80ff, and raises swap-done 0x803f=1; any other player number tails to swapOutActivePlayerPages (0x0726). Runs at 2-player turn transitions.", cert: "seen" },
  ...
  // ── batch-4 dispatchers ──
  0x06a2: { name: "loc_06a2", role: "home-slot marker dispatcher on the home-column selector; five values each stamp one slot's empty 2x2 marker at its fixed VRAM base, one value fills all five slots and awards the completion life, any other returns", cert: "code" },
  0x0b9b: { name: "loc_0b9b", role: "[code] Prints a 16-bit packed-BCD value (DE) as four tilemap digits via writePackedBcdByte twice -- the high byte (D) then the low byte (E) -- leaving HL advanced four 32-cell rows for the caller's next field.", cert: "code" },
  0x11bf: { name: "loc_11bf", role: "[code] lower half of the horizontal-move dispatcher: gated on 0x83cd==0 and 0x8004==0; the frog position 0x8047 low nibble (>=9) or high nibble selects one of sixteen ROM arms (arm-pointer table at 0x11e9), six delegating to the upper half 0x12e4 and ten scanning a lane object list (0x8100..0x815a) for an object in the frog's move band -- an in-band object kills via 0x12d0 in the upper position band and delegates otherwise, a clear lane kills via 0x12d0 in the lower band and delegates otherwise. Live-out memory-only.", cert: "code" },
  0x2b83: { name: "loc_2b83", role: "sprite-object dispatcher: run the five IX/IY sprite-object arms in fixed order (spawnSpriteObject, steerSpriteObjectTowardTarget, writeSpriteObjectSlotX, flagSpriteObjectFrogHitAhead, writeSpriteObjectSlotAttr) on the object record and sprite slot the caller selected; no register live-out, memory-only", cert: "code" },
};
