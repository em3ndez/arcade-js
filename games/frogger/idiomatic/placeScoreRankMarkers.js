 ...
 *   two markers.
 *
 *   The actual byte-poke is delegated to the shared row helper stampRankMarkerIfPlaced (ROM 0x0c4a): given a page, a row
...
// The three fixed arguments this routine feeds to the row helper stampRankMarkerIfPlaced on both passes. They map to the
...
export function placeScoreRankMarkers(m) {
  ...
  // Hand the code to stampRankMarkerIfPlaced as its offset (C). For a nonzero code the helper writes MARKER_TILE into
  ...
  stampRankMarkerIfPlaced(m, largerScoreRank, ROW_BASE, RAM_PAGE, MARKER_TILE);
  ...
  stampRankMarkerIfPlaced(m, smallerScoreRank, ROW_BASE, RAM_PAGE, MARKER_TILE);
}
