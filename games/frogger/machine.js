...
//
// Frogger machine — mirrors games/timeplt/machine.js (same coroutine/NMI engine) on the NEW
// Galaxian/Scramble-family board (boards/frogger). Differences from timeplt are only the board wiring,
// the timing constants (60.606 Hz / 50688 cyc-per-frame), the video render opts (flipScreenX/Y, no
// video-enable latch), and the cloned RAM arrays. SKELETON: routines.js is empty, so reset() dispatches
// 0x0000 and throws NotImplemented at the first (and only, for now) gap — that is the Step-3 entry point.

import { AddressSpace } from "../../boards/frogger/memory.js";
import { Io, NotImplemented } from "../../boards/frogger/io.js";
import {
  SCREEN_W,
  SCREEN_H,
  decodeGraphics,
  renderFrameRGB,
  renderRowsRGB,
} from "../../boards/frogger/video.js";
import { Regs } from "../../core/cpu/z80.js";
import { makeIndexedView } from "../../core/mem-views.js";
import { buildRoutines } from "./routines.js";

/** 3072000 / 60.606061 = 50688. The galaxian/pooyan/dkong refresh, NOT timeplt's exact 60/51200.
 *  boards/frogger/hardware.json cyclesPerFrame; galaxian.cpp:7498. */
export const CYCLES_PER_FRAME = 50688;

/** The vblank NMI vector. Asserted only while the irq_enable latch (0xB808 D0) is set. */
export const NMI_VECTOR = 0x0066;

/**
 * Zero: screen_vblank() asserts at vbstart, which IS our frame origin — the same reasoning as timeplt.
 * Derived from the driver, not measured. galaxian.cpp:761-767, 7501.
 */
export const NMI_CYCLE_IN_FRAME = 0;

/** 50688 / VTOTAL 264 lines. Render-timing only (raster capture); frogger exposes no CPU scanline read. */
export const CYCLES_PER_SCANLINE = 192;

/**
 * From the frame origin (vpos = vbstart = 240) to the first visible line (vpos = vbend = 16), wrapping
 * at VTOTAL 264: (264-240)+16 = 40 lines. Render-timing only; derived from the driver, not measured.
 */
...
export class Machine {
  constructor(rom, routines, opts = {}) {
    // Two forms. Legacy (internal): (rom, routinesMap, opts). Browser worker convention:
    // (rom, opts) — when the 2nd arg is not a routines Map, treat it as opts and build routines here
    // (oracle table + opts.overrides), so web/worker.js can call `new Machine(rom, {inputs, ...gfx,
    // overrides})` synchronously.
    ...
    // MAME Z80 reset state, same for every MAME Z80 (boards/frogger/hardware.json z80Reset).
    this.regs.af = 0x0040;
    this.regs.bc = 0;
    this.regs.de = 0;
    this.regs.hl = 0;
    ...
    // (galaxian_charlayout/spritelayout read different strides of the same halves). Without the gfx +
    // proms images this.video stays null and the raster hooks are inert.
    ...
    // NOTE: frogger_map exposes NO CPU-readable scanline/vpos counter (unlike timeplt's readScanline);
    // the ROM's frame timing comes only from the vblank NMI, so there is nothing to wire here.
  }
  ...
  tick(n) {
    ...
    // DRAIN BEFORE THE BOUNDARY — the ORDER is the guarantee. Entering the boundary loop needs
    // cycles past every row's due time, so draining first paints them all.
    this.drainRaster();

    while (this.cycles >= this.nextBoundary && this.frames.length < this.maxFrames) {
      // Frame N is about to execute: assert its inputs and apply its pokes first, so both are in
      // effect DURING frame N. The notifier fires at the END of a frame, so a per-game offset must be
      // measured.
    ...
    if (this.cycles >= this.nextNmi) {
      ...
      // The irq_enable latch IS the gate; the handler clears the bit itself (0xB808 D0).
      if (this.io.nmiMask) this.fireNmi();
    }

    // A bare tick() records no successor, so the PC is stale until the next step(). Invalidating AFTER
    // the NMI check is what lets fireNmi's guard ever pass.
    this.pcKnown = false;
  }
  ...
  call(addr, ...args) {
    ...
    if (fn === undefined) {
      // NotImplemented, not a bare Error: runFrames absorbs THIS and rethrows the rest, so a real JS
      // bug cannot be reported as a translation gap.
   ...
   * undocumented F3/F5 from (A + the byte copied), S/Z/C untouched. F is diffed, so a site that
   * open-codes this without F diverges for real.
   */
  ldi(nextAddr) {
    const { regs, mem } = this;
   ...
   * repeat, 16 on exit. F IS WRITTEN ON EVERY ITERATION: a repeat ends in a `step` that can accept the
   * NMI, whose handler pushes AF into work RAM, which IS diffed.
   */
  ldirAt(self, nextAddr) {
    ...
    for (;;) {
      ...
      // A REPEAT then OVERWRITES F3/F5 from the high byte of this instruction's own address: MAME's
      // `ldir` does `PC -= 2` back onto the opcode, then `yx_val = PC >> 8`.
...
export function withOmittedRet(fn, addr = null) {
  ...
  // body, so the seam would read the unmoved stack as an omitted ret.
