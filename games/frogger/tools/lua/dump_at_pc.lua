...
-- Frogger PC-EXACT state capture: a state that exists only PARTWAY through a frame is invisible
...
-- reads, and this ROM's boot self-check reads bytes long before most routines run). Same 3328-byte
-- format as dump_state.lua. META carries the register file AT THE TAP; a ONE-SHOT snapshot, no pair.
...
-- Machine CONFIGURATION on EVERY capture path (see dump_state.lua for why galaxian has no
-- standalone DSW; dsw0=IN2, dsw1=IN1). All sampled AT SCRIPT LOAD: dips as MAME resolved them,
-- the ROM byte proving the image loaded, reg_* as the Z80 RESET state -- NOT the tapped-PC regs.
local cfgf = io.open(os.getenv("CONFIG_OUT") or "config.txt", "w")
if cfgf then
  cfgf:setvbuf("no")
  cfgf:write(string.format("dsw0=0x%02X\ndsw1=0x%02X\ncontrol_rom0000=0x%02X\n",
    sp:read_u8(0xE004), sp:read_u8(0xE002), sp:read_u8(0x0000)))
...
-- Mirrors hardware.json "stateRegions" AND boards/frogger/memory.js dumpState() order.
local REGIONS = {
  { 0x8000, 0x87FF, "ram" },
  { 0xA800, 0xABFF, "vram" },
