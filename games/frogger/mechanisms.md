...
While bays sit empty they are not blank — a creature is animated cycling through them, one bay at a time, driven by a rotating slot cursor and a free-running frame counter. The cursor is `HOME_BAY_SLOT_CURSOR` (0x8123) [seen], advanced once per in-play frame by `advanceHomeBaySlotCursor` [seen], which increments it and wraps it to 0 at 6, so it cycles 0,1,2,3,4,5 mod 6. Values 1..5 name a home bay (a 1-based index into the five bays); value 0 is a rest phase where nothing is stamped.
...
When the active player's tally reaches 5, the per-frame service routine takes the board-completion branch (for player 1: `PLAYER1_SLOT` == 5; the player-2 path checks `PLAYER2_SLOT` == 5). It zeros that player's five occupancy gates and slot count, then arms the reveal through `armBoardCompleteReveal` [seen,poked], which sets the board-advance request and start/demo flags, seeds `HOME_REVEAL_COUNTDOWN` (0x8297) [seen,poked] to 255 and `HOME_REVEAL_DELAY_TIMER` (0x8298) [seen,poked] to 64.
...
`placeScoreRankMarkers` [seen] (0x0c3d) stamps the "your rank" markers. For each of the two bytes of the packed field `INTRO_DIGIT_FIELD` (0x83fb) — the rank codes `packScoreRankPair` stored — a non-zero code writes the constant marker tile 4 into work-RAM page 0x80 at offset (48 − code), via the row helper `stampRankMarkerIfPlaced` [seen] (0x0c4a) which writes a byte into page-H RAM at row (D − C) and skips when C is 0. A zero code stamps nothing; the rank is encoded as a *position*, not a rendered numeral.
...
- **`driveDiveAnimByLevel`** — `[seen,poked]`, the two-pair-figure per-frame driver (above); named by mechanism,
  but its game-purpose is open: a turtle-dive-specific clock versus a generic figure clock, undecided.
