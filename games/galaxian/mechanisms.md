...
of state, the seed `RNG_SEED` (0x401e [seen]), and advances it with a linear-congruential step:
...
`fillMemoryBlock` [seen] at ROM 0x0010 is the block-fill: given a destination pointer, a fill byte, and
...
`fetchIndexedTableByte` [seen] at ROM 0x0020 is the indexed table read: given a table base pointer and
...
### State dispatch: an index selects a handler

A recurring shape across the machine is the data-driven state dispatch: a single state index selects
and runs one of a list of handlers, so the value in one byte drives a whole state machine — the byte
simply names which handler runs this frame. This same shape drives every state machine in the game,
most importantly the per-frame game-state fork described next, which the born-live vblank service runs
as a five-entry handler table indexed by `GAME_STATE`.

### One frame: the vblank interrupt

The per-frame handler is `enterVblankService`, run on every vertical-blank interrupt. It opens by
clearing the interrupt-enable latch `IRQ_ENABLE` (0x7001 [seen]) to acknowledge the interrupt; re-arming
for the *next* frame is deferred to the very end. In the born-live engine the interrupt is a direct JS
call with no interrupt stack, so it saves and restores no registers — it does the frame's work and
returns.

Immediately after the acknowledgement the handler consults the self-test mode flag `SELFTEST_MODE`
(0x401a [seen]) and forks. When that flag is zero — the normal case, all through attract and play — the
frame takes the main path below. When it is non-zero the frame is diverted wholesale to the alternate
handler `dispatchSelfTestMode` (described in its own subsection), which does its focused boot-time job
and re-arms the interrupt itself, and none of the main-path work happens that frame.
...
0x7000) into `IN2_SHADOW` (0x4012 [seen]), IN1 (from 0x6800) into `IN1_SHADOW` (0x4011 [seen]), and IN0
(from 0x6000) into `IN0_SHADOW` (0x4010 [seen]). One bit of IN0 is checked on the spot: bit 6 is the
...
`updateCoinLockoutFromCredits` [seen] (0x197c), which engages or releases the coin lockout by the
current credit count; `driveSoundFrame` [seen] (0x16f5), the sound driver's per-frame tick;
`driveSoundLfoLevel` [seen] (0x1898), which drives the sound LFO level; and `advanceMessageScroller`
...
Finally the frame is handed to the game-state machine. The handler reads `GAME_STATE` (0x4005 [seen])
and dispatches through a five-entry handler table indexed by that value, routing the
...
`runAttractSequenceAndAdvanceOnCredit` [seen] (0x0156), the attract/demo loop; state 2 to
`runStartScreenAndLaunchGame` [seen] (0x03f2), the post-credit press-start screen; state 3 to
`runPlayerOnePlayFrame` [seen] (0x0536), the first player's play frame; and state 4 to
`runPlayerTwoPlayFrame` [seen] (0x077b), the second player's play frame. After the selected handler
runs, the service re-arms the interrupt with a single write of `IRQ_ENABLE` (0x7001) back to 1, so the
interrupt fires again next frame. That re-arm is the single point that keeps the heartbeat going; if it
were skipped the interrupt would never fire again and the machine would freeze.

### The alternate per-frame path

When `SELFTEST_MODE` (0x401a) is non-zero the frame is diverted to the handler `dispatchSelfTestMode`,
used during the boot self-test and screen-fill phases. It is itself a small one-of-four dispatch keyed on the flag's
value; it returns its selected sub-handler's result to the vblank service, which then re-arms the
interrupt inline for the next frame. Value 1 routes to
`driveSoundFrameAndScanInput` [seen] (0x1c3a), which ticks the sound driver and sweep, pets the
watchdog, and scans IN0 for its arm bits. Value 2 routes to `advanceScreenFillStrip` [seen] (0x1d28),
...
[seen], and, within every non-boot phase, a second selector, `SEQUENCE_STATE` (0x400a) [seen]. The
vblank interrupt is the whole heartbeat: each frame it acknowledges the interrupt by
clearing the hardware enable latch (0x7001), copies the OBJRAM shadow at 0x4020 out to the sprite/scroll
hardware at 0x5800, latches this frame's raw input ports into their shadow cells behind a two-frame
history chain, decrements the free-running `FRAME_COUNTER` (0x425f) [seen], runs a fixed cluster of per-frame
...
(`runAttractSequenceAndAdvanceOnCredit` [seen]), press-start (`runStartScreenAndLaunchGame` [seen]), and the
two play handlers `runPlayerOnePlayFrame` [seen] and `runPlayerTwoPlayFrame` [seen]. After the selected
handler runs, the service re-arms the interrupt inline so the next frame can fire; the machine idles
between frames and every frame's work happens inside this interrupt. The self-test mode flag
`SELFTEST_MODE` (0x401a) diverts the whole frame to an alternate per-frame path — the boot self-test /
screen fill — when it is nonzero, which does its focused job and re-arms the interrupt itself.
...
carry-out of the dwell timer expiring**. `tickSequenceDwellTimer` [seen] ticks `loc_4009` directly each
...
re-arm the tiers. `reloadSequenceDwellTimer` [seen] stamps `loc_4009` back to 80; `advanceSubstateAndReloadDwell`
[seen] bumps the state index and then re-arms it; `setSequenceStateByModeAndReloadDwell` [seen] jumps the
index to one of two fixed values chosen by the mode flag `loc_4006` before re-arming; and
`advanceSequenceStateAndReseedObjectShadow` [seen] performs the same in-place carry (its pointer arrives on
`loc_4009`, so stepping to the next byte bumps `SEQUENCE_STATE`) and then reseeds the object shadow.
`enterSequenceStep1` [seen] is the cold hand-in: it forces `SEQUENCE_STATE` to 1 and seeds a very short
...
each frame while ticking the prescaled timer, and `holdStartLampsThenAdvanceSequence` [seen] counts it down,
...
(0x400b) [seen] and advances that cursor, marching a clean fill across the whole tilemap while the
...
`FLAG_BITS_BASE` (0x4100) [seen] via `unpackBitmaskToFlagBytes` [seen], seeds the object shadow and the
...
`runAttractSequenceAndAdvanceOnCredit` [seen] is the attract phase (`GAME_STATE` = 1). Each frame it runs the
...
turns the starfield on (`STARS_ENABLE` 0x7004 [seen]), resets the per-sequence cells, and seeds the dwell
cascade (`loc_4008` = 96, `loc_4009` = 16); `primeVramFillAndAdvanceStep` [seen], `fillVramRowThenResetObjectState`
[seen], and `blankVramRowThenResetSpriteState` [seen] repaint the field a row at a time between beats;
`emitMessageColumnsThenAdvanceSequence` [seen], `queueColumnDrawAndAdvanceSequence` [seen],
`activateDescriptorSlotsThenAdvanceSequence` [seen], and `postCreditAndMessageDrawsAndAdvance` [seen] emit
the title and message text as deferred draws; `dwellThenAdvanceSequence` [seen] is a pure hold that only
runs the shared subsystem updates and ticks the prescaled timer; and `clearFlagBlockAndReseedObjectShadow`
[seen] plus `loadDescriptorAndAdvanceSequence` [seen] set up the demo. Notably, the late attract steps reuse
the live play handlers as their own sub-states — `activateObjectsAndBeginPlayPhase` [seen],
`runGameplayFrameAndAdvanceOnFieldClear` [seen], and `stepPlaySubstate6` [seen] appear as attract steps
...
After the sub-state, attract tail-runs `advanceGameStateOnCredit` [seen], and this is the coin hand-off: if
...
`MESSAGE_SCROLL_ENABLE` (0x40b0) [seen] — so the next frame comes up cleanly on the press-start screen. A
...
off to `presetCreditCount` [seen], which pins the credit count to 9; otherwise it combines the input
shadows, masks them against the coin guard cells, and routes a coin edge either to `addCreditForCoin` [seen]
...
`incrementCreditCount` [seen] or the two-coins-per-credit path `awardCreditEverySecondCoin` [seen] — the
latter toggling the coin-phase flag through `setCoinPhaseFlag` [seen] so the credit only lands on the second
...
`clampCreditsToMax` [seen] exists purely to pin an overshoot back to exactly 99. Finally
`updateCoinLockoutFromCredits` [seen] drives the coin-lockout latch `COIN_LOCKOUT` (0x6002) [seen] straight
off the credit count, releasing it (via `clearCoinLockout` [seen]) once nine credits are banked and engaging
it below that.

### Press-start and launching a round

`runStartScreenAndLaunchGame` [seen] is the credit-inserted screen (`GAME_STATE` = 2). It runs the same
formation prep, dispatches on `SEQUENCE_STATE` across four short sub-states — `resetObjectRamAndAdvanceSequence`
[seen] to clear the object records and reseed the shadow, `holdStartLampsThenAdvanceSequence` [seen] to hold
while its countdown runs, `blankVramRowsThenDriveStartLamps` [seen] to wipe two rows, and
`driveStartButtonLamps` [seen] to light the buttons — then tail-runs `beginGameOnStartButton` [seen] every
...
`startOnePlayerGame` [seen]; bit 1 (two-player start) fires only with at least two credits banked, spending
two, copying the 32-byte row template `loc_051b` into `SAVED_STATE_SNAPSHOT` (0x41a0) [seen], optionally
...
`startGameRoundAndClearScores` [seen], which stores the player/spawn word into `CURRENT_PLAYER` (0x400d) [seen]
(a 16-bit store whose high byte becomes the two-player flag), blits the row template into the packed board
buffer `PACKED_FLAG_BITMAP` (0x4180) [seen], sets `SEQUENCE_STATE` = 0 and `GAME_STATE` = 3 to enter play,
...
`runPlayerOnePlayFrame` [seen] at `GAME_STATE` = 3 and `runPlayerTwoPlayFrame` [seen] at `GAME_STATE` = 4.
...
- **0 — setup.** `initPlayfieldState` [seen] turns off both start lamps, clears the flag block, the object
...
- **1 — clear.** `blankScreenRowsThenAdvanceSequence` [seen] blanks 32 cells per frame at the cursor,
...
- **2 — restore the board.** This is where the two phases first differ: player one's
  ...
  two's `restoreSavedStateAndEnterPlaySubstate` [seen] unpacks it from `SAVED_STATE_SNAPSHOT`. Either way it
...
- **5 — play.** `runGameplayFrameAndAdvanceOnFieldClear` [seen] is the actual per-frame gameplay pipeline.
  ...
  been cleared.
- **6 — branch.** `stepPlaySubstate6` [seen] (and its player-two twin `stepAltPlaySubstate6` [seen]) branch
  ...
  `advanceDwellOrResetToState1` [seen], is the fork between "carry on" and "game over": with `loc_4006`
...
- **7 — hand off.** The terminal ticks the dwell one last time and, on expiry, packs the live board back
  into storage and switches players. Player one's `packFlagsToBitmapAndSwitchPlayerState` [seen] packs the
  flag bytes into `PACKED_FLAG_BITMAP`, sets `CURRENT_PLAYER` = 1, and sets `GAME_STATE` = 4; player two's
  `saveFlagsToSnapshotAndSwitchPlayerState` [seen] packs into `SAVED_STATE_SNAPSHOT`, sets `CURRENT_PLAYER`
...
`enqueueCommandWordBurst` [seen] pushes a fixed five-word sound cue in one call. A per-frame consumer,
...
`driveSoundFrame` [seen] and `driveSoundLfoLevel` [seen] tick; and `advanceMessageScroller` [seen] steps one
glyph up its VRAM column each frame while `MESSAGE_SCROLL_ENABLE` [seen] is set, stopping itself on its own
delay counter. Over gameplay specifically, the play pipeline layers two more timers on top of the sequence
machine's dwell clock. `scheduleDelayedEvent` [seen] arms a two-tier delay while the object subsystem is
active and uninhibited; `fireDelayedEventRequest` [seen] counts `DELAYED_EVENT_TIMER` (0x422f) [seen] down
each frame and, as a one-shot on the zero tick, disarms itself and — only while the object subsystem is
enabled — raises `DELAYED_EVENT_REQUEST` (0x4229) [seen]. Alongside it, `expireActivityGatedTimer` [seen] is
...
timers round out the layer. `rampCounterToCeiling` [seen] is a slow pace/difficulty ramp: a two-tier prescaler
...
stage advance resets it to 0). And a behavior-gate pair drives `loc_4208`: `armBehaviorGateOnInputOrTimer` [seen]
...
also arming the companion `loc_41cc` — while `clearGateOnPendingRequest` [seen] is its acknowledger, consuming a
pending request in `loc_420b` bit0 by clearing both it and the gate `loc_4208`.

### The clamp helper

The one small shared arithmetic helper in this cluster is `clampCreditsToMax` [seen]: it is the overshoot
...
(0x42b0) [seen]. `OBJ_TABLE` (0x42d0) [seen] names the second slot of that same array and is the
handle the spawn and collision code use as the "primary" object table; the descriptor slots at
`DESCRIPTOR_SLOT_TABLE` (0x4330) [seen] sit four records further in. Within a record the fields that
...
sprite-shadow records at `SPRITE_SHADOW_BASE` (0x4060) [seen], laying them down as a band of three
...
stages the player object's negated position into the same region's `OBJ_STAGE_BLOCK` (0x4054) [seen],
...
entry that aims it at `STRIDED_TABLE_SRC` (0x1d71) [seen]; two sibling templates seed the same lane in
other phases — `loc_1d91` at the start screen and `OBJ_SHADOW_RESEED_TEMPLATE` (0x1db1) [seen] in the
...
The standing formation itself is a bitmap of live aliens. `FLAG_BITS_BASE` (0x4100) [seen] is a block
...
into the 128 flag bytes, and `packFlagBytesToBitmask` [seen] is its exact inverse. Those packed forms
are where a board is stored between turns — `PACKED_FLAG_BITMAP` (0x4180) [seen] and
`SAVED_STATE_SNAPSHOT` (0x41a0) [seen] hold the two players' boards, packed on a player switch and
unpacked back into the flag block when that player resumes.

Co-resident in the same page is the object/occupancy grid based at `OBJECT_GRID_BASE` (0x4120) [seen]:
six rows at a 16-byte stride, whose occupancy lane is `OCCUPANCY_GRID` (0x4123) [seen].
`summarizeFormationOccupancy` [seen] reduces that six-row-by-ten-column occupancy view into the
summaries the rest of the field consults. It ORs each row into `ROW_OCCUPANCY` (0x41e8) [seen] (behind
two always-empty guard cells) and each column into `COLUMN_OCCUPANCY` (0x41f0) [seen] (behind three
guards). From the column ORs it derives the horizontal extent of the block into `FORMATION_X_BOUNDS`
(0x4210) [seen]: scanning inward from the right end it starts at 34 and steps 16 per empty column to
...
`positionObjectFromGridCell` [seen] turns it into on-screen coordinates using the same field
...
frames by the low two bits of `FRAME_COUNTER` (0x425f) [seen]. `OBJ_SWEEP_DIRECTION` (0x420d) [seen]
...
Once per frame `driveAllObjectSlots` [seen] walks all eight records and hands each to `driveObjectSlot`
[seen]. That driver reads the record's flags and state index and routes: a record whose dying-animation
flag (byte 1 bit0) is set goes to `dispatchDeactivatedObjectAnim` [seen], which sub-dispatches on byte
...
Those sixteen states are the attacker life-cycle. State 0, `initSpawnedObjectFromGridCell` [seen], is
the first tick of a freshly launched object: it raises the spawn flag, positions the sprite from its
packed cell, looks up the sprite number and flight-curve seed from `SPAWN_RECORD_TABLE` (0x1dd1) [seen]
by row (the top row also tallies its two neighbours into `ACTIVE_NEIGHBOR_COUNT` (0x422a) [seen]), seeds
the motion counters, and sets the heading sign from the direction bit. State 1,
`advanceObjectPathStep` [seen], walks a per-object cursor through `PATH_STEP_TABLE` (0x1e00) [seen],
...
[seen], picks the object's next horizontal target — a stored/player target for one kind of object, a
cross-player target for the rest. State 3, `advanceObjectFlightAndFire` [seen], and its homing sibling
state 9, `advanceHomingObjectFlightAndFire` [seen], run the swoop: each frame they advance the flight
...
`advanceObjectDiveStep` [seen], steps the diving object down one or two pixels on frame parity and folds
the flight curve into Y until it runs off the bottom. State 5, `reseedFormationObjectState` [seen],
...
[seen], is how a diver returns home: it repositions from the grid cell each frame and, when its counter
...
The shared motion-planning primitives sit beneath those handlers: `commitMoveAcrossPlayerX` [seen] and
`commitMoveToTargetX` [seen] set up a horizontal run toward a chosen X; `advanceObjectFlightCurve`
[seen] runs a cross-coupled fixed-point rotation over two accumulators in the record, whose high bytes
...
`spawnAimedProjectileAtPlayer` [seen] claims a free slot in the fourteen-entry projectile table at
...
The trickle of divers is metered by `paceEnemyLaunchTrigger` [seen]. Gated on the object subsystem
...
`SUBCOUNTER_RELOAD_TABLE` (0x15e3) [seen] and sweeps a span of per-attacker sub-counters — the span
widening with the pace counter and the stage — refilling each that reaches zero and raising
`SUBCOUNTER_REFILL_FLAG` (0x4228) [seen] if any refilled. That refill flag is the one-shot trigger the
launcher consumes.

`launchAttackerFromFormation` [seen] fires exactly when that flag is up and the region-clear gate is
...
`chooseNextAttackerDirection` [seen]: when the anchor sits within 28 pixels of a bound the direction is
forced away from that edge, otherwise a fresh random bit decides. `selectAttackerRowScan` [seen]
...
being armed. `flagPlayerShotHitOnFormation` [seen] handles the standing block directly in grid space: it bands
...
[seen] sweeps the seven object-table records and box-tests each against the shot reference via
`flagPlayerShotHitOnObject` [seen]; a hit tail-calls `awardKillScoreByBandAndDeactivate` [seen], which
...
Beyond the paced launcher, a delayed-event path spawns extra objects. `scheduleDelayedEvent` [seen] is a
gated two-tier timer that, on a cascade elapse, arms the delayed-event triple — `DELAYED_EVENT_TIMER`
(0x422f) [seen], a companion counter, and `DELAYED_EVENT_ARMED` (0x422e) [seen] — either to fixed values
or, in its mode-set branch, to values derived from work-RAM word sums. `fireDelayedEventRequest` [seen]
then counts that timer down while armed and, on the zero tick, disarms and (when the subsystem is
enabled) raises `DELAYED_EVENT_REQUEST` (0x4229) [seen]. `spawnObjectsOnDelayedEvent` [seen] consumes
...
spawn (`spawnObjectsFromTriggerFlags` [seen]), the other scans the `PRIMARY_TRIGGER_BLOCK` (0x4176)
[seen] high-to-low for a set flag and spawns a primary plus up to two secondaries
(`spawnPrimaryAndSecondaryObjects` [seen]), then falls to the secondary window and seeds a free
descriptor slot (`spawnIntoFreeDescriptorSlot` [seen]). At the bottom of all these paths,
`activateObjectSlotAndEnqueueSpawn` [seen] consumes the trigger flag it was found under, marks the slot
...
When the block is emptied, the field advances to a harder stage. `armFormationAdvanceTrigger` [seen] is
...
`advanceStageAndReseedFormation` [seen] later consumes that: on the tick where the enable is set and its
...
this subsystem is gated by the master switch `OBJ_ACTIVE_FLAG` [seen]: its bit 0 is
...
`moveControlledObjectAndStageSprite` [seen] moves it once per frame. When
`OBJ_ACTIVE_FLAG` bit 0 is set it first chooses a source of movement bits: with the
mode flag `loc_4006` bit 0 clear it takes the auto/AI command `OBJ_MOVE_CMD` [seen],
otherwise it reads a live controller — the second port `IN1_SHADOW` [seen] when the
orientation flag `loc_4018` bit 0 is set, else the first port `IN0_SHADOW` [seen]. From
...
`OBJ_STAGE_BLOCK` [seen]. When the ship is *not* active the routine still runs: in the
...
`computeControlledObjectMoveCommand` [seen]. This runs only on one frame in
...
object tables (the seven attacker records at `OBJ_TABLE` [seen], stride 32, and the
...
`accumulateObjectPositionWeight` [seen] is the per-object scorer behind that sum, and it
...
and the row band into a 0–15 index, reads a signed step from `OBJ_STEP_TABLE` [seen],
...
[seen] is the whole timing engine. A gate byte `loc_4208` marks a shot in flight: while
...
`advancePlayerShotAndStageSprite` [seen] wraps that: it services the shot, then reads
...
same memory two ways. `advanceAndRenderProjectiles` [seen] treats it as seven ten-byte
...
`SPRITE_SOURCE_OBJ_BASE` [seen] (0x42b0) with a 32-byte stride; `OBJ_TABLE` [seen]
(0x42d0) names the second of these, the base of the seven records the spawn and
collision code treats as the live attackers. Every frame `driveAllObjectSlots` [seen]
walks all eight, handing each in turn to `driveObjectSlot` [seen].
...
New attackers are metered out, not spawned all at once. `paceEnemyLaunchTrigger` [seen]
...
holds the refill flag `SUBCOUNTER_REFILL_FLAG` [seen] clear. On expiry it reloads the
master from `SUBCOUNTER_RELOAD_TABLE` [seen] and sweeps a difficulty-scaled span of the
...
`launchAttackerFromFormation` [seen] consumes that one-shot flag. If the refill flag is
...
scanning `COLUMN_OCCUPANCY` [seen] from one end or the other according to that
direction, walks that column of the formation flag grid `FLAG_BITS_BASE` [seen] looking
...
Which direction a launch takes is decided by `chooseNextAttackerDirection` [seen]. It
reads the 16-bit formation anchor `loc_420e`; the anchor's sign picks which of the two
sweep bounds in `FORMATION_X_BOUNDS` [seen] to measure against. When the anchor sits
...
A second, delayed spawn path runs off a timer. `scheduleDelayedEvent` [seen], guarded by
...
the fixed delayed-event triple — `DELAYED_EVENT_TIMER` [seen] to 90, `loc_424a` to 45,
`DELAYED_EVENT_ARMED` [seen] to 1; with it set the routine instead derives the payload
from `loc_4221` or from the byte-sums of `loc_4177` and `loc_421a` and fans it into the
triple by rotation. `fireDelayedEventRequest` [seen] then counts `DELAYED_EVENT_TIMER`
down while armed and, on the zero tick, disarms (one-shot) and — only if both enables
hold — raises `DELAYED_EVENT_REQUEST` [seen].

That request is picked up by `spawnObjectsOnDelayedEvent` [seen], which is gated on the
...
`PRIMARY_TRIGGER_BLOCK` [seen] high-to-low for a set flag (spawning a primary plus its
...
`activateObjectSlotAndEnqueueSpawn` [seen] is the common core: it consumes the trigger
...
keyed by that index. `spawnPrimaryAndSecondaryObjects` [seen] uses it to spawn a primary
...
`spawnSecondaryObjectIntoSlot` [seen] — which refuses a slot if either of its two live
...
[seen] is the per-step form of that walk, advancing the slot pointer and forcing the
enclosing loop to end when its budget is spent. `spawnObjectsFromTriggerFlags` [seen]
scans the primary trigger block (four flags): the first set flag spawns a primary plus up
to two secondaries from the secondary block `SECONDARY_TRIGGER_BLOCK` [seen]; if no
primary flag is set it scans the secondary block and, on the first hit, seeds a free
descriptor slot. `spawnIntoFreeDescriptorSlot` [seen] is the shared free-slot finder,
scanning the four descriptor slots in `DESCRIPTOR_SLOT_TABLE` [seen] high-to-low for one
whose two guard bytes are both zero and seeding it.

Once a slot is claimed and reaches state 0, `initSpawnedObjectFromGridCell` [seen] does
...
`SPAWN_RECORD_TABLE` [seen] indexed by the cell's row. For a top-row object it also tallies
how many of its two neighbour slots are still active into `ACTIVE_NEIGHBOR_COUNT` [seen]
...
`positionObjectFromGridCell` [seen] is the shared unpacker: the display axes are rotated
...
(`advanceObjectPathStep` [seen], state 1) reads pairs of deltas from
`PATH_STEP_TABLE` [seen] through a per-object cursor: the first byte steps the sprite X
(record+3), the second — direction-chosen by the object's direction bit — steps the sprite
Y (record+4). If that Y value (plus a seven-pixel margin) crosses the near screen edge the
object is thrown into the fall-away state (5).
...
leg finishes. `advanceObjectPathStepDescending` [seen] (state 10) is the mirror that
*subtracts* the deltas, handing the Y half to `advanceObjectPathStepAscending` [seen]
...
cursor. `advanceObjectPathStepAlias` [seen] (state 11) is a bare trampoline that forwards
to the ordinary path walk for whichever object is current — it holds no state of its own.

The **flight curve** is the swoop. `advanceObjectFlightCurve` [seen] runs
...
match. `advanceObjectDiveStep` [seen] (state 4) is the plunge: it steps the position
...
`advanceHomingObjectFlightAndFire` [seen] (state 9) is the tracking variant. Before the
...
`homeObjectXTowardPlayer` [seen] (state 7) steers the object's 16-bit X:subpixel pair
...
`reseedFormationObjectState` [seen] (state 5) is the return-to-formation / recycle
...
toward a ceiling of two. `settleObjectIntoFormationCell` [seen] (state 6) repositions the
...
`commitMoveAcrossPlayerX` [seen] chooses that target relative to the ship: it takes the
...
`commitMoveToStoredOrPlayerTargetX` [seen] picks between two sources: if the object
table's mode flag has bit 0 set it steers toward the table's stored target X, otherwise it
runs the ordinary cross-player pick. `beginObjectCrossPlayerMove` [seen] is the shared
...
Those primitives are driven by three states. `advanceActorPhaseAndCommitMove` [seen]
...
select. `armDirectedMoveWhenInWindow` [seen] (state 8) ticks an arm counter and, until
...
[seen] (state 12) bumps a sub-counter, forces the state back to 8 (re-entering the
...
top three bits. The division itself is `divideUnsigned8` [seen], an eight-round
compare-subtract-shift restoring divide that returns the quotient with no memory effect.

`spawnAimedProjectileAtPlayer` [seen] creates the shot. It claims the first free entry in
...
`computeJitteredXVelocity` [seen], which divides the slope, adds a bounded random draw
...
Which row an attacker fires from is chosen by `selectAttackerRowScan` [seen]. It seeds a
slot index and base value from the alt flag `loc_421b` (slot 2 / base 157 when set, else
slot 1 / base 132), scans up to four two-byte slots of `ROW_OCCUPANCY` [seen] advancing
...
`SPRITE_SHADOW_BASE` [seen] (stride 4), as a band of three followed by a band of five,
...
The player's shot is tested against two things. `flagPlayerShotHitsOnObjects` [seen],
gated on the shot gate `loc_4208`, box-tests the shot reference (`loc_4209`/`loc_420a`)
against each of the seven objects in `OBJ_TABLE` via `flagPlayerShotHitOnObject` [seen],
which uses a six-wide by twelve-tall window; on an overlap it raises the shot-retire flag
`loc_420b` and scores-and-deactivates the object. `flagPlayerShotHitOnFormation` [seen]
...
The player is tested against the two enemy hazards. `flagObjectHitsOnPlayer` [seen],
gated on `OBJ_ACTIVE_FLAG`, box-tests each of the seven objects against `loc_4202` via
`flagObjectHitOnPlayer` [seen], which classifies the object's field into a near or far
proximity window and, on an overlap, raises `HIT_EVENT_FLAG` [seen] and scores the
object. `flagProjectileHitsOnPlayer` [seen], also gated on `OBJ_ACTIVE_FLAG`, loops all
fourteen entries of `loc_4260` through `flagProjectileHitOnPlayer` [seen], which
...
`awardKillScoreByBandAndDeactivate` [seen] is the shared kill handler the
...
[seen], which adds one only when both look-ahead slots are inactive), records the bonus in
`loc_422d`, and enqueues the folded score request — this is how shooting the last of a
diving trio pays a bonus.

`HIT_EVENT_FLAG` [seen] is the player-death event, and `handlePlayerHitEvent` [seen] is
...
`loc_421d`, and pulses the sound latch `SOUND_W_REG3` [seen] when the mode bit `loc_4006`
...
`dispatchDeactivatedObjectAnim` [seen] reads the record's animation sub-state (record+2)
and runs one of four phase handlers. Sub-state 0, `armObjectAnimAndRequestSound` [seen],
...
[seen], runs a fast timer down (reloading it and stepping a companion field on each
...
[seen], simply counts a timer down and clears the record's state byte to retire the object
when it reaches zero. Sub-state 3, `noopAnimDispatchSlot` [seen], does nothing — a valid
...
(0x6800-0x6807, all [seen]) — note the read side of 0x6800 is the input port `IN1` [seen] — the four
LFO frequency latches beginning at `SOUND_LFO_FREQ` (0x6004-0x6007, [seen]), and the single pitch latch
`SOUND_PITCH_W` (0x7800, [seen]). Nothing plays directly off the game logic; instead the various voices
...
`driveSoundFrame` [seen] is the heart of the audio system, and it runs once per frame as part of the
per-frame audio-and-input service `driveSoundFrameAndScanInput` [seen]. It opens each frame from a clean
slate: it clears the composite flag byte `loc_41c0` and primes the staged-pitch shadow `SOUND_PITCH`
[seen] to 0xff. It then runs seven voice updaters in a fixed order — the sequence-arm-on-request step,
...
`loc_41c0` goes to `SOUND_W_REG6` [seen]; a rotate-right of that same byte (its bit 0 wrapping up into
bit 7) goes to `SOUND_W_REG7` [seen]; and the staged `SOUND_PITCH` shadow is copied out to the pitch
...
watchdog by reading `WATCHDOG_RESET` (0x7800, the read side of the pitch port), and reads the input port `IN0` [seen]: if any of that port's arm
...
`advanceAllSoundSequenceChannels` [seen], which steps each of the three sequence descriptors —
`loc_41d2`, `loc_41cf`, and `SOUND_SEQ_ACTIVE` [seen] — one tick per frame through the shared per-channel
worker `advanceSoundSequenceChannel` [seen].
...
command byte from its shared playback cursor `SOUND_SEQ_PTR` [seen]. A command byte of 0xe0 is an end
marker that deactivates the channel by zeroing its descriptor. Any other byte is a packed note: its low
five bits index the tone table `SOUND_TONE_TABLE` [seen] to fetch the new tone into `loc_41d5`, its high
three bits index the duration table `SOUND_DURATION_TABLE` [seen] to fetch the new timer into `loc_41d6`,
...
`loc_1e68`. `armSoundSequenceBySelector` [seen] runs only while the sound driver is enabled (bit 0 of
...
selector value is handed on to `armSoundSequenceForSelector16` [seen]. That last handler recognizes only
...
`updateSoundSweepVoice` [seen] drives a pitch sweep and, like the sequence arming, only acts while the
...
`advanceSoundSweepAndStagePitch` [seen]. On the frame where the countdown finally reaches zero, it
...
The pitch selector `stageSoundPitchBySelector` [seen] reads the low two bits of the sweep cell it is
handed. When those bits are zero it stages a fixed pitch of 96; when they are nonzero it recomputes a
pitch from the sound counter via `stagePitchFromSoundCounter` [seen]. That recompute takes the counter
...
`advancePulseToneEnvelope` [seen] runs a two-byte decrementing envelope word held in `loc_41c7` (low) and
`loc_41c8` (high). When bit 0 of the low byte is clear it hands the high byte to
`pulseSoundToneFromCountdown` [seen]; when bit 0 is set it re-arms the word to its top-bit sentinel,
...
`stagePitchAndRaiseSoundFlag` [seen]. That helper writes its argument minus one (modulo 256) into
`SOUND_PITCH` and raises the composite flag `loc_41c0` to 1, marking the shadow pair filled for the
frame.

### The rising pitch ramp

`driveRisingPitchRamp` [seen] produces a rising glide. It uses the arm cell `loc_41c9` as a one-frame
...
`advanceGatedSquareTone` [seen] runs a phase counter `loc_41cc` for a square-wave voice. While that
counter is more than one step from expiry it drives the tone toggler `driveGatedSquareTone` [seen] each
tick; on the expiry step it parks the counter at 0 and re-arms the tone's duration `SOUND_TONE_DURATION`
[seen] to 8. The toggler itself, while `SOUND_TONE_DURATION` is nonzero, counts one tick off that
duration and drives `SOUND_W_REG5` [seen] with the frame flag `loc_4007` XORed by 1 — a bit that
...
over time. `driveSoundLfoLevel` [seen] runs from the vblank interrupt service and dispatches on a reset
request: when `SOUND_LFO_RESET_REQUEST` [seen] is zero it runs the ordinary per-frame decay through
`decaySoundLfoLevel` [seen]; otherwise it consumes the request (clearing the flag) and slams the full
...
[seen]) is still nonzero — dropping the level by one and re-broadcasting it. Both the reset and the decay
...
`driveSoundVoicesFromOccupancy` [seen] is a hum whose thickness — how many of its voices are lit —
tracks how many enemies remain (it stages no pitch value). It runs only
on even frames (bit 0 of the frame flag `loc_4007` clear). It sums the six-by-ten flag grid
`OCCUPANCY_GRID` [seen] (rows sixteen bytes apart) into an eight-bit tally seeded at 1, then lights the
...
`driveDecayingSoundSweep` [seen] emits a fading sweep on alternate frames. On even frames, while its
countdown `loc_41df` is nonzero, it writes the countdown rotated right by two bits into `SOUND_W_REG4`
[seen] and ticks the countdown down by one, so the emitted value shrinks toward silence over successive
...
`driveGatedSoundStepSequence` [seen] plays a stepped burst gated by bit 0 of `loc_4201`. A prescaler in
...
`armObjectAnimAndRequestSound` [seen], which — as the entry phase of an object's death animation, run via
`dispatchDeactivatedObjectAnim` [seen] — sets up the animation timers and then posts a sound request
keyed on the object's screen position: below a threshold it requests one effect, at or above it another.
Player input seeds it through the input-scan chain: `requestSoundOnInput1AndContinueScan` [seen] reads
the input port `IN1` and seeds the selector to 0x16 when either of its low two bits is set, then hands
off to `requestSound6AndContinueInputScan` [seen], which seeds the selector to 6 when the folded input
ports show bit 2 or 3 set. Board and level starts post their cue a different way, through
`queueBoardStartSoundBurst` [seen], which enqueues a prologue command word on the cue channel (channel 5)
followed by the standard five-word burst that spans channels 5, 6, and 7.

### Silencing the hardware

`silenceSoundAndDisableIrqStars` [seen] is the full quiesce path, used on attract-mode and reset
...
the audio hardware — clears `IRQ_ENABLE` [seen] to stop the vblank interrupt and `STARS_ENABLE` [seen] to
halt the starfield.
...
per slot. A producer appends through the command-queue write head kept in `loc_40a0` [guess]; the drain consumes
through the read cursor `DISPLAY_LIST_CURSOR` (`0x40a1`) [seen]. Both indices are single bytes that
...
head back to `loc_40a0` and returns through the shared stack epilogue `loc_090b` [seen], which
restores the caller's saved pointer and does no work of its own.

Callers that need a whole sound-and-draw cue at once use `enqueueCommandWordBurst` [seen], which
...
The eight channels are: **0** `drawAnimatedTileFigureAtPackedCoord` [seen] — map the argument, a
packed VRAM coordinate, to a tilemap cell (via `mapPackedCoordToVram` [seen], which shuffles the
...
`drawFixedTileFigureAtPackedCoord` [seen] — the same mapping but a fixed (unanimated) figure, a 2x2
block when the coordinate's bit 4 is set else a vertical tile pair; **2** `draw4x4TileForm` [seen] —
draw one of three 4x4 indicator forms at a fixed screen region (blank-plus-icon, blank-only, or four
2x2 blocks of a folded tile code); **3** `addBcdScoreIncrementAndUpdateHighScore` [seen] — add a
score increment for a scored event; **4** `clearAndRedrawScoreField` [seen] — zero and repaint a
score field; **5** `drawScoreFieldByIndex` [seen] — repaint a score field in place; **6**
`renderMessageColumn` [seen] — paint, arm, or erase a text column; and **7**
`renderHudFieldBySelector` [seen] — repaint one HUD field. Channels 0-2 are the tile-figure painters
...
free play) it hands off to `presetCreditCount` [seen], which simply pins the credit count to 9 every
frame (clearing the coin-phase byte `loc_4001` [guess] and writing 9 into the credit count
`loc_4002` [guess]). Otherwise it folds the two input shadows (`IN0_SHADOW` `0x4010` [seen] with
...
[seen]; otherwise each of the low two bits ticks the coarse coin counter `loc_4004` [guess] once, up
...
in bit 0 — the only wired bit — and latches it to `COIN_COUNTER_0_LATCH` (`0x6003`) [seen], then
...
`awardCreditEverySecondCoin` [seen], a toggle on the coin-phase flag `loc_4001`: the first coin of a
pair merely raises the flag via `setCoinPhaseFlag` [seen]; the second clears it and advances the
credit count. Every credit award funnels through `incrementCreditCount` [seen], which walks the count
toward a ceiling of 99 — at the ceiling it does nothing, past it `clampCreditsToMax` [seen] pins it
...
The coin mechanism's lockout coil follows the bank. `updateCoinLockoutFromCredits` [seen] reads the
credit count and, at nine or more credits, releases the lockout through `clearCoinLockout` [seen]
(writing 0 to `COIN_LOCKOUT`, `0x6002` [seen]), de-energizing the coil so the mechanism accepts coins;
below nine it engages the lockout by writing 1, energizing the coil so the slot rejects coins. So the
coin slot is held closed while fewer than nine credits are banked and opens once nine or more are.

### From credit to game start

While no game is running the machine sits in its attract state, and the transition out of it is
credit-driven. `advanceGameStateOnCredit` [seen] runs as the tail of the attract handler: whenever
the credit count `loc_4002` is nonzero it steps the top-level `GAME_STATE` (`0x4005`) [seen] forward
one and clears the attract sub-state cluster — the frame flag `loc_4007` [guess], the sequence step
`SEQUENCE_STATE` (`0x400a`) [seen], a pair of sound cells (`loc_41c2`, `loc_41df` [guess]), and the
scroller enable `MESSAGE_SCROLL_ENABLE` (`0x40b0`) [seen] — moving the machine to the "press start"
...
[seen] gates on bit 5 of the free-running `FRAME_COUNTER` (`0x425f`) [seen]: while that bit is clear
both lamps (`START_LAMP_0` `0x6000`, `START_LAMP_1` `0x6001` [seen] — the same board addresses that
...
Pressing start launches through `beginGameOnStartButton` [seen], which reads the start buttons out of
`IN1_SHADOW` (`0x4011`): bit 0 begins a one-player game outright through `startOnePlayerGame` [seen];
...
(`0x41a0`) [seen], optionally arming the state advance gate (`armStateAdvanceGate`, writing `loc_41b5`
...
`GAME_STATE` back to 1. Both funnel into `startGameRoundAndClearScores` [seen], the common round
setup: it stores the 16-bit spawn word into `CURRENT_PLAYER` (`0x400d`) [seen] and its neighbour —
...
(`0x4180`) [seen], arms the sub-state advance gate (`armSubstateAdvanceGate`, writing `loc_4195` [guess])
...
`PLAYER2_SCORE_BCD` (`0x40a5`), both [seen] — and the machine keeps one shared `HIGH_SCORE_BCD`
(`0x40a8`) [seen]. Scoring is a channel-3 command carrying a score-type index, dispatched to
`addBcdScoreIncrementAndUpdateHighScore` [seen]. Gated off when the frame-skip flag `loc_4007`'s bit 0
is set, it otherwise selects the active player's field via `selectCurrentPlayerScore` [seen] (player 1
→ `PLAYER1_SCORE_BCD`, else `PLAYER2_SCORE_BCD`), reads the matching 3-byte increment from
`SCORE_INCREMENT_TABLE` (`0x22d0`) [seen] at `index*3`, and adds it into the score low byte first with
...
The digit painting is a small stack of helpers. `drawScoreToSelectedPlayerField` [seen] picks the
VRAM digit-field cursor from a selector — `DIGIT_FIELD_PRIMARY` (`0x5381`) [seen] for the primary
player, `DIGIT_FIELD_ALT` (`0x5121`) [seen] otherwise — and calls `drawBcdNumberColumn` [seen], which
...
[seen], which maps a nibble to its glyph (digits `0`-`9` are the contiguous tiles `0x90`-`0x99`) and
...
paints a real `0`. `byteToPackedBcd` [seen] is the binary-to-BCD converter (value mod 100 as two
nibbles) that the HUD and coin lines lean on; `drawHighScoreDigits` [seen] is the fixed-cursor variant
that paints six digits into the high-score field `loc_5241` [guess].

Two channel handlers repaint whole fields on demand. `drawScoreFieldByIndex` [seen] redraws the field
...
[seen] is a one-shot per current player: it indexes the per-player flag table at
`PLAYER1_BONUS_MARKER_AWARDED` (`0x40ad`, with the player-2 slot at `PLAYER2_BONUS_MARKER_AWARDED`
`0x40ae`, both [seen]) by `CURRENT_PLAYER`, does nothing if that player's slot is already flagged, and
otherwise flags it, raises a sound-envelope trigger (`loc_41c7` [guess]), bumps the marker counter
`loc_421d` [guess], and repaints the row with the new count. `drawMarkerRow` [seen] paints the count
as marker tiles (tile 102) growing upward from `MARKER_ROW_VRAM` (`0x539e`) [seen] across five slots,
then blanks the remaining slots; when the object subsystem is live (`OBJ_ACTIVE_FLAG` `0x4200` [seen]
...
`renderHudFieldBySelector` [seen] is channel 7: a four-way switch on its argument that repaints one
HUD element. Selector 0 draws the coin/credit icon-tally row — gated off on odd frames by
`loc_4007` bit 0, it reads the tally `COIN_CREDIT_ROW_COUNT` (`0x421c`) [seen], adds one, clamps to a
ceiling, converts to BCD, and stamps that many tens-blocks and units-pairs into
`COIN_CREDIT_ROW_VRAM` (`0x507e`) [seen] before blanking the remainder of sixteen slots (and, when a
region flag `loc_4220` [guess] is set, it also requests a sound-LFO reset via `SOUND_LFO_RESET_REQUEST`
`0x41d0` [seen]). Selector 1 draws the credit-count line — skipped while `loc_4006` bit 0 is set. When both dip bits in
...
`CREDIT_COUNT_TENS_VRAM` (`0x529f`) and `CREDIT_COUNT_UNITS_VRAM` (`0x527f`), both [seen]. Selector 2 draws a two-nibble status readout from `loc_40ac` (skipped when
...
(`0x5158`), both [seen]. Any other selector repaints the bonus-marker row through `drawMarkerRow`,
...
[seen] stamps a column of three cells from a base pointer stepping by a stride — the top cell an
...
[seen] picks the base: player 1 → `PLAYER1_STATUS_VRAM` (`0x5340`), else `PLAYER2_STATUS_VRAM`
(`0x50e0`), both [seen]. `repaintPlayerStatusColumn` [seen] drives the whole thing off a flags byte:
...
`repaintPlayerStatusColumnIfSaved` [seen] repaints only when `loc_40ab` is already nonzero, and
...
`MESSAGE_PTR_TABLE` (`0x235c`) [seen] as an array of 2-byte record pointers; each record begins with a
...
both [seen], derives a per-column delay/cursor pointer into the table at `loc_4020` [guess] and stores
it in `MESSAGE_CURSOR_PTR` (`0x40b1`) [seen], clears the whole column to the clear tile, seeds the
...
Static (non-scrolling) text uses a separate descriptor path. `drawTextColumn` [seen] is the inner
...
negative stride walks up a column). `drawTextColumnFromDescriptor` [seen] unpacks a five-byte
descriptor — source word, destination word, count byte — and paints that many characters up the
column, and `drawTextColumnByIndex` [seen] indexes the ROM descriptor table `TEXT_DESCRIPTOR_TABLE`
(`0x1cf6`) [seen] by a small index to pick which descriptor to paint.
...
as it lays out a title screen; `postCreditAndMessageDrawsAndAdvance` [seen] queues a credit-line
...
the input ports — bits 6-7 of `IN1` (`0x6800`) [seen], bits 0-1 and bit 2 of `IN2_PORT` (`0x7000`)
[seen] — into text-descriptor indices and paints a column for each through `drawTextColumnByIndex`.
Then, unless bit 6 of `IN0` (`0x6000`) [seen] is asserted, it seeds the screen-fill state: clears the
mode flag `loc_4006`, sets the dispatch flag `loc_401a` [guess] to 2, seeds the two dwell tiers
`loc_4008`/`loc_4009` [guess], rewinds the VRAM write cursor `VRAM_WRITE_PTR` (`0x400b`) [seen] to
`VRAM_BASE` (`0x5000`) [seen], clears the four-byte lamp/coin latch block, and silences the sound
hardware. `armInputFlagAndDrawInputColumns` [seen] is the caller that first folds the two input bytes
and, if their shared bit 4 is set, raises the companion input flag `loc_41cc` [guess], then falls
through into this init.

The fill itself is an animated tile strip drawn a slice at a time. `advanceScreenFillStrip` [seen]
...
the strip's first half. `drawScreenFillStripFirstHalf` [seen] stamps sixteen two-tile pairs (tiles 48
and 50) forward from the cursor and falls into `drawScreenFillStripSecondHalf` [seen], which stamps
...
sub-counters against `SUBCOUNTER_RELOAD_TABLE` (`0x15e3`) [seen] and raises the launch-trigger flag
`SUBCOUNTER_REFILL_FLAG` (`0x4228`) [seen] on the refills. The related block-fill primitive
`fillMemoryBlock` [seen] — store a byte across a run of cells (a count of zero meaning a full 256) —
...
[seen]. It takes a packed coordinate byte and produces a tilemap-VRAM cell address by
...
`VRAM_BASE` [seen] plus `(high << 8)` plus that low byte. Beyond the address itself the
...
Table lookups elsewhere in the kit go through `fetchIndexedTableByte` [seen], an indexed
...
At the bottom sits `stampTilePair` [seen]: it writes a tile code at the destination and
...
Vertical work uses `drawDoubleHeightTile` [seen], which paints a glyph as a top/bottom
...
The 2x2 block is `drawTileBlock2x2` [seen]: two `stampTilePair` passes with a stride of
...
[seen] is its mirror, growing upward with a -33 step and seeding the upper pair four
codes lower. `drawTileBlock2x2AtDe` [seen] is a pointer-swap adapter: it exchanges the
...
caller path. `drawBottomTilePairRestoreDe` [seen] is the shared tail those writers fall
...
start at 44 (0x2c): `drawFixedTileBlock2x2` [seen] lays a 2x2 block of codes 44..47,
`drawFixedTileBlock2x2Up` [seen] does the upward block from seed 46,
`drawFixedTilePairHorizontal` [code] stamps a horizontal pair from 44, and
`drawFixedTilePairVertical` [seen] stamps the double-height pair from 44.

### Choosing block, indexed block, or glyph

Two selectors decide what a figure looks like. `drawSelectedTileBlockOrFallback` [seen]
reads a *signed* selector: a non-negative value is treated as an index into the ROM
tile-block source table `TILE_BLOCK_TABLE` [seen] — the block code is fetched by index
(`drawIndexedTileBlock` [seen]) and stamped at the pending destination — while a negative
value stamps a fixed fallback block seeded with tile 0xa4 instead.

The higher-level dispatch is `drawTileGlyphOrBlock` [seen], keyed on the mapper's carry
...
`computeTileVariantFromTimer` [seen] folds an input value into a two-bit variant index:
...
`computeTileVariantFromValueAndTimer` [seen] is the front door used by the figure
...
`DISPLAY_LIST_CURSOR` [seen] past them (wrapping back to the list base 0xc0 when it
...
`drawFixedTileFigureAtPackedCoord` [seen] maps its packed coordinate to a cell and stamps
a fixed decorative figure there — a 2x2 block when the coordinate's bit 4 is set, a
vertical tile pair otherwise. `drawAnimatedTileFigureAtPackedCoord` [seen] is its animated
...
its own position. `draw4x4TileForm` [seen] draws a larger indicator at a fixed 4x4-cell
...
block. Its blanking helpers `blankTileBlock4x4` [seen] (four rows of four cells of the
blank tile 0x40) and `blank4x4AndDraw2x2Icon` [seen] are shared with form 0.
...
other phase it forms a pointer into the object grid `OBJECT_GRID_BASE` [seen] offset by the
...
`drawTileColumnTriple` [seen] copies three source bytes up a single VRAM column — each
...
[seen] is its eraser twin, stamping the blank tile 16 into three cells per column (stepping
-32 per row, +98 per column) from `loc_5193`.

`redrawTileColumnsPeriodically` [seen] schedules those two against the frame phase. It does
nothing unless at least two columns are queued in `DRAWN_COLUMN_COUNT` [seen]. Then, keyed
...
row picked by the frame counter's top two bits out of `TILE_COLUMN_TABLE` [seen], and the
remaining columns stream consecutively from `TILE_COLUMN_TABLE_CONT` [seen]. This is the
...
is `fillMemoryBlock` [seen], a memory fill that stores a byte into a count of cells from a
pointer (a count of zero wrapping to a full 256). The bulk-fill sequence states each carry a
live write cursor in `VRAM_WRITE_PTR` [seen] and walk it across the 0x5000..0x53ff character
...
operations rely on;
`blankVramRowThenResetSpriteState` [seen], `blankVramRowsThenDriveStartLamps` [seen], and
`blankScreenRowsThenAdvanceSequence` [seen] are the same row-blank pattern for their
respective sequence and play states.

The animated diagonal-fill effect is a three-piece chain. `advanceScreenFillStrip` [seen] is
the head: it pets the watchdog (a read of `WATCHDOG_RESET`, the read side of 0x7800), and if the strip gate `loc_4008`
has run out it restarts the outer fill dwell, otherwise it loads the write cursor and draws
the strip's first half. `drawScreenFillStripFirstHalf` [seen] stamps sixteen two-tile pairs
(codes 48 and 50) forward from the cursor and falls straight into
`drawScreenFillStripSecondHalf` [seen], which stamps sixteen more pairs (codes 52 and 54),
...
The block primitives are also what draws the HUD's marker row: `drawMarkerRow` [seen]
repaints the five-slot marker row at `MARKER_ROW_VRAM` [seen] by stamping marker tiles
...
`advanceAndRenderProjectiles` [seen] both integrates and draws the moving projectiles. It
walks seven ten-byte records from the projectile / enemy-shot table `loc_4260`, each holding two five-byte sub-slots; the low
...
`flagProjectileHitsOnPlayer` [seen] runs only when the object subsystem flag
`OBJ_ACTIVE_FLAG` [seen] bit 0 is set, and then sweeps all fourteen sub-slot entries (base
`loc_4260`, stride 5 — the same 70 bytes the renderer sees as seven ten-byte records)
through `flagProjectileHitOnPlayer` [seen]. That per-entry test box-tests an active entry's
X against the player-X reference `loc_4202` and its Y across a near/far band, and on overlap
deactivates the entry and raises `HIT_EVENT_FLAG` [seen] for the hit handler to consume.

## Open questions

- The idiomatic layer is complete: `idiomatic_gate` reports 0 cruft — every reachable routine runs as
  born-live JS with no Z80-level register or raw-address primitives, and the vblank-NMI heartbeat
  (enterVblankService) plus the free-running main loop (mainLoop) drive the frame. The one routine still
  `[code]` is `drawFixedTilePairHorizontal` (0x259e), a tile painter that is unreachable on a good ROM
  (accounted for in `grounding-debt.txt`, R38-confirmed).
- Some work-RAM cells are still named `loc_<addr>`: their role is understood from the routines that touch them
  (the credit count 0x4002, the player-X reference 0x4202, the formation anchor 0x420e, the command-queue
  write head 0x40a0, the projectile table 0x4260), but a descriptive identifier is a follow-up quality item —
  the naming gate governs routines, and these cells are not yet promoted.
