 ...
 * dispatch state (record+2) or scans the row table and hands off to spawnAimedProjectileAtPlayer. Every effect lands in work
 ...
 * D active + delayed-event clear + row match -> aimObjectAtTarget octant then spawnAimedProjectileAtPlayer handoff; E delayed-event
 * set -> aimObjectAtTarget octant then ret. Teeth: no-op, wrong state delta, wrong Y, reset curve, undo handoff,
...
import { advanceObjectFlightAndFire as cand } from "../advanceObjectFlightAndFire.js";
...
const TARGET_X = 0x4202;   // target X anchor read by aimObjectAtTarget
const SLOT0 = 0x4260;      // first entry of the 14-slot spawnAimedProjectileAtPlayer fill table
...
// D: active + delayed-event clear + immediate row match (H == pos 0x11) -> octant then spawnAimedProjectileAtPlayer fills slot 0.
const pathD = () => craft((mem, mm) => {
  ...
  mem[SLOT0] = 0x00;                          // slot 0 free -> spawnAimedProjectileAtPlayer fills it
});
...
test("EQUAL (crafted): loc_0e2b == oracle, active-flag gate -> early ret", { skip }, () => {
  ...
  assert.equal(a.mem8[OCTANT], 0xaa, "positive control: octant NOT written (aimObjectAtTarget skipped)");
});

test("EQUAL (crafted): loc_0e2b == oracle, row match -> octant + spawnAimedProjectileAtPlayer handoff", { skip }, () => {
  assert.equal(ramDiff(oracle, cand, pathD()), null, "loc_0e2b diverged on the row-match handoff");
  const a = pathD(); oracle(a);
  assert.notEqual(a.mem8[OCTANT], 0xaa, "positive control: octant written (aimObjectAtTarget ran)");
  assert.equal(a.mem8[SLOT0], 0x01, "positive control: spawnAimedProjectileAtPlayer marked slot 0 active");
  assert.equal(a.mem8[SLOT0 + 1], 0x11, "positive control: spawnAimedProjectileAtPlayer stored the object counter");
});
...
test("EQUAL (crafted): loc_0e2b == oracle, row match after one stride -> spawnAimedProjectileAtPlayer handoff", { skip }, () => {
  assert.equal(ramDiff(oracle, cand, pathF()), null, "loc_0e2b diverged on the multi-stride row scan");
  const a = pathF(); oracle(a);
  assert.equal(a.mem8[SLOT0], 0x01, "positive control: stride reached the match -> spawnAimedProjectileAtPlayer filled slot 0");
});
...
test("TEETH: broken twins are caught", { skip }, () => {
  ...
  assert.ok(ramDiff(oracle, undoHandoff, pathD()), "undo-handoff twin escaped (spawnAimedProjectileAtPlayer skipped)");
