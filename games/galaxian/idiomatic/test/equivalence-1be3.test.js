...
/**
 * fillObjRamTestRamp — memory-equivalent to the frozen self-test OBJRAM ramp fill at ROM 0x1be3. It lays a stepped
 * colour ramp (seed +0x2f per cell) across the 256-byte OBJRAM page, then reloads the seed from RNG_SEED and
 * falls into the ramp scan (scanObjRamTestRampAndFinishSelfTest). A crafted entry seats HL = OBJRAM and A = seed (as dispatchSelfTestMode hands over)
...
import { fillObjRamTestRamp as cand } from "../fillObjRamTestRamp.js";
...
function entry(seed, passes) {
  return craft((mem8, e) => {
    e.push16(0x9999);
    mem8[RNG_SEED] = seed;   // the scan seed fillObjRamTestRamp reloads before falling into the scan
...
test("EQUAL: fillObjRamTestRamp == oracle across seeds and pass counts", { skip }, () => {
  ...
  console.log("  EQUAL: fillObjRamTestRamp == oracle (fill + fall-through scan)");
});
