 ...
 * otherwise, gated by the active flag, it computes the octant (aimObjectAtTarget) and scans the row table, handing
 * off to spawnAimedProjectileAtPlayer on a match. Every effect lands in work RAM, so ramDiff covers the live-out; the handler
 ...
 * -> aimObjectAtTarget octant + spawnAimedProjectileAtPlayer handoff; FF mode==4 parity-clear no-chase then active-flag gate -> ret.
...
import { advanceHomingObjectFlightAndFire as cand } from "../advanceHomingObjectFlightAndFire.js";
...
const TARGET_X = 0x4202; // target column (chase target) + aimObjectAtTarget X anchor
...
//     delayed-event clear + immediate row match (H == pos 0x11) -> octant then spawnAimedProjectileAtPlayer fills slot 0.
...
test("EQUAL (crafted): loc_0faf == oracle, row match after one stride -> spawnAimedProjectileAtPlayer handoff", { skip }, () => {
...
test("TEETH: broken twins are caught", { skip }, () => {
  ...
  assert.ok(ramDiff(oracle, undoHandoff, pathE()), "undo-handoff twin escaped (spawnAimedProjectileAtPlayer skipped)");
