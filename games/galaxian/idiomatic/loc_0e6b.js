...
// then window-check it. Outside the window -> bump the state index. Inside, advance the flight curve and
...
const OBJ_STATE = 0x02;   // state index (bumped when the object leaves the window)
...
export function loc_0e6b(m, obj = m.regs.ix) {
  const { mem8 } = m;
  ...
  // Outside the [6, 9) window -> bump the state index and stop.
  if (((pos - 6) & 0xff) < 3) return bumpState();

  // Inside: advance the flight curve, then fold heading + increment into a new Y.
  ...
  // A carry that crosses the signed boundary means the fold left the window: bump state; else store Y.
