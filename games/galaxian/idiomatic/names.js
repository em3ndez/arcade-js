...
export const ROUTINES = {
  ...
  0x1d51: { name: "restartScreenFillOnDwellExpiry", role: "[seen] Screen-fill outer-dwell tick: entered from the tile-strip fill updater (loc_1d28 gate-zero path / drawScreenFillStripSecondHalf fall-through, HL=0x4008), bump to the high-tier dwell byte 0x4009 and tick it; if already 0 or on the zero-crossing, restart the fill via resetScreenFillState (0x1d58).", cert: "seen" },
  ...
  0x1801: { name: "stageSoundPitchBySelector", role: "[code] Pitch-selector dispatcher, tail-called from the sound-counter managers loc_17e5/tickSoundCounterAndStagePitch: reads (HL)&0x03; a nonzero selector -> stagePitchFromSoundCounter(selector) (warble recomputed from sound counter 0x41c4), zero -> stageSoundPitch(96) (fixed pitch). Both delegates stage SOUND_PITCH (0x41c1).", cert: "code" },
  0x194f: { name: "incrementCreditCount", role: "[code] Step the credit count at (HL) (both callers loc_1931/awardCreditEverySecondCoin pass 0x4002) toward the 99 ceiling: ==99 return; >99 clampCreditsToMax (pin to 99); else increment, set ready flag 0x41c9=1 (arms driveRisingPitchRamp), and enqueue command word (7,1). 0x4002 as credit count is corroborated by clampCreditsToMax/driveStartButtonLamps/advanceGameStateOnCredit.", cert: "code" },
  ...
  0x05e2: { name: "enqueueCommandWordBurst", role: "[code] Post a five-word burst into the command queue via enqueueCommandWord: (D,2),(D+1,2),(D+1,4),(7,3),(7,0), each word (channel<<8)|param with D=caller's channel. Tail of loc_05a5 (falls through after 0x0500) and entered from queueBoardStartSoundBurst.", cert: "code" },
  ...
  0x04f2: { name: "startOnePlayerGame", role: "[code] One-player game-start arm (loc_0492 bit0 path): spend one credit (0x4002--), zero the saved-state snapshot 0x41a0, and enter play via startGameRoundAndClearScores with spawn word 0 (GAME_STATE=3, single player); no credit -> force GAME_STATE=1. Own write is only the credit spend (0x4002, itself [code]); the start is delegated, so not write-grounded", cert: "code" },
  0x05fc: { name: "queueBoardStartSoundBurst", role: "[code] Board/level-start sound cue, paired-player variant (sole caller loc_05a5 when sound gate 0x4006 bit0 and paired-player flag 0x400e bit0 are set): enqueue a prologue command word (channel 5, param 3) then the standard channel 5/6/7 burst via enqueueCommandWordBurst", cert: "code" },
