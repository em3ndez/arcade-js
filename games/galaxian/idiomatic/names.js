...
export const ROUTINES = {
  ...
  0x01be: { name: "armStepCountdownAndTickSequenceTimer", role: "[seen] loc_0156 sub-state 1: arm countdown cell 0x4019=1, then tick the prescaled sequence timer (tickPrescaledSequenceTimer). 0x4019 is decremented by loc_0430, which advances SEQUENCE_STATE and clears the 0x4100 flag block on its expiry", cert: "seen" },
  ...
  0x13e1: { name: "chooseNextAttackerDirection", role: "[code] Sets the shared attacker launch/curve direction flag loc_4215 (0 or 1): the 16-bit formation anchor loc_420e's sign picks which FORMATION_X_BOUNDS (0x4210) edge to measure the 8-bit gap against; within 28px of that edge the flag is forced away from the edge (0 near the high bound, 1 near the low bound), otherwise a fresh random bit (advanceRandomSeed & 1). Consumed by the spawner loc_1344 (seeds a new attacker's direction field record+6) and loc_140c (left/right column scan).", cert: "code" },
  0x1515: { name: "paceEnemyLaunchTrigger", role: "[code] Gated prescaler that ticks a master counter (0x424a) and, on expiry, sweeps a difficulty-scaled span of attacker sub-counters, refilling each expired one and raising the spawn-trigger flag SUBCOUNTER_REFILL_FLAG (0x4228) that loc_1344 consumes to launch a new attacker.", cert: "code" },
  ...
  0x1d51: { name: "restartScreenFillOnDwellExpiry", role: "[seen] Screen-fill outer-dwell tick: entered from the tile-strip fill updater (loc_1d28 gate-zero path / loc_1d43 fall-through, HL=0x4008), bump to the high-tier dwell byte 0x4009 and tick it; if already 0 or on the zero-crossing, restart the fill via resetScreenFillState (0x1d58).", cert: "seen" },
