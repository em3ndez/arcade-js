...
export const ROUTINES = {
  ...
  0x145c: { name: "activateObjectSlotAndEnqueueSpawn", role: "[code] Activate a claimed free object slot: consume the trigger flag it was found under (write 0 at HL), mark the slot at IX active (byte0=1, phase byte2=0), record the spawn code (C -> byte6) and the trigger pointer's low byte as the source index (byte7), then enqueue a type-1 spawn command word keyed by that source index; returns the trigger pointer unchanged for the caller. Tail/called from the free-slot finders loc_1446, loc_1472, loc_14be (IX=0x42d0).", cert: "code" },
  ...
  0x2591: { name: "drawFixedTileBlock2x2Up", role: "[code] Draw a 2x2 tile block upward from the pointer using the fixed decorative tile seed 46 (drawTileBlock2x2Up); used by loc_22b3 to blank unused marker-row slots.", cert: "code" },
  0x0722: { name: "advanceDwellOrResetToState1", role: "[code] Shared tail of state-6 handlers loc_06d8/loc_07e8: when mode flag 0x4006 bit0 is clear, advanceSubstateAndReloadDwell (advance the sub-state counter at HL, reload the dwell timer); when set, reset to state 1 -- GAME_STATE (0x4005)=1, clear 0x4006 and SEQUENCE_STATE (0x400a), silenceSoundAndDisableIrqStars, then enqueue command word 6.", cert: "code" },
  ...
  0x1801: { name: "stageSoundPitchBySelector", role: "[code] Pitch-selector dispatcher, tail-called from the sound-counter managers loc_17e5/loc_17f9: reads (HL)&0x03; a nonzero selector -> stagePitchFromSoundCounter(selector) (warble recomputed from sound counter 0x41c4), zero -> stageSoundPitch(96) (fixed pitch). Both delegates stage SOUND_PITCH (0x41c1).", cert: "code" },
  0x194f: { name: "incrementCreditCount", role: "[code] Step the credit count at (HL) (both callers loc_1931/loc_1964 pass 0x4002) toward the 99 ceiling: ==99 return; >99 clampCreditsToMax (pin to 99); else increment, set ready flag 0x41c9=1 (arms driveRisingPitchRamp), and enqueue command word (7,1). 0x4002 as credit count is corroborated by clampCreditsToMax/driveStartButtonLamps/advanceGameStateOnCredit.", cert: "code" },
  ...
  0x1d43: { name: "drawScreenFillStripSecondHalf", role: "[code] Second half of the tile-strip fill (fallen into from loc_1d39, which stamps tile pair 0x30/0x32 and sets B=0x10): stamps B pairs of tiles 52/54 forward from the cursor (HL, two cells per pass, count 0 wraps to 256), saves the advanced cursor to VRAM_WRITE_PTR (0x400b), ticks strip dwell 0x4008; on its expiry restartScreenFillOnDwellExpiry(0x4008).", cert: "code" },
  ...
  0x05e2: { name: "enqueueCommandWordBurst", role: "[code] Post a five-word burst into the command queue via enqueueCommandWord: (D,2),(D+1,2),(D+1,4),(7,3),(7,0), each word (channel<<8)|param with D=caller's channel. Tail of loc_05a5 (falls through after 0x0500) and entered from loc_05fc.", cert: "code" },
  0x149b: { name: "spawnSecondaryObjectIntoSlot", role: "[code] Called per-slot from loc_148e's spawn walk (IY stepped by 0x20): if neither of the IY slot's two live flags (byte0 bit0 / byte1 bit0) is set, consume the trigger flag at HL (set 0), mark the slot alive (byte0=1), clear its state (byte2=0), inherit field 6 from the IX source record, stash the trigger index (byte7), and enqueue activation command (1<<8)|index.", cert: "code" },
  ...
  0x191e: { name: "addCreditForCoin", role: "[code] Coin-service credit bump (loc_18ef bit7 branch): if credit count 0x4002 < 99 increment it, raise event flag 0x41c9=1, and tail-enqueue a channel-7 command that redraws the credit-count HUD (dispatch 0x24b7 arg 1 renders 0x4002 capped at 99 as two BCD digits); at the 99 cap do nothing.", cert: "code" },
  ...
  0x04bc: { name: "startGameRoundAndClearScores", role: "[code] Game/round-start entry (fall-through from loc_0492, jp from loc_04f2): store the player-count/spawn word in CURRENT_PLAYER(0x400d), blit the 32-byte ROM row template 0x051b into PACKED_FLAG_BITMAP(0x4180), arm the substate-advance gate when 0x401f bit0 is set, enter play (GAME_STATE=3, SEQUENCE_STATE=0, 0x4006=1, 0x41d1=1), and queue a start message (ch6) plus two channel-4 score-slot clears.", cert: "code" },
