...
>>> binary 0000:roms/136002-133.d1 + roms/136002-134.f1 + roms/136002-235.j1 + roms/136002-136.lm1 + roms/136002-237.p1
...
loc_9254:
9254: AD CA 60        LDA     $60CA               ; take a 4-bit POKEY random for this slot
9257: 29 0F           AND     #$0F                
...
92CB: AD DA 60        LDA     $60DA               ; substitute a POKEY random key when off the top
92CE: 29 1F           AND     #$1F                
92D0: 09 40           ORA     #$40                
...
9967: AD CA 60        LDA     $60CA               ; ...replaced by a random even lane so spawns spread across the rim
...
loc_9a61:
9A61: AD DA 60        LDA     $60DA               ; POKEY-random start column for the round-robin sweep
9A64: 29 03           AND     #$03                
9A66: AA              TAX                         
...
9ABB: AD CA 60        LDA     $60CA               ; POKEY-random start lane
...
9F66: AD DA 60        LDA     $60DA               ; draw a fresh secondary-RNG byte
...
9F8F: 2C CA 60        BIT     $60CA               ; test a primary-RNG bit (bit6)
...
A031: AD DA 60        LDA     $60DA               ; draw a secondary-RNG start
...
A2CC: AD CA 60        LDA     $60CA               ; read the RNG
...
A31A: AD DA 60        LDA     $60DA               ; draw a random variant seed
A31D: 29 07           AND     #$07                
...
A66C: AD DA 60        LDA     $60DA               ; seed axis-1 velocity low from a random source
...
A678: AD CA 60        LDA     $60CA               ; seed axis-0 velocity low from a random source
...
A68B: AD CA 60        LDA     $60CA               ; seed axis-2 velocity low from a random source
...
A69C: AD DA 60        LDA     $60DA               ; take a 3-bit random magnitude, 0..7
A69F: 29 07           AND     #$07                
...
A8DE: AD E4 31        LDA     $31E4               ; read the glyph snapshot
A8E1: 8D A6 2F        STA     $2FA6               ; mirror it into the first snapshot cell
A8E4: 8D A8 2F        STA     $2FA8               ; and the second snapshot cell
...
A936: A8              TAY                         
A937: B9 FA 31        LDA     $31FA,Y             ; read the glyph word
A93A: 9D 60 2F        STA     $2F60,X             ; copy it into the strided mirror
A93D: E8              INX                         
A93E: E8              INX                         
...
A994: 9D 60 2F        STA     $2F60,X             ; write the head word into the glyph buffer
...
loc_a9a9:
A9A9: AD 84 32        LDA     $3284               ; filled-tick glyph
...
A9B2: AD 86 32        LDA     $3286               ; past the count -- the empty-tick glyph

loc_a9b5:
A9B5: 9D 60 2F        STA     $2F60,X             ; write the glyph into the buffer
...
AA08: A8              TAY                         
AA09: B9 E4 31        LDA     $31E4,Y             ; fetch the digit's stroke byte from the glyph table
AA0C: 9D 60 2F        STA     $2F60,X             ; store it into the text buffer at the cursor
...
AB7B: AA              TAX                         
AB7C: BD E4 31        LDA     $31E4,X             ; fetch the point's low byte from the coordinate table
AB7F: A4 2A           LDY     $2A                 ; {hard.workRam+2A}
AB81: 91 74           STA     ($74),Y             ; {hard.workRam+74} write the point low byte into the display buffer
AB83: C8              INY                         
AB84: BD E5 31        LDA     $31E5,X             ; fetch the point's high byte
...
AE20: AD CA 60        LDA     $60CA               ; sample the first POKEY random register
AE23: AC CA 60        LDY     $60CA               
...
AE30: AD DA 60        LDA     $60DA               ; sample the second POKEY random register
AE33: AC DA 60        LDY     $60DA               
...
AF0F: AA              TAX                         
AF10: BD FA 31        LDA     $31FA,X             ; copy the glyph's low byte into the display list
AF13: 91 74           STA     ($74),Y             ; {hard.workRam+74}
AF15: C8              INY                         
AF16: BD FB 31        LDA     $31FB,X             ; copy the glyph's high byte
...
B1B9: AD 00 20        LDA     $2000               ; read the display-list header guard word
...
B1FD: 8D 00 20        STA     $2000               ; store it into the first display word
B200: AD C5 CE        LDA     $CEC5               ; {hard.rom+3EC5} latch the play-mode header high byte
B203: 8D 01 20        STA     $2001               ; store it into the second display word
B206: B8              CLV                         
...
B2B4: 8D 00 20        STA     $2000               ; store it into the first display word
B2B7: AD C3 CE        LDA     $CEC3               ; {hard.rom+3EC3} latch the per-frame header high byte
B2BA: 8D 01 20        STA     $2001               ; store it into the second display word
B2BD: 60              RTS                         
...
B335: CD 00 20        CMP     $2000               ; compare it against the checkpoint copy
B338: F0 05           BEQ     $B33F               ; {code.loc_b33f} unchanged: go emit the frame link
B33A: 8D 00 20        STA     $2000               ; source moved mid-frame: re-latch the checkpoint
...
loc_b796:
B796: 8C 08 08        STY     $0808               ; latch the per-level rim colour into colour RAM
B799: 60              RTS                         
...
B87E: 98              TYA                         
B87F: 9D 09 08        STA     $0809,X             ; mirror it into visible colour RAM
B882: 68              PLA                         
...
B899: 8D FC 2F        STA     $2FFC               ; jump target low = the current tail cursor
B89C: AD 3A 01        LDA     $013A               ; {hard.workRam+13A}
B89F: 09 70           ORA     #$70                ; tag with the vector-generator jump opcode bits
B8A1: 8D FD 2F        STA     $2FFD               ; jump target high
B8A4: A9 C0           LDA     #$C0                
B8A6: 8D FF 2F        STA     $2FFF               ; halt word that ends the beam scan
...
BD47: 8D 95 60        STA     $6095               ; into the math-box operand low
BD4A: A9 00           LDA     #$00                
BD4C: E5 5B           SBC     $5B                 ; {hard.workRam+5B}
BD4E: 8D 96 60        STA     $6096               ; math-box operand high
BD51: A9 18           LDA     #$18                
BD53: 8D 8C 60        STA     $608C               ; iteration count
BD56: A5 A0           LDA     $A0                 ; {hard.workRam+A0}
BD58: 8D 8E 60        STA     $608E               ; divisor
BD5B: 8D 94 60        STA     $6094               ; issue the divide

loc_bd5e:
BD5E: 2C 40 60        BIT     $6040               ; poll the math-box busy bit
BD61: 30 FB           BMI     $BD5E               ; {code.loc_bd5e} spin until the divide is done
BD63: AD 60 60        LDA     $6060               ; read the result low
BD66: 85 79           STA     $79                 ; {hard.workRam+79}
BD68: AD 70 60        LDA     $6070               ; read the result high
BD6B: 85 7A           STA     $7A                 ; {hard.workRam+7A}
BD6D: A2 0F           LDX     #$0F                
BD6F: 8E 8C 60        STX     $608C               ; reload the iteration count
BD72: 38              SEC                         
BD73: E9 01           SBC     #$01                
...
C09D: 8D 95 60        STA     $6095               ; feed the math-box divisor low byte
C0A0: A9 00           LDA     #$00                
C0A2: E5 5B           SBC     $5B                 ; {hard.workRam+5B} borrow through the depth guard byte
C0A4: 8D 96 60        STA     $6096               ; feed the math-box divisor high byte
C0A7: 10 0A           BPL     $C0B3               ; {code.loc_c0b3} skip the clamp when the divisor stayed non-negative
C0A9: A9 00           LDA     #$00                
C0AB: 8D 96 60        STA     $6096               ; clamp the divisor high byte to zero
C0AE: A9 01           LDA     #$01                
C0B0: 8D 95 60        STA     $6095               ; floor the divisor low byte at 1 -- never divide by zero
...
loc_c0c7:
C0C7: 8D 8E 60        STA     $608E               ; feed the X magnitude as the math-box operand
C0CA: 8D 94 60        STA     $6094               ; trigger the divide for the X projection
...
loc_c0e7:
C0E7: 2C 40 60        BIT     $6040               ; read the math-box busy status
C0EA: 30 FB           BMI     $C0E7               ; {code.loc_c0e7} spin while the math box is busy
C0EC: AD 60 60        LDA     $6060               ; read the math-box result low byte
C0EF: 85 63           STA     $63                 ; {hard.workRam+63} into the projected X accumulator low
C0F1: AD 70 60        LDA     $6070               ; read the math-box result high byte
...
C0F8: 8D 8E 60        STA     $608E               ; feed it as the math-box operand
C0FB: 8D 94 60        STA     $6094               ; trigger the divide for the Y projection
...
loc_c12f:
C12F: 2C 40 60        BIT     $6040               ; read the math-box busy status
C132: 30 FB           BMI     $C12F               ; {code.loc_c12f} spin while the math box is busy
C134: AD 60 60        LDA     $6060               ; read the math-box result low byte
C137: 85 61           STA     $61                 ; {hard.workRam+61} into the projected Y accumulator low
C139: AD 70 60        LDA     $6070               ; read the math-box result high byte
...
C182: 8D 00 58        STA     $5800               ; strobe the vector-generator reset when the trigger was already clear
...
C18D: 8D 00 20        STA     $2000               ; latch it into the vector-list header low
C190: AD C7 CE        LDA     $CEC7               ; {hard.rom+3EC7} load the second head word
C193: 8D 01 20        STA     $2001               ; latch it into the vector-list header high
...
C1AE: 99 00 08        STA     $0800,Y             ; mirror it into color RAM
...
C1BB: 99 08 08        STA     $0808,Y             ; mirror it into color RAM
C1BE: CA              DEX                         
C1BF: 88              DEY                         
...
C1D1: A9 00           LDA     #$00                
C1D3: 8D 80 60        STA     $6080               ; zero the math-box coprocessor input ports
C1D6: 8D 81 60        STA     $6081               
C1D9: 8D 84 60        STA     $6084               
C1DC: 8D 85 60        STA     $6085               
C1DF: 8D 86 60        STA     $6086               
C1E2: 8D 87 60        STA     $6087               
C1E5: 8D 89 60        STA     $6089               
C1E8: 8D 83 60        STA     $6083               
C1EB: 8D 8D 60        STA     $608D               
C1EE: 8D 8E 60        STA     $608E               
C1F1: 8D 8F 60        STA     $608F               
C1F4: 8D 90 60        STA     $6090               
C1F7: A9 0F           LDA     #$0F                
C1F9: 8D 8C 60        STA     $608C               ; load step count 0x0f to arm the math box
C1FC: 60              RTS                         
...
C2EE: AD CA 60        LDA     $60CA               ; otherwise pull the POKEY random byte
...
C705: AD CA 60        LDA     $60CA               ; read a bit of the sound chip's random register
C708: 29 02           AND     #$02                
C70A: 18              CLC                         
...
C72C: C8              INY                         
C72D: AD B2 3D        LDA     $3DB2               ; write the blank-slot vector low byte
C730: 91 74           STA     ($74),Y             ; {hard.workRam+74}
C732: C8              INY                         
C733: AD B3 3D        LDA     $3DB3               ; write the blank-slot vector high byte
...
C7BD: AD 00 0D        LDA     $0D00               ; coinage dip config 0x82 disables this whole update pass
C7C0: 29 83           AND     #$83                
C7C2: C9 82           CMP     #$82                
...
C891: AD 00 0C        LDA     $0C00               ; coin input bit4 clear -> force game mode 0x22
C894: 29 10           AND     #$10                
...
CD85: 9D C8 60        STA     $60C8,X             ; publish the level to the second sound chip's voice register
CD88: B8              CLV                         
CD89: 50 03           BVC     $CD8E               ; {code.loc_cd8e}

loc_cd8b:
CD8B: 9D C0 60        STA     $60C0,X             ; publish the level to the first sound chip's voice register
...
resetBothPokeyChips:
CD95: A9 00           LDA     #$00                
CD97: 8D CF 60        STA     $60CF               ; hold the first sound chip in serial reset (control latch 0)
CD9A: 8D DF 60        STA     $60DF               ; hold the second sound chip in serial reset
...
CDA2: AD CA 60        LDA     $60CA               ; snapshot the first chip's free-running random register
CDA5: AC DA 60        LDY     $60DA               ; snapshot the second chip's random register

loc_cda8:
CDA8: CD CA 60        CMP     $60CA               ; has the first chip's random register advanced
CDAB: D0 03           BNE     $CDB0               ; {code.loc_cdb0}
CDAD: CC DA 60        CPY     $60DA               ; has the second chip's advanced
...
CDBC: 8D CF 60        STA     $60CF               ; release the first chip (control latch 7, the init handshake)
CDBF: 8D DF 60        STA     $60DF               ; release the second chip
CDC2: A2 07           LDX     #$07                ; walk the eight audio registers of each chip
CDC4: A9 00           LDA     #$00                

loc_cdc6:
CDC6: 9D C0 60        STA     $60C0,X             ; silence a first-chip audio register
CDC9: 9D D0 60        STA     $60D0,X             ; silence a second-chip audio register
...
CDD3: A9 00           LDA     #$00                
CDD5: 8D C8 60        STA     $60C8               ; clear the first chip's control register
CDD8: A9 00           LDA     #$00                
CDDA: 8D D8 60        STA     $60D8               ; clear the second chip's control register
CDDD: 60              RTS                         
...
D6BB: AD 00 0E        LDA     $0E00               ; read the options DIP-switch port
...
D6CC: AD 00 0D        LDA     $0D00               ; read the coinage DIP-switch port
...
loc_d717:
D717: 8D 00 50        STA     $5000               ; kick the watchdog and acknowledge the interrupt
D71A: 8D CB 60        STA     $60CB               ; pulse the first chip's pot-scan start
D71D: AD C8 60        LDA     $60C8               ; read the spinner pot
...
D73B: 8D DB 60        STA     $60DB               ; mirror the accumulator to the second chip's pot register
D73E: AC D8 60        LDY     $60D8               ; read the coin/switch input
D741: AD 00 0C        LDA     $0C00               ; latch the raw input port
...
loc_d777:
D777: 8D 00 40        STA     $4000               ; write the coin-counter / screen-flip output latch
...
D79C: 8D E0 60        STA     $60E0               ; mirror the draw mode to the LED / screen-flip latch
...
loc_d7c9:
D7C9: 2C 00 0C        BIT     $0C00               ; is the vector-generator-done input asserted
D7CC: 50 09           BVC     $D7D7               ; {code.loc_d7d7}
D7CE: EE 33 01        INC     $0133               ; {hard.workRam+133} bump the redraw counter
D7D1: 8D 00 58        STA     $5800               ; strobe the vector-generator reset
D7D4: 8D 00 48        STA     $4800               ; strobe the vector-generator go -- launch the redraw
...
D7EE: AD 00 0C        LDA     $0C00               ; read the feature-enable input
D7F1: 29 10           AND     #$10                
...
D82F: 0A              ASL     A                   
D830: A8              TAY                         
D831: B9 1F 3F        LDA     $3F1F,Y             ; read the difficulty coordinate pair (high byte)
D834: BE 1E 3F        LDX     $3F1E,Y             ; read the difficulty coordinate pair (low byte)
...
D845: 48              PHA                         
D846: A8              TAY                         
D847: B9 17 3F        LDA     $3F17,Y             ; read the segment coordinate pair (high byte)
D84A: BE 16 3F        LDX     $3F16,Y             ; read the segment coordinate pair (low byte)
...
loc_d8da:
D8DA: A9 A2           LDA     #$A2                
D8DC: 8D C1 60        STA     $60C1               ; set voice-1 control -- tone on
D8DF: BA              TSX                         
...
loc_d8ed:
D8ED: 8D C0 60        STA     $60C0               ; write voice-1 frequency -- the tone
D8F0: A9 03           LDA     #$03                
D8F2: 8D E0 60        STA     $60E0               ; strobe the LED / screen-flip latch
D8F5: A2 00           LDX     #$00                

loc_d8f7:
D8F7: 2C 00 0C        BIT     $0C00               ; sync to the 3kHz line
D8FA: 30 FB           BMI     $D8F7               ; {code.loc_d8f7}

loc_d8fc:
D8FC: 2C 00 0C        BIT     $0C00               
D8FF: 10 FB           BPL     $D8FC               ; {code.loc_d8fc}
D901: 8D 00 50        STA     $5000               ; kick the watchdog through the tone drain
D904: CA              DEX                         
D905: D0 F0           BNE     $D8F7               ; {code.loc_d8f7}
D907: 88              DEY                         
D908: D0 ED           BNE     $D8F7               ; {code.loc_d8f7}
D90A: 8E C1 60        STX     $60C1               ; silence voice-1
D90D: A9 00           LDA     #$00                
D90F: 8D E0 60        STA     $60E0               ; clear the LED / screen-flip latch
D912: A0 09           LDY     #$09                

loc_d914:
D914: 2C 00 0C        BIT     $0C00               
D917: 30 FB           BMI     $D914               ; {code.loc_d914}

loc_d919:
D919: 2C 00 0C        BIT     $0C00               
D91C: 10 FB           BPL     $D919               ; {code.loc_d919}
D91E: 8D 00 50        STA     $5000               ; kick the watchdog through the silent gap
D921: CA              DEX                         
...
D940: 8D 00 50        STA     $5000               ; kick the watchdog
D943: 8D 00 58        STA     $5800               ; strobe the vector-generator reset
D946: A2 FF           LDX     #$FF                
...
D961: 8D 00 50        STA     $5000               ; kick the watchdog during the wipe
D964: 90 E7           BCC     $D94D               ; {code.loc_d94d} continue the wipe
D966: 85 01           STA     $01                 ; {hard.workRam+1}
D968: 8D E0 60        STA     $60E0               ; clear the LED / screen-flip latch
D96B: 8D CF 60        STA     $60CF               ; hold the first sound chip in reset
D96E: 8D DF 60        STA     $60DF               ; hold the second sound chip in reset
D971: A2 07           LDX     #$07                
D973: 8E CF 60        STX     $60CF               ; release the first sound chip (control latch 7)
D976: 8E DF 60        STX     $60DF               ; release the second sound chip
D979: E8              INX                         

loc_d97a:
D97A: 9D C0 60        STA     $60C0,X             ; silence a first-chip audio register
D97D: 9D D0 60        STA     $60D0,X             ; silence a second-chip audio register
D980: CA              DEX                         
D981: 10 F7           BPL     $D97A               ; {code.loc_d97a}
D983: AD 00 0C        LDA     $0C00               ; read the self-test switch
D986: 29 10           AND     #$10                
D988: F0 1F           BEQ     $D9A9               ; {code.loc_d9a9} switch held -- run the RAM diagnostic

loc_d98a:
D98A: 8D 00 50        STA     $5000               ; normal boot: kick the watchdog through a settle delay
...
D9BF: BA              TSX                         
D9C0: 8A              TXA                         
D9C1: 8D 00 50        STA     $5000               ; kick the watchdog during the test
D9C4: C8              INY                         
...
D9FC: 8D 00 50        STA     $5000               ; kick the watchdog
D9FF: E8              INX                         
...
DA20: 8D 00 50        STA     $5000               ; kick the watchdog once per page
...
DA41: 8E C5 60        STX     $60C5               ; error-tone voice-3 control

loc_da44:
DA44: A2 05           LDX     #$05                
DA46: AD CA 60        LDA     $60CA               ; sample the first chip's random register

loc_da49:
DA49: CD CA 60        CMP     $60CA               ; re-read until it reads stable
...
loc_da53:
DA53: A2 05           LDX     #$05                
DA55: AD DA 60        LDA     $60DA               ; sample the second chip's random register

loc_da58:
DA58: CD DA 60        CMP     $60DA               ; re-read until stable
...
DA7D: 9D 00 08        STA     $0800,X             ; into colour RAM
DA80: CA              DEX                         
DA81: 10 F7           BPL     $DA7A               ; {code.loc_da7a}
DA83: A9 00           LDA     #$00                
DA85: 8D E0 60        STA     $60E0               ; blank the LED / screen-flip latch
DA88: A9 10           LDA     #$10                
DA8A: 8D 00 40        STA     $4000               ; idle the coin-counter / flip output latch
...
DAB4: 8D CB 60        STA     $60CB               ; pulse the first chip's pot scan
DAB7: AD C8 60        LDA     $60C8               ; read the spinner pot
...
DAC0: AD 00 0C        LDA     $0C00               ; read and invert the input port
DAC3: 49 FF           EOR     #$FF                
...
DAE2: 8D 00 48        STA     $4800               ; strobe the vector-generator go
...
loc_daf0:
DAF0: AD 00 0C        LDA     $0C00               ; read the self-test switch
DAF3: 29 10           AND     #$10                
...
DB22: A9 00           LDA     #$00                
DB24: 8D E0 60        STA     $60E0               ; zero the LED/flip/coin latch
DB27: 8D 80 60        STA     $6080               ; zero the mathbox R0-low load register
DB2A: 8D C0 60        STA     $60C0               ; zero POKEY 1 pitch
DB2D: 8D D0 60        STA     $60D0               ; zero POKEY 2 pitch
DB30: 8D 00 60        STA     $6000               ; zero the EAROM data latch
DB33: 8D 40 60        STA     $6040               ; zero the EAROM control latch (0x6040 write) -- a write here hits EAROM control, not the read-only mathbox status
DB36: AD 40 60        LDA     $6040               ; dummy read to let the mathbox status flip-flop settle
DB39: AD 60 60        LDA     $6060               ; dummy read of the mathbox result low byte
DB3C: AD 70 60        LDA     $6070               ; dummy read of the mathbox result high byte
DB3F: AD 50 60        LDA     $6050               ; dummy read of the EAROM read latch
DB42: A9 08           LDA     #$08                
DB44: 8D E0 60        STA     $60E0               ; raise the LED/flip latch
...
loc_db4c:
DB4C: 9D 80 60        STA     $6080,X             ; write the walking-bit pattern into mathbox load slot x
...
loc_db8f:
DB8F: 9D C1 60        STA     $60C1,X             ; silence POKEY 1 voice control at even slot x
DB92: 9D D1 60        STA     $60D1,X             ; silence POKEY 2 voice control at even slot x
DB95: CA              DEX                         
DB96: CA              DEX                         
...
DBAA: A9 00           LDA     #$00                
DBAC: 99 C1 60        STA     $60C1,Y             ; silence that POKEY 1 voice
...
DBB5: 99 C0 60        STA     $60C0,Y             ; set the fired voice's frequency
DBB8: A9 A8           LDA     #$A8                
DBBA: 99 C1 60        STA     $60C1,Y             ; fire it at fixed volume and distortion
DBBD: A9 34           LDA     #$34                
DBBF: A2 56           LDX     #$56                
...
DBE0: 8D DB 60        STA     $60DB               ; strobe POKEY 2 pot scan with the incoming value
DBE3: AD D8 60        LDA     $60D8               ; read POKEY 2 pot lines (ALLPOT at 0x60d8)
DBE6: 29 07           AND     #$07                ; keep the low three control bits
DBE8: 85 37           STA     $37                 ; {hard.workRam+37} mirror them into scratch
DBEA: 8D CB 60        STA     $60CB               ; and into the POKEY 1 pot-scan strobe
DBED: AD C8 60        LDA     $60C8               ; read POKEY 1 pot lines (ALLPOT at 0x60c8)
...
DBFB: 8D 95 60        STA     $6095               ; seed mathbox R7 low with the counter
DBFE: 8D 8D 60        STA     $608D               ; and mathbox RA low
DC01: A5 2F           LDA     $2F                 ; {hard.workRam+2F} read the sweep counter high byte
DC03: 8D 96 60        STA     $6096               ; seed mathbox R7 high
DC06: A2 00           LDX     #$00                
...
loc_dc27:
DC27: 8D DB 60        STA     $60DB               ; write the decided status byte to the POKEY 2 pot-scan strobe
DC2A: AD D8 60        LDA     $60D8               ; read POKEY 2 pot lines (ALLPOT at 0x60d8)
...
DC33: 8D C0 60        STA     $60C0               ; set POKEY 1 voice 1 frequency
DC36: A2 A4           LDX     #$A4                ; arm voice 1 control

loc_dc38:
DC38: 8E C1 60        STX     $60C1               ; write POKEY 1 voice 1 control
DC3B: A2 00           LDX     #$00                
...
DC41: 0A              ASL     A                   
DC42: 8D C2 60        STA     $60C2               ; set POKEY 1 voice 2 frequency (edge flags << 1)
DC45: A2 A4           LDX     #$A4                ; arm voice 2 control

loc_dc47:
DC47: 8E C3 60        STX     $60C3               ; write POKEY 1 voice 2 control
...
loc_dc78:
DC78: 8D E0 60        STA     $60E0               ; write the LED/flip latch
DC7B: 8C 00 40        STY     $4000               ; write the coin-counter/flip output latch
...
loc_dcc0:
DCC0: B9 E4 31        LDA     $31E4,Y             ; read the coordinate word low byte from the glyph table
DCC3: BE E5 31        LDX     $31E5,Y             ; read the coordinate word high byte from the glyph table
...
DCED: 8D 8E 60        STA     $608E               ; load mathbox Ra high operand
DCF0: 8E 8F 60        STX     $608F               ; load mathbox Rb low operand
DCF3: 8C 90 60        STY     $6090               ; clear mathbox Rb high
DCF6: A2 10           LDX     #$10                
DCF8: 8E 8C 60        STX     $608C               ; seed the mathbox iteration count
DCFB: 8E 94 60        STX     $6094               ; strobe the divide to start

loc_dcfe:
DCFE: CA              DEX                         ; count down the poll window
DCFF: 30 0B           BMI     $DD0C               ; {code.loc_dd0c} window exhausted -- no result ready
DD01: AD 40 60        LDA     $6040               ; read the mathbox status
DD04: 30 F8           BMI     $DCFE               ; {code.loc_dcfe} still busy -- keep polling
DD06: AD 60 60        LDA     $6060               ; ready -- latch the result low byte
DD09: AC 70 60        LDY     $6070               ; latch the result high byte
...
DD53: 8D 95 60        STA     $6095               ; store as mathbox R7 low operand
...
DD5D: 8D 96 60        STA     $6096               ; store as mathbox R7 high operand
...
DD64: A9 01           LDA     #$01                
DD66: 8D 95 60        STA     $6095               ; floor the operand to one so the divide never sees zero

loc_dd69:
DD69: AD 09 04        LDA     $0409               ; {hard.workRam+409} load mathbox Ra low from the timer low byte
DD6C: 8D 8D 60        STA     $608D               
...
loc_de6b:
DE6B: A0 00           LDY     #$00                
DE6D: 8C 40 60        STY     $6040               ; reset the math-box / EAROM control port
...
DE7F: 9D 00 60        STA     $6000,X             ; stage a data byte into the EAROM data window
DE82: A9 40           LDA     #$40                
...
loc_deab:
DEAB: 9D 00 60        STA     $6000,X             ; stage the byte into the EAROM data window
DEAE: A0 0C           LDY     #$0C                
DEB0: B8              CLV                         
...
loc_deb3:
DEB3: A9 08           LDA     #$08                
DEB5: 8D 40 60        STA     $6040               ; begin the EAROM read handshake on the control port
DEB8: 9D 00 60        STA     $6000,X             
DEBB: A9 09           LDA     #$09                
DEBD: 8D 40 60        STA     $6040               ; clock the control port
DEC0: EA              NOP                         
DEC1: A9 08           LDA     #$08                
DEC3: 8D 40 60        STA     $6040               ; return the control port
DEC6: EC CD 01        CPX     $01CD               ; {hard.workRam+1CD} reached the region limit?
DEC9: AD 50 60        LDA     $6050               ; read the EAROM read-back port
...
loc_deff:
DEFF: 8C 40 60        STY     $6040               ; write the exit code to the control port
DF02: 98              TYA                         
...
DF26: A0 00           LDY     #$00                
DF28: AA              TAX                         
DF29: BD E4 31        LDA     $31E4,X             ; read the glyph's first stroke byte from the vector-ROM stroke table
DF2C: 91 74           STA     ($74),Y             ; {hard.workRam+74} copy it into the display list at the draw cursor
DF2E: BD E5 31        LDA     $31E5,X             ; read the glyph's second stroke byte
DF31: C8              INY                         
...
DFEC: 00 F4 EA E2 E0 E2 EA F4 00 0C 16 1E 00 00 04 D7
DFFC: 3F D9 04 D7
```
