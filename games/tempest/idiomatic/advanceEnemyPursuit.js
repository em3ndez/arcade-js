...
export function advanceEnemyPursuit(m, x = m.regs.x) {
  ...
  if (fire) return maybeFireEnemyStep(m, x);              // carry set -> fire step
  if (mem8[ENEMY_FIRE_SELECT] & 0x80) return flipEnemyLaneTowardTarget(m, x); // ENEMY_FIRE_SELECT sign set -> seek step
  return flipEnemyLaneRandomSide(m, x);                          // ENEMY_FIRE_SELECT sign clear -> the other seek step
}
