...
export function computeCoordListBackDelta(m, y = m.regs.y) {
  ...
  mem8[loc_29] = y;
  const operand = mem8[u16(mem16[COORD_LIST_PTR_LO] + ((y - 2) & 0xff))];
  return (base - operand) & 0xff;
}

// Sibling entry: run computeCoordListBackDelta, advance the cursor by the low bit of its result, and return the entry there.
export function selectListEntryByDeltaParity(m, y = m.regs.y) {
  const { mem8, mem16 } = m;
  const delta = computeCoordListBackDelta(m, y);
  if (delta & 0x01) y = u8(y + 1);
  return mem8[u16(mem16[COORD_LIST_PTR_LO] + y)];
}
