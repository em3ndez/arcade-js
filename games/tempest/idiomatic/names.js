...
export const ROUTINES = {
  ...
  0xa888: { name: "sweepLaneSlotsForRespawn", role: "[seen] on the timed phase (phase cell 0x125 >= 3 and even) sweep 0x2df,y downward from y=0x11c for the first nonzero slot: on a hit clear the low two bits of 0x28a,y and tail-delegate to respawnEnemyAndAward, on no hit reset 0x125 to 0.", cert: "seen" },
  0xda0a: { name: "checksumRomAndSettleEntropy", role: "[seen] power-on ROM checksum + entropy settle: walk 12 banks of 8 pages, XOR every byte into a per-bank checksum seeded with the bank index (strobing watchdog 0x5000 per page), land the 12 checksums from 0x7d; if bank 0's checksum is nonzero arm the error tone (0x60c4/0x60c5); then settle each entropy register (0x60ca->0x7a, 0x60da->0x7b) storing only when six re-reads match, and continue into runSelfTestLoop.", cert: "seen" },
  ...
  0x902b: { name: "resetWorkingRamForStateEntry", role: "[seen] master state-entry sweep: run the six reset/seed leaves back to back (loc_928f, loc_926f, loc_9246, loc_929f, loc_92ad, loc_c16e), then arm loc_124 and loc_148 to 0xff and clear loc_123.", cert: "seen" },
  ...
  0x928f: { name: "clearActiveShots", role: "[seen] reset the shot bank: zero the 12-byte depth array loc_2d3 (0x0b..0) and both count cells loc_135 and loc_a6 to baseline.", cert: "seen" },
  ...
  0x92b2: { name: "swapParallelTables", role: "[seen] swap the two parallel 18-entry tables loc_3aa and loc_3bc slot-for-slot (index 0x11 down to 0), so each table ends holding what its sibling held.", cert: "seen" },
  ...
  0x96b7: { name: "fetchCoordListEntryByCounter", entry: "fetchCoordListEntryByIndex", role: "[seen] sibling of 0x96ab that uses the raw index 0x2b (not the counter-wrapped value): stash Y at 0x29, subtract the list byte two entries back, add back saved Y, re-index the pointer, and load and return that entry.", cert: "seen" },
  0x96c4: { name: "fetchCoordListEntryByCounter", entry: "readCoordListEntry", role: "[seen] the bare coordinate-list read: return the byte at pointer 0x2c offset by Y, with no index arithmetic.", cert: "seen" },
  0x96c7: { name: "advanceCursorPastPackedRecord", role: "[seen] cursor-skip helper: advance the list cursor Y by a fixed run to step over a packed record without reading it -- the first entry moves Y forward by three (bumps by one then falls into the second entry), the second entry by two; register only, writes no memory", cert: "seen" },
  0x96c8: { name: "advanceCursorPastPackedRecord", entry: "advanceListCursorByTwo", role: "[seen] step the list cursor forward past a two-byte record without reading it: return Y+2 (register only).", cert: "seen" },
  ...
  0x96db: { name: "resolveCoordListEntryToAbsolute", role: "[seen] resolve one list entry to an absolute coordinate: read the byte at pointer 0x2c offset by Y and add the base value in 0x160, returning the 8-bit sum in A.", cert: "seen" },
  0x96e2: { name: "sumCoordListEntryRun", role: "[seen] fold a run of coordinate-list entries: use 0x96f4 as a repeat count, load the first entry at pointer 0x2c offset Y, then add that many further consecutive entries into a single wrapped one-byte total.", cert: "seen" },
  0x96f4: { name: "computeCoordListBackDelta", role: "[seen] compute the coordinate-list index delta: record Y at 0x29 and return (0x2b minus the list byte two slots back at pointer+(Y-2)) & 0xff; a leaf whose result is consumed downstream.", cert: "seen" },
  0x9700: { name: "computeCoordListBackDelta", entry: "selectListEntryByDeltaParity", role: "[seen] pick one source-list entry: run computeCoordListBackDelta to form base 0x2b minus the table byte two slots back, advance cursor Y by the low bit of that delta, and return the entry at (mem16[0x2c]+Y).", cert: "seen" },
  0x9749: { name: "rotateBlasterAroundRim", role: "[seen] advance the rim rotation/spinner state: skip while fine-angle flag loc_201 bit7 set; take the delta from manual reading loc_50 (clamped to band [0xe1,0x1f], then consumed) when loc_5 bit7 is set else the auto-aim from aimSpinnerAtNearestEnemy; fold it into work cells loc_2b/loc_2c, and on a live level (loc_111!=0) cap loc_2c to 0xef and saturate toward the stored sign on a sign flip; the high nibble becomes coarse angle loc_2a, ring sound cueRimRotationSound on a changed coarse angle, then commit loc_200/loc_201/loc_51.", cert: "seen" },
  ...
  0x9a9d: { name: "seatDemoCoordListPointer", role: "[seen] seat the index-0 coordinate list: take the low pointer byte from fixed table byte 0x9b02, mark index 0 at 0x2b, take the high pointer from held source cell 0x15d into 0x2d, reload A from 0x29.", cert: "seen" },
  0x9ab7: { name: "seatDemoCoordListPointer", entry: "seatCoordListPointerAtIndex3", role: "[seen] preset the selecting index to 3 and run the shared 0x9aee seating, parking the coordinate-list pointer 0x2c/0x2d from ROM tables 0x9b02[3]/0x9afd[3].", cert: "seen" },
  ...
  0x9c21: { name: "setFlagIfSlotPastSegmentBound", role: "[seen] boundary-test opcode: read the slot's segment loc_2b9,x, look up its boundary in loc_3ac (a zero entry reads as the maximum 0xff), and set the branch flag loc_10c to 1 when the boundary is at or beyond the slot's depth loc_2df,x, else 0", cert: "seen" },
  0x9c3b: { name: "setFlagFromPhaseAccumulatorSign", role: "[seen] phase-probe opcode: form ((loc_147<<2)+loc_148) & loc_148 & 0x80, XOR against 0x80, and store to the branch flag loc_10c so it becomes 0x00 when that high bit is set and 0x80 when clear", cert: "seen" },
  ...
  0x9c99: { name: "stepEnemyDepthInLaneDirection", entry: "reverseEnemyLaneDepth", role: "[seen] subtract-direction depth step: loc_29f,x -= loc_160,seg (16-bit, with borrow) into loc_2df,x, flooring the high byte to 0xf2 when it underflows past 0xf0; returns the new (or floored) high byte.", cert: "seen" },
  ...
  0x9d67: { name: "faceEnemyTowardPlayerSegment", role: "[seen] aim slot x's turn side toward the player: take the signed segment delta of loc_2b9,x against player segment loc_200 via signedSegmentDelta, then clear bit6 of loc_283,x when the delta is negative (bit7 set) and set it otherwise.", cert: "seen" },
  ...
  0x9e2f: { name: "spawnType5OnCoordMatch", role: "[seen] spawn a type-5 object when a live slot (loc_283,x bit7 clear) has its coordinate pair loc_2b9,x/loc_2cc,x matching the current target pair loc_200/loc_201.", cert: "seen" },
  0x9e48: { name: "fireHitOnPlayerCollision", role: "[seen] fire the player-collision hit when a slot's depth loc_2df,x matches loc_202 and its segment loc_2b9,x matches loc_200.", cert: "seen" },
  ...
  0x9eab: { name: "keepClimberFlipBitByDepth", role: "[seen] keep the climber's flip bit (bit6 of loc_283,x) in step with its segment depth loc_2b9,x while gate loc_111 is on: when bit6 is set clear it once depth reaches 0x0e; when bit6 is clear set it only while depth is 0.", cert: "seen" },
  0x9ed7: { name: "lookupRingHeading", role: "[seen] look up a climber heading from the ring-direction table loc_3ee,y with bit7 forced on; when the caller's bit6 is set take the half-turn first -- index y=(y-1)&0x0f, value (loc_3ee,y+8)&0x0f.", cert: "seen" },
  0x9ef1: { name: "advanceEnemyPursuit", role: "[seen] per-slot pursuit mover: when loc_28a,x bit7 is set re-seek the subtract way (reverseEnemyLaneDepth with delta index 0x04) and dispatch on the returned high (maybeFireEnemyStep fire step for hi<0x80, else flipEnemyLaneRandomSide/flipEnemyLaneTowardTarget by loc_159 bit6); when clear advance the 16-bit depth (loc_29f,x += loc_164; loc_2df,x += loc_169) clamped to floor loc_202, producing a fire carry only when loc_3ab!=0 and (loc_9f>=0x11 or hi>=0x20), then run maybeFireEnemyStep or pick flipEnemyLaneTowardTarget/flipEnemyLaneRandomSide by loc_159 sign.", cert: "seen" },
  0x9f5f: { name: "maybeFireEnemyStep", role: "[seen] per-slot fire gate: run a step only when the slot's fire bit loc_2df,x & 0x20 is set and a fresh POKEY draw loc_60da >= threshold loc_15f; then route to flipEnemyLaneRandomSide when loc_159 bit6 is clear or the slot index is even, otherwise flipEnemyLaneTowardTarget.", cert: "seen" },
  0x9f81: { name: "flipEnemyLaneTowardTarget", role: "[seen] flip-step entry that re-derives slot x's turn side toward its target (faceEnemyTowardPlayerSegment) then toggles bit6 of loc_283,x (toggleEnemyTurnSide), then runs the shared tail: on a live board (loc_111!=0) flip bit6 by ring depth via loc_2b9,x, mark loc_10b=0x66, and continue into stepClimberSegmentAndHeading -- the flipper's lane-to-adjacent-lane hop.", cert: "seen" },
  0x9f8a: { name: "flipEnemyLaneTowardTarget", entry: "flipEnemyLaneRandomSide", role: "[seen] flip-step entry that reseeds slot x's turn side from a random bit (loc_60ca & 0x40) into bit6 of loc_283,x, then runs the shared tail: on a live board (loc_111!=0) flip bit6 again by ring depth (loc_2b9,x==0 when bit6 set, >=0x0f when clear), mark loc_10b=0x66, and continue into stepClimberSegmentAndHeading.", cert: "seen" },
  ...
  0xa1e4: { name: "primeTopObjectOnTargetMatch", role: "[seen] prime the top-priority object only when the live byte loc_200 matches slot x's target loc_2ad,x and ready flag loc_201 bit7 is clear -- run the prime and latch loc_201=0x81.", cert: "seen" },
  0xa1fa: { name: "advanceShotAndScoreLaneHit", role: "[seen] advance shot slot x's counter loc_2d3,x toward the per-lane target-depth loc_3ac,y (y=loc_2ad,x); on reaching it shrink/clear loc_3ac,y, bump the hit tally loc_2f2,x, flag the target loc_39a,y=0xc0, chime (requestSegmentHitSound) and award (addBcdScoreAndAwardAtThreshold), returning the live slot; a second hit spends the shot (clear loc_2d3,x, drop the shot count loc_135).", cert: "seen" },
  ...
  0xa343: { name: "insertObjectHeadTag9", role: "[seen] insert a type-1 object after stamping head flag loc_13b with 0x09.", cert: "seen" },
  0xa347: { name: "insertObjectHeadTag9", entry: "insertObjectHeadTag7", role: "[seen] insert a type-1 object after stamping head flag loc_13b with 0x07.", cert: "seen" },
  0xa34b: { name: "primeTopPriorityObject", role: "[seen] prime a fresh top-priority object: stamp head flag loc_13b=0xff and run the type-1 insert.", cert: "seen" },
  ...
  0xa3d4: { name: "insertTimedObjectOfType", role: "[seen] insert a timed object of a given type: stash A into the type scratch loc_2c, then insert into the 8-slot table.", cert: "seen" },
  ...
  0xa618: { name: "stepEnemyFleetAndSpawn", role: "[seen] drive one frame of the enemy bank: seed frame-active flag loc_10d from spawn budget loc_10e, walk slots 0x0f..0, integrate+decay each live slot (loc_283,x!=0) via the flight steps and refill each free slot from the budget, tick loc_10e on even frames (loc_3 bit0 clear), settle loc_37=0xff, and raise mode-request loc_0=0x12 when nothing was live or spawned.", cert: "seen" },
  0xa65b: { name: "spawnEnemyInSlot", role: "[seen] spawn an enemy into free slot x: mark loc_263,x/loc_283,x/loc_2a3,x = 0x80, seed three velocity/coordinate pairs from POKEY draws loc_60da/loc_60ca (raw into loc_2c3/2e3/303,x, signed nudge from drawSignedVelocityNudge into loc_323/343/363,x, forcing the middle nudge non-positive), then cue the spawn sound gateSound1f.", cert: "seen" },
  0xa69b: { name: "drawSignedVelocityNudge", role: "[seen] produce a signed random velocity nudge: take a 3-bit magnitude (loc_60da & 0x07, 0..7) and negate it when the caller's incoming value has bit0 set, yielding a signed step in [-7,+7] consumed by spawnEnemyInSlot as a whole-velocity seed.", cert: "seen" },
  0xa6a9: { name: "advanceEnemyFreeFlight", role: "[seen] integrate slot x's three free-flight axes: fold each low velocity (loc_2e3/2c3/303,x) into its fraction (loc_223/203/243,x) and add carry+signed whole (loc_343/323/363,x) into a whole coordinate, committing loc_263,x and loc_2a3,x and loc_283,x, but forcing loc_283,x to 0 (retiring the slot) if any axis crosses the tube ring [0x10,0xf0).", cert: "seen" },
  0xa721: { name: "decayEnemyFreeFlightVelocity", role: "[seen] decay slot x's free-flight velocities: seed saturation counter loc_29=0xfd, step each velocity pair (loc_2c3/323, loc_2e3/343, loc_303/363,x) one increment toward zero via stepVelocityTowardZero, and clear the slot's whole coordinate loc_283,x only when all three axes saturate in the same frame.", cert: "seen" },
  0xa75d: { name: "stepVelocityTowardZero", role: "[seen] step one signed 16-bit velocity (whole:low) one fixed increment loc_a788 (0x20) toward zero (add when whole negative, subtract otherwise), snapping low loc_2a=0 and bumping saturation counter loc_29 on crossing zero; returns the stepped [low,whole].", cert: "seen" },
  0xa789: { name: "resetPerSlotStateTable", role: "[seen] slot-arm init leaf: zero the sixteen-byte per-slot state/flags table loc_283..loc_283+0x0f, then re-seed scalars loc_10e=0x20, loc_10d=0x20, loc_1=0x04, loc_68=0 and loc_69=0.", cert: "seen" },
  ...
  0xa7bd: { name: "rebuildSpikeTable", role: "[seen] rebuild the spike table: zero the 8-byte array loc_3fe (7..0), stamp its last slot loc_405=0xf0, and arm the remap reference loc_115=0xff.", cert: "seen" },
  ...
  0xaa13: { name: "stageTextLineWithCount", role: "[seen] stage a text line into the vector text buffer at 0x2f60: pick a string index in loc_3e (forced to 0x01 when loc_5 bit7 is clear and any of loc_43/loc_44/loc_45 is set), seat the write cursor loc_74/loc_75 at 0x2f60, copy loc_ce66[index]+1 stroke bytes from ROM loc_cde6 into it, and on the loc_5 bit7 path also emit the BCD of (loc_9f+1) at 0x2fa6 before restoring the cursor low byte and terminating the record via emitRecordBodyC0.", cert: "seen" },
  0xaa5a: { name: "drawFrameWithSlot08", role: "[seen] compose a frame led by slot 0x08: draw slot 0x08 (drawSlotShapeRecord) then chain prepCountThenComposeFrame (the count prep followed by the per-frame composition loc_a8e7).", cert: "seen" },
  0xaa62: { name: "drawFrameWithSlot00", role: "[seen] compose a frame led by slot 0x00: prime slot 0x00 with header 0x30 (drawSlotShapeWithHeader), run the shared count prep (drawSlotThenDigitRun), then dispatch the per-frame composition loc_a8e7.", cert: "seen" },
  0xaa69: { name: "prepCountThenComposeFrame", role: "[seen] run the shared count prep (drawSlotThenDigitRun: slot 0x02 + digit run) then dispatch the per-frame composition loc_a8e7.", cert: "seen" },
  0xaa6f: { name: "composeFrameThenDrawSlot06", role: "[seen] run the alternate per-frame composition loc_a8b4, then prime slot 0x06 through the shared entry drawSlotShapeWithHeader (header 0x00).", cert: "seen" },
  0xaa79: { name: "drawFrameWithSlot32", role: "[seen] compose a frame led by slot 0x32: draw slot 0x32 (drawSlotShapeWithHeader, header 0x00), add a second draw of slot 0x22 (header 0xe0) only while status nibble loc_3&0x1f < 0x10, then finish with the alternate per-frame composition loc_a8b4.", cert: "seen" },
  0xaa92: { name: "drawSlotThenDigitRun", role: "[seen] draw fixed slot 0x02 (drawSlotShapeRecord) then hand off to emitCountDigitRun to lay down the numeric run.", cert: "seen" },
  0xaa97: { name: "emitCountDigitRun", role: "[seen] publish a zero scale header (emitScaleWordIfChanged with 0x00) then emit the one-byte digit run for the slot named by loc_3d via loc_aa9e, drawing the count as nibble digits.", cert: "seen" },
  0xaa9e: { name: "emitSlotIndexDigit", role: "[seen] Increments the slot index, publishes it into loc_61, and emits that single byte as a digit run.", cert: "seen" },
  ...
  0xaaf5: { name: "packBinaryToBcd", role: "[seen] Converts the binary byte in A to packed BCD by double-dabble and writes the packed digits into loc_29 and loc_2c.", cert: "seen" },
  0xab0d: { name: "emitFixedVectorWord", role: "[seen] emit one fixed vector word (bytes 0x20, 0x80) through the draw cursor 0x74 and step past it (via loc_df57).", cert: "seen" },
  ...
  0xab98: { name: "drawShapeListAtPosition", role: "[seen] thin front over the shared builder: seat loc_35=x (slot), loc_2a=a (position seed), and a zero colour header loc_2b=0x00, then fall into expandShapeListToVectors to emit the slot's vector run.", cert: "seen" },
  0xaba2: { name: "rebuildControlBlocksIfRequested", role: "[seen] guarded front for the rebuilder: refresh via loc_ac20, and if loc_1c9's low two request bits are clear take the no-op tail loc_ac07, otherwise run the rebuild path loc_abac.", cert: "seen" },
  0xabac: { name: "rebuildControlBlocksFromTemplate", role: "[seen] control-block rebuilder: refresh via requestRebuildIfSwitchesChanged, write loc_100=0x08, raise the requests via raiseRebuildRequestBits when (loc_71b|loc_71c|loc_71d) are idle, then keyed on loc_1c9's low two bits copy template loc_ac08 into loc_606 (top 0x17 if bit0 else 0x0e) and fill loc_706 with 0x01 (top 0x17 if bit1 else 0x0e), latch loc_71e=(loc_a&0xf8)/loc_71f=(loc_16a&0x03) when any request bit was set, and clear the two request bits (loc_1c9 &= 0xfc).", cert: "seen" },
  0xac07: { name: "noRebuildRequestReturn", role: "[seen] no-op return tail taken by rebuildControlBlocksIfRequested when no rebuild request bit is set in loc_1c9; does nothing and returns.", cert: "seen" },
  0xac20: { name: "requestRebuildIfSwitchesChanged", role: "[seen] refresh the live option/switch snapshot via decodeOptionSwitches, then compare (loc_a & 0xf8) to cached loc_71e and (loc_16a & 0x03) to cached loc_71f: a match takes the no-op tail switchesUnchangedReturn, a mismatch calls raiseRebuildRequestBits to raise the pending-rebuild request bits.", cert: "seen" },
  ...
  0xac3e: { name: "switchesUnchangedReturn", role: "[seen] shared no-op return tail taken by requestRebuildIfSwitchesChanged when the option/switch snapshot still matches the cached targets; does nothing and returns.", cert: "seen" },
  ...
  0xad6e: { name: "tickActiveSoundSlot", role: "[seen] per-frame tick of the active slot: set loc_1=0x06; while the low 5 bits of loc_3 are clear run countdown loc_605 down and on zero write loc_00=0x14 and return; otherwise clamp the active slot value loc_606,slot via foldStepIntoFraction (negative->0x1a, >=0x1b->0), gate on bits 3-4 of loc_4e (clearing them plus bit7), and when gated step cursor loc_602 and counter loc_604 -- on underflow re-arm (requestWriteLowRegions when loc_600,loc_3d < 4, then armRequestedSoundSlot), else clear the retired slot loc_606,slot-1.", cert: "seen" },
  0xadce: { name: "foldStepIntoFraction", role: "[seen] fold the signed sub-step loc_50 (times 8) into the fraction cell loc_51, add the fold carry plus loc_50's sign-extension into A, and clear loc_50; returns the updated whole byte in A.", cert: "seen" },
  0xadea: { name: "drawScoreDeltaPanel", role: "[seen] draw a fixed frame (loc_ab17/loc_ab14 chain), tick the countdown loc_16e down by one, then hand the score delta loc_602-loc_604 to the glyph-row builder drawHighlightedGlyphRowList.", cert: "seen" },
  ...
  0xaf6e: { name: "sharedReturnTail", role: "[seen] no-op leaf: return immediately; the shared RTS tail that drawCounterPair falls through to when both counters loc_600/loc_601 are zero.", cert: "seen" },
  0xaf71: { name: "emitCappedCount", role: "[seen] clamp the incoming byte to a max of 0x63 (99), then pack-and-emit it via loc_af77.", cert: "seen" },
  0xaf77: { name: "emitByteAsBcdDigits", role: "[seen] Packs the incoming byte to BCD (into loc_29) then emits that single zeropage byte as its two decimal nibbles.", cert: "seen" },
  ...
  0xb0ab: { name: "nudgeBlasterRimPosition", role: "[seen] nudge the blaster's rim position loc_200 by its signed sub-step (via foldStepIntoFraction), clamp into [0, ceiling loc_127] (a negative result floors to 0, an over-ceiling result pins to loc_127), write it back to loc_200, and return it in both A and Y.", cert: "seen" },
  0xb0c6: { name: "emitTableValueDigitRun", role: "[seen] Seats a table pointer by index (via loc_91b5) then emits the three-byte zeropage run at loc_29 as nibble digits.", cert: "seen" },
  ...
  0xb60f: { name: "emitJumpModeSlot", role: "[seen] Pairs a jump-mode byte from table loc_b61e (indexed by loc_28a+x & 0x03) with slot target loc_2b9+x and emits via bcfd.", cert: "seen" },
  0xb622: { name: "emitAnimatedPhaseSlot", role: "[seen] Pairs slot target loc_2b9+x with a four-phase animation offset (((loc_3 & 0x03)<<1)+0x12) and emits via bcfd.", cert: "seen" },
  ...
  0xb7eb: { name: "animateShapeOneVector", role: "[seen] advance and emit the shape-1 enemy animation: refresh axis params loc_56/loc_58 from loc_435/loc_445 at loc_29, run frame updaters (projectPointThroughMathbox, layHeaderAndBuildRecord), tick sub-timer loc_13c and on wrap advance phase loc_13b and reload loc_13c from loc_b82a+loc_13b, run phase handler dispatchDrawSetup when loc_b83d+loc_13b<0x80, then emit the phase's vector-pair word from loc_cec8/loc_cec9 at ((loc_13b<<1)+0x28)&0xff.", cert: "seen" },
  0xb84e: { name: "dispatchDrawSetup", role: "[seen] computed-jump dispatcher: caller's Y is a byte offset (0,2,4,6) selecting one of four draw-setup targets loc_b85f/loc_b875/loc_b888/loc_b896, and it tail-returns that routine's result.", cert: "seen" },
  0xb85f: { name: "seedTripleArrays", role: "[seen] seed the paired three-entry arrays 0x22-0x24 and 0x809-0x80b with the fixed values 0x00, 0x04, 0x0c.", cert: "seen" },
  0xb875: { name: "rotateTripleArray", role: "[seen] rotate the three-entry array 0x22-0x24 down by one, threading the wrapped value, and mirror each new entry into paired array 0x809-0x80b.", cert: "seen" },
  0xb888: { name: "resetVectorTailCursor", role: "[seen] rebuild the packed-nibble table via unpackLevelNibbleTables, then seat the vector-list tail cursor 0x139=0x7f and 0x13a=0x04.", cert: "seen" },
  0xb896: { name: "emitVectorTailRecord", role: "[seen] emit a vector-RAM tail record from cursor 0x139/0x13a — low byte to 0x2ffc, high byte tagged 0x70 to 0x2ffd, 0xc0 terminator to 0x2fff — then step the cursor down by 0x20 with a 16-bit borrow into 0x13a and mask the low byte to 0x7f.", cert: "seen" },
  0xb8ba: { name: "drawMovingObjectSlots", role: "[seen] draw the 16-slot moving-object cascade into the display list: reset accumulators (loc_6a-loc_6d, loc_202, loc_68/loc_69) with loc_5f=0xe0/loc_5b=0xff, cache the base draw-struct pointer from selectPointerPair into loc_76/loc_77, lay an opening coord (emitCoordinateVectorWord), then count loc_37 from 0x0f down and for each active slot (loc_283+x nonzero) load loc_57/loc_56/loc_58 from loc_283/loc_263/loc_2a3+x, integrate deltas (projectPointThroughMathbox), emit the record body (emitCoordDeltaRecord, emitBlankValueRecord, emitObjectPositionVector) with pointer-swap shadow passes (swapDrawPointers), and set the slot phase into loc_9e; closes by swapping pointers back and finishing the base list (emitBlankVectorWordTag70, emitRecordBodyC0).", cert: "seen" },
  0xb944: { name: "swapDrawPointers", role: "[seen] swap the two 16-bit draw pointers 0x74/0x75 and 0x76/0x77 so the shared cursor addresses the other structure.", cert: "seen" },
  ...
  0xb967: { name: "selectPointerPair", role: "[seen] select a pointer pair by flag 0x415: zero picks 0xce87/0xce86, nonzero picks 0xce6f/0xce6e; returns the pair as (A,X).", cert: "seen" },
  ...
  0xc765: { name: "layHeaderAndBuildRecord", role: "[seen] Writes the fixed header word 0x00/0x71 at the draw cursor start (loc_74/loc_75) then resumes the shared vector-record builder from cursor slot 2.", cert: "seen" },
  ...
  0xc81b: { name: "advanceLevelCounter", role: "[seen] from the 2-bit gate loc_4e&0x60 (then cleared) and a >=2 test on counter loc_6 derive a step 0..2 and drain loc_6; when the gate is clear optionally seed intro cells (loc_1/loc_4/loc_0/loc_2, gated by loc_50 and loc_5 bit7); when the step is nonzero set loc_5|=0xc0, zero loc_16/loc_18/loc_0, bump the 16-bit tally loc_40c,x/loc_40d,x, and advance the level cell loc_100 by step+1 clamped to 0x63.", cert: "seen" },
  ...
  0xccb0: { name: "gateSound5f", role: "[seen] cue the fixed sound id 0x5f through the sound gate, carrying the caller's X/Y.", cert: "seen" },
  0xccb5: { name: "cueRimRotationSound", role: "[seen] load fixed sound id 0x0f and pass it through the sound gate loc_ccc3 (keeping caller X/Y); rung by loc_9749 when the coarse rim angle changes, i.e. the spinner-rotation sound cue.", cert: "seen" },
  0xccb9: { name: "requestScoreAwardSound", role: "[seen] one-line cue: request fixed sound id 0x4f through the enable gate loc_ccc3; caller loc_c98c fires it on the bonus/level-advance handler chain, and the same id 0x4f is fired by the score-award path loc_ca6c, so it voices a score/bonus award.", cert: "seen" },
  0xccbd: { name: "gateSound8f", role: "[seen] cue the fixed sound id 0x8f through the sound gate, carrying the caller's X/Y.", cert: "seen" },
  0xccc1: { name: "gateSound1f", role: "[seen] cue the fixed sound id 0x1f through the sound gate, carrying the caller's X/Y.", cert: "seen" },
  0xccc3: { name: "requestSoundIfEnabled", role: "[seen] sound enable gate: only when bit7 of the enable flag loc_5 is set, forward the sound id in A (with X/Y) to the loader loadSoundVoiceSlots; with sound disabled it returns making no write.", cert: "seen" },
  ...
  0xccea: { name: "requestEnemySpawnSound", role: "[seen] one-line cue: request fixed sound id 0x2f through the enable gate loc_ccc3, forwarding slot index X; its sole caller is the enemy spawner loc_a23f, so it voices a new enemy entering the tube.", cert: "seen" },
  0xccee: { name: "cueMovingSpikeStartSound", role: "[seen] register the fixed sound id 0x6f through the sound-enable gate loc_ccc3 -- the moving spike's start cue, fired by loc_97f8 at trigger height loc_202==0x10.", cert: "seen" },
  0xccf2: { name: "cueMovingSpikeEndSound", role: "[seen] register the fixed sound id 0x7f through the sound-enable gate loc_ccc3 -- the moving spike's end cue, fired by loc_97f8 when the height overflows the ceiling.", cert: "seen" },
  0xccf6: { name: "requestSegmentHitSound", role: "[seen] one-line cue: request fixed sound id 0x9f through the enable gate loc_ccc3, forwarding X/Y; its sole caller is the per-slot hit/award routine loc_a1fa, so it chimes when a segment/enemy is hit.", cert: "seen" },
  0xccfa: { name: "requestActiveSoundCue", role: "[seen] request fixed sound id 0xaf by jumping straight into the loader loc_ccc7, bypassing the enable gate; the frame dispatcher loc_c891 calls it every frame while loc_c is live, so it (re)voices the currently-active held cue regardless of the loc_5 enable flag.", cert: "seen" },
  0xccfe: { name: "requestLevelIntroSound", role: "[seen] one-line cue: request fixed sound id 0xbf through the enable gate loc_ccc3; its sole caller loc_90c4 fires it at phase 3 of the start-slot pick / wave working-set reseed, so it voices the level-intro / skill-step start.", cert: "seen" },
  0xcd02: { name: "requestMotionFlipSound", role: "[seen] one-line cue: request fixed sound id 0x3f through the enable gate loc_ccc3, threading X; its sole caller loc_9b1e fires it as a correction when the per-slot motion accumulator loc_148 sign-flips, so it voices an enemy direction reversal on the rim.", cert: "seen" },
  0xcd06: { name: "cueSpikeCollisionSound", role: "[seen] register the fixed sound id 0xcf through the sound-enable gate loc_ccc3, threading X/Y -- the spike-collision cue fired by loc_97f8 when the spike reaches the player segment.", cert: "seen" },
  ...
  0xd7e1: { name: "armModeAndRebuildIfEnabled", role: "[seen] arm mode bytes loc_5=0x00 and loc_1=0x02, then rebuild only while idle and enabled: bail if loc_1ca is nonzero (busy) or bit4 of option port loc_c00 is clear (disabled), write loc_0=0x00, and run rebuildControlBlocksFromTemplate only when loc_1c9's low two bits show a pending request.", cert: "seen" },
  0xd8a9: { name: "emitScaledByteDigit", role: "[seen] Stashes A into loc_29, scales the two coordinates (y, x), and emits that one stashed byte as a single-entry digit run.", cert: "seen" },
  0xd804: { name: "buildVectorItemList", role: "[seen] a trampoline target that assembles the frame's vector item list into display RAM: four setup passes, a header pair, a marker emitted 0x158 times (dec-counted via 0x37), then table-indexed coordinate records selected by state cells 0x16a/0x200/0x4d and the 0xd8b6/0xd8ba/0xd8c2 tables, with a mask-table gated branch (eraseEaromLowRegions/queueEaromRegionErase).", cert: "seen" },
  ...
  0xdb22: { name: "initVectorDisplayRegisters", role: "[seen] reset the $60xx vector-display register bank (zero 0x60e0/0x6080/0x60c0/0x60d0/0x6000/0x6040, four settling reads, then raise 0x60e0 to 0x08), march a single set bit across the 32 slots at 0x6080, and emit one framing word via emitCoordinateVectorWord(0x34,0xa6).", cert: "seen" },
  0xdb5a: { name: "beginEaromSequenceIfIdle", role: "[seen] When guards loc_1ca and loc_1c7 are both clear, runs the EAROM seeder (de11) and stamps loc_7c=loc_1c9 and loc_0=0x02.", cert: "seen" },
  0xdb6f: { name: "emitHalvedCountHeaderAndClearVectorSlots", role: "[seen] emit a header from the halved slot count via emitTaggedVectorWord(0x68, 0x50>>1), then run loc_db88(0x33,0x4e) to emit its header and blank the four even slots of 0x60c1 and 0x60d1.", cert: "seen" },
  0xdb7e: { name: "emitPrimedHeaderAndClearVectorSlots", role: "[seen] front onto emitVectorHeaderAndClearSlots with the fixed header pair (0x32,0xb6): emit that header word then blank the four even slots of 0x60c1 and 0x60d1.", cert: "seen" },
  ...
  0xdb88: { name: "emitVectorHeaderAndClearSlots", role: "[seen] emit a caller-supplied header word via emitCoordinateVectorWord(a,x), then blank the four even-indexed slots of output tables 0x60c1 and 0x60d1.", cert: "seen" },
  0xdb9a: { name: "stepVectorPhaseAnimation", role: "[seen] advance phase counter 0x39 (only while frame gate 0x3 & 0x3f is clear), index three parallel ROM rows (loc_dbd5/loc_dbd6/loc_dfdc) by 0x39 & 0x07 to seed output cells (clear 0x60c1+slotA, write ROM value into 0x60c0+slotB and 0xa8 into 0x60c1+slotB), then emit three words via emitCoordinateVectorWord(0x34,0x56)/emitVectorWordTag70(0x01, 0x3 & 0x7f)/emitCoordinateVectorWord(0x34,0xaa).", cert: "seen" },
  0xdbe0: { name: "assemblePotStatusByte", role: "[seen] Assembles a pot-status byte from the low three bits of loc_60d8 (mirrored into loc_37 and loc_60cb) merged with one relocated bit lifted from loc_60c8.", cert: "seen" },
  0xdce6: { name: "runMathboxDivide", role: "[seen] Primes the math-coprocessor operand/count registers loc_608c-loc_6096 from A and X, kicks its divide, spins a 16-step window on loc_6040 for the first ready result, and returns the loc_6060/loc_6070 low/high pair.", cert: "seen" },
  ...
  0xdd0d: { name: "buildPotReadoutVectorList", role: "[seen] Builds a diagnostic readout vector list: a fixed header word (loc_df53), a zero word (emitBlankVectorWordTag70), then eight-digit runs keyed by DIP ports loc_d00/loc_e00 and a pot-status byte.", cert: "seen" },
  0xdd27: { name: "emitByteBitsAsDigitsFixed", role: "[seen] Fixed-position front feeding the coordinate deltas 0xd0/0xf8 into the eight-bit byte-digit emit, displaying the byte in y.", cert: "seen" },
  0xdd29: { name: "emitByteBitsAsDigitsAtF8", role: "[seen] Fixed-position front feeding coordinate x = 0xf8 into the eight-bit byte-digit emit.", cert: "seen" },
  0xdd2b: { name: "emitByteBitsAsDigits", role: "[seen] Stashes the byte y into loc_35, scales the two coordinates, then shifts loc_35 out MSB-first emitting each of its eight bits as one vector digit.", cert: "seen" },
  0xdd41: { name: "buildLargeDecimalNumber", role: "[seen] Doubles-and-adds two little-endian input pairs into the math-box operands loc_6095/loc_6096 (floored to one), seeds a divide, emits a header, then makes repeated passes of binary-to-BCD double-dabble over the three-byte source at loc_3b/loc_3c emitting each pass's digits with a scaled coordinate record.", cert: "seen" },
  ...
  0xddf7: { name: "requestWriteLowRegions", role: "[seen] request a (non-blanked) EAROM write of the two low NVRAM regions: pass the fixed mask 0x03 to the shared zeroed-index merge tail loc_ddfd, which ORs the mask into the region-pending loc_1c7 and direction loc_1c8 cells with a cleared blank-index loc_1c6.", cert: "seen" },
  ...
  0xddfd: { name: "queueEaromRegionSave", entry: "queueEaromRequestAtIndexZero", role: "[seen] queue an EAROM (high-score NVRAM) request at cell index 0: store 0x00 into the target-index cell loc_1c6 and OR the caller's mask A into both request-flag cells loc_1c7 and loc_1c8 (shared tail queueEaromRequest); entry queueEaromRegionSave presets mask 0x04.", cert: "seen" },
  0xddff: { name: "queueEaromRegionSave", entry: "queueEaromRequest", role: "[seen] queue an EAROM (high-score NVRAM) request: store Y into the target-index cell loc_1c6 and OR the request mask A into both request-flag cells loc_1c7 and loc_1c8.", cert: "seen" },
  ...
  0xdf09: { name: "emitRecordBodyC0", role: "[seen] Stores the fixed body byte 0xc0 at the draw cursor origin loc_74 and runs the shared record-tail emit.", cert: "seen" },
  0xdf0d: { name: "emitHeaderedBodyRecord", role: "[seen] build a two-byte-header record: emit the {0x40,0x80} header via 0x74, then store the fixed body byte 0x20 at the cursor origin and run the shared record tail.", cert: "seen" },
  0xdf12: { name: "emitHeaderedBodyRecord", entry: "emitRecordBodyByte", role: "[seen] store one body byte (A) at the display cursor origin (0x74)+0 and continue into the shared record tail (0xdfac).", cert: "seen" },
  ...
  0xdf24: { name: "emitStrokeWordFromNibblePlusOne", entry: "emitStrokeWordByIndex", role: "[seen] emit one glyph/stroke vector word by table index A: copy the two bytes of ROM stroke-table loc_31e4 entry (loc_31e4 + (A<<1)) into the vector list at write cursor mem16[loc_74] and step the cursor past them via advanceDisplayCursor (entry emitStrokeWordFromNibblePlusOne first maps a low nibble to index (nibble&0x0f)+1).", cert: "seen" },
  0xdf39: { name: "emitCoordinateVectorWord", role: "[seen] Emits a coordinate vector word: high byte is a's upper nibble tagged 0xa0, low byte is x shifted right with a's carry rotated into bit7, written through loc_74 then advanced (or, on cursor wrap, tailing emitTaggedVectorWord with key loc_73).", cert: "seen" },
  0xdf4a: { name: "emitVectorWordTag60FromKey", role: "[seen] Emits a vector word tagged 0x60 using the key byte loc_73 as its data byte through the draw cursor loc_74.", cert: "seen" },
  0xdf4c: { name: "emitTaggedVectorWord", role: "[seen] emit one vector word through cursor 0x74: first byte the Y payload, second byte the A payload tagged with header bits 0x60 (via loc_df57).", cert: "seen" },
  ...
  0xdf6a: { name: "emitBlankVectorWordTag70", role: "[seen] Emits a 0x70-tagged vector word with a zero data byte through the draw cursor loc_74.", cert: "seen" },
  0xdf6c: { name: "emitVectorWordTag70", role: "[seen] Emits one vector word through the draw cursor loc_74 -- first byte the y payload, second byte the a payload OR 0x70 -- then advances.", cert: "seen" },
  0xdf73: { name: "emitKeyedScaledCoordinateRecord", role: "[seen] Stashes the index byte into the key cell loc_73, then scales the two coordinates and emits the scaled coordinate record.", cert: "seen" },
  ...
  0xdfac: { name: "emitCoordinateRecord", entry: "emitRecordTailByte", role: "[seen] Stores one final record byte at the next draw-cursor slot loc_74 and either advances the cursor or, on wrap to zero, runs the terminating nibble run.", cert: "seen" },
