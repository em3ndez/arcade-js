...
export const ROUTINES = {
  ...
  0x9a87: { name: "dispatchListSetupByColumn", role: "[seen] route to the list-setup entry for column X: copy X into the dispatch index and enter the computed jump loc_9a88, tail-returning the selected entry's result to placeSpawnListForColumnDeficit.", cert: "seen" },
  ...
  0xda62: { name: "runSelfTestLoop", role: "[code] run the self-test session: seed the state machine (armEaromReadback), forward a pending request (queueEaromEraseAllRegions), copy the 8-byte colour table loc_daf9 into colour RAM at 0x800, idle coin/flip 0x4000; then each pass strobe watchdog 0x5000/display-reset 0x5800, sample option switches 0x60c8 and diagnostics 0xc00, build/show a frame via loc_db0f/emitHeaderedBodyRecord, call stepEaromTransfer every fourth frame, until the self-test switch (0xc00 bit4) is released.", cert: "code" },
  ...
  0xc90c: { name: "resetLevelPlayfieldSlots", role: "[seen] reset the per-slot playfield state: run setup passes loc_aba2/buildLevelLayout (and clearChannelStagingBlock when 0x5 negative), clear 0x49, walk every slot from 0x3e down to 0 seeding 0x48,slot from 0x158 and 0x46,slot=0xff, clear 0x3f and 0x115, reload 0x3d from 0x3e, then tail-delegate to selectWaveStartSlot.", cert: "seen" },
  0xc940: { name: "setupLevelTimers", role: "[seen] set up the level: seed sizing/timer cells loc_1=0, loc_00=30, loc_2=30, and when the level id loc_3f differs from last-seen loc_3d latch loc_3d and (with loc_5 negative) install the new-level timers loc_1=14/loc_00=10/loc_4=40or80 (by loc_117) via swapParallelTables, then run selectProjectionScale, index loc_46 by loc_3d into loc_9f, run loc_9025 and tail-delegate to the readout reset resetBothPokeyChips.", cert: "seen" },
  0xc98c: { name: "bumpLevelEnemyQuota", role: "[seen] ramp the per-level enemy quota: while the loc_46-slot indexed by level loc_3d is below 0x62, bump that slot and the working copy loc_9f together, seed loc_00=0x18, and when the loc_102-slot is nonzero run the handler chain (seatInPagePointer, addBcdScoreAndAwardAtThreshold, loc_ccb9), then tail-delegate to init loc_9009.", cert: "seen" },
  ...
  0x97f8: { name: "advanceMovingSpike", role: "[code] step the moving spike each frame while loc_201 bit7 is clear and arm flag loc_106 bit7 is set: cue a start sound at trigger height loc_202==0x10, advance 16-bit height loc_107/loc_202 by loc_104/loc_105 (park loc_202=0xff, request mode loc_0=0x0e, cue end sound on ceiling overflow), rebuild the spike table (rebuildSpikeTable) past 0x50, rederive the per-frame delta from loc_9f, then scan loc_3ac for the player-segment lane loc_200 and register a collision (loc_cd06/loc_a347/clearActiveShots, clear loc_115).", cert: "code" },
  ...
  0x9b07: { name: "setupEnemyCoordList", role: "[seen] set up the coordinate/shape list for the packed index in loc_2b (saving/restoring the caller index in loc_36): when the held count loc_29 >= 0x20 select a list-setup entry through dispatcher loc_9a88, otherwise seat the pointer pair directly at that index via seatCoordListPointer.", cert: "seen" },
  ...
  0xa1fa: { name: "advanceShotAndScoreLaneHit", role: "[code] advance shot slot x's counter loc_2d3,x toward the per-lane target-depth loc_3ac,y (y=loc_2ad,x); on reaching it shrink/clear loc_3ac,y, bump the hit tally loc_2f2,x, flag the target loc_39a,y=0xc0, chime (loc_ccf6) and award (addBcdScoreAndAwardAtThreshold), returning the live slot; a second hit spends the shot (clear loc_2d3,x, drop the shot count loc_135).", cert: "code" },
  ...
  0xac07: { name: "noRebuildRequestReturn", role: "[code] no-op return tail taken by loc_aba2 when no rebuild request bit is set in loc_1c9; does nothing and returns.", cert: "code" },
  ...
  0xae1c: { name: "seedRngAndDrawCounterPanel", role: "[seen] fold two POKEY random samples loc_60ca/loc_60da into scratch loc_29 and stored nibble loc_11f, draw the counters via loc_af26, then draw the highlighted glyph-row list via loc_ae4e(0xff).", cert: "seen" },
  ...
  0xaf6e: { name: "sharedReturnTail", role: "[code] no-op leaf: return immediately; the shared RTS tail that loc_af26 falls through to when both counters loc_600/loc_601 are zero.", cert: "code" },
  0xaf71: { name: "loc_af71" },
  0xaf77: { name: "loc_af77" },
  0xaf81: { name: "drawTubeWell", role: "[seen] draw the whole playfield well: refresh gates (selectProjectionScale), tick loc_16e, draw the eight rim segments walking loc_37 7->0 (drawSlotShapeRecord(loc_b09b+loc_37)), nudge the depth window pair loc_7b/loc_7c one step toward target loc_200 (bounded by ceiling loc_127), draw five depth rows deepest-first from loc_91fe+loc_3a (skipping rows >=0x63; loc_b0c6/drawTubeShapeOutline), and close with a framing draw and a four-entry trailer walk over loc_b0a3.", cert: "seen" },
  ...
  0xb0d1: { name: "emitColorStatIfChanged", role: "[seen] emit a colour/intensity stat word (loc_df4c with 0x08 and y) into the display list only when it changes: return if loc_9e already equals y, otherwise latch loc_9e=y and emit — the coarse (high-nibble) scale/colour attribute for the current slot.", cert: "seen" },
  ...
  0xb2fe: { name: "closeLayerPointer", role: "[seen] close a layer: emit a header record (loc_df09), seat base pointer from 0xce8c/0xce8d at stride 2*slot, toggle the layer's parity flag 0x415+slot, and write the ($3b) pointer target with the word chosen by parity — 0xceb0/0xceb1 when set, else 0xce9e/0xce9f.", cert: "seen" },
  ...
  0xb5ad: { name: "drawStyledSlotList", role: "[seen] draw a per-slot vector record for seven slots (0x37 from 6 down) when guard 0x106 bit7 is clear: for each non-empty control 0x2df+x cache it in 0x57, split paired byte 0x283+x into style nibble 0x55=(paired&0x18)>>3 and a doubled selector (paired&7)<<1, and dispatch the style's draw handler via loc_b5d7 carrying slot index x.", cert: "seen" },
  ...
  0xc54d: { name: "drawTimedObjectList", role: "[seen] draw eight slots from table 0x3fe while guard 0x115 is set (forcing 0x5f=0xe8, 0x5b=0xff, 0xa0=0x28): per non-empty entry seat 0x57 with 0x56=0x58=0x80, pick a colour mode into 0x9e (slot&7 with 7->4 when 0x9f>=5, else 6), emit via loc_df4c, set style 0x55=((slot&3)<<1)+0x0a, and draw via emitColoredShapeVector; restore the forced cells; tail increments 0x200+0x40 when 0x11f set and 0x42>=0x15.", cert: "seen" },
  ...
  0xc7a0: { name: "runMainFrameLoop", role: "[seen] the main frame loop (a generator): after a one-time board-init pass (resetBothPokeyChips) and seeding 0x00=0, free-run forever -- each pass yields until the interrupt counter 0x53 reaches 9, clears it, and runs the three per-update passes loc_c7bd, seedFramePhaseAndTick, buildFrameVectors (~26.5Hz).", cert: "seen" },
  ...
  0xdbf7: { name: "emitReadoutVectorList", role: "[code] per-frame vector-list emit (handler index 1 of loc_db0f): when the 16-bit counter 0x2e/0x2f is nonzero, seed the POKEY operand cells and run the math-coprocessor scan (runMathboxDivide) to decide 0x78=0xff and the POKEY status byte; advance the 15-bit counter 0x2e/0x2f; build the POKEY work word from 0x4d/0x4e; fire the readout draws; conditionally emit the 0x52-bit marker; walk the 0x7d (x=11..0) and 0x78 (x=4..0) emit tables; then tail-delegate the 0x50-indexed colour pair to the colour-pair emitter emitKeyedScaledCoordinateRecord.", cert: "code" },
  ...
  0xdf39: { name: "emitCoordinateVectorWord", role: "[code] Emits a coordinate vector word: high byte is a's upper nibble tagged 0xa0, low byte is x shifted right with a's carry rotated into bit7, written through loc_74 then advanced (or, on cursor wrap, tailing loc_df4c with key loc_73).", cert: "code" },
