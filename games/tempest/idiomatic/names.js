...
export const ROUTINES = {
  ...
  0xda62: { name: "runSelfTestLoop", role: "[code] run the self-test session: seed the state machine (armEaromReadback), forward a pending request (queueEaromEraseAllRegions), copy the 8-byte colour table loc_daf9 into colour RAM at 0x800, idle coin/flip 0x4000; then each pass strobe watchdog 0x5000/display-reset 0x5800, sample option switches 0x60c8 and diagnostics 0xc00, build/show a frame via dispatchDrawHandler/emitHeaderedBodyRecord, call stepEaromTransfer every fourth frame, until the self-test switch (0xc00 bit4) is released.", cert: "code" },
  ...
  0x902b: { name: "resetWorkingRamForStateEntry", role: "[code] master state-entry sweep: run the six reset/seed leaves back to back (loc_928f, loc_926f, loc_9246, loc_929f, loc_92ad, loc_c16e), then arm loc_124 and loc_148 to 0xff and clear loc_123.", cert: "code" },
  ...
  0x928f: { name: "clearActiveShots", role: "[code] reset the shot bank: zero the 12-byte depth array loc_2d3 (0x0b..0) and both count cells loc_135 and loc_a6 to baseline.", cert: "code" },
  ...
  0x96b7: { name: "fetchCoordListEntryByCounter", entry: "fetchCoordListEntryByIndex", role: "[code] sibling of 0x96ab that uses the raw index 0x2b (not the counter-wrapped value): stash Y at 0x29, subtract the list byte two entries back, add back saved Y, re-index the pointer, and load and return that entry.", cert: "code" },
  0x96c4: { name: "fetchCoordListEntryByCounter", entry: "readCoordListEntry", role: "[code] the bare coordinate-list read: return the byte at pointer 0x2c offset by Y, with no index arithmetic.", cert: "code" },
  0x96c7: { name: "advanceCursorPastPackedRecord", role: "[code] cursor-skip helper: advance the list cursor Y by a fixed run to step over a packed record without reading it -- the first entry moves Y forward by three (bumps by one then falls into the second entry), the second entry by two; register only, writes no memory", cert: "code" },
  0x96c8: { name: "advanceCursorPastPackedRecord", entry: "advanceListCursorByTwo", role: "[code] step the list cursor forward past a two-byte record without reading it: return Y+2 (register only).", cert: "code" },
  ...
  0x96db: { name: "resolveCoordListEntryToAbsolute", role: "[code] resolve one list entry to an absolute coordinate: read the byte at pointer 0x2c offset by Y and add the base value in 0x160, returning the 8-bit sum in A.", cert: "code" },
  0x96e2: { name: "sumCoordListEntryRun", role: "[code] fold a run of coordinate-list entries: use 0x96f4 as a repeat count, load the first entry at pointer 0x2c offset Y, then add that many further consecutive entries into a single wrapped one-byte total.", cert: "code" },
  0x96f4: { name: "computeCoordListBackDelta", role: "[code] compute the coordinate-list index delta: record Y at 0x29 and return (0x2b minus the list byte two slots back at pointer+(Y-2)) & 0xff; a leaf whose result is consumed downstream.", cert: "code" },
  0x9700: { name: "computeCoordListBackDelta", entry: "selectListEntryByDeltaParity", role: "[code] pick one source-list entry: run computeCoordListBackDelta to form base 0x2b minus the table byte two slots back, advance cursor Y by the low bit of that delta, and return the entry at (mem16[0x2c]+Y).", cert: "code" },
  0x9749: { name: "rotateBlasterAroundRim", role: "[code] advance the rim rotation/spinner state: skip while fine-angle flag loc_201 bit7 set; take the delta from manual reading loc_50 (clamped to band [0xe1,0x1f], then consumed) when loc_5 bit7 is set else the auto-aim from aimSpinnerAtNearestEnemy; fold it into work cells loc_2b/loc_2c, and on a live level (loc_111!=0) cap loc_2c to 0xef and saturate toward the stored sign on a sign flip; the high nibble becomes coarse angle loc_2a, ring sound cueRimRotationSound on a changed coarse angle, then commit loc_200/loc_201/loc_51.", cert: "code" },
  ...
  0x9a9d: { name: "seatDemoCoordListPointer", role: "[code] seat the index-0 coordinate list: take the low pointer byte from fixed table byte 0x9b02, mark index 0 at 0x2b, take the high pointer from held source cell 0x15d into 0x2d, reload A from 0x29.", cert: "code" },
  0x9ab7: { name: "seatDemoCoordListPointer", entry: "seatCoordListPointerAtIndex3", role: "[code] preset the selecting index to 3 and run the shared 0x9aee seating, parking the coordinate-list pointer 0x2c/0x2d from ROM tables 0x9b02[3]/0x9afd[3].", cert: "code" },
  ...
  0x9cb6: { name: "steerSlotCoordinate", role: "[code] step slot x's coordinate keyed on loc_28a,x bit7 (sub-step via reverseEnemyLaneDepth flipping direction at the loc_157 threshold when loc_3ab is set, else add-step via advanceEnemyLaneDepth), and on the common tail -- loc_148 bit7 clear AND loc_2df,x < loc_157 AND loc_200==loc_2b9,x AND loc_201==loc_2cc,x -- seed a fresh object for that slot via insertObjectHeadTag7 with the stepped Y.", cert: "code" },
  0x9bcf: { name: "noopDispatchStub", role: "[code] no-op leaf that returns immediately, occupying a slot in a computed-dispatch set so selecting it falls straight back to the caller.", cert: "code" },
  ...
  0x9c21: { name: "setFlagIfSlotPastSegmentBound", role: "[code] boundary-test opcode: read the slot's segment loc_2b9,x, look up its boundary in loc_3ac (a zero entry reads as the maximum 0xff), and set the branch flag loc_10c to 1 when the boundary is at or beyond the slot's depth loc_2df,x, else 0", cert: "code" },
  0x9c3b: { name: "setFlagFromPhaseAccumulatorSign", role: "[code] phase-probe opcode: form ((loc_147<<2)+loc_148) & loc_148 & 0x80, XOR against 0x80, and store to the branch flag loc_10c so it becomes 0x00 when that high bit is set and 0x80 when clear", cert: "code" },
  ...
  0x9c99: { name: "stepEnemyDepthInLaneDirection", entry: "reverseEnemyLaneDepth", role: "[code] subtract-direction depth step: loc_29f,x -= loc_160,seg (16-bit, with borrow) into loc_2df,x, flooring the high byte to 0xf2 when it underflows past 0xf0; returns the new (or floored) high byte.", cert: "code" },
  ...
  0x9d67: { name: "faceEnemyTowardPlayerSegment", role: "[code] aim slot x's turn side toward the player: take the signed segment delta of loc_2b9,x against player segment loc_200 via signedSegmentDelta, then clear bit6 of loc_283,x when the delta is negative (bit7 set) and set it otherwise.", cert: "code" },
  ...
  0x9e2f: { name: "spawnType5OnCoordMatch", role: "[code] spawn a type-5 object when a live slot (loc_283,x bit7 clear) has its coordinate pair loc_2b9,x/loc_2cc,x matching the current target pair loc_200/loc_201.", cert: "code" },
  0x9e48: { name: "fireHitOnPlayerCollision", role: "[code] fire the player-collision hit when a slot's depth loc_2df,x matches loc_202 and its segment loc_2b9,x matches loc_200.", cert: "code" },
  ...
  0x9eab: { name: "keepClimberFlipBitByDepth", role: "[code] keep the climber's flip bit (bit6 of loc_283,x) in step with its segment depth loc_2b9,x while gate loc_111 is on: when bit6 is set clear it once depth reaches 0x0e; when bit6 is clear set it only while depth is 0.", cert: "code" },
  0x9ed7: { name: "lookupRingHeading", role: "[code] look up a climber heading from the ring-direction table loc_3ee,y with bit7 forced on; when the caller's bit6 is set take the half-turn first -- index y=(y-1)&0x0f, value (loc_3ee,y+8)&0x0f.", cert: "code" },
  ...
  0xa343: { name: "insertObjectHeadTag9", role: "[code] insert a type-1 object after stamping head flag loc_13b with 0x09.", cert: "code" },
  0xa347: { name: "insertObjectHeadTag9", entry: "insertObjectHeadTag7", role: "[code] insert a type-1 object after stamping head flag loc_13b with 0x07.", cert: "code" },
  0xa34b: { name: "primeTopPriorityObject", role: "[code] prime a fresh top-priority object: stamp head flag loc_13b=0xff and run the type-1 insert.", cert: "code" },
  ...
  0xa3d4: { name: "insertTimedObjectOfType", role: "[code] insert a timed object of a given type: stash A into the type scratch loc_2c, then insert into the 8-slot table.", cert: "code" },
  ...
  0xa65b: { name: "spawnEnemyInSlot", role: "[code] spawn an enemy into free slot x: mark loc_263,x/loc_283,x/loc_2a3,x = 0x80, seed three velocity/coordinate pairs from POKEY draws loc_60da/loc_60ca (raw into loc_2c3/2e3/303,x, signed nudge from drawSignedVelocityNudge into loc_323/343/363,x, forcing the middle nudge non-positive), then cue the spawn sound gateSound1f.", cert: "code" },
  0xa69b: { name: "drawSignedVelocityNudge", role: "[code] produce a signed random velocity nudge: take a 3-bit magnitude (loc_60da & 0x07, 0..7) and negate it when the caller's incoming value has bit0 set, yielding a signed step in [-7,+7] consumed by spawnEnemyInSlot as a whole-velocity seed.", cert: "code" },
  0xa6a9: { name: "advanceEnemyFreeFlight", role: "[code] integrate slot x's three free-flight axes: fold each low velocity (loc_2e3/2c3/303,x) into its fraction (loc_223/203/243,x) and add carry+signed whole (loc_343/323/363,x) into a whole coordinate, committing loc_263,x and loc_2a3,x and loc_283,x, but forcing loc_283,x to 0 (retiring the slot) if any axis crosses the tube ring [0x10,0xf0).", cert: "code" },
  0xa721: { name: "decayEnemyFreeFlightVelocity", role: "[code] decay slot x's free-flight velocities: seed saturation counter loc_29=0xfd, step each velocity pair (loc_2c3/323, loc_2e3/343, loc_303/363,x) one increment toward zero via stepVelocityTowardZero, and clear the slot's whole coordinate loc_283,x only when all three axes saturate in the same frame.", cert: "code" },
  0xa75d: { name: "stepVelocityTowardZero", role: "[code] step one signed 16-bit velocity (whole:low) one fixed increment loc_a788 (0x20) toward zero (add when whole negative, subtract otherwise), snapping low loc_2a=0 and bumping saturation counter loc_29 on crossing zero; returns the stepped [low,whole].", cert: "code" },
  0xa789: { name: "resetPerSlotStateTable", role: "[code] slot-arm init leaf: zero the sixteen-byte per-slot state/flags table loc_283..loc_283+0x0f, then re-seed scalars loc_10e=0x20, loc_10d=0x20, loc_1=0x04, loc_68=0 and loc_69=0.", cert: "code" },
  ...
  0xa7bd: { name: "rebuildSpikeTable", role: "[code] rebuild the spike table: zero the 8-byte array loc_3fe (7..0), stamp its last slot loc_405=0xf0, and arm the remap reference loc_115=0xff.", cert: "code" },
  ...
  0xaa13: { name: "stageTextLineWithCount", role: "[code] stage a text line into the vector text buffer at 0x2f60: pick a string index in loc_3e (forced to 0x01 when loc_5 bit7 is clear and any of loc_43/loc_44/loc_45 is set), seat the write cursor loc_74/loc_75 at 0x2f60, copy loc_ce66[index]+1 stroke bytes from ROM loc_cde6 into it, and on the loc_5 bit7 path also emit the BCD of (loc_9f+1) at 0x2fa6 before restoring the cursor low byte and terminating the record via emitRecordBodyC0.", cert: "code" },
  0xaa5a: { name: "drawFrameWithSlot08", role: "[code] compose a frame led by slot 0x08: draw slot 0x08 (drawSlotShapeRecord) then chain prepCountThenComposeFrame (the count prep followed by the per-frame composition loc_a8e7).", cert: "code" },
  0xaa62: { name: "drawFrameWithSlot00", role: "[code] compose a frame led by slot 0x00: prime slot 0x00 with header 0x30 (drawSlotShapeWithHeader), run the shared count prep (drawSlotThenDigitRun), then dispatch the per-frame composition loc_a8e7.", cert: "code" },
  0xaa69: { name: "prepCountThenComposeFrame", role: "[code] run the shared count prep (drawSlotThenDigitRun: slot 0x02 + digit run) then dispatch the per-frame composition loc_a8e7.", cert: "code" },
  0xaa6f: { name: "composeFrameThenDrawSlot06", role: "[code] run the alternate per-frame composition loc_a8b4, then prime slot 0x06 through the shared entry drawSlotShapeWithHeader (header 0x00).", cert: "code" },
  0xaa79: { name: "drawFrameWithSlot32", role: "[code] compose a frame led by slot 0x32: draw slot 0x32 (drawSlotShapeWithHeader, header 0x00), add a second draw of slot 0x22 (header 0xe0) only while status nibble loc_3&0x1f < 0x10, then finish with the alternate per-frame composition loc_a8b4.", cert: "code" },
  0xaa92: { name: "drawSlotThenDigitRun", role: "[code] draw fixed slot 0x02 (drawSlotShapeRecord) then hand off to emitCountDigitRun to lay down the numeric run.", cert: "code" },
  0xaa97: { name: "emitCountDigitRun", role: "[code] publish a zero scale header (emitScaleWordIfChanged with 0x00) then emit the one-byte digit run for the slot named by loc_3d via loc_aa9e, drawing the count as nibble digits.", cert: "code" },
  0xaa9e: { name: "emitSlotIndexDigit", role: "[code] Increments the slot index, publishes it into loc_61, and emits that single byte as a digit run.", cert: "code" },
  ...
  0xaaf5: { name: "packBinaryToBcd", role: "[code] Converts the binary byte in A to packed BCD by double-dabble and writes the packed digits into loc_29 and loc_2c.", cert: "code" },
  0xab0d: { name: "emitFixedVectorWord", role: "[code] emit one fixed vector word (bytes 0x20, 0x80) through the draw cursor 0x74 and step past it (via loc_df57).", cert: "code" },
  ...
  0xab3b: { name: "expandShapeListToVectors", role: "[seen] the shared tail all three builders converge on: with loc_2a/loc_2b/loc_35 pre-seeded, clear loc_73, set loc_72=0x01, emit the intensity header (loc_df6a) and beam-position record (emitScaledCoordinateRecord), reload the shape-list pointer from (loc_ac)+loc_35, split scale key loc_d121+loc_35 into emitColorStatIfChanged/emitScaleWordIfChanged, then walk the list copying point pairs from loc_31e4/loc_31e5 into (loc_74) until a high-bit terminator and close with advanceDisplayCursor.", cert: "seen" },
  0xab98: { name: "drawShapeListAtPosition", role: "[code] thin front over the shared builder: seat loc_35=x (slot), loc_2a=a (position seed), and a zero colour header loc_2b=0x00, then fall into expandShapeListToVectors to emit the slot's vector run.", cert: "code" },
  0xaba2: { name: "rebuildControlBlocksIfRequested", role: "[code] guarded front for the rebuilder: refresh via loc_ac20, and if loc_1c9's low two request bits are clear take the no-op tail loc_ac07, otherwise run the rebuild path loc_abac.", cert: "code" },
  0xabac: { name: "rebuildControlBlocksFromTemplate", role: "[code] control-block rebuilder: refresh via requestRebuildIfSwitchesChanged, write loc_100=0x08, raise the requests via raiseRebuildRequestBits when (loc_71b|loc_71c|loc_71d) are idle, then keyed on loc_1c9's low two bits copy template loc_ac08 into loc_606 (top 0x17 if bit0 else 0x0e) and fill loc_706 with 0x01 (top 0x17 if bit1 else 0x0e), latch loc_71e=(loc_a&0xf8)/loc_71f=(loc_16a&0x03) when any request bit was set, and clear the two request bits (loc_1c9 &= 0xfc).", cert: "code" },
  0xac07: { name: "noRebuildRequestReturn", role: "[code] no-op return tail taken by rebuildControlBlocksIfRequested when no rebuild request bit is set in loc_1c9; does nothing and returns.", cert: "code" },
  0xac20: { name: "requestRebuildIfSwitchesChanged", role: "[code] refresh the live option/switch snapshot via decodeOptionSwitches, then compare (loc_a & 0xf8) to cached loc_71e and (loc_16a & 0x03) to cached loc_71f: a match takes the no-op tail switchesUnchangedReturn, a mismatch calls raiseRebuildRequestBits to raise the pending-rebuild request bits.", cert: "code" },
  ...
  0xac3e: { name: "switchesUnchangedReturn", role: "[code] shared no-op return tail taken by requestRebuildIfSwitchesChanged when the option/switch snapshot still matches the cached targets; does nothing and returns.", cert: "code" },
  ...
  0xad6e: { name: "tickActiveSoundSlot", role: "[code] per-frame tick of the active slot: set loc_1=0x06; while the low 5 bits of loc_3 are clear run countdown loc_605 down and on zero write loc_00=0x14 and return; otherwise clamp the active slot value loc_606,slot via foldStepIntoFraction (negative->0x1a, >=0x1b->0), gate on bits 3-4 of loc_4e (clearing them plus bit7), and when gated step cursor loc_602 and counter loc_604 -- on underflow re-arm (requestWriteLowRegions when loc_600,loc_3d < 4, then armRequestedSoundSlot), else clear the retired slot loc_606,slot-1.", cert: "code" },
  0xadce: { name: "foldStepIntoFraction", role: "[code] fold the signed sub-step loc_50 (times 8) into the fraction cell loc_51, add the fold carry plus loc_50's sign-extension into A, and clear loc_50; returns the updated whole byte in A.", cert: "code" },
  0xadea: { name: "drawScoreDeltaPanel", role: "[code] draw a fixed frame (loc_ab17/loc_ab14 chain), tick the countdown loc_16e down by one, then hand the score delta loc_602-loc_604 to the glyph-row builder drawHighlightedGlyphRowList.", cert: "code" },
  ...
  0xaf26: { name: "drawCounterPair", role: "[seen] when either counter byte loc_600/loc_601 is nonzero, draw a shared header (loc_ab14(0x12), loc_af71(0x63)) and both counter slots via drawCounterSlot(0x00) and drawCounterSlot(0x01); otherwise return through the bare tail loc_af6e.", cert: "seen" },
  0xaf3f: { name: "drawCounterSlot", role: "[seen] draw one counter slot x: return when count loc_600+x is zero, else record the slot in loc_2e, position it at loc_af6f+loc_2e (loc_df75), and emit the capped count (loc_af71, loc_b56a, loc_ab98, loc_aa9e).", cert: "seen" },
  0xaf6e: { name: "sharedReturnTail", role: "[code] no-op leaf: return immediately; the shared RTS tail that drawCounterPair falls through to when both counters loc_600/loc_601 are zero.", cert: "code" },
  0xaf71: { name: "loc_af71" },
  0xaf77: { name: "emitByteAsBcdDigits", role: "[code] Packs the incoming byte to BCD (into loc_29) then emits that single zeropage byte as its two decimal nibbles.", cert: "code" },
  ...
  0xb0ab: { name: "nudgeBlasterRimPosition", role: "[code] nudge the blaster's rim position loc_200 by its signed sub-step (via foldStepIntoFraction), clamp into [0, ceiling loc_127] (a negative result floors to 0, an over-ceiling result pins to loc_127), write it back to loc_200, and return it in both A and Y.", cert: "code" },
  0xb0c6: { name: "emitTableValueDigitRun", role: "[code] Seats a table pointer by index (via loc_91b5) then emits the three-byte zeropage run at loc_29 as nibble digits.", cert: "code" },
  ...
  0xb0dd: { name: "emitScaleWordIfChanged", role: "[seen] emit a scale word (loc_df6a with a) into the display list only when it changes: return if a already equals loc_72, otherwise latch loc_72=a and emit — the fine (low-nibble) scale attribute for the current slot.", cert: "seen" },
  ...
  0xb102: { name: "advanceSpreadingSpanAnimation", role: "[code] redraw the paired-cursor span via emitSegmentedSpanBetweenCursors(0x34,0xaa) then spread the two cursors apart one frame: wrap the far cursor loc_14e up by 0x14 while below 0xa0, and once it clears 0x50 step the near cursor loc_14d up by 0x08, pinning it at 0xa0 and latching phase byte loc_1=0x14 when near reaches far.", cert: "code" },
  0xb131: { name: "advancePinchingSpanAnimation", role: "[code] redraw the paired-cursor span via emitSegmentedSpanBetweenCursors(0x3f,0x4e) then squeeze the two cursors together one frame: decrement the near cursor loc_14d while at/above 0x30 (bail if it wraps to >=0x80) and pull the far cursor loc_14e down by one but never below the near cursor.", cert: "code" },
  ...
  0xb498: { name: "buildObjectDisplayList", role: "[seen] build a vector display list for up to 0x12 active objects (kind from tag table 0x243, coords via 0x203 into the four coord banks 0x35a/0x36a/0x37a/0x38a): emit a rotated header code, screen-relative coordinate words and their negated shadow words per object through cursor 0x74/0x75, flushing the cursor when the byte offset saturates, then close with a trailing header (loc_df6a).", cert: "seen" },
  ...
  0xb60f: { name: "emitJumpModeSlot", role: "[code] Pairs a jump-mode byte from table loc_b61e (indexed by loc_28a+x & 0x03) with slot target loc_2b9+x and emits via bcfd.", cert: "code" },
  0xb622: { name: "emitAnimatedPhaseSlot", role: "[code] Pairs slot target loc_2b9+x with a four-phase animation offset (((loc_3 & 0x03)<<1)+0x12) and emits via bcfd.", cert: "code" },
  ...
  0xb7eb: { name: "animateShapeOneVector", role: "[code] advance and emit the shape-1 enemy animation: refresh axis params loc_56/loc_58 from loc_435/loc_445 at loc_29, run frame updaters (projectPointThroughMathbox, layHeaderAndBuildRecord), tick sub-timer loc_13c and on wrap advance phase loc_13b and reload loc_13c from loc_b82a+loc_13b, run phase handler dispatchDrawSetup when loc_b83d+loc_13b<0x80, then emit the phase's vector-pair word from loc_cec8/loc_cec9 at ((loc_13b<<1)+0x28)&0xff.", cert: "code" },
  0xb84e: { name: "dispatchDrawSetup", role: "[code] computed-jump dispatcher: caller's Y is a byte offset (0,2,4,6) selecting one of four draw-setup targets loc_b85f/loc_b875/loc_b888/loc_b896, and it tail-returns that routine's result.", cert: "code" },
  0xb85f: { name: "seedTripleArrays", role: "[code] seed the paired three-entry arrays 0x22-0x24 and 0x809-0x80b with the fixed values 0x00, 0x04, 0x0c.", cert: "code" },
  0xb875: { name: "rotateTripleArray", role: "[code] rotate the three-entry array 0x22-0x24 down by one, threading the wrapped value, and mirror each new entry into paired array 0x809-0x80b.", cert: "code" },
  0xb888: { name: "resetVectorTailCursor", role: "[code] rebuild the packed-nibble table via unpackLevelNibbleTables, then seat the vector-list tail cursor 0x139=0x7f and 0x13a=0x04.", cert: "code" },
  0xb896: { name: "emitVectorTailRecord", role: "[code] emit a vector-RAM tail record from cursor 0x139/0x13a — low byte to 0x2ffc, high byte tagged 0x70 to 0x2ffd, 0xc0 terminator to 0x2fff — then step the cursor down by 0x20 with a 16-bit borrow into 0x13a and mask the low byte to 0x7f.", cert: "code" },
  0xb8ba: { name: "drawMovingObjectSlots", role: "[code] draw the 16-slot moving-object cascade into the display list: reset accumulators (loc_6a-loc_6d, loc_202, loc_68/loc_69) with loc_5f=0xe0/loc_5b=0xff, cache the base draw-struct pointer from selectPointerPair into loc_76/loc_77, lay an opening coord (emitCoordinateVectorWord), then count loc_37 from 0x0f down and for each active slot (loc_283+x nonzero) load loc_57/loc_56/loc_58 from loc_283/loc_263/loc_2a3+x, integrate deltas (projectPointThroughMathbox), emit the record body (emitCoordDeltaRecord, emitBlankValueRecord, emitObjectPositionVector) with pointer-swap shadow passes (swapDrawPointers), and set the slot phase into loc_9e; closes by swapping pointers back and finishing the base list (loc_df6a, emitRecordBodyC0).", cert: "code" },
  0xb944: { name: "swapDrawPointers", role: "[code] swap the two 16-bit draw pointers 0x74/0x75 and 0x76/0x77 so the shared cursor addresses the other structure.", cert: "code" },
  ...
  0xb967: { name: "selectPointerPair", role: "[code] select a pointer pair by flag 0x415: zero picks 0xce87/0xce86, nonzero picks 0xce6f/0xce6e; returns the pair as (A,X).", cert: "code" },
  ...
  0xc30d: { name: "initAndDrawRimDepthCounters", role: "[seen] first-time rim-counter setup: when loc_110==0 seed loc_110/loc_10f through the projection integrator projectAllLanesThroughMathbox (nudging the low counter via snapCoordUpToReference when it lags at index 0x0f), always emit a header (loc_df6a, loc_9e=0x06), return unless both counters live and loc_113!=0, then clear the record slots two-at-a-time via drawFramedCounterSlot and draw each counter's record set with drawGatedRecordLoop.", cert: "seen" },
  ...
  0xc765: { name: "layHeaderAndBuildRecord", role: "[code] Writes the fixed header word 0x00/0x71 at the draw cursor start (loc_74/loc_75) then resumes the shared vector-record builder from cursor slot 2.", cert: "code" },
  ...
  0xc774: { name: "emitObjectPositionVector", entry: "emitObjectPositionRecord", role: "[code] emit a six-byte object position record into the vector list at write cursor loc_74/loc_75 offset Y: a fixed header (0x40,0x80) then the object's X pair (loc_2+x low, loc_3+x high masked to 5 bits) and Y pair (loc_00+x low, loc_1+x high masked to 5 bits), caching the raw bytes into loc_6c/loc_6d/loc_6a/loc_6b, then advancing the cursor past the bytes via advanceDisplayCursor (entry emitObjectPositionVector starts the offset at 0).", cert: "code" },
  ...
  0xccb0: { name: "gateSound5f", role: "[code] cue the fixed sound id 0x5f through the sound gate, carrying the caller's X/Y.", cert: "code" },
  0xccb5: { name: "cueRimRotationSound", role: "[code] load fixed sound id 0x0f and pass it through the sound gate loc_ccc3 (keeping caller X/Y); rung by loc_9749 when the coarse rim angle changes, i.e. the spinner-rotation sound cue.", cert: "code" },
  0xccb9: { name: "requestScoreAwardSound", role: "[code] one-line cue: request fixed sound id 0x4f through the enable gate loc_ccc3; caller loc_c98c fires it on the bonus/level-advance handler chain, and the same id 0x4f is fired by the score-award path loc_ca6c, so it voices a score/bonus award.", cert: "code" },
  0xccbd: { name: "gateSound8f", role: "[code] cue the fixed sound id 0x8f through the sound gate, carrying the caller's X/Y.", cert: "code" },
  0xccc1: { name: "gateSound1f", role: "[code] cue the fixed sound id 0x1f through the sound gate, carrying the caller's X/Y.", cert: "code" },
  0xccc3: { name: "requestSoundIfEnabled", role: "[code] sound enable gate: only when bit7 of the enable flag loc_5 is set, forward the sound id in A (with X/Y) to the loader loadSoundVoiceSlots; with sound disabled it returns making no write.", cert: "code" },
  ...
  0xccea: { name: "requestEnemySpawnSound", role: "[code] one-line cue: request fixed sound id 0x2f through the enable gate loc_ccc3, forwarding slot index X; its sole caller is the enemy spawner loc_a23f, so it voices a new enemy entering the tube.", cert: "code" },
  0xccee: { name: "cueMovingSpikeStartSound", role: "[code] register the fixed sound id 0x6f through the sound-enable gate loc_ccc3 -- the moving spike's start cue, fired by loc_97f8 at trigger height loc_202==0x10.", cert: "code" },
  0xccf2: { name: "cueMovingSpikeEndSound", role: "[code] register the fixed sound id 0x7f through the sound-enable gate loc_ccc3 -- the moving spike's end cue, fired by loc_97f8 when the height overflows the ceiling.", cert: "code" },
  0xccf6: { name: "requestSegmentHitSound", role: "[code] one-line cue: request fixed sound id 0x9f through the enable gate loc_ccc3, forwarding X/Y; its sole caller is the per-slot hit/award routine loc_a1fa, so it chimes when a segment/enemy is hit.", cert: "code" },
  0xccfa: { name: "requestActiveSoundCue", role: "[code] request fixed sound id 0xaf by jumping straight into the loader loc_ccc7, bypassing the enable gate; the frame dispatcher loc_c891 calls it every frame while loc_c is live, so it (re)voices the currently-active held cue regardless of the loc_5 enable flag.", cert: "code" },
  0xccfe: { name: "requestLevelIntroSound", role: "[code] one-line cue: request fixed sound id 0xbf through the enable gate loc_ccc3; its sole caller loc_90c4 fires it at phase 3 of the start-slot pick / wave working-set reseed, so it voices the level-intro / skill-step start.", cert: "code" },
  0xcd02: { name: "requestMotionFlipSound", role: "[code] one-line cue: request fixed sound id 0x3f through the enable gate loc_ccc3, threading X; its sole caller loc_9b1e fires it as a correction when the per-slot motion accumulator loc_148 sign-flips, so it voices an enemy direction reversal on the rim.", cert: "code" },
  0xcd06: { name: "cueSpikeCollisionSound", role: "[code] register the fixed sound id 0xcf through the sound-enable gate loc_ccc3, threading X/Y -- the spike-collision cue fired by loc_97f8 when the spike reaches the player segment.", cert: "code" },
  ...
  0xd7e1: { name: "armModeAndRebuildIfEnabled", role: "[code] arm mode bytes loc_5=0x00 and loc_1=0x02, then rebuild only while idle and enabled: bail if loc_1ca is nonzero (busy) or bit4 of option port loc_c00 is clear (disabled), write loc_0=0x00, and run rebuildControlBlocksFromTemplate only when loc_1c9's low two bits show a pending request.", cert: "code" },
  0xd8a9: { name: "emitScaledByteDigit", role: "[code] Stashes A into loc_29, scales the two coordinates (y, x), and emits that one stashed byte as a single-entry digit run.", cert: "code" },
  0xd804: { name: "buildVectorItemList", role: "[code] a trampoline target that assembles the frame's vector item list into display RAM: four setup passes, a header pair, a marker emitted 0x158 times (dec-counted via 0x37), then table-indexed coordinate records selected by state cells 0x16a/0x200/0x4d and the 0xd8b6/0xd8ba/0xd8c2 tables, with a mask-table gated branch (loc_dded/loc_dde9).", cert: "code" },
  0xdb0f: { name: "dispatchDrawHandler", role: "[code] draw-handler dispatch on the display-finalize/self-test path: byte offset in 0x00 (0,2,..,12) selects one of seven per-frame draw handlers (loc_db5a, loc_dbf7, ...) at offset>>1; an out-of-range offset (>=0x0e) is clamped to 0x02 and the clamp persisted to 0x00, then tail-returns the handler's result.", cert: "code" },
  0xdb22: { name: "initVectorDisplayRegisters", role: "[code] reset the $60xx vector-display register bank (zero 0x60e0/0x6080/0x60c0/0x60d0/0x6000/0x6040, four settling reads, then raise 0x60e0 to 0x08), march a single set bit across the 32 slots at 0x6080, and emit one framing word via emitCoordinateVectorWord(0x34,0xa6).", cert: "code" },
  ...
  0xdb84: { name: "emitFixedHeaderAndClearVectorSlots", role: "[code] emit a fixed framing word via emitCoordinateVectorWord(0x33,0x0a), then blank the four even-indexed slots of output tables 0x60c1 and 0x60d1.", cert: "code" },
  0xdb88: { name: "emitVectorHeaderAndClearSlots", role: "[code] emit a caller-supplied header word via emitCoordinateVectorWord(a,x), then blank the four even-indexed slots of output tables 0x60c1 and 0x60d1.", cert: "code" },
  0xdb9a: { name: "stepVectorPhaseAnimation", role: "[code] advance phase counter 0x39 (only while frame gate 0x3 & 0x3f is clear), index three parallel ROM rows (loc_dbd5/loc_dbd6/loc_dfdc) by 0x39 & 0x07 to seed output cells (clear 0x60c1+slotA, write ROM value into 0x60c0+slotB and 0xa8 into 0x60c1+slotB), then emit three words via emitCoordinateVectorWord(0x34,0x56)/emitVectorWordTag70(0x01, 0x3 & 0x7f)/emitCoordinateVectorWord(0x34,0xaa).", cert: "code" },
  0xdbe0: { name: "assemblePotStatusByte", role: "[code] Assembles a pot-status byte from the low three bits of loc_60d8 (mirrored into loc_37 and loc_60cb) merged with one relocated bit lifted from loc_60c8.", cert: "code" },
  0xdce6: { name: "runMathboxDivide", role: "[code] Primes the math-coprocessor operand/count registers loc_608c-loc_6096 from A and X, kicks its divide, spins a 16-step window on loc_6040 for the first ready result, and returns the loc_6060/loc_6070 low/high pair.", cert: "code" },
  0xdbf7: { name: "emitReadoutVectorList", role: "[code] per-frame vector-list emit (handler index 1 of dispatchDrawHandler): when the 16-bit counter 0x2e/0x2f is nonzero, seed the POKEY operand cells and run the math-coprocessor scan (runMathboxDivide) to decide 0x78=0xff and the POKEY status byte; advance the 15-bit counter 0x2e/0x2f; build the POKEY work word from 0x4d/0x4e; fire the readout draws; conditionally emit the 0x52-bit marker; walk the 0x7d (x=11..0) and 0x78 (x=4..0) emit tables; then tail-delegate the 0x50-indexed colour pair to the colour-pair emitter emitKeyedScaledCoordinateRecord.", cert: "code" },
  0xdd0d: { name: "buildPotReadoutVectorList", role: "[code] Builds a diagnostic readout vector list: a fixed header word (loc_df53), a zero word (loc_df6a), then eight-digit runs keyed by DIP ports loc_d00/loc_e00 and a pot-status byte.", cert: "code" },
  ...
  0xdd2b: { name: "emitByteBitsAsDigits", role: "[code] Stashes the byte y into loc_35, scales the two coordinates, then shifts loc_35 out MSB-first emitting each of its eight bits as one vector digit.", cert: "code" },
  0xdd41: { name: "buildLargeDecimalNumber", role: "[code] Doubles-and-adds two little-endian input pairs into the math-box operands loc_6095/loc_6096 (floored to one), seeds a divide, emits a header, then makes repeated passes of binary-to-BCD double-dabble over the three-byte source at loc_3b/loc_3c emitting each pass's digits with a scaled coordinate record.", cert: "code" },
  0xdde9: { name: "loc_dde9" },
  0xdded: { name: "loc_dded" },
  0xddf1: { name: "queueEaromEraseAllRegions", role: "[code] queue a blanked write (erase) of all three EAROM regions: stamp 0xff into blank-flag 0x1c6 and OR mask 0x07 into region-pending 0x1c7 and direction 0x1c8.", cert: "code" },
  ...
  0xddf7: { name: "requestWriteLowRegions", role: "[code] request a (non-blanked) EAROM write of the two low NVRAM regions: pass the fixed mask 0x03 to the shared zeroed-index merge tail loc_ddfd, which ORs the mask into the region-pending loc_1c7 and direction loc_1c8 cells with a cleared blank-index loc_1c6.", cert: "code" },
  ...
  0xddfd: { name: "queueEaromRegionSave", entry: "queueEaromRequestAtIndexZero", role: "[code] queue an EAROM (high-score NVRAM) request at cell index 0: store 0x00 into the target-index cell loc_1c6 and OR the caller's mask A into both request-flag cells loc_1c7 and loc_1c8 (shared tail queueEaromRequest); entry queueEaromRegionSave presets mask 0x04.", cert: "code" },
  0xddff: { name: "queueEaromRegionSave", entry: "queueEaromRequest", role: "[code] queue an EAROM (high-score NVRAM) request: store Y into the target-index cell loc_1c6 and OR the request mask A into both request-flag cells loc_1c7 and loc_1c8.", cert: "code" },
  ...
  0xdf09: { name: "emitRecordBodyC0", role: "[code] Stores the fixed body byte 0xc0 at the draw cursor origin loc_74 and runs the shared record-tail emit.", cert: "code" },
  0xdf0d: { name: "emitHeaderedBodyRecord", role: "[code] build a two-byte-header record: emit the {0x40,0x80} header via 0x74, then store the fixed body byte 0x20 at the cursor origin and run the shared record tail.", cert: "code" },
  0xdf12: { name: "emitHeaderedBodyRecord", entry: "emitRecordBodyByte", role: "[code] store one body byte (A) at the display cursor origin (0x74)+0 and continue into the shared record tail (0xdfac).", cert: "code" },
  ...
  0xdf24: { name: "emitStrokeWordFromNibblePlusOne", entry: "emitStrokeWordByIndex", role: "[code] emit one glyph/stroke vector word by table index A: copy the two bytes of ROM stroke-table loc_31e4 entry (loc_31e4 + (A<<1)) into the vector list at write cursor mem16[loc_74] and step the cursor past them via advanceDisplayCursor (entry emitStrokeWordFromNibblePlusOne first maps a low nibble to index (nibble&0x0f)+1).", cert: "code" },
  0xdf39: { name: "emitCoordinateVectorWord", role: "[code] Emits a coordinate vector word: high byte is a's upper nibble tagged 0xa0, low byte is x shifted right with a's carry rotated into bit7, written through loc_74 then advanced (or, on cursor wrap, tailing emitTaggedVectorWord with key loc_73).", cert: "code" },
  0xdf4a: { name: "emitVectorWordTag60FromKey", role: "[code] Emits a vector word tagged 0x60 using the key byte loc_73 as its data byte through the draw cursor loc_74.", cert: "code" },
  0xdf4c: { name: "emitTaggedVectorWord", role: "[code] emit one vector word through cursor 0x74: first byte the Y payload, second byte the A payload tagged with header bits 0x60 (via loc_df57).", cert: "code" },
  ...
  0xdf6c: { name: "emitVectorWordTag70", role: "[code] Emits one vector word through the draw cursor loc_74 -- first byte the y payload, second byte the a payload OR 0x70 -- then advances.", cert: "code" },
  0xdf73: { name: "emitKeyedScaledCoordinateRecord", role: "[code] Stashes the index byte into the key cell loc_73, then scales the two coordinates and emits the scaled coordinate record.", cert: "code" },
  ...
  0xdfac: { name: "emitCoordinateRecord", entry: "emitRecordTailByte", role: "[code] Stores one final record byte at the next draw-cursor slot loc_74 and either advances the cursor or, on wrap to zero, runs the terminating nibble run.", cert: "code" },
