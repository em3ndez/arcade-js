...
// Memory-equivalence for loc_a398 (ROM 0xa398-0xa3c4) -- seeds COORD_LIST_PTR_HI for slot Y (the ENEMY_SEGMENT,y value,
...
// (LANE_SCORE_INDEX_TABLE indexed by ENEMY_SLOT_FLAGS,y & 7) to loc_ca6c. Both spawn callees preserve Y (they park it in SAVED_INDEX2
...
// Seed slot Y for a clean, non-throwing run: gate STATUS_FLAGS bit7 clear (loc_ca6c returns early) and the
// spawn gate ENEMY_SLOT_DIR,y low bits clear (loc_a06f retires the slot but skips the draw sub-calls). `desc`
// carries the top-two-bit flag under test plus a lane in bits 0..2.
const SLOT = 0x03;
function seed(m, desc) {
  m.regs.y = SLOT; m.regs.x = 0x00;
  m.mem.write8(STATUS_FLAGS, 0x00);                     // loc_ca6c gated off -> no draw/award
  ...
  m.mem.write8(ENEMY_DEPTH + SLOT, 0x05);           // slot occupied -> loc_a06f retires it
  m.mem.write8(ENEMY_SLOT_DIR + SLOT, 0x00);           // spawn gate 0 -> loc_a06f skips the draw sub-calls
  m.mem.write8(PLAYER_SHOT_DEPTH, 0x00);                  // no match -> loc_a06f drops the total counter
...
test("TEETH: a twin that never seats COORD_LIST_PTR_HI MUST diverge in RAM", () => {
  ...
  // signature write. loc_a398 unconditionally seats COORD_LIST_PTR_HI, so this alone guarantees a RAM divergence.
