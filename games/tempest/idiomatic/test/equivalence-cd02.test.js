...
// (always taken) into the sound gate. The idiomatic dissolves the m.call(0xccc3) into loc_ccc3(m, 0x3f);
...
test("TEETH: a twin that seeds the WRONG sound id diverges from the oracle (gate open)", () => {
  ...
  const brokenCd02 = (m, x = m.regs.x, y = m.regs.y) => {
    loc_ccc7(m, 0x00, x, y); // BUG: wrong sound id (0x00 instead of 0x3f)
  };
  brokenCd02(c);
  const d = ramDiff(o, c);
