...
// Memory-equivalence for noRebuildRequestReturn (ROM 0xac07) -- a bare RTS leaf / dispatch stub. No RAM write, no
...
// Run: node --test games/tempest/idiomatic/test/equivalence-ac07.test.js

import nodeTest from "node:test";
import assert from "node:assert/strict";
import { existsSync, readFileSync } from "node:fs";

import { loc_ac07 as oracle } from "../../translated/loc_ac07.js";
...
test("CAPTURE: real 0xac07 dispatches -- noRebuildRequestReturn == oracle in RAM (-stack)", () => {
  for (const cap of CAPS) {
    const o = cap.clone(), c = cap.clone();
    oracle(o); noRebuildRequestReturn(c);
...
test("SP-TOOTH: the omitted-ret leaf (moved 0) is seam-placeable", () => {
  ...
  const r = seamPlaceable(withOmittedRet, noRebuildRequestReturn, TARGET, m);
  assert.equal(r.placeable, true, `noRebuildRequestReturn must be seam-placeable; got: ${r.error}`);
