...
// Memory-equivalence for requestSegmentHitSound (ROM 0xccf6-0xccf9) -- seeds the fixed sound id 0x9f then BNE-delegates
// (always taken) into the sound gate. The idiomatic dissolves the m.call(0xccc3) into requestSoundIfEnabled(m, 0x9f, x, y);
...
// Run: node --test games/tempest/idiomatic/test/equivalence-ccf6.test.js

import nodeTest from "node:test";
import assert from "node:assert/strict";
import { existsSync, readFileSync } from "node:fs";

import { loc_ccf6 as oracle } from "../../translated/loc_ccf6.js";
...
test("CAPTURE: real 0xccf6 dispatches -- requestSegmentHitSound == oracle in RAM (-stack)", () => {
  for (const cap of CAPS) {
    const o = cap.clone(), c = cap.clone();
    oracle(o); requestSegmentHitSound(c);
...
test("TEETH: a twin that seeds the WRONG sound id diverges from the oracle (gate open)", () => {
  ...
  const brokenCcee = (m, x = m.regs.x, y = m.regs.y) => {
    loadSoundVoiceSlots(m, 0x00, x, y); // BUG: wrong sound id (0x00 instead of 0x9f)
  };
  brokenCcee(c);
  const d = ramDiff(o, c);
...
test("SP-TOOTH: the omitted-ret leaf (moved 0) is seam-placeable", () => {
  ...
  const r = seamPlaceable(withOmittedRet, requestSegmentHitSound, TARGET, m);
  assert.equal(r.placeable, true, `requestSegmentHitSound must be seam-placeable; got: ${r.error}`);
