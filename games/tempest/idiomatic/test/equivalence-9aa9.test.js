...
import { STACK_SCRATCH, loc_29, loc_2b, COORD_LIST_PTR_LO, COORD_LIST_PTR_HI, LIST_SELECT_FLAGS, LIST_PTR_TABLE_HI, LIST_PTR_TABLE_LO1 } from "../names.js";
...
const oracle = (m) => { m.regs.a = m.mem.read8(LIST_PTR_TABLE_LO1) | m.mem.read8(LIST_SELECT_FLAGS); m.regs.y = 0x01; return oracleSetup(m); };
...
function seed(m, a29, held) {
  ...
  m.mem.write8(LIST_SELECT_FLAGS, held); // the second held byte the low pointer ORs in
...
test("CRAFTED: OR marshalling and index-1 setup -- RAM and A equal, both operands feed $2c", () => {
  const rom9b03 = new Machine(ROM, OPTS).mem.read8(LIST_PTR_TABLE_LO1);
  for (const held of [0x00, 0xff]) {
    ...
    assert.equal(c.mem.read8(COORD_LIST_PTR_LO), (rom9b03 | held) & 0xff, `$2c = held-byte OR held-byte (held=${held})`);
...
test("MUTATION: a twin that drops the OR of the second held byte diverges in RAM", () => {
  ...
  const broken = (m) => {
    const { mem8 } = m;
    mem8[COORD_LIST_PTR_LO] = mem8[LIST_PTR_TABLE_LO1]; // BUG: drops the | of the second held byte
    mem8[COORD_LIST_PTR_HI] = mem8[u16(LIST_PTR_TABLE_HI + 0x01)];
    mem8[loc_2b] = 0x01;
