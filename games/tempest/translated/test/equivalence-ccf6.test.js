...
// Equivalence test for loc_ccf6 (ROM 0xccf6) -- lda #$9f then BNE $ccc3 (always taken). Author-derived harness.
...
test("loc_ccf6: A=0x9f, BNE always taken -> tail-call loc_ccc3; 5 T", () => {
  const m = makeMachine();
  loc_ccf6(m);
  assert.equal(m.regs.a, 0x9f, "A = 0x9f");
  assert.equal(m.regs.fZ, false, "0x9f -> Z clear (BNE taken)");
  assert.equal(m.regs.fN, true, "0x9f -> N set");
...
test("loc_ccf6 MUTATION: mischarging the BNE as 4T breaks the golden total", () => {
  const m = makeMachine();
