...
// Equivalence test for loc_ccee (ROM 0xccee-0xccf1) -- seeds A=$6f, unconditional branch-delegate
// to $ccc3 (bne always taken, same-page 3 cyc, no push).
// Run: node --test games/tempest/translated/test/equivalence-ccee.test.js
...
test("seeds A=$6f, delegates to $ccc3 (no push)", () => {
  const m = makeMachine();
  loc_ccee(m);
  assert.equal(m.regs.a, 0x6f, "A seeded to $6f");
