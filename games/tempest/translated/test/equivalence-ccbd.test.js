...
// Equivalence test for loc_ccbd (ROM 0xccbd) -- lda #$8f then BNE $ccc3 (always taken). Author-derived harness.
...
test("loc_ccbd: A=0x8f, BNE always taken -> tail-call loc_ccc3; 5 T", () => {
  const m = makeMachine();
  loc_ccbd(m);
  assert.equal(m.regs.a, 0x8f, "A = 0x8f");
  assert.equal(m.regs.fZ, false, "0x8f -> Z clear (BNE taken)");
  assert.equal(m.regs.fN, true, "0x8f -> N set");
...
test("loc_ccbd MUTATION: mischarging the BNE as 4T breaks the golden total", () => {
  const m = makeMachine();
