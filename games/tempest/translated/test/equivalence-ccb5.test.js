...
// Equivalence test for loc_ccb5 (ROM 0xccb5) -- lda #$0f then BNE $ccc3 (always taken). Author-derived harness.
...
test("loc_ccb5: A=0x0f, BNE always taken -> tail-call loc_ccc3; 5 T", () => {
  const m = makeMachine();
  loc_ccb5(m);
  assert.equal(m.regs.a, 0x0f, "A = 0x0f");
  assert.equal(m.regs.fZ, false, "0x0f -> Z clear (BNE taken)");
...
test("loc_ccb5 MUTATION: mischarging the BNE as 4T breaks the golden total", () => {
  const m = makeMachine();
