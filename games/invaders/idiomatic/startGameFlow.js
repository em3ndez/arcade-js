 ...
 *   (page:0xfc coordinate word loc_21fc/loc_22fc, page:0xfe round counter loc_21fe/loc_22fe, page:0xff
 *   reserve-ship count loc_21ff/loc_22ff). The two flag pairs loc_20e7 and loc_20e5 (one byte per
 *   player) are seeded 0x01/0x01; the loc_20e5 pair is the extra-ship-award flag awardExtraShip later
...
export function* startGameFlow(m, twoPlayerFlag, creditDelta) {
  ...
  // open) and loc_20e5 (the extra-ship-award-available flag awardExtraShip reads and later clears).
  ...
  // and stow it as both players' reserve-ship count (page:0xff, loc_21ff / loc_22ff). loc_00d7 seeds the
  // per-player fleet-delta cells and blanks the fixed strip; then zero both players' round counters
  // (page:0xfe, loc_21fe / loc_22fe) for a fresh game.
  ...
  // Seat both players' field-save coordinate word (page:0xfc, loc_21fc / loc_22fc) to the initial fleet
