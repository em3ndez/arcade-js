...
import { TASK_FLAGS, ATTRACT_ANIM_ACK, ALIEN_SHOT_STEP, ATTRACT_OBJECT_TABLE, loc_2080, ATTRACT_ANIM_OBJ_TEMPLATE, loc_3311 } from "./names.js";
 ...
 *   routine block-copies a fixed ROM descriptor at ATTRACT_ANIM_OBJ_TEMPLATE into the attract-demo object table at
 *   ATTRACT_OBJECT_TABLE (0x10 bytes) -- which is what makes the walker dispatch object handler 0x050e each reveal
 *   cycle -- seeds loc_2080=2 and ALIEN_SHOT_STEP=0xff (an alien-shot step cell; the =2 role at loc_2080 is
...
export function* runHandshakedAttractAnim(m) {
  // Seed the attract-demo object record: block-copy the fixed ROM descriptor (ATTRACT_ANIM_OBJ_TEMPLATE) into the
  // attract object table at ATTRACT_OBJECT_TABLE, which arms object handler 0x050e for the walker to dispatch.
  blockCopy(m, ATTRACT_ANIM_OBJ_TEMPLATE, ATTRACT_OBJECT_TABLE, 0x10);
  // Prime the step's state cells (loc_2080=2 role ungrounded; ALIEN_SHOT_STEP=0xff is an alien-shot step cell).
  m.mem8[loc_2080] = 0x02;
  m.mem8[ALIEN_SHOT_STEP] = 0xff;
