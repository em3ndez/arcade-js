...
export const ROUTINES = {
  ...
  0x062f: { name: "loc_062f", role: "scan five object slots (stride 0x0b) on ACTIVE_PLAYER_PAGE from low byte C-1; live-outs: carry set on the first non-empty slot (else the final pointer add's carry), C decremented once, and L (the found slot's low byte, which the caller feeds to alienIndexToScreenCoords)", cert: "seen" },
  ...
  0x0707: { name: "stopSaucerSound", role: "clear the saucer sound bit: SOUND_PORT3_SHADOW &= 0xfe via loc_19dc, mirror to sound port 3; value-out A", cert: "seen" },
  ...
  0x154a: { name: "clearShotHitAndSilence", role: "clear PLAYER_SHOT_HIT, then loc_19dc(0xf7) masks bit 3 off SOUND_PORT3_SHADOW; value-out A", cert: "seen" },
  0x1554: { name: "countStepsToThreshold", role: "count in C the 0x10 steps that lift A to/above threshold H (pre-normalizing a negative A via loc_1590); live-out A, C, carry clear", cert: "seen" },
  ...
  0x172c: { name: "loc_172c", role: "mode-gated sound step: PLAYER_SHOT_STATUS!=0 -> startSound(0x02), else loc_19dc(0xfd)", cert: "seen" },
  ...
  0x190a: { name: "loc_190a", role: "run the state-2 handler resolvePlayerShotHit, then tail into the fleet edge/direction update reverseFleetAtEdge; RAM-only, callers ignore the result", cert: "seen" },
  ...
  0x19d7: { name: "clearGameActive", role: "store 0 -> GAME_ACTIVE (shared tail loc_19d3); clear the game-active flag", cert: "seen" },
  ...
  0x08d8: { name: "loc_08d8", role: "if ALIEN_COUNT < 9: loc_207e = 0xfb", cert: "seen" },
  ...
  0x097c: { name: "loc_097c", role: "HL = loc_1da0 + clamp-index of A (offset 0 if A<2, 1 if 2<=A<4, 2 if A>=4)", cert: "seen" },
  ...
  0x1581: { name: "loc_1581", role: "compute record pointer HL from index B, offset C, and the record-page cell", cert: "seen" },
  0x1590: { name: "loc_1590", role: "normalize A up in 0x10 steps until non-negative, counting the steps in C", cert: "seen" },
  0x1611: { name: "activePlayerPageBase", role: "HL := page byte (mem[ACTIVE_PLAYER_PAGE]) << 8", cert: "seen" },
  0x176d: { name: "loc_176d", role: "OUT 5 := mem[SOUND_PORT5_SHADOW] & 0x30 (sound-off helper)", cert: "seen" },
  ...
  0x073c: { name: "loc_073c", role: "resolve the sprite descriptor at loc_2087 to its screen address + gfx pointer (resolveSpriteScreenAddr), then blit the sprite column into video RAM (drawSpriteColumn); live-out HL", cert: "seen" },
