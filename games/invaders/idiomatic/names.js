...
export const loc_1c00 = 0x1c00;
export const loc_1cdc = 0x1cdc;
export const loc_1d4c = 0x1d4c;
export const loc_1d50 = 0x1d50;
export const loc_1da0 = 0x1da0;
export const loc_2004 = 0x2004;
export const loc_2005 = 0x2005;
export const loc_2006 = 0x2006;
...
export const ROUTINES = {
  ...
  0x0100: { name: "loc_0100", role: "redraw the pending sprite object: bail to tickDespawnTimer when PRIZE_ACTIVE set; else if the object at (ACTIVE_PLAYER_PAGE:loc_2006) is live build the sprite pointer (loc_2004 id bit0-cleared, rol3 into loc_1c00 bank; +0x30 alt frame via selectAlternateSpriteFrame when loc_2005!=0) and blitShiftedSprite 16 rows at ALIEN_DRAW_ADDR; clears ALIEN_DRAW_PENDING on every non-bail path", cert: "code" },
  ...
  0x0644: { name: "loc_0644", role: "alien-shot cadence step: decrement counter loc_2078; at 3 eraseAlienShot then re-seat ALIEN_SHOT_SPRITE_PTR=loc_1cdc + timers (loc_207c-=2, loc_207b-=2, loc_207d=6) and drawAlienShotWithCollision (tail); at 0 just eraseAlienShot (tail); else idle", cert: "code" },
  ...
  0x070c: { name: "loc_070c", role: "mystery-ship score/draw: set SCORE_ADD_PENDING, load the key at [loc_208d], scan the 4-entry table loc_1d4c for it, copy the parallel loc_1d50 entry into record loc_2087, store key*16 to SCORE_ADD_VALUE, resolveSpriteScreenAddr then drawThreeSprites (tail); live-out HL/DE/C", cert: "code" },
  ...
  0x0935: { name: "loc_0935", role: "extra-ship award: when the active player's flag (loc_1910-2) is set and its tally reached the port-2-bit3 threshold, bump the stored count at the active-page top, redraw the reserve-ship column (RESERVE_SHIP_SPRITE) + lives digit, clear the flag, seat SFX_OFF_TIMER=0xff and cue the award sound (tail startSound 0x10)", cert: "code" },
  0x09ad: { name: "loc_09ad", role: "draw the 16-bit value in DE as four BCD glyphs -- high byte D then low byte E -- via drawBcdByte; live-out HL (advanced two glyph-pairs), DE preserved", cert: "code" },
  ...
  0x14d8: { name: "loc_14d8", role: "state-2 prize-landing handler: rets unless PLAYER_SHOT_STATUS==2; bounds-checks the descent (loc_2029), stands the prize down (state 3 + deactivatePrize) or retires it (markExitingAndRetire) at the edges, else scales coords to grid blocks (scaleXToBlock/scaleYToBlock -> loc_2064), enters state 5, awards score/sound (loc_0a5f), loadSpriteDescriptor + blitShiftedSprite, arms despawn timer loc_2003", cert: "code" },
  ...
  0x1947: { name: "loc_1947", role: "draw the BCD credit tally (loc_20eb) as two decimal glyphs at its slot (loc_3c01) via drawBcdByte (tail dissolved); live-out HL", cert: "code" },
  ...
  0x1a7f: { name: "loc_1a7f", role: "ships display: readActivePlayerPageTopByte gives the count at the active page top; if zero bail; else write count-1 back, drawReserveLifeIcons(count-1) the reserve row, fall through to drawLivesDigit(count)", cert: "code" },
