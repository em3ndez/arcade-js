...
import {
  loc_1b32, loc_2032, ALIEN_SHOT2_STEP_GATE, loc_2035, loc_2046, ALIEN_SHOT_RATE_GATE0, loc_2056, ALIEN_SHOT_RATE_GATE_1,
  ALIEN_SHOT_BLOWUP_TIMER, ALIEN_SHOT_SLOT2_RECORD, ALIEN_SHOT_SLOT2_TEMPLATE,
} from "./names.js";
 ...
 *   the handler for object record 2, whose 16-byte record lives at ALIEN_SHOT_SLOT2_RECORD (0x2030). It drives one of
 ...
 *   16-bit word at ALIEN_SHOT2_STEP_GATE; primes/restores the 11-byte descriptor strip at loc_2035 through the shared
 *   work buffer (copyRecordToWorkBuffer / copyWorkBufferToRecord); stages the two per-column shot-rate
 *   cells loc_2046/loc_2056 into ALIEN_SHOT_RATE_GATE0/ALIEN_SHOT_RATE_GATE_1 for stepAlienShot to read; reads ALIEN_SHOT_BLOWUP_TIMER
 *   (0x2078) to learn whether the shot is exploding; and reseeds the record from the ROM template at
 *   ALIEN_SHOT_SLOT2_TEMPLATE (0x1b30) via blockCopy.
 *
...
export function alienShotSlot2Handler(m) {
  ...
  // The 16-bit gate word ALIEN_SHOT2_STEP_GATE (rec2+8) decides whether the shot steps this pass. Read it once.
  ...
  // Prime the shared 11-byte work buffer (OBJECT_WORK_BUFFER) from this record's descriptor strip at loc_2035
  // (rec2+5). The first argument 0xf9 is the accumulator value copyRecordToWorkBuffer parks into its
  // scratch cell ALIEN_SHOT_SPRITE_FRAME_CEILING (stepAlienShot reads it back), so the copy loop cannot clobber it.
  copyRecordToWorkBuffer(m, 0xf9, loc_2035);
  // Stage the two per-column alien-shot rate cells (loc_2046, loc_2056) into ALIEN_SHOT_RATE_GATE0/ALIEN_SHOT_RATE_GATE_1 — the
  ...
  // Otherwise reseed the entire 16-byte record from its ROM template (ALIEN_SHOT_SLOT2_TEMPLATE -> ALIEN_SHOT_SLOT2_RECORD), returning
