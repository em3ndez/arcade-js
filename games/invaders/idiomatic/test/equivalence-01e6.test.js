...
// Memory-equivalence for loc_01e6 (ROM 0x01e6) -- boot-init: seat DE=ROM template / HL=work-RAM base
...
import { STACK_SCRATCH, loc_1b00, loc_2000 } from "../names.js";
...
test("CAPTURE: real 0x01e6 dispatches -- loc_01e6 == oracle in RAM (-stack)", () => {
...
test("CRAFTED: B bytes copied (ROM template)->(work-RAM base) for several counts, incl. B=0 => 256", () => {
  for (const b of [1, 7, 0xc0, 0x00]) {
    ...
    for (const i of [0, 1, n - 1]) {
      assert.equal(c.mem.read8(loc_2000 + i), c.mem.read8(loc_1b00 + i),
...
test("TEETH: a broken twin (off-by-one copied value) is caught", () => {
  ...
  function loc_01e6_broken(m, b = m.regs.b) {
    const n = b === 0 ? 256 : b;
    for (let i = 0; i < n; i++) m.mem8[loc_2000 + i] = (m.mem8[loc_1b00 + i] + 1) & 0xff; // BUG: value+1
  }
  const b = 0xc0;
  const o = new Machine(ROM); o.regs.b = b; o.regs.sp = 0x2400;
  const c = new Machine(ROM); c.regs.b = b; c.regs.sp = 0x2400;
  ...
  assert.equal(d.addr, loc_2000 & 0xffff);
});
