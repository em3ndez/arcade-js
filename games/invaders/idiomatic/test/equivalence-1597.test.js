...
// Memory-equivalence for loc_1597 (ROM 0x1597) -- fleet edge / direction reversal. Reads FLEET_MOVE_DIR to
// pick an edge column, scans it (loc_15c5); on a hit flips the direction and republishes the derived cells,
// else bails unchanged. Live-out (DERIVED FROM THE ORACLE): RAM only -- FLEET_MOVE_DIR, loc_2008 (step, via
// loc_18f1), loc_2007 (mirrored from loc_200e). Its only caller (loc_190a) ignores the result, so no
...
test("CAPTURE: real 0x1597 dispatches -- loc_1597 == oracle in RAM (-stack)", () => {
...
test("CRAFTED: flips direction + republishes on a hit; bails unchanged on all-zero", () => {
  ...
  for (const t of cases) {
    ...
    if (t.found) {
      ...
      assert.equal(c.mem.read8(CELL_2007), 0x7c, `loc_2007 mirrored from loc_200e: ${tag}`);
    } else {
...
test("SP-TOOTH: the omitted-ret leaf (moved 0) is seam-placeable", () => {
  ...
  m.io.setInte(false);
  const r = seamPlaceable(withOmittedRet, loc_1597, TARGET, m);
  assert.equal(r.placeable, true, `loc_1597 must be seam-placeable; got: ${r.error}`);
});
