...
// Memory-equivalence for loc_0bf1 (ROM 0x0bf1) -- run 0x190a then tail-jump into 0x199a. 0x190a is still
...
// where it found it and the seam completes the ret (SP-TOOTH). Live-out is RAM only (its caller loc_0aea
...
test("CAPTURE: real 0x0bf1 dispatches -- loc_0bf1 == oracle in RAM (-stack)", () => {
...
test("CRAFTED: painting state -- copyright drawn, still equal to the oracle", () => {
  ...
  oracle(o); loc_0bf1(c);
  assert.equal(diffBelow(0x23fe)(o, c), null);
  const p = new Machine(ROM); seatPainting(p); loc_0bf1(p);
...
test("SP-TOOTH: the kept-call omitted-ret leaf (moved 0) is seam-placeable", () => {
  ...
  const r = seamPlaceable(withOmittedRet, loc_0bf1, TARGET, m);
  assert.equal(r.placeable, true, `loc_0bf1 must be seam-placeable; got: ${r.error}`);
