...
// Memory-equivalence for loc_14d8 -- the state-2 prize-landing handler: bail unless PLAYER_SHOT_STATUS == 2
// (a 5 also rets); bounds-check the descent (loc_2029), stand the prize down (state 3 + deactivatePrize) or
// retire it (markExitingAndRetire) at the edges; else scale the descriptor coords to grid blocks (stashed at
// loc_2064), enter state 5, and if the target cell is set blank it, award (loc_0a5f), load the sprite
...
import { STACK_SCRATCH, PLAYER_SHOT_STATUS, PRIZE_ACTIVE, SAUCER_EXITING, ACTIVE_PLAYER_PAGE,
  GAME_IN_PROGRESS, loc_2009, loc_200a, loc_2029, loc_202a, loc_2064, loc_2003 } from "../names.js";
...
test("CAPTURE: real 0x14d8 dispatches -- loc_14d8 == oracle in RAM (-stack)", () => {
...
test("CRAFTED: state != 2 (and the 5 early-out) -> return, nothing touched", () => {
  for (const st of [0x00, 0x01, 0x05, 0x07]) {
    const seed = (m) => { seatCaller(m); m.mem.write8(PLAYER_SHOT_STATUS, st); m.mem.write8(PRIZE_ACTIVE, 0xaa); };
    const o = new Machine(ROM); seed(o);
    const c = new Machine(ROM); seed(c);
    oracle(o); loc_14d8(c);
    assert.equal(ramDiff(o, c), null, `state=0x${st.toString(16)}`);
    assert.equal(c.mem.read8(PRIZE_ACTIVE), 0xaa, `no side effects: state=0x${st.toString(16)}`);
  }
});
...
test("CRAFTED: state 2, descent past 0xd8 -> stand down (state 3, prize deactivated)", () => {
  const seed = (m) => { seatCaller(m); m.mem.write8(PLAYER_SHOT_STATUS, 0x02);
    m.mem.write8(loc_2029, 0xe0); m.mem.write8(PRIZE_ACTIVE, 0x01); };
  const o = new Machine(ROM); seed(o);
  const c = new Machine(ROM); seed(c);
  oracle(o); loc_14d8(c);
  assert.equal(ramDiff(o, c), null);
  assert.equal(c.mem.read8(PLAYER_SHOT_STATUS), 0x03, "state forced to 3");
  assert.equal(c.mem.read8(PRIZE_ACTIVE), 0x00, "prize deactivated");
});

test("CRAFTED: state 2, prize inactive -> return", () => {
  const seed = (m) => { seatCaller(m); m.mem.write8(PLAYER_SHOT_STATUS, 0x02);
    m.mem.write8(loc_2029, 0x40); m.mem.write8(PRIZE_ACTIVE, 0x00); };
  const o = new Machine(ROM); seed(o);
  const c = new Machine(ROM); seed(c);
  oracle(o); loc_14d8(c);
  assert.equal(ramDiff(o, c), null);
...
test("CRAFTED: state 2, descent in [0xce,0xd8) -> mark exiting and retire", () => {
  const seed = (m) => { seatCaller(m); m.mem.write8(PLAYER_SHOT_STATUS, 0x02);
    m.mem.write8(loc_2029, 0xd0); m.mem.write8(PRIZE_ACTIVE, 0x01); };
  const o = new Machine(ROM); seed(o);
  const c = new Machine(ROM); seed(c);
  oracle(o); loc_14d8(c);
  assert.equal(ramDiff(o, c), null);
  assert.equal(c.mem.read8(SAUCER_EXITING), 0x01, "exit flag raised");
  assert.equal(c.mem.read8(PRIZE_ACTIVE), 0x00, "prize deactivated by retire");
});
...
test("CRAFTED: state 2, full commit -> block stashed, state 5, sprite blitted, despawn armed", () => {
  ...
  assert.equal(c.mem.read8(loc_2003), 0x10, "despawn timer armed");
  assert.equal(c.mem.read16(loc_2064), o.mem.read16(loc_2064), "grid-block word matches oracle");
});
...
test("CRAFTED: state 2, gate in [key,0x90) -> gate-compare stand down (before state 5)", () => {
  ...
  const seed = (m) => { seatCaller(m); m.mem.write8(PLAYER_SHOT_STATUS, 0x02);
    m.mem.write8(loc_2029, 0x40); m.mem.write8(PRIZE_ACTIVE, 0x01);
...
test("CRAFTED: state 2, commit path with a zero record cell -> stand down after entering state 5", () => {
  ...
  assert.equal(c.mem.read8(PLAYER_SHOT_STATUS), o.mem.read8(PLAYER_SHOT_STATUS), "state matches oracle");
  assert.notEqual(o.mem.read8(loc_2003), 0x10, "the zero-record bail skips the despawn-arm the full commit does");
});

test("TEETH: a module-mutating twin (grid-block bytes swapped) diverges at loc_2064", () => {
  // Real module shape, one broken step: writes the block word with X/Y residuals swapped.
  function loc_14d8_broken(m) {
    const standDown = () => { m.mem8[PLAYER_SHOT_STATUS] = 0x03; return deactivatePrize(m); };
    ...
    m.mem16[loc_2064] = (residualX << 8) | residualY; // BUG: residuals swapped (should be Y<<8 | X)
  ...
  assert.equal(d.addr, loc_2064 & 0xffff, "first divergence is the low byte of the grid-block word");
});
