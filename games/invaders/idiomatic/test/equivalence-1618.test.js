...
// Memory-equivalence for loc_1618 (ROM 0x1618) -- gated pre-round advance: bail unless the round is armed
// (loc_2015==0xff) and loc_2010/loc_2011/loc_2025 are clear, then step the march pointer (loc_20ed ->
...
import {
  ...
  loc_2010, loc_2011, loc_2015, loc_201d, loc_2025, loc_202d, loc_20ed, loc_20ef,
} from "../names.js";
...
function brokenLoc_1618(m) {
  ...
  if (m.mem8[loc_20ef] === 0) {
    ...
    const ptr = u16(m.mem16[loc_20ed] + 1); // BUG: dropped the >=0x7e wrap
...
function mk(over = {}) {
  const build = () => {
    ...
    m.mem.write8(loc_2010, over.g2010 ?? 0x00);
    m.mem.write8(loc_2011, over.g2011 ?? 0x00);
    m.mem.write8(loc_2025, over.g2025 ?? 0x00);
    m.mem.write8(loc_20ef, over.ef ?? 0x00);
    m.mem.write8(loc_202d, over._2d ?? 0x00);
    if (over.ed != null) m.mem.write16(loc_20ed, over.ed);
...
test("CAPTURE: real 0x1618 dispatches -- loc_1618 == oracle in RAM (-stack)", () => {
...
test("CRAFTED: 0x20ef==0 steps the march pointer and wraps its low byte at 0x7e", () => {
  // no wrap: low byte stays below 0x7e
  {
    ...
    assert.equal(c.mem.read8(loc_2025), 0x01);
    assert.equal(c.mem.read16(loc_20ed), 0x2c01);
  ...
  // wrap: low byte reaches 0x7f (>=0x7e) -> reset to 0x74
  {
    ...
    assert.equal(c.mem.read16(loc_20ed), 0x2c74);
...
test("CRAFTED: 0x20ef!=0 arms/clears the fire latch by the input bit", () => {
  // 0x202d!=0, fire pressed -> no change
  {
    ...
    assert.equal(c.mem.read8(loc_202d), 0x01);
    assert.equal(c.mem.read8(loc_2025), 0x00);
  }
  // 0x202d!=0, fire released -> clear the latch
  {
    ...
    assert.equal(c.mem.read8(loc_202d), 0x00);
  }
