...
// Memory-equivalence for loc_0a59 (ROM 0x0a59) -- "is [loc_2015] == 0xff?". The oracle loads A from
...
// and loc_0a59 sets it (return-assignment bridge) and returns the boolean. It writes no memory, so the
...
test("CAPTURE: real 0x0a59 dispatches -- loc_0a59 == oracle in RAM (-stack) and the Z flag", () => {
  for (const cap of CAPS) {
    const o = cap.clone(), c = cap.clone();
    oracle(o); const b = loc_0a59(c);
...
test("TEETH: a broken twin (compares to 0x00) sets the wrong Z flag", () => {
  // Broken twin of loc_0a59: tests the wrong constant, so for 0xff it clears Z where the oracle sets it.
  function loc_0a59_broken(m) { return (m.regs.fZ = m.mem8[loc_2015] === 0x00); }
