...
import { STACK_SCRATCH, loc_2003, loc_2064, PLAYER_SHOT_STATUS, PRIZE_ACTIVE, SOUND_PORT3_SHADOW } from "../names.js";
...
test("CRAFTED expiry: timer 0x01 -> 0x00, column cleared, tail runs; HL + A live-out", () => {
  ...
  const seed = (m) => {
    m.regs.sp = 0x2400;
    m.mem.write8(loc_2003, 0x01);
    m.mem.write16(loc_2064, prizePos);
  ...
  assert.equal(c.mem.read8(PRIZE_ACTIVE), 0x00, "prize deactivated by the tail");
...
test("TEETH: a module-mutating twin (skips the column clear on expiry) diverges in RAM", () => {
  ...
  const loc_1538_broken = (m) => {
    ...
    return retirePrize(m); // BUG: dropped the column clear
  };
