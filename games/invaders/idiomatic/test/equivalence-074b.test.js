...
// Memory-equivalence for loc_074b -- OR a sound-select bit into the port-5 shadow and latch it
...
import { STACK_SCRATCH, SOUND_PORT5_SHADOW, loc_2087, loc_1d7c } from "../names.js";
...
test("CAPTURE: real 0x074b dispatches -- loc_074b == oracle in RAM (-stack) and HL live-out", () => {
...
// record at loc_2087 -- a=[loc_2087+2], c=[loc_2087+3], b=[loc_2087+4]. loc_074b overwrites the record's
// first word (e,d) with loc_1d7c (the ROM gfx table), so DE := loc_1d7c and drawSpriteColumn reads its B
...
test("CRAFTED: shadow bit raised, record repointed to the table, sprite column blitted; HL := 0x2480", () => {
  ...
  assert.equal(c.mem.read16(loc_2087), loc_1d7c, "record gfx pointer reset to the ROM table");
...
test("TEETH: a twin that ORs the wrong sound bit diverges at the shadow cell", () => {
  // Mutate loc_074b's OWN logic: OR bit 5 instead of bit 4 into the shadow -- a real-logic change.
  function loc_074b_broken(m) {
    const a = (m.mem8[SOUND_PORT5_SHADOW] |= 0x20); // BUG: wrong sound-select bit (0x20 not 0x10)
    latchSoundPort5(m, a);
    m.mem16[loc_2087] = loc_1d7c;
    return loc_073c(m);
  }
  const o = new Machine(ROM); seat(o);
  const c = new Machine(ROM); seat(c);
