...
// Memory-equivalence for loc_1947 -- read the BCD credit tally, seat its screen slot, then draw the byte
...
import { STACK_SCRATCH, loc_20eb, loc_3c01 } from "../names.js";
...
test("CAPTURE: real 0x1947 dispatches -- loc_1947 == oracle in RAM (-stack), HL and DE", () => {
...
test("CRAFTED: the credit cell draws as two glyphs at its slot; HL advances 0x200, DE preserved", () => {
  for (const credit of [0x00, 0x37, 0x99, 0x5a, 0x10]) {
    const seed = (m) => { m.regs.sp = 0x2400; m.regs.de = 0xbeef; m.mem.write8(loc_20eb, credit); };
    ...
    assert.equal(c.regs.hl, (loc_3c01 + 0x200) & 0xffff, `HL advanced two glyph columns: ${tag}`);
  ...
  const c = new Machine(ROM); c.regs.sp = 0x2400; c.mem.write8(loc_20eb, 0x37); c.io.setInte(false);
  loc_1947(c);
  let drew = 0;
  for (let i = 0; i < 8; i++) drew |= c.mem.read8(loc_3c01 + i * 0x20);
  assert.notEqual(drew, 0, "first glyph plotted");
});

test("TEETH: a module-mutating twin (wrong glyph slot) diverges in RAM", () => {
  // Broken twin of loc_1947: seats the glyph base one page too low, so the digits land off their slot.
  const loc_1947_broken = (m) =>
    (m.regs.hl = (loc_3c01 + 0x100) & 0xffff, drawBcdByte(m, m.mem8[loc_20eb])); // BUG: not loc_3c01
  const seed = (m) => {
    m.regs.sp = 0x2400; m.mem.write8(loc_20eb, 0x37);
    for (let a = loc_3c01; a < loc_3c01 + 0x200; a++) m.mem.write8(a, 0xaa);
  };
  const o = new Machine(ROM); seed(o); o.io.setInte(false);
  const c = new Machine(ROM); seed(c); c.io.setInte(false);
  ...
  assert.equal(d.addr, loc_3c01 & 0xffff, "first divergence is the real glyph slot");
});
