...
// glyphs prove the value word and screen address landed. The oracle's draw chain leaves call/ret residue just
// below the entry SP, so that band is excluded from the RAM diff. Each side runs on a fresh clone.
// Run: node --test games/invaders/idiomatic/test/equivalence-192b.test.js
...
const TARGET = 0x192b;
const BASE = 0x20fc; // the player-2 score record this routine seats
...
test("CAPTURE: real 0x192b dispatches -- module == oracle in RAM (-stack)", () => {
...
test("TEETH: a twin seating the WRONG record base diverges in RAM", () => {
  ...
  function module_wrongBase(m) {
    ...
    return (m.regs.hl = (h << 8) | a, drawBcdWord(m, d, e)); // BUG: seats the P1 record, not P2
  }
  const o = new Machine(ROM);
  o.regs.sp = 0x2400;
  // Seed the P2 record (what the real routine draws) and a DIFFERENT P1 record (what the twin draws).
