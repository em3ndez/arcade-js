...
// 0xff -> ret, 0xfe -> skip; else test a 16-bit field + gate byte, edit record in place or DISPATCH the
// record's target via pchl @ 0x026e (dynamic m.call + the 0x026f advance-and-loop continuation).
export function loc_024b(m) {
  ...
  for (;;) {
    ...
    if (regs.fNZ) {
      ...
      regs.c = regs.a; m.step(0x0257, 5);
      regs.or(regs.b); m.step(0x0258, 4);                   // 0257  ora b
      regs.a = regs.c; m.step(0x0259, 5);
      if (regs.fNZ) {
        m.step(0x0277, 10);
        regs.b = regs.dec8(regs.b); m.step(0x0278, 5);      // 0277  dcr b
        regs.b = regs.inc8(regs.b); m.step(0x0279, 5);
        if (regs.fNZ) {
          m.step(0x027d, 10);
        } else {
          m.step(0x027c, 10);
        ...
        if (regs.fNZ) {
          ...
          // 026e pchl: dispatch to the record target (HL); the handler pops its record ptr + rets to the
          // 0x026f continuation, inlined below (pop outer record ptr, HL += 0x10, loop to next record).
          m.step(regs.hl, 5); m.call(regs.hl);              // 026e  pchl
          regs.hl = m.pop16(); m.step(0x0270, 10);          // 026f  pop h
          regs.de = 0x000c; m.step(0x0273, 10);             // 0270  lxi d,0x000c
          regs.addHl(regs.de); m.step(0x0274, 10);          // 0273  dad d
          m.step(0x024b, 10);                               // 0274  jmp 0x024b
          continue;
        }
      }
    } else {
