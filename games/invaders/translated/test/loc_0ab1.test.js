...
// Equivalence test for loc_0ab1 (ROM 0x0ab1-0x0ab5): A=0x40 then tail-delegate into 0x0ad7.
// Pins A, the T-states, and the delegate target (recorded, not run).
//
// Run: node --test games/invaders/translated/test/loc_0ab1.test.js
...
test("loc_0ab1: A=0x40, tail-delegates to 0x0ad7; 17 T", () => {
  const m = makeMachine();

  loc_0ab1(m);

  assert.equal(m.regs.a, 0x40, "A := 0x40");
  ...
  assert.deepEqual(m.pcSeq, [0x0ab3, 0x0ad7], "step boundaries");
});

test("loc_0ab1 MUTATION: `mvi a,0x40` mis-charged 4T (not 7T) is caught", () => {
  const m = makeMachine();
  const rs = m.step.bind(m);
  m.step = (n, c) => rs(n, n === 0x0ab3 ? 4 : c);
  loc_0ab1(m);
