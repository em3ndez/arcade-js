...
| **Per-frame object step from control** | `stepObjectFromControl` (0x1420): picks `DEMO_STEER_DIR` (demo) vs debounced joystick by `GAME_MODE`, hands to the update dispatcher `advanceObjectFrame` | [code] |
| **Object action dispatch** (per-state) | `advanceTrackedObject` (0x13de) / `advanceObjectFrame` (0x1434) / `loc_144c` (first-set-bit command dispatcher) → `windUpObjectMove` (0x1468, gates the move behind a startup wind-up counter) / `loc_16b9`, `stepHighScoreInitialsEntry` (0x4eea) | [code] state machines keyed on a mode byte |
...
| **Object move + terrain-resolve** (both axes) | `stepObjectAndResolveTile` (0x1a02, vertical/climb) and `loc_1704` (horizontal): step the tracked object one frame, then resolve the tile it enters — collect loot, carve, hold, bump-react, or walk on; `resolveObjectTile` (0x186f) locates the cell + dispatches loot/dig/walk | [code] the paired move handlers; axis labels inferred |
...
| **Dig reaction + dig-entity spawn** | `triggerDigReaction` (0x191f): on a diggable tile stages `REACTION_STATE`=3 + dig sprite + dig sound; `spawnDigEntity` (0x28ab) stages a dig entity at the aligned cell and commits it into the map | [code] |
| **Player walk-frame animation + step** | `walkActor` (0x184a step-by-velocity + walk-sprite cycle), `advanceActorWalk` (0x19d0), `drawActorWalkFrame` (0x19e3), `advanceObjectWalkFrame` (0x1659); `loc_186a`→`resolveObjectTile` (cell geometry + tile dispatch) | [code] |
...
| **Background scroll sprite** (the sky UFO?) | `BG_SPRITE_*` (0x80db.. X/frame/attr/Y) bounced by `loc_2f71`, frame set by `setBgSpriteFrame` (0x2fd9), animation advanced by `advanceBackgroundAnimation` (0x2fc0) | [guess] the patrolling top-band object |
| **Column / vertical tile reveal animation** | `reseedColumnAnimation` (0x23e8) + `advanceColumnAnimation` (0x241c) walk; `REVEAL_*` (0x80e4/5/6); the `loc_2f71`/`revealTerrainColumn` (0x2f88)/`drawTerrainColumn` (0x2fb7)/`advanceBackgroundAnimation` (0x2fc0) blitters | [code] the dirt/shaft animation |
| **Board-display setup** (screen rebuild per board mode) | `setupBoardDisplay(m, boardMode)` (0x4b46): clear sprites, wipe tilemap, flood colour RAM, blank staging; doors `loc_4b40`(0x90)/`blankScreen`(0x00)/`loc_4b3c`(0xC0) | [code] |
| **Static attract screens** | `showCreditScreen` (0x021c), `showSetupScreen` (0x3a6f), `showFixedScreen` (0x3b81), `showBonusScreen` (0x3bec), `blankScreen` (0x4b44) — paint a canned screen and hold | [code] |
| **HUD panels & labels** | `drawSharedPanel` (0x3cc1 skeleton), `drawMenLeftPanel` (0x483a "MEN LEFT"/"LAST MAN" + count `0x802b`), `drawCreditsDisplay` (0x4894 "CREDITS"+count), `drawPlayerLabel` (0x47e1 "PLAYERS"), `drawBestScoresTodayLabel` (0x4785), `drawCopyrightLine` (0x492a), `drawSetupCreditsPanel` (0x3d49), `showBonusScreen` (0x3bec), `drawGameOverText`/`drawGameOverLabel` | [code] ROM glyphs decoded + confirmed |
