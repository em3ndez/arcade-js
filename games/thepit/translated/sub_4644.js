...
/**
 * sub_4644  (ROM 0x4644–0x4672) — copies the active player's 5-byte state block
 ...
 * sub_4632 does the reverse copy (save the shared slot back to the player).
 * `ld` moves touch no flags, so the flag residue at `ret` is the `dec a` result.
 */
export function sub_4644(m) {
  const { regs, mem } = m;
