...
/**
 * loc_4683  (ROM 0x4683-0x46a1) — award 20 (BCD) points to the active player's
 * score, then repaint the digits. Calls 0x4c8f (a sound/effect kick), loads the
 * BCD increment 0x0020 into BC, and falls into the shared scorer at loc_4689:
 ...
 * with `jr 0x46af`, a TAIL-jump into loc_46af (splits the BCD bytes into on-screen
 * digit cells); the jr pushes nothing, so loc_46af's own `ret` returns to OUR
 * caller — hence `m.step(0x46af,12); return m.call(0x46af)`, NOT m.call()+m.ret()
 * (which would pop a second word off the stack).
 *
 * loc_4689 is a shared entry (loc_4673 preloads BC=0x0001, loc_467b BC=0x0010);
 ...
 *   46a1  18 0c        jr   0x46af         ; TAIL into loc_46af (repaint digits)
 */
export function loc_4683(m) {
  const { regs, mem } = m;
  ...
  // loc_4689 (shared scorer entry; reached here by fall-through with BC=0x0020)
  ...
  // 46a1  jr 0x46af -- TAIL into loc_46af; it pushes nothing, so loc_46af's own
