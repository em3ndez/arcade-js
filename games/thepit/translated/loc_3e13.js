...
/**
 * loc_3e13  (ROM 0x3E13–0x3E31) — advance the shared colour index at 0x8057 and
 ...
 *   3e1b  18 01        jr   0x3e1e        ; skip loc_3e1d's own `ld e,a`
 *
 *   ---- loc_3e1e : shared tail, also entered by loc_3e1d ----
 ...
 * `ld e,a` at 0x3e1d, which is loc_3e1d's separate entry (it takes the offset in
 ...
 * — the loc_3e01 / sub_003d shape. `inc a` leaves carry alone; `and` clears it.
 *
 * B IS NOT CHECKED FOR ZERO, but here it is a fixed 0x1C, so the loop always runs
 * exactly 28 times.
 */
export function loc_3e13(m) {
  const { regs, mem } = m;
  ...
  m.step(0x3e1b, 4);
  m.step(0x3e1e, 12); // jr 0x3e1e -- skips loc_3e1d's `ld e,a` at 0x3e1d
