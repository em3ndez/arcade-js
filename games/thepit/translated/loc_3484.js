...
/**
 * loc_3484  (ROM 0x3484-0x34d9, The Pit) -- one of four velocity-preset entry
 ...
 * loc_3484, presets BC=0x0001 (C=0x01 = step +1 in X, B=0x00) and D=0x02. The
 * other three entries are 0x3476 (BC=0x00ff,D=0), 0x347d (BC=0x0100,D=1) and
 * 0x348b (BC=0xff00,D=3) -- separate routines that jump into the same 0x3490
 * tail; they are NOT part of loc_3484.
 *
 ...
 *
 * FROM THE loc_3484 ENTRY THE SPRITE BRANCH IS DEAD: B is 0x00, so `bit 0,b` at
 * 0x34a3 always sets Z and `jr z,0x34d2` at 0x34a5 always jumps -- 0x34a7-0x34cf
 * are never executed via this entry (they are live only for the 0x347d/0x348b
 * entries whose B has bit 0 set). They are still translated below because 0x3490
 * is the shared body; the label-dispatch loop simply never dispatches them here.
 * C=0x01 means this entry adds +1 to the X position at 0x34d2; D=0x02 is the
 ...
 * exactly like the sibling loc_3476: `next` holds the ROM address of the block to
 ...
 * is byte-identical to loc_3476's; it is inlined here (not m.call'd) because
 * 0x3490 is not a routine of its own -- it is reachable only by `jr` from the four
 * entry points, exactly as the siblings loc_3476/loc_347d model it.
 */
export function loc_3484(m) {
  ...
  for (;;) {
    switch (next) {
      case 0x3484: {
        regs.bc = 0x0001; m.step(0x3487, 10); // 3484  ld bc,0x0001 -- C=step +1 in X, B=0 (bit0 clear -> sprite branch dead)
        regs.d = 0x02; m.step(0x3489, 7); // 3487  ld d,0x02 -- direction index 2
        m.step(0x3490, 12); next = 0x3490; break; // 3489  jr 0x3490
      }
      case 0x3490: {
        ...
        mem.write8(0x8092, regs.a); m.step(0x34a3, 13); // 34a0  ld (0x8092),a -- publish direction index (D=2)
        regs.bit(0, regs.b); m.step(0x34a5, 8); // 34a3  bit 0,b -- Z = !(bit0 of B)
        if (regs.fZ) { m.step(0x34d2, 12); next = 0x34d2; break; } // 34a5  jr z,0x34d2 (taken; always, for B=0)
        m.step(0x34a7, 7); // 34a5  jr z,0x34d2 (not taken -- dead from loc_3484, B=0)
      ...
      case 0x34c8: {
        ...
        if (regs.fNZ) { m.step(0x34cf, 12); next = 0x34cf; break; } // 34cb  jr nz,0x34cf (taken)
      ...
      case 0x34d2: {
        ...
        regs.add(regs.c); m.step(0x34d6, 4); // 34d5  add a,c -- advance by the X delta (C=+1 here)
        mem.write8(0x8086, regs.a); m.step(0x34d9, 13); // 34d6  ld (0x8086),a
        m.ret(); return; // 34d9  ret
      }
      default:
        throw new Error(
          "loc_3484: dispatch to non-block address 0x" + next.toString(16),
        );
    }
  }
}
