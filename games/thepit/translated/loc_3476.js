...
/**
 * loc_3476  (ROM 0x3476-0x34d9, The Pit) -- one of four velocity-preset entry
 ...
 * whose bit 0 gates the sprite-orientation update) and a direction index D, then
 * converges at 0x3490. This entry, loc_3476, presets BC=0x00ff (C=0xff = step -1
 * in X, B=0x00) and D=0x00. The other three entries are 0x347d (BC=0x0100,D=1),
 * 0x3484 (BC=0x0001,D=2) and 0x348b (BC=0xff00,D=3) -- separate routines that
 * jump into the same 0x3490 tail; they are NOT part of loc_3476.
 *
 ...
 *
 * FROM THE loc_3476 ENTRY THE SPRITE BRANCH IS DEAD: B is 0x00, so `bit 0,b` at
 ...
 * exactly like loc_03e8: `next` holds the ROM address of the block to run; each
 * block runs its instructions and ends by returning (ret) or by setting `next`.
 * The loop charges NO T-states -- only m.step does.
 */
export function loc_3476(m) {
  ...
  for (;;) {
    switch (next) {
      case 0x3476: {
        regs.bc = 0x00ff; m.step(0x3479, 10); // 3476  ld bc,0x00ff -- C=step -1 in X, B=0
        regs.d = 0x00; m.step(0x347b, 7); // 3479  ld d,0x00 -- direction index 0
        m.step(0x3490, 12); next = 0x3490; break; // 347b  jr 0x3490
      }
      case 0x3490: {
        ...
        m.step(0x34a7, 7); // 34a5  jr z,0x34d2 (not taken -- dead from loc_3476, B=0)
      ...
      case 0x34d2: {
        ...
        regs.add(regs.c); m.step(0x34d6, 4); // 34d5  add a,c -- advance by the X delta
        mem.write8(0x8086, regs.a); m.step(0x34d9, 13); // 34d6  ld (0x8086),a
        m.ret(); return; // 34d9  ret
      }
      default:
        throw new Error(
          "loc_3476: dispatch to non-block address 0x" + next.toString(16),
        );
    }
  }
}
