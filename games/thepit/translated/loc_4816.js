...
/**
 * loc_4816  (ROM 0x4816-0x4839) — draws one HUD strip: sets the column/row cursor
 * (0x8058=0x01 col, 0x8059=0x0b row), resolves that cell's screen offset and its
 * colour/video-RAM addresses (loc_3dae, loc_3dc9), then fills a 0x0a-cell vertical
 * strip from the ROM table at 0x494f (fill byte 0x8057=0x00, count 0x8055=0x0a via
 * loc_3ddb) and TAIL-JUMPS into loc_3e01 to paint the paired colour column.
 *
 ...
 *   4830  dd 21 4f 49  ld   ix,0x494f       ; source table (walked by loc_3ddb)
 *   4834  cd db 3d     call 0x3ddb          ; fill the tilemap strip
 *   4837  c3 01 3e     jp   0x3e01          ; TAIL into loc_3e01 (fill colour column)
 *
 * Returns via loc_3e01's own `ret`: the closing `jp 0x3e01` pushes nothing, so
 * loc_3e01's `ret` pops loc_4816's caller and returns there. The three earlier
...
export function loc_4816(m) {
  ...
  // 4837  jp 0x3e01 -- TAIL into loc_3e01; no push, so loc_3e01's ret returns
  // to loc_4816's caller.
  m.step(0x3e01, 10);
