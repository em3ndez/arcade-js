...
/**
 * sub_492a  (ROM 0x492A–0x4945) — paints a 32-tile column into video RAM from the
 * ROM table at 0x49C7, then tail-jumps to sub_3e1d to colour that column.
 *
 ...
 *   493f  0e 02        ld   c,0x02         ; colour byte for sub_3e1d
 *   4941  3e 19        ld   a,0x19         ; column offset for sub_3e1d
 ...
 * modelled as `return m.call(0x3e1d)`: sub_3e1d's own `ret` pops sub_492a's caller's
 ...
 * the diffed stack and charge a second ret's cycles (see entry_0000.js / sub_24cf.js).
 * A and C are the arguments sub_3e1d reads (A = column offset 0x19, C = fill 0x02).
 */
export function sub_492a(m) {
  const { regs, mem } = m;
  ...
  regs.c = 0x02;
  m.step(0x4941, 7); // ld c,0x02 -- colour for sub_3e1d
  regs.a = 0x19;
  m.step(0x4943, 7); // ld a,0x19 -- column offset for sub_3e1d
