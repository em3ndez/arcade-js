...
/**
 * loc_4c5b  (ROM 0x4c5b-0x4c5e -> shared tail 0x4ca5-0x4cbe, The Pit) —
 * the A=0x03 door of a large fan-in of one-line entry points (loc_4c5b..sub_4ca3
 * load A = 0x03..0x15) that all `jr 0x4ca5` into the common tail loc_4ca5. The
 ...
 * 0x03 | 0x80 = 0x83.
 *
 *   4c5b  3e 03        ld   a,0x03      ; this door's command code
 *   4c5d  18 46        jr   0x4ca5      ; unconditional -> shared tail loc_4ca5
 *
 * loc_4ca5: (shared by every loc_4c5b..sub_4ca3 entry)
 *   4ca5  f6 80        or   0x80        ; A = code | 0x80  (0x03 -> 0x83)
 ...
 * its own `ret`, so the tail is INLINED here (as loc_4b10/loc_4b14 inline loc_4b16)
 * and the routine ends with m.ret — not `return m.call(0x4ca5)`, since 0x4ca5 is an
 * internal label, not a registered routine.
 */
export function loc_4c5b(m) {
  const { regs, mem } = m;

  regs.a = 0x03; // 4c5b  ld a,0x03 -- this door's command code
  m.step(0x4c5d, 7);

  m.step(0x4ca5, 12); // 4c5d  jr 0x4ca5 -- unconditional, into the shared tail

  // loc_4ca5: shared enqueue tail (A = command code on entry; 0x03 for this door).
  regs.or(0x80); // 4ca5  or 0x80 -- A = code | 0x80 (0x83)
  m.step(0x4ca7, 7);
