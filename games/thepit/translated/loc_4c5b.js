...
/**
 * loc_4c5b  (ROM 0x4C5B–0x4C5E, The Pit) — sound-request stub: seeds A with the
 * sound-command index 0x03 and TAIL-jumps into the shared enqueue handler
 * loc_4ca5.
 *
 *   4c5b  3e 03        ld   a,0x03        ; sound-command index 3
 *   4c5d  18 46        jr   0x4ca5        ; unconditional TAIL-jump -> loc_4ca5
 *
 ...
 * tail at loc_4ca5. That tail sets bit 7 (`or 0x80` -> 0x83), then writes the
 ...
 * loc_4ca5's `ret` unwinds to loc_4c5b's OWN caller. Modelled per the translation doc as
 ...
 * 0x03 (neighbours load 0x02 at 0x4c57, 0x04 at 0x4c5f); that byte is the entire
 * payload, which is what the test's mutation attacks.
 */
export function loc_4c5b(m) {
  const { regs } = m;

  regs.a = 0x03; // 4c5b  ld a,0x03 -- sound-command index 3 (no flags)
  m.step(0x4c5d, 7);
  m.step(0x4ca5, 12); // 4c5d  jr 0x4ca5 -- unconditional TAIL-jump into loc_4ca5
  return m.call(0x4ca5); // loc_4ca5's ret unwinds to loc_4c5b's caller (no m.ret)
}
