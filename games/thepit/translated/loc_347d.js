...
/**
 * loc_347d  (ROM 0x347d-0x34d9, The Pit) -- one of four velocity-preset entry
 * points into a shared object-mover tail at 0x3490. Each entry loads a BC
 ...
 * loc_347d, presets BC=0x0100 (C=0x00 = NO X step, B=0x01) and D=0x01. The
 * other three entries are 0x3476 (BC=0x00ff,D=0), 0x3484 (BC=0x0001,D=2) and
 * 0x348b (BC=0xff00,D=3) -- separate routines that jump into the same 0x3490
 * tail; they are NOT part of loc_347d.
 *
 ...
 *
 * FROM THE loc_347d ENTRY THE SPRITE BRANCH IS LIVE (unlike loc_3476): B is 0x01,
 ...
 * the counter-zero frame. And because bit 7 of B is CLEAR (B=0x01), `bit 7,b` at
 * 0x34c9 sets Z, `jr nz,0x34cf` is NOT taken, and the `xor 0x80` at 0x34cd always
 * fires here (bit 7 of the sprite code is flipped). C=0x00 means this entry adds
 * NOTHING to the X position at 0x34d2 -- it only advances the counter/orientation.
 *
 ...
 *   34c8  7b           ld   a,e
 *   34c9  cb 78        bit  7,b
 *   34cb  20 02        jr   nz,0x34cf       ; bit7(B)=1 -> keep as-is
 ...
 * exactly like loc_3476: `next` holds the ROM address of the block to run; each
 * block runs its instructions and ends by returning (ret) or by setting `next`.
 * The loop charges NO T-states -- only m.step does. The 0x3490 tail below is
 * byte-identical to loc_3476's; it is inlined here (not m.call'd) because 0x3490
 * is not a routine of its own -- it is reachable only by `jr` from the four
 * entry points, exactly as the sibling loc_3476 models it.
 */
export function loc_347d(m) {
  ...
  for (;;) {
    switch (next) {
      case 0x347d: {
        regs.bc = 0x0100; m.step(0x3480, 10); // 347d  ld bc,0x0100 -- C=0 (no X step), B=1 (bit0 set -> sprite branch live)
        regs.d = 0x01; m.step(0x3482, 7); // 3480  ld d,0x01 -- direction index 1
        m.step(0x3490, 12); next = 0x3490; break; // 3482  jr 0x3490
      }
      case 0x3490: {
        ...
        mem.write8(0x8092, regs.a); m.step(0x34a3, 13); // 34a0  ld (0x8092),a -- publish direction index
        regs.bit(0, regs.b); m.step(0x34a5, 8); // 34a3  bit 0,b -- Z = !(bit0 of B)
        if (regs.fZ) { m.step(0x34d2, 12); next = 0x34d2; break; } // 34a5  jr z,0x34d2 (not taken; for B=1 the sprite branch runs)
        m.step(0x34a7, 7); // 34a5  jr z,0x34d2 (not taken -- live from loc_347d, B=1)
      ...
      case 0x34c8: {
        ...
        if (regs.fNZ) { m.step(0x34cf, 12); next = 0x34cf; break; } // 34cb  jr nz,0x34cf (not taken; for B=1 bit7=0 -> xor fires)
        m.step(0x34cd, 7); // 34cb  jr nz,0x34cf (not taken -- xor path from loc_347d)
        regs.xor(0x80); m.step(0x34cf, 7); // 34cd  xor 0x80 -- mirror (flip bit 7)
        next = 0x34cf; break;
      }
      case 0x34cf: {
      ...
      case 0x34d2: {
        ...
        regs.add(regs.c); m.step(0x34d6, 4); // 34d5  add a,c -- advance by the X delta (C=0 here)
        mem.write8(0x8086, regs.a); m.step(0x34d9, 13); // 34d6  ld (0x8086),a
        m.ret(); return; // 34d9  ret
      }
      default:
        throw new Error(
          "loc_347d: dispatch to non-block address 0x" + next.toString(16),
        );
    }
  }
}
