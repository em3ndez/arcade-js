...
/**
 * loc_4c67  (ROM 0x4C67–0x4C6A, The Pit) — sound-request stub: seeds A with the
 * sound-command index 0x06 and TAIL-jumps into the shared enqueue handler
 * loc_4ca5.
 *
 *   4c67  3e 06        ld   a,0x06        ; sound-command index 6
 *   4c69  18 3a        jr   0x4ca5        ; unconditional TAIL-jump -> loc_4ca5
 *
 ...
 * tail at loc_4ca5. That tail sets bit 7 (`or 0x80` -> 0x86), then writes the
 ...
 * loc_4ca5's `ret` unwinds to loc_4c67's OWN caller. Modelled per the translation doc as
 ...
 * 0x06 (neighbour loads 0x07 at 0x4c6b); that byte is the entire payload, which
 * is what the test's mutation attacks.
 */
export function loc_4c67(m) {
  const { regs } = m;

  regs.a = 0x06; // 4c67  ld a,0x06 -- sound-command index 6 (no flags)
  m.step(0x4c69, 7);
  m.step(0x4ca5, 12); // 4c69  jr 0x4ca5 -- unconditional TAIL-jump into loc_4ca5
  return m.call(0x4ca5); // loc_4ca5's ret unwinds to loc_4c67's caller (no m.ret)
}
