...
/**
 * sub_4689  (ROM 0x4689-0x46A1) — conditional BCD score-add: when the state byte
 ...
 * then TAIL-JUMPS to sub_46af; otherwise it returns immediately (score untouched)
 ...
 *   46a1  18 0c        jr   0x46af        ; TAIL into sub_46af
 *
 * Two exits, both transfers of control: `jr nc,0x4672` lands on a single `ret`
 * (loc_4672) so it is a conditional return to sub_4689's OWN caller; the closing
 * `jr 0x46af` is an unconditional tail-jump, so sub_46af's eventual `ret` returns
 * to whoever called sub_4689.
 */
export function sub_4689(m) {
  const { regs, mem } = m;
  ...
  // 0x4672 and returns to sub_4689's caller with the score left untouched.
  if (regs.fNC) {
  ...
  m.step(0x46af, 12); // 46a1  jr 0x46af -- TAIL into sub_46af
