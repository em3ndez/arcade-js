...
/**
 * loc_4b14  (ROM 0x4b14–0x4b19) — ENABLE the NMI: sets LS259 latch bit 0
 ...
 * therefore SETS the mask (NMI enabled) — the same tail loc_0066 uses at 0x01a1
 * to re-enable the NMI. This is the "enable" entry of a two-door helper: loc_4b10
 * (`ld a,0x00`) falls into the SAME loc_4b16 tail to CLEAR the bit (NMI disabled).
 * Only loc_4b14 (a = 0x01) is translated here; it enters above the shared tail.
 *
 ...
 */
export function loc_4b14(m) {
  const { regs, mem } = m;

  regs.a = 0x01; // ld a,0x01 -- datum for LS259 b0 (NMI mask = 1)
  m.step(0x4b16, 7);

  // loc_4b16 (shared tail with loc_4b10) -- write the datum to LS259 b0.
