...
/**
 * loc_4785  (ROM 0x4785-0x47A0) — draws a border/HUD column. Copies a 0x20-byte
 * ROM tile strip (0x4ACB..0x4AEA) UP a video-RAM column (0x93FE, descending by
 * one row = 0x20 each pass), then TAIL-jumps into loc_3e1d with C=0x01, A=0x1E to
 * paint the matching colour-RAM column (base 0x8840+0x1E, 28 rows of colour 0x01).
 *
 * The tile source 0x4ACB is exactly loc_46f4's source + 0x20 — this routine draws
 ...
 * entering the tail are the last loop `add hl,de`'s — but loc_3e1d overwrites them
 * before any conditional reads them. On exit from the loop IX = 0x4AEB (source +
 * 0x20), HL = 0x8FFE (0x93FE - 0x20*0x20), DE = 0xFFE0, B = 0.
 *
 * `jp 0x3e1d` is a TAIL-JUMP: an unconditional JP pushes nothing, so loc_3e1d's own
 ...
 * charge a second ret (see loc_3d7e / loc_3b81). loc_3e1d reads A (0x1E) as the
...
export function loc_4785(m) {
  ...
  regs.c = 0x01; // 479a  ld c,0x01 -- fill byte for loc_3e1d's colour column
  m.step(0x479c, 7);

  regs.a = 0x1e; // 479c  ld a,0x1e -- column offset for loc_3e1d
  m.step(0x479e, 7);

  // 479e  jp 0x3e1d -- tail-jump; loc_3e1d's ret returns to OUR caller.
  m.step(0x3e1d, 10);
