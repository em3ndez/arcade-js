...
/**
 * loc_467b  (ROM 0x467b-0x4682) — score-add entry point (+10). One of a family
 * of thin entry points (loc_4673 +1, loc_467b +10, loc_4683 +20) that each fire
 * a sound effect and then tail-jump into the shared BCD adder loc_4689 with the
 ...
 *
 * The terminal `jr 0x4689` pushes nothing, so loc_4689's own `ret` (whether the
 ...
 * charge a second ret (the double-pop trap the loc_3d8a header calls out).
 *
 * loc_467b's OWN three instructions touch no flags: a CALL, an LD BC,nn, and a
 * JR are all flag-neutral (the callees change flags, but that is their business).
 */
export function loc_467b(m) {
  const { regs } = m;
  ...
  // 4681  jr 0x4689 -- tail-jump; loc_4689's ret returns to OUR caller.
  m.step(0x4689, 12);
