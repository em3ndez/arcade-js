...
/**
 * loc_1515  (ROM 0x1515-0x1657) — the tile-under-object classifier / collision
 * dispatch. Entered (from loc_14cd, at the `jr nz 0x1515` guards) with IX pointing
 * at the object's video-RAM cell (0x806e), D holding the biased screen column
 * (bits 0..2 = the object's sub-tile offset within a cell) and 0x806e already set.
 * It:
 *
 *   1. Reads the tile the object sits ON at (IX+0) into 0x80a5, 0x80a7 and B.
 *   2. Only when the object is grid-aligned (D & 7 == 0) does it special-case the
 *      collectible / gate tiles: 0x3a -> call 0x467b and bump the 0x8081 counter
 *      (loc_1552-skipped); 0x3b/0x3c/0x3d -> arm 0x8078 (unless 0x80bd is already
 *      set), call 0x4683, bump 0x8082. Both of those stamp the cell with 0x70 and
 *      tail-jump to the shared tail 0x1659.
 *   3. Otherwise (loc_1568) it records tile 0x26 -> 0x8076 and 0x27 -> 0x80e7, then
 *      runs the classification ladder (loc_1578): the "solid" ids
 *      (0x2a/0x41/0xc1/0x95/0xc4, the 0x96..0x99 band and >=0x9a via the c/nc
 *      tests, plus 0xc5 gated on D bit2) defer the frame via 0x1b5b; the 0x71..0x9d
 *      "pushable" band indexes table 0x1b78 by ((tile-0x71) << 3 | (D & 7)), stores
 *      the looked-up byte to 0x80a7, and on a mismatch arms the 0xb5 push state
 *      (0x80a2=1, 0x80a4=0x80a3, 0x8069=0xb5) and defers.
 *   4. loc_15ea repeats the same ladder for the tile AHEAD at (IX+1) (recorded to
 *      0x80a6) against table 0x1ce0 into 0x80a8, and loc_164f cross-checks
 *      0x80a5 vs 0x80a7 before falling through into 0x1659.
 *
 * CONTROL FLOW. Every `jp/jp cc 0x1b5b`, the `jr z 0x1659`, the two `jp 0x1659`
 * and the 0x1657 fall-through into 0x1659 are TAIL-JUMPS to SEPARATE registered
 * routines: `return m.call(target)`, whose own ret returns to OUR caller (no
 * trailing m.ret — a second ret would double-pop). The two `call`s (0x467b,
 * 0x4683) are ORDINARY returning calls: push16(return), step(target,17),
 * m.call(target), then execution continues at the pushed return address.
 *
 * The 15 internal labels (0x1535 .. 0x164f) are reached ONLY from inside this
 * routine, so they are address-suffixed helpers called directly
 * (`return loc_1515_1568(m)`), never registered at their own address — the
 * registry regex `^[a-z]+_([0-9a-f]+)$` rejects the embedded second underscore,
 * so a bare `m.call(0x1568)` can never resolve here. `sla a`/`rl b` are ordered so
 * the carry the first spills is the carry the second reads.
 *
 * (This body is the ROM's 0x1515 label. loc_14cd — which reaches 0x1515 by
 * fall-through — carries a byte-identical private copy as its own loc_14cd_1515
 * helpers; that copy is a direct-called internal of loc_14cd and is not the
 * registered 0x1515 entry, which is this file.)
 *
 * loc_1515:
...
export function loc_1515(m) {
  ...
  m.step(0x1518, 19); // ld a,(ix+0x00) -- tile the object sits on
  ...
  if (regs.fNZ) {
    m.step(0x1568, 12); // jr nz taken -- not aligned, skip the collectible cases
  ...
  return loc_1515_155c(m);
}

/**
 * loc_1515_1535  (ROM 0x1535-0x1540) — tile ids 0x3b/0x3c/0x3d route to the gate
 * handler loc_1542; every other id falls out to loc_1568.
 */
export function loc_1515_1535(m) {
  const { regs } = m;
...
/**
 * loc_1515_1542  (ROM 0x1542-0x1551) — the gate tiles (0x3b..0x3d): if the busy
 * flag 0x8078 is already set, skip to loc_1552; else if 0x80bd is set, bail back
 * to the ladder at loc_1568; otherwise arm 0x8078 = 1 and fall into loc_1552.
 */
export function loc_1515_1542(m) {
  const { regs, mem } = m;
...
/**
 * loc_1515_1552  (ROM 0x1552-0x155b) — call 0x4683, bump the 0x8082 counter, then
 * fall into loc_155c.
 */
export function loc_1515_1552(m) {
  const { regs, mem } = m;
...
/**
 * loc_1515_155c  (ROM 0x155c-0x1567) — reload the cell pointer, stamp it with 0x70
 * and tail-jump to the shared tail 0x1659.
 */
export function loc_1515_155c(m) {
  const { regs, mem } = m;
...
/**
 * loc_1515_1568  (ROM 0x1568-0x156f) — record tile 0x26 into 0x8076, then fall
 * into loc_1570.
 */
export function loc_1515_1568(m) {
  const { regs, mem } = m;
...
/**
 * loc_1515_1570  (ROM 0x1570-0x1577) — record tile 0x27 into 0x80e7, then fall
 * into the classification ladder loc_1578.
 */
export function loc_1515_1570(m) {
  const { regs, mem } = m;
...
/**
 * loc_1515_1578  (ROM 0x1578-0x15a1) — the "tile under object" classification
 * ladder. Solid ids (0x2a/0x41/0xc1/0x95/0xc4, plus 0x96..0x99 via the c test)
 * defer to 0x1b5b; 0xc5 and the 0x9a..0x9d band route to the bit2 / pushable
 * checks; >= 0x9e goes straight to loc_15e5. A holds the tile id throughout (cp
 * does not change it).
 */
export function loc_1515_1578(m) {
  const { regs } = m;
...
/**
 * loc_1515_15a2  (ROM 0x15a2-0x15a6) — the D bit2 gate: if clear, defer to
 * 0x1b5b; else fall into the pushable-range test loc_15a7.
 */
export function loc_1515_15a2(m) {
  const { regs } = m;
...
/**
 * loc_1515_15a7  (ROM 0x15a7-0x15e4) — the pushable-tile range 0x71..0x9d.
 ...
 * to the tile. Equal -> loc_15e5. Unequal, and only when the object is grid-aligned
 * (D & 7 == 0), arm the 0xb5 push state (0x80a2=1, 0x80a4=0x80a3, 0x8069=0xb5) and
 * defer to 0x1b5b; if not aligned -> loc_15ea.
 */
export function loc_1515_15a7(m) {
  const { regs, mem } = m;
...
/**
 * loc_1515_15ea  (ROM 0x15ea-0x160f) — the same solid-tile ladder applied to the
 ...
 * >= 0x9a -> loc_164f.
 */
export function loc_1515_15ea(m) {
  const { regs, mem } = m;
...
/**
 * loc_1515_1610  (ROM 0x1610-0x1615) — decrement D, retest bit2: if set, defer to
 * 0x1b5b; else fall into the pushable-ahead test loc_1616.
 */
export function loc_1515_1610(m) {
  const { regs } = m;
...
/**
 * loc_1515_1616  (ROM 0x1616-0x163b) — the pushable-tile range test for the tile
 ...
 */
export function loc_1515_1616(m) {
  const { regs, mem } = m;
  ...
  return loc_1515_163c(m);
}

/**
 * loc_1515_163c  (ROM 0x163c-0x164e) — arm the 0xb5 push state (0x80a4=0x80a3,
 * 0x80a2=1, 0x8069=0xb5) and defer to 0x1b5b. Same body as the aligned-mismatch
 * arm inside loc_15a7.
 */
export function loc_1515_163c(m) {
  const { regs, mem } = m;
