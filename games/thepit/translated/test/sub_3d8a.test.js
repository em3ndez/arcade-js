...
//
// Drafter test for the translated sub_3d8a (ROM 0x3d8a-0x3dad), The Pit.
//
...
// asserted T-state total is sub_3d8a's OWN instruction cost:
//   7+13+7+13 + 17+17 + 7+13+7+13 + 14 + 17 + 10 = 155 T.
//
// The load-bearing decision is the terminal `jp 0x3e01`: it is a TAIL-JUMP (a JP
// pushes nothing), so sub_3e01's own `ret` returns to sub_3d8a's caller, and the
...
//
// sub_3d8a's own instructions touch NO flag (straight-line stores/calls/jump), so
...
import { sub_3d8a } from "../sub_3d8a.js";
...
class TestMachine {
  ...
  // charge -- a callee's cost is not sub_3d8a's.
  call(addr) {
...
function checkSpec(res) {
  assert.equal(res.cycles, EXP_CYCLES, "T-state total (155, sub_3d8a's own instructions)");
...
test("sub_3d8a: seed state + call chain + tail-jump 0x3e01; 155 T", () => {
  checkSpec(run(sub_3d8a));
});
...
function sub_3d8a_countMutant(m) {
  const { regs, mem } = m;
  regs.a = 0x06;
  m.step(0x3d8c, 7);
...
test("mutation (row count 0x08 not 0x09) is caught by the memory/register asserts", () => {
  const bad = run(sub_3d8a_countMutant);
...
function sub_3d8a_cycleMutant(m) {
  const { regs, mem } = m;
  regs.a = 0x06;
  m.step(0x3d8c, 7);
  mem.write8(0x8058, regs.a);
...
test("mutation (ld ix,nn mischarged 10 T) is caught by the cycle assertion", () => {
  const bad = run(sub_3d8a_cycleMutant);
...
// `m.call(); m.ret()` shape. The tail-jump pushed nothing, so sub_3e01's ret
...
// exact double-pop trap the sub_3d8a / sub_3d7e headers warn against, and only
// the pc/sp (control-flow) asserts pin it down.
function sub_3d8a_doubleRetMutant(m) {
  const { regs, mem } = m;
  regs.a = 0x06;
  m.step(0x3d8c, 7);
  mem.write8(0x8058, regs.a);
...
test("mutation (tail-jump written as m.call+m.ret, a double-pop) is caught by pc/sp", () => {
  const bad = run(sub_3d8a_doubleRetMutant);
