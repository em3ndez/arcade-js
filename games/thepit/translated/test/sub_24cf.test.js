...
/**
 * Drafter test for sub_24cf (ROM 0x24cf) — the work-RAM state-block initialiser
 ...
 *
 * Run: node --test games/thepit/translated/test/sub_24cf.test.js
 */
...
import { sub_24cf } from "../sub_24cf.js";
...
function makeMachine() {
  return {
    ...
    call(addr) {
      ...
      // state at the instant control leaves sub_24cf and hand back a sentinel.
      this.tail = {
        addr,
...
function checkContract(m, ret) {
  ...
  assert.equal(ret, SENTINEL, "sub_24cf must return the tail target's result");
...
test("sub_24cf: constant stores, T-states, boundaries, flags, tail-jump", () => {
  const { m, ret } = run(sub_24cf);
  checkContract(m, ret);
  ...
  console.log(`  sub_24cf: 146 T, 8 constant stores, jp -> 0x${TAIL.toString(16)} returned through`);
});
