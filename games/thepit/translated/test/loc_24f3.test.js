...
/**
 * Drafter test for the translated loc_24f3 (ROM 0x24f3, The Pit).
 *
 ...
 *
 * Run: node --test games/thepit/translated/test/loc_24f3.test.js
 */
...
// Minimal machine matching the surface loc_24f3 uses: regs, mem, step, call, ret,
...
test("loc_24f3 Path A: phase-1 timer expiry re-seeds, builds record, tail-jumps 0x29ad", () => {
  const m = makeMachine();
  runPathA(m);
  assertPathAGolden(m);
});
...
test("loc_24f3 Path B: (0x8077) != 0 -> loc_24ff -> loc_2677 -> tail-jump 0x29ad", () => {
  const m = makeMachine();
  seatCaller(m);
...
test("loc_24f3 Path C: 0x8077==0 & 0x80c1!=0 -> `jr z` not taken -> loc_24ff", () => {
  const m = makeMachine();
  seatCaller(m);
...
test("loc_24f3 MUTATION: `ld ix,(nn)` mis-charged 16T (not 20T) is caught", () => {
  const m = makeMachine();
