...
//
// Drafter test for loc_3e1d (ROM 0x3E1D-0x3E31): the colour-RAM column fill that
...
// seam loc_3e1d drives (no m.call in this routine).
function makeMachine({ a = 0x00, c = 0x00 } = {}) {
  ...
  m.push16(RET); // the return address loc_3e1d's `ret` must pop
  return m;
}
...
// Faithful copy of loc_3e1d with the row-advance removed: every write lands on
...
test("the assertions have teeth: the dropped-stride mutation is caught", () => {
  assertColumnFill(loc_3e1d); // the real routine passes it
  assert.throws(
    () => assertColumnFill(loc_3e1d_mut),
    /row 1 .* filled|939 T/,
