...
//
// Drafter test for loc_4c9f (ROM 0x4c9f-0x4ca2, The Pit): a sound-request stub that
// loads the command index 0x14 into A and UNCONDITIONALLY tail-jumps into the shared
// enqueue handler loc_4ca5 (0x4ca5). loc_4ca5's own ret unwinds to loc_4c9f's caller,
...
//   * A = 0x14 seeded; exact 19 T total (ld a,n = 7, jr = 12); boundary sequence
//     [0x4ca1, 0x4ca5]; control leaves via return m.call(0x4ca5) (no ret); the
//     caller's seeded flags survive (neither instruction writes F).
//
// TEETH: a faithful twin whose only break is the payload immediate `ld a,0x14`
// swapped for `ld a,0x15` (its reachable neighbour loc_4ca3's value) -- the single
// byte that distinguishes this stub from its ~20 siblings -- must be CAUGHT (A and
// the arg into loc_4ca5 both diverge 0x14 -> 0x15).
//
// Run: node --test games/thepit/translated/test/loc_4c9f.test.js
...
function makeMachine({ f = 0x00 } = {}) {
  ...
  const m = {
    ...
    call(addr) {
      ...
      // machine state at the instant control leaves loc_4c9f, return a sentinel.
...
const EXPECTED_PC_SEQ = [0x4ca1, 0x4ca5];
...
function checkContract(m, ret) {
  // Payload: A = the command index 0x14.
  assert.equal(m.regs.a, 0x14, "A = the sound-command index 0x14");
  ...
  assert.equal(m.tail.a, 0x14, "A into loc_4ca5 = 0x14");
  assert.equal(ret, SENTINEL, "loc_4c9f returns loc_4ca5's result straight through");
...
test("loc_4c9f: seeds A=0x14 and tail-jumps to loc_4ca5, 19 T, no ret", () => {
  const m = makeMachine();
  const ret = loc_4c9f(m);
  checkContract(m, ret);
  console.log("  loc_4c9f: A=0x14, 19 T, [0x4ca1, 0x4ca5], jr -> 0x4ca5 (tail)");
});

test("loc_4c9f: ld a / jr touch no flags -- the caller's F survives unchanged", () => {
...
// -- TEETH -------------------------------------------------------------------
// A faithful copy of loc_4c9f with EXACTLY ONE break: the payload immediate
// `ld a,0x14` (3e 14) swapped for `ld a,0x15` (3e 15) -- its reachable neighbour
// loc_4ca3's value, and the ONLY byte that distinguishes this stub from its ~20
// siblings. The contract rejects it: A and the arg into loc_4ca5 both flip
// 0x14 -> 0x15.
function brokenLoc4c9f(m) {
  const { regs } = m;
  regs.a = 0x15; // BUG: ld a,0x15 instead of ld a,0x14 (neighbour loc_4ca3's index)
  m.step(0x4ca1, 7);
  m.step(0x4ca5, 12);
  return m.call(0x4ca5);
}

test("TEETH: the ld a,0x14 -> ld a,0x15 twin is CAUGHT by the contract", () => {
  // The real routine passes the contract.
  {
    const m = makeMachine();
    checkContract(m, loc_4c9f(m));
  }
  // The mutant fails it -- A (and the arg into loc_4ca5) diverge.
  const m = makeMachine();
  const ret = brokenLoc4c9f(m);
  assert.throws(
    () => checkContract(m, ret),
    /0x14/,
    "the contract FAILED to catch ld a,0x14 -> ld a,0x15 -- it has no teeth",
  );
  // Concretely: the wrong index reaches A and would enqueue the wrong sound.
  assert.equal(m.regs.a, 0x15, "mutant: A = 0x15 (wrong sound-command index)");
  assert.equal(m.tail.a, 0x15, "mutant: 0x15 would be enqueued, not 0x14");
});
