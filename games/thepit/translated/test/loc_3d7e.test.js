...
//
// Drafter test for the translated loc_3d7e (ROM 0x3d7e-0x3d89), The Pit.
//
...
// loc_3d7e's OWN instruction cost: 13+4+7+13+10 = 47 T.
//
...
function checkSpec(res) {
  ...
  assert.deepEqual(res.calls, [0x3e01], "tail-jumps into loc_3e01 exactly once");
}

test("loc_3d7e: bump 0x8057 masking bit 3, tail-jump 0x3e01; 47 T", () => {
  checkSpec(run(loc_3d7e));
});

// A sanity anchor for the mask: a value that does NOT hit bit 3 counts normally.
test("loc_3d7e: 0x03 + 1 = 0x04 (no bit-3 involvement)", () => {
  const res = run(loc_3d7e, 0x03);
...
function loc_3d7e_maskMutant(m) {
  const { regs, mem } = m;
...
test("mutation (and 0xff drops the bit-3 mask) is caught by the memory/flag asserts", () => {
  const bad = run(loc_3d7e_maskMutant);
...
function loc_3d7e_cycleMutant(m) {
  const { regs, mem } = m;
  regs.a = mem.read8(0x8057);
...
test("mutation (ld a,(nn) mischarged 7 T) is caught by the cycle assertion", () => {
  const bad = run(loc_3d7e_cycleMutant);
