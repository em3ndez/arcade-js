...
/**
 * Drafter test for the translated loc_4b10 (ROM 0x4b10, The Pit).
 *
 ...
 *
 * Run: node --test games/thepit/translated/test/loc_4b10.test.js
 */
...
// Minimal machine matching the surface loc_4b10 uses: regs, mem (with a hardware
...
function makeMachine() {
  ...
  return {
    ...
    call(addr) {
      ...
      return undefined; // loc_4b10 makes no calls
    },
  };
}
...
test("loc_4b10: writes A=0x00 to the 0xB000 control latch, 42 T, rets", () => {
  const m = makeMachine();
  run(m);
  assertGolden(m);
});
...
test("loc_4b10 MUTATION: unconditional jr mis-charged 7T (not 12T) is caught", () => {
  const m = makeMachine();
