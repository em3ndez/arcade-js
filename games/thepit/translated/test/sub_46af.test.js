...
//
// Drafter test for sub_46af (ROM 0x46AF-0x46F3). Asserts the T-state total and
...
import { sub_46af } from "../sub_46af.js";
...
// charges T-states; ret charges its own T and marks the return). sub_46af makes
...
function checkPath1(m) {
  ...
  assert.equal(m.calls.length, 0, "sub_46af makes no calls");
  assert.equal(m.returned, true, "routine returned");
}

test("sub_46af blank path: 0x90c1 base, 0x8034 pair blanked to tile 0x24", () => {
...
test("sub_46af digits path: 0x9301 base, both nibbles of each byte rendered", () => {
  const m = new MockMachine();
