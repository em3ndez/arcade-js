...
/**
 * Drafter test for the translated loc_33da (ROM 0x33DA-0x340F, The Pit).
 *
 ...
 * checker must catch.
 *
 * Run: node --test games/thepit/translated/test/loc_33da.test.js
 */
...
// Minimal machine matching the surface loc_33da uses: regs, mem, step, ret,
...
test("loc_33da Path A: both cpir searches hit, ix/hl/a as disassembled", () => {
  const m = makeMachine();
  setupFull(m);
  loc_33da(m);
  checkFull(m);
...
test("loc_33da Path B: key1 absent -> cpir exhausts 32 entries -> ret nz", () => {
  const m = makeMachine();
  seatCaller(m);
  m.mem.write8(0x808d, 0x05);
  m.mem.write16(0x8089, 0x8200);
...
test("loc_33da Path C: first hit but (0x808d)==0 -> ret z, no second search", () => {
  const m = makeMachine();
  seatCaller(m);
...
// sibling loc_3425 (ROM 0x3425) genuinely does `ld bc,0x0020 / add hl,bc` at that
...
test("loc_33da MUTATION caught: `ld bc,0x0020` for `ld bc,0xffe0` shifts the pointer", () => {
