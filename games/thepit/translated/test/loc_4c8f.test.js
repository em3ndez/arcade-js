...
//
// Drafter test for loc_4c8f (ROM 0x4c8f-0x4c92, The Pit): a sound-request stub that
// loads the command index 0x10 into A and UNCONDITIONALLY tail-jumps into the shared
// enqueue handler loc_4ca5 (0x4ca5). loc_4ca5's own ret unwinds to loc_4c8f's caller,
...
//   * A = 0x10 seeded; exact 19 T total (ld a,n = 7, jr = 12); boundary sequence
//     [0x4c91, 0x4ca5]; control leaves via return m.call(0x4ca5) (no ret); the
//     caller's seeded flags survive (neither instruction writes F).
//
// TEETH: a faithful twin whose only break is the payload immediate `ld a,0x10`
// swapped for `ld a,0x11` (its neighbour sub_4c93's value) -- the single byte that
// distinguishes this stub from its ~20 siblings -- must be CAUGHT (A and the arg
// into loc_4ca5 both diverge 0x10 -> 0x11).
//
// Run: node --test games/thepit/translated/test/loc_4c8f.test.js
...
function makeMachine({ f = 0x00 } = {}) {
  ...
  const m = {
    ...
    call(addr) {
      ...
      // machine state at the instant control leaves loc_4c8f, return a sentinel.
...
const EXPECTED_PC_SEQ = [0x4c91, 0x4ca5];
...
function checkContract(m, ret) {
  // Payload: A = the command index 0x10.
  assert.equal(m.regs.a, 0x10, "A = the sound-command index 0x10");
  ...
  assert.equal(m.tail.a, 0x10, "A into loc_4ca5 = 0x10");
  assert.equal(ret, SENTINEL, "loc_4c8f returns loc_4ca5's result straight through");
...
test("loc_4c8f: seeds A=0x10 and tail-jumps to loc_4ca5, 19 T, no ret", () => {
  const m = makeMachine();
  const ret = loc_4c8f(m);
  checkContract(m, ret);
  console.log("  loc_4c8f: A=0x10, 19 T, [0x4c91, 0x4ca5], jr -> 0x4ca5 (tail)");
});

test("loc_4c8f: ld a / jr touch no flags -- the caller's F survives unchanged", () => {
...
// -- TEETH -------------------------------------------------------------------
// A faithful copy of loc_4c8f with EXACTLY ONE break: the payload immediate
// `ld a,0x10` (3e 10) swapped for `ld a,0x11` (3e 11) -- its neighbour sub_4c93's
// value, and the ONLY byte that distinguishes this stub from its ~20 siblings.
// The contract rejects it: A and the arg into loc_4ca5 both flip 0x10 -> 0x11.
function brokenLoc4c8f(m) {
  const { regs } = m;
  regs.a = 0x11; // BUG: ld a,0x11 instead of ld a,0x10 (neighbour sub_4c93's index)
  m.step(0x4c91, 7);
  m.step(0x4ca5, 12);
  return m.call(0x4ca5);
}

test("TEETH: the ld a,0x10 -> ld a,0x11 twin is CAUGHT by the contract", () => {
  // The real routine passes the contract.
  {
    const m = makeMachine();
    checkContract(m, loc_4c8f(m));
  }
  // The mutant fails it -- A (and the arg into loc_4ca5) diverge.
  const m = makeMachine();
  const ret = brokenLoc4c8f(m);
  assert.throws(
    () => checkContract(m, ret),
    /0x10/,
    "the contract FAILED to catch ld a,0x10 -> ld a,0x11 -- it has no teeth",
  );
  // Concretely: the wrong index reaches A and would enqueue the wrong sound.
  assert.equal(m.regs.a, 0x11, "mutant: A = 0x11 (wrong sound-command index)");
  assert.equal(m.tail.a, 0x11, "mutant: 0x11 would be enqueued, not 0x10");
});
