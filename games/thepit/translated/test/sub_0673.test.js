...
//
// Drafter test for the translated sub_0673 (ROM 0x0673-0x06AB), The Pit.
//
...
// but add no cycles, so the asserted T-state total is sub_0673's OWN instruction
...
import { sub_0673 } from "../sub_0673.js";
...
test("sub_0673: 0x8028 bit0 SET copies source A, total 43220 T", () => {
  checkSpec(run(sub_0673, 1), EXP_VIDEO_A, 43220);
});

// bit0 clear path: falls through to ld hl,0x0b62 (source B). jr not-taken (7 T)
//   + ld hl (10 T) => +5 T over the taken path => 43225 T.
test("sub_0673: 0x8028 bit0 CLEAR copies source B, total 43225 T", () => {
  const res = run(sub_0673, 0);
...
test("mutation (branch polarity flipped) is caught by the spec", () => {
  ...
  const bad = run(sub_0673_mutant, 1);
