...
/**
 * Drafter test for sub_33bc (ROM 0x33bc) — the 32-byte table-row search.
 *
 ...
 *
 * Run: node --test games/thepit/translated/test/sub_33bc.test.js
 */
...
import { sub_33bc } from "../sub_33bc.js";

// Leaf-routine machine double: exactly the surface sub_33bc touches. `seed` gets
...
function makeMachine(seed) {
  ...
  const m = {
    ...
    call() {
      throw new Error("sub_33bc makes no calls");
    },
  };
...
test("sub_33bc: dec-hl branch, byte found (Z set, cpir n=6, 249 T)", () => {
  const m = makeMachine(seedA);
  sub_33bc(m);
  checkContractA(m);
  console.log("  sub_33bc A: 249 T, dec hl taken, cpir found @0x3523 -> HL=0x3524, BC=0x1a, Z=1");
});
...
test("sub_33bc: jr-taken branch skips dec hl, byte found (185 T)", () => {
  ...
  console.log("  sub_33bc B: 185 T, jr taken (no dec hl), cpir found @0x3500 -> HL=0x3501, BC=0x1d");
});
...
test("sub_33bc: byte not found -> Z clear, BC=0, HL=base+0x20 (795 T)", () => {
  ...
  console.log("  sub_33bc C: 795 T, cpir miss -> Z=0, BC=0, HL=0x353e");
});
