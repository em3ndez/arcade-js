...
//
// Drafter test for loc_4c8b (ROM 0x4c8b-0x4c8e, The Pit): a sound-request stub that
// loads the command index 0x0f into A and UNCONDITIONALLY tail-jumps into the shared
// enqueue handler loc_4ca5 (0x4ca5). loc_4ca5's own ret unwinds to loc_4c8b's caller,
...
//   * A = 0x0f seeded; exact 19 T total (ld a,n = 7, jr = 12); boundary sequence
//     [0x4c8d, 0x4ca5]; control leaves via return m.call(0x4ca5) (no ret); the
//     caller's seeded flags survive (neither instruction writes F).
//
// TEETH: a faithful twin whose only break is the payload immediate `ld a,0x0f`
// swapped for `ld a,0x10` (its reachable neighbour loc_4c8f's value) -- the single
// byte that distinguishes this stub from its ~20 siblings -- must be CAUGHT (A and
// the arg into loc_4ca5 both diverge 0x0f -> 0x10).
//
// Run: node --test games/thepit/translated/test/loc_4c8b.test.js
...
function makeMachine({ f = 0x00 } = {}) {
  ...
  const m = {
    ...
    call(addr) {
      ...
      // machine state at the instant control leaves loc_4c8b, return a sentinel.
...
const EXPECTED_PC_SEQ = [0x4c8d, 0x4ca5];
...
function checkContract(m, ret) {
  // Payload: A = the command index 0x0f.
  assert.equal(m.regs.a, 0x0f, "A = the sound-command index 0x0f");
  ...
  assert.equal(m.tail.a, 0x0f, "A into loc_4ca5 = 0x0f");
  assert.equal(ret, SENTINEL, "loc_4c8b returns loc_4ca5's result straight through");
...
test("loc_4c8b: seeds A=0x0f and tail-jumps to loc_4ca5, 19 T, no ret", () => {
  const m = makeMachine();
  const ret = loc_4c8b(m);
  checkContract(m, ret);
  console.log("  loc_4c8b: A=0x0f, 19 T, [0x4c8d, 0x4ca5], jr -> 0x4ca5 (tail)");
});

test("loc_4c8b: ld a / jr touch no flags -- the caller's F survives unchanged", () => {
...
// -- TEETH -------------------------------------------------------------------
// A faithful copy of loc_4c8b with EXACTLY ONE break: the payload immediate
// `ld a,0x0f` (3e 0f) swapped for `ld a,0x10` (3e 10) -- its reachable neighbour
// loc_4c8f's value, and the ONLY byte that distinguishes this stub from its ~20
// siblings. The contract rejects it: A and the arg into loc_4ca5 both flip
// 0x0f -> 0x10.
function brokenLoc4c8b(m) {
  const { regs } = m;
  regs.a = 0x10; // BUG: ld a,0x10 instead of ld a,0x0f (neighbour loc_4c8f's index)
  m.step(0x4c8d, 7);
  m.step(0x4ca5, 12);
  return m.call(0x4ca5);
}

test("TEETH: the ld a,0x0f -> ld a,0x10 twin is CAUGHT by the contract", () => {
  // The real routine passes the contract.
  {
    const m = makeMachine();
    checkContract(m, loc_4c8b(m));
  }
  // The mutant fails it -- A (and the arg into loc_4ca5) diverge.
  const m = makeMachine();
  const ret = brokenLoc4c8b(m);
  assert.throws(
    () => checkContract(m, ret),
    /0x0f/,
    "the contract FAILED to catch ld a,0x0f -> ld a,0x10 -- it has no teeth",
  );
  // Concretely: the wrong index reaches A and would enqueue the wrong sound.
  assert.equal(m.regs.a, 0x10, "mutant: A = 0x10 (wrong sound-command index)");
  assert.equal(m.tail.a, 0x10, "mutant: 0x10 would be enqueued, not 0x0f");
});
