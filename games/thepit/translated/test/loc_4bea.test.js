...
//
// Drafter test for loc_4bea (ROM 0x4BEA-0x4BFE). Asserts the T-state total and
...
//
// loc_4bea has a single control-flow path (two unconditional djnz fill loops),
...
// Minimal, faithful stand-in for the machine (identical to loc_23e8.test.js):
// real Regs, real AddressSpace/Io, step/call/ret/push16 modelled as the DK
// machine does. loc_4bea makes no calls, so `calls` should stay empty.
class MockMachine {
  constructor() {
    this.regs = new Regs();
    this.io = new Io();
...
test("loc_4bea clears both blocks, preserves flags, and returns", () => {
