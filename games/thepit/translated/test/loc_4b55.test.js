...
//
// Drafter test for loc_4b55 (ROM 0x4b55-0x4bc6, The Pit): decode the DSW byte at
...
//
// Run: node --test games/thepit/translated/test/loc_4b55.test.js
...
function makeMachine({ dsw = 0x00, mem8002 = 0x00, f = 0x00 } = {}) {
  ...
  const m = {
    ...
    call(addr) {
      ...
      // machine state at the instant control leaves loc_4b55 and hand back a sentinel.
      this.tail = {
        addr,
  ...
  m.push16(RET); // the return address loc_4b55's `ret` must pop on the non-tail path
  return m;
}
...
test("DSW=0x00: decodes the parameter block, 419 T, boundaries, returns", () => {
  ...
  console.log("  loc_4b55 DSW=0x00: 419 T, block=[01 02|0a|37|00|00|00(0x8051)|03], ret");
});
...
function checkContractFF(m, ret) {
  ...
  assert.equal(ret, SENTINEL, "loc_4b55 returns loc_4f47's result straight through");
...
test("DSW=0xFF: mirror path decodes the block and tail-jumps to loc_4f47", () => {
  ...
  console.log("  loc_4b55 DSW=0xFF: 373 T, block=[00 00|0c|2d|01|01|02(0x8051)|04], jp -> 0x4f47");
});

// -- TEETH -------------------------------------------------------------------
// A faithful copy of loc_4b55 with EXACTLY ONE break: the core fold's `and c`
