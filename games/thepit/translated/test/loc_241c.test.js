...
/**
 * Drafter test for the translated loc_241c (ROM 0x241c, The Pit).
 *
 ...
 *
 * Run: node --test games/thepit/translated/test/loc_241c.test.js
 */
...
// Minimal machine matching the surface loc_241c uses: regs, mem, step, call, ret,
...
test("loc_241c Path A: timer fires, tile advances, tail-jumps to 0x23e8", () => {
  const m = makeMachine();
  runPathA(m);
  assertPathAGolden(m);
});
...
test("loc_241c Path B: (0x8010) < 0x0A -> immediate `ret c`, no work", () => {
  const m = makeMachine();
  seatCaller(m);
...
test("loc_241c Path C: (0x8067) != 1 -> decrement, store, `ret nz`", () => {
  const m = makeMachine();
  seatCaller(m);
...
test("loc_241c MUTATION: `ld ix,(nn)` mis-charged 16T (not 20T) is caught", () => {
  const m = makeMachine();
