...
//
// Drafter test for loc_319d (ROM 0x319D-0x33B9) -- the per-object move/collision
...
test("loc_319d fast path: matched target column tail-jumps to 0x3458", () => {
...
test("loc_319d countdown path: decrements 0x808b and returns via ret nz", () => {
  const m = new MockMachine();
...
test("loc_319d deep path: decodes 0x8089/0x808d then tail-jumps to 0x347d", () => {
  const m = new MockMachine();
  setupDeep(m);
  loc_319d(m);
  checkDeep(m);
...
function loc_319d_MUTANT_deep(m) {
  const { regs, mem } = m;
