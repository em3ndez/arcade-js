...
//
// Drafter test for the translated sub_483a (ROM 0x483a-0x4893), The Pit.
//
...
// cycles, so the asserted T-state total is sub_483a's OWN instruction cost and
// the register/memory effects are isolated from the callees'.
//
// sub_483a's defining feature is the two-way branch on (0x802b): `dec a` then
...
import { sub_483a } from "../sub_483a.js";
...
// as a real CALL into sub_483a would leave it, so the tail-jump's ret has a
...
test("sub_483a fall-through (0x802b != 1): 0x0b/0x97 panel, 4 draws via 0x3dea x2; 250 T", () => {
  checkFallThrough(run(sub_483a, 0x02));
});

test("sub_483a branch (0x802b == 1): loc_4875 0x0c/0x96 panel, 1 draw; 184 T", () => {
  checkBranch(run(sub_483a, 0x01));
});
...
test("mutation (inverted jr z -> jr nz) is caught: wrong arm on the fall-through input", () => {
  ...
  const bad = run(sub_483a_mutant, 0x02);
