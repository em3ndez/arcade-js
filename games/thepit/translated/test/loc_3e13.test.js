...
//
// Drafter test for loc_3e13 (ROM 0x3E13-0x3E31): take a column offset in A,
...
// seam loc_3e13 drives (no m.call in this routine).
function makeMachine(presets = {}) {
  ...
  m.push16(RET); // the return address loc_3e13's `ret` must pop
  return m;
}
...
// Faithful copy of loc_3e13 with the mask instruction removed: the index becomes
...
test("the assertions have teeth: the dropped-AND-mask mutation is caught", () => {
  assertColumnCycle(loc_3e13); // the real routine passes it
  assert.throws(
    () => assertColumnCycle(loc_3e13_mut),
