...
//
// Drafter test for the translated sub_4689 (ROM 0x4689-0x46A1), The Pit.
//
...
//     with DAA carry propagation, then tail-jump (m.call) to sub_46af.
// sub_46af is stubbed as a "pop-and-return" callee (balances the stack, adds no
// cycles) so the asserted T-state total is sub_4689's OWN instruction cost.
...
import { sub_4689 } from "../sub_4689.js";
...
class TestMachine {
  ...
  // the caller. No cycle charge -- sub_46af's own cost is not sub_4689's.
  call(addr) {
...
test("sub_4689: skip path ((0x8001)-1 >= 2) returns without touching the score", () => {
  checkSkip(run(sub_4689, { state8001: 5, lo: 0x40, hi: 0x17 }));
});

// gate byte 3 -> dec=2 -> cp 2 == 0, still NO borrow -> NC -> skip (boundary).
test("sub_4689: gate byte 3 is the skip boundary (cp 0x02 == 0 => NC)", () => {
  const res = run(sub_4689, { state8001: 3, lo: 0x11, hi: 0x22 });
...
function checkAdd(res) {
  ...
  assert.deepEqual(res.calls, [0x46af], "tail-jumps into sub_46af");
  assert.equal(res.lo, 0x03, "0x8031 = BCD low after 0995+8");
  assert.equal(res.hi, 0x10, "0x8034 = BCD high after carry propagates");
}

test("sub_4689: add path carries BCD across bytes (0995 + 8 = 1003)", () => {
  checkAdd(run(sub_4689, { state8001: 1, lo: 0x95, hi: 0x09, b: 0x00, c: 0x08 }));
});

// gate byte 2 -> dec=1 -> cp 2 borrows -> also the add path (no carry case).
test("sub_4689: gate byte 2 also adds; simple BCD 25 + 5 = 30", () => {
  const res = run(sub_4689, { state8001: 2, lo: 0x25, hi: 0x01, b: 0x00, c: 0x05 });
...
// the branch, and tail-jumps to sub_46af. The skip spec MUST reject it.
function sub_4689_mutant(m) {
  const { regs, mem } = m;
...
test("mutation (branch polarity flipped) is caught by the skip spec", () => {
  ...
  const bad = run(sub_4689_mutant, { state8001: 5, lo: 0x40, hi: 0x17 });
