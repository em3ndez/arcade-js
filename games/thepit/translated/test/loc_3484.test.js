...
//
// Drafter test for loc_3484 (ROM 0x3484-0x34d9): the BC=0x0001 / D=0x02 entry into
// the shared object-mover tail at 0x3490. loc_3484 always presets B=0x00, so the
...
//             D=0x02 to 0x8092, then `bit 0,b` (B=0) Z-set -> `jr z` at 0x34a5 to
//             0x34d2.
// Both converge at 0x34d2, which adds C (0x01 = +1) to the X position byte 0x8086.
...
// reject it -- this is what pins the X update to loc_3484's C=+1, not B=0.
...
// Leaf-routine machine double: exactly the surface loc_3484 touches (regs, mem,
...
// 0x3497 taken -> 0x34d2. Position 0x8086: 0x20 + 0x01 = 0x21 (no carry). The
// reload/direction path is NOT reached, so 0x8092 stays untouched.
const PATH_A = {
  steps: [0x3487, 0x3489, 0x3490, 0x3493, 0x3494, 0x3497, 0x34d2, 0x34d5, 0x34d6, 0x34d9],
...
// 0x808b from 0x8091 (0x0a), publish D=0x02 to 0x8092, bit 0,b (B=0) Z-set -> jr z
// taken -> 0x34d2. Same position math as Path A.
const PATH_B = {
  steps: [
    0x3487, 0x3489, 0x3490, 0x3493, 0x3494, 0x3497, 0x3499, 0x349c, 0x349f,
  ...
  dir: 0x02, // D=0x02 published
  posX: 0x21,
};

test("path A: counter still running -> jr nz to 0x34d2, position only (+1 in X)", () => {
...
test("path B: counter expires -> reload + publish D=0x02, jr z (B=0) to 0x34d2", () => {
  const m = makeMachine({ 0x808b: 0x01, 0x8091: 0x0a, 0x8086: 0x20, 0x8092: 0x77 });
  loc_3484(m);
  assertPath(m, PATH_B);
});

test("X position wraps 0xff + 0x01 -> 0x00 with carry (C=+1 step)", () => {
  // Pins C=0x01 specifically: 0xff + 0x01 = 0x00 and the carry OUT is set.
  const m = makeMachine({ 0x808b: 0x05, 0x8086: 0xff });
  loc_3484(m);
  assert.equal(m.mem.read8(0x8086), 0x00, "0x8086 wraps to 0x00");
  assert.equal((m.regs.f & F_C) !== 0, true, "carry set on the 0xff+1 wrap");
});

test("B=0 forces the jr z at 0x34a5: the sprite byte 0x8084 is never written", () => {
  // The 0x34a7-0x34cf branch (which alone writes 0x8084) is dead from loc_3484,
...
test("mutation: `add a,b` for `add a,c` at 0x34d5 is caught", () => {
  // Byte-identical to loc_3484 except the position update adds B (0x00) instead
  // of C (0x01). Cycles are UNCHANGED (add a,r is 4T either way), so only the
  // value assertions can reject it: 0x8086 stays 0x20 (and A ends 0x20, no carry)
  // instead of 0x20 + 0x01 = 0x21.
  function loc_3484_mutant(m) {
    const { regs, mem } = m;
    let next = 0x3484;
    for (;;) {
      switch (next) {
        case 0x3484: {
          regs.bc = 0x0001; m.step(0x3487, 10);
          regs.d = 0x02; m.step(0x3489, 7);
  ...
  const expA = { ...PATH_A, dir: 0x77 };
  assert.throws(() => assertPath(m, expA), /A register|X position/);
});
