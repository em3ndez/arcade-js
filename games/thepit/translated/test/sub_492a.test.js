...
//
// Drafter test for sub_492a (ROM 0x492A-0x4945): the 32-tile video-RAM column
// painter that copies ROM 0x49C7..0x49E6 into video RAM at 0x93F9 stride -0x20,
// then tail-jumps to sub_3e1d (colour-RAM column fill) with A=0x19, C=0x02.
//
...
// the jump) and returns a sentinel standing in for sub_3e1d's return-through.
//
...
//
// Run: node --test games/thepit/translated/test/sub_492a.test.js
...
import { sub_492a } from "../sub_492a.js";

const TAIL = 0x3e1d;
const SENTINEL = "RET_FROM_3e1d"; // stands in for sub_3e1d's return-through
...
function makeMachine() {
  ...
  return {
    ...
    call(addr) {
      ...
      // instant control leaves sub_492a and hand back a sentinel.
      this.tail = {
        addr,
...
function checkContract(m, ret) {
  ...
  // 3. control flow: tail-jump to sub_3e1d, result returned through.
  assert.ok(m.tail, "must have tail-jumped (called a target)");
  assert.equal(m.tail.addr, TAIL, "tail-jump target must be 0x3e1d");
  assert.equal(ret, SENTINEL, "sub_492a must return the tail target's result");
  ...
  assert.equal(m.tail.aAtJump, 0x19, "A into the jp = 0x19 (ld a,0x19 -- sub_3e1d column)");
  assert.equal(m.tail.cAtJump, 0x02, "C into the jp = 0x02 (ld c,0x02 -- sub_3e1d colour)");
  assert.equal(m.tail.carryAtJump, true, "carry set from the final add hl,de (0x9019+0xffe0)");
}

test("sub_492a: 32-tile column copy, T-states, boundaries, exit regs, tail-jump", () => {
  const { m, ret } = run(sub_492a);
  checkContract(m, ret);
  ...
  console.log(`  sub_492a: 1980 T, 32 tiles 0x${SRC.toString(16)}->VRAM stride -0x20, jp -> 0x${TAIL.toString(16)}`);
});

// -- TEETH --------------------------------------------------------------------
// Faithful copy of sub_492a EXCEPT the loop `add hl,de` (and its 11 T) is dropped,
