...
//
// Drafter test for sub_06ac (ROM 0x06AC-0x0737): the per-frame animator that
...
import { sub_06ac } from "../sub_06ac.js";
...
// seam sub_06ac drives. It only needs regs, mem, step, ret (no m.call here).
function makeMachine(presets = {}) {
  ...
  m.push16(RET); // the return address sub_06ac's `ret` must pop
  return m;
}
...
test("the assertions have teeth: the mutation is caught", () => {
  assertMatchPath(sub_06ac); // the real routine passes it
  assert.throws(() => assertMatchPath(sub_06ac_mut), /colour must advance|144 T/);
});
