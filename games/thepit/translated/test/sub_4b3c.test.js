...
/**
 * Drafter test for the translated sub_4b3c (ROM 0x4b3c, The Pit).
 *
 * sub_4b3c is two instructions: `ld a,0xc0` then an UNCONDITIONAL `jr 0x4b46`
 * into the shared control-latch setup sub_4b46. It is modelled as a tail-jump
 ...
 * real routine -- "control transferred to 0x4b46 and never came back". sub_4b3c
 ...
 *
 * Run: node --test games/thepit/translated/test/sub_4b3c.test.js
 */
import test from "node:test";
import assert from "node:assert/strict";

import { Regs } from "../../../../core/cpu/z80.js";
import { sub_4b3c } from "../sub_4b3c.js";

// Leaf-machine double: exactly the surface sub_4b3c touches (regs, step, call).
// step records target + charges cycles; call records the tail-jump target without
// invoking a real routine (sub_4b46 is a separate unit); ret would flip `returned`
...
test("sub_4b3c: loads A=0xC0 and tail-jumps to sub_4b46, 19 T", () => {
  const m = makeMachine();
...
// catch it.
test("sub_4b3c MUTATION: jr target 0x4b46 -> 0x4b44 is caught", () => {
  function sub_4b3c_mutant(m) {
    const { regs } = m;
    regs.a = 0xc0;
    m.step(0x4b3e, 7);
