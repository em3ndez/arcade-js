...
//
// Drafter test for the translated loc_4816 (ROM 0x4816-0x4839), The Pit.
//
// Runs the routine on a lightweight machine built from the REAL thepit address
// space (boards/thepit/memory.js) + Io + the shared Z80 Regs. loc_4816 is
...
// total is loc_4816's OWN instruction cost and the tail-jump's return lands on
...
function checkAll(res) {
  ...
  assert.deepEqual(
    ...
    "three returning calls then a tail-jump into loc_3e01",
  );
...
test("loc_4816: writes the HUD cursor/fill/count, loads IX, calls + tail-jumps", () => {
  checkAll(run(loc_4816));
});
