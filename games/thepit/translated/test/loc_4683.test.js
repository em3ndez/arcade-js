...
/**
 * Drafter test for loc_4683 (ROM 0x4683) — the "+20 BCD points" scorer that
 * guards on the active-player slot at 0x8001, does a two-byte `daa`-corrected BCD
 * add of BC into the score at 0x8031/0x8034, and TAIL-jumps to loc_46af.
 *
 ...
 *   - the head call 0x4c8f and the tail dispatch to loc_46af (result returned through).
 *
 ...
 *
 * Run: node --test games/thepit/translated/test/loc_4683.test.js
 */
...
const TAIL = 0x46af; // loc_46af, repaint the digits (tail-jump target)
const SENTINEL = "RET_FROM_46af"; // stands in for loc_46af's return-through
...
function checkAwardContract(m, ret) {
  ...
  // control flow: tail-jumped to loc_46af, its result returned through.
  assert.ok(m.tail, "must have tail-jumped");
  assert.equal(m.tail.addr, TAIL, "tail target must be loc_46af (0x46af)");
  assert.equal(ret, SENTINEL, "loc_4683 must return the tail target's result");
...
test("award: BCD +0x20 with daa carry propagation, then tail-jump to loc_46af", () => {
  const { m, ret } = run(loc_4683, AWARD);
  ...
  console.log(`  loc_4683 award: 138 T, 0x1285+0x20=0x1305, tail -> 0x${TAIL.toString(16)}`);
});
...
test("guard: slot NOT in {1,2} skips the award and returns via the 0x4672 ret", () => {
  ...
  const { m, ret } = run(loc_4683, { 0x8001: 0x05, 0x8031: 0x85, 0x8034: 0x12 });
  ...
  console.log("  loc_4683 guard: 73 T, jr nc -> ret @0x4672, score untouched");
});

test("slot=2 also awards (boundary of the cp 0x02 guard)", () => {
  // slot=2 -> dec a=1, cp 0x02 -> carry set -> jr NOT taken -> award.
  const { m } = run(loc_4683, { 0x8001: 0x02, 0x8031: 0x00, 0x8034: 0x00 });
