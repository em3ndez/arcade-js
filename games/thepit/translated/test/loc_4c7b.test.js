...
//
// Drafter test for loc_4c7b (ROM 0x4c7b-0x4c7e, The Pit): a sound-request stub that
// loads the command index 0x0b into A and UNCONDITIONALLY tail-jumps into the shared
// enqueue handler loc_4ca5 (0x4ca5). loc_4ca5's own ret unwinds to loc_4c7b's caller,
...
//   * A = 0x0b seeded; exact 19 T total (ld a,n = 7, jr = 12); boundary sequence
//     [0x4c7d, 0x4ca5]; control leaves via return m.call(0x4ca5) (no ret); the
//     caller's seeded flags survive (neither instruction writes F).
//
// TEETH: a faithful twin whose only break is the payload immediate `ld a,0x0b`
...
// into loc_4ca5 both diverge 0x0b -> 0x0a).
//
// Run: node --test games/thepit/translated/test/loc_4c7b.test.js
...
function makeMachine({ f = 0x00 } = {}) {
  ...
  const m = {
    ...
    call(addr) {
      ...
      // machine state at the instant control leaves loc_4c7b, return a sentinel.
...
const EXPECTED_PC_SEQ = [0x4c7d, 0x4ca5];
...
function checkContract(m, ret) {
  // Payload: A = the command index 0x0b.
  assert.equal(m.regs.a, 0x0b, "A = the sound-command index 0x0b");
  ...
  assert.equal(m.tail.a, 0x0b, "A into loc_4ca5 = 0x0b");
  assert.equal(ret, SENTINEL, "loc_4c7b returns loc_4ca5's result straight through");
...
test("loc_4c7b: seeds A=0x0b and tail-jumps to loc_4ca5, 19 T, no ret", () => {
  const m = makeMachine();
  const ret = loc_4c7b(m);
  checkContract(m, ret);
  console.log("  loc_4c7b: A=0x0b, 19 T, [0x4c7d, 0x4ca5], jr -> 0x4ca5 (tail)");
});

test("loc_4c7b: ld a / jr touch no flags -- the caller's F survives unchanged", () => {
...
// -- TEETH -------------------------------------------------------------------
// A faithful copy of loc_4c7b with EXACTLY ONE break: the payload immediate
// `ld a,0x0b` (3e 0b) swapped for `ld a,0x0a` (3e 0a) -- its neighbour loc_4c77's
// value, and the ONLY byte that distinguishes this stub from its ~20 siblings.
// The contract rejects it: A and the arg into loc_4ca5 both flip 0x0b -> 0x0a.
function brokenLoc4c7b(m) {
  const { regs } = m;
  regs.a = 0x0a; // BUG: ld a,0x0a instead of ld a,0x0b (neighbour loc_4c77's index)
  m.step(0x4c7d, 7);
  m.step(0x4ca5, 12);
  return m.call(0x4ca5);
}

test("TEETH: the ld a,0x0b -> ld a,0x0a twin is CAUGHT by the contract", () => {
  // The real routine passes the contract.
  {
    const m = makeMachine();
    checkContract(m, loc_4c7b(m));
  }
  // The mutant fails it -- A (and the arg into loc_4ca5) diverge.
  const m = makeMachine();
  const ret = brokenLoc4c7b(m);
  assert.throws(
    () => checkContract(m, ret),
    /0x0b/,
    "the contract FAILED to catch ld a,0x0b -> ld a,0x0a -- it has no teeth",
  );
  ...
  assert.equal(m.tail.a, 0x0a, "mutant: 0x0a would be enqueued, not 0x0b");
});
