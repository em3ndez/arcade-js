...
//
// Drafter test for loc_4b14 (ROM 0x4b14-0x4b19): ENABLE the NMI by writing 0x01
...
// disassembly. Ends with a MUTATION (`ld a,0x00`, the sibling loc_4b10 datum
...
function makeMachine({ latch = 0x00, f = 0x00 } = {}) {
  ...
  m.push16(RET); // the return address loc_4b14's `ret` must pop
  return m;
}
...
// ---- MUTATION: `ld a,0x00` (the sibling loc_4b10 datum) --------------------
// loc_4b10 loads 0x00 and falls into the SAME loc_4b16 tail to CLEAR the mask
...
function loc_4b14_mut(m) {
  const { regs, mem } = m;
  regs.a = 0x00; // BUG: 0x00 (loc_4b10) instead of 0x01 -- CLEARS the mask
  m.step(0x4b16, 7);
...
test("the assertions have teeth: the ld-a-0x00 mutation is caught", () => {
  assertEnablesNmi(loc_4b14); // the real routine passes it
  assert.throws(() => assertEnablesNmi(loc_4b14_mut), /ENABLE the NMI|datum 0x01/);
});
