...
//
// Drafter test for sub_4c11 (ROM 0x4C11-0x4C1B): clear 128 (0x80) bytes of
...
import { sub_4c11 } from "../sub_4c11.js";
...
// seam sub_4c11 drives (no m.call in this routine).
function makeMachine() {
  ...
  m.push16(RET); // the return address sub_4c11's `ret` must pop
  return m;
}
...
// Faithful copy of sub_4c11 with the `inc l` removed: HL never advances, so all
...
test("the assertions have teeth: the dropped-`inc l` mutation is caught", () => {
  assertClearSpan(sub_4c11); // the real routine passes it
  assert.throws(
    () => assertClearSpan(sub_4c11_mut),
