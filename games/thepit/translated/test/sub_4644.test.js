...
//
// Drafter test for sub_4644 (ROM 0x4644-0x4672): copies the active player's
...
import { sub_4644 } from "../sub_4644.js";
...
// sub_4644 drives. No m.call here (the routine has none).
function makeMachine(presets = {}) {
  ...
  m.push16(RET); // the return address sub_4644's `ret` must pop
  return m;
}
...
test("the assertions have teeth: the dropped-inc-iy mutation is caught", () => {
  assertPlayer2Path(sub_4644); // the real routine passes it
  assert.throws(() => assertPlayer2Path(sub_4644_mut), /player-2 block|262 T/);
});
