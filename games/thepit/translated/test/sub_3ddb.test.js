...
//
// Drafter test for the translated sub_3ddb (ROM 0x3ddb-0x3e00), The Pit.
//
...
// ROM (the copyrighted image is never committed). sub_3ddb calls nothing, so the
...
// the first pass too) -- the exact error of confusing sub_3ddb with its sibling
// sub_3dea. It corrupts the first written cell (poison 0xFF instead of the
...
import { sub_3ddb } from "../sub_3ddb.js";
...
test("sub_3ddb: cap + backwards (ix) column, cursor stored back; 310 T", () => {
  checkSpec(run(sub_3ddb));
});

// -- MUTATION: enter the loop at loc_3df4 (the sub_3dea entry) instead of
