...
//
// Drafter test for loc_472c (ROM 0x472C-0x4784, The Pit). Asserts the T-state
...
// loc_46af here -- so we seed IX to a VRAM base (as loc_46af would leave it), so
...
class MockMachine {
  constructor() {
    ...
    this.regs.ix = 0x90c1; // VRAM base loc_46af would have set (else-branch value)
    this.cycles = 0;
    this.pc = 0x472c;
    this.calls = [];
    this.returned = false;
...
test("loc_472c 1-2 player path: draws both scores, calls 0x47E1, paints HUD columns", () => {
...
test("loc_472c 3+ player path: jr nc taken selects 0x48E5 instead of 0x47E1", () => {
  const m = new MockMachine();
  setupPathB(m);
  loc_472c(m);
  checkPathB(m);
