...
//
// Drafter test for the translated loc_467b (ROM 0x467b-0x4682), The Pit.
//
// loc_467b is a thin score-add entry: fire sound effect 0x10, preload the BCD
// increment BC=0x0010, then TAIL-jump into the shared adder loc_4689. Runs on a
...
// T-state total is loc_467b's OWN instruction cost:
//   17 (call) + 10 (ld bc,nn) + 12 (jr) = 39 T.
//
// The load-bearing decision is the terminal `jr 0x4689`: it is a TAIL-JUMP (a JR
// pushes nothing), so loc_4689's own `ret` returns to loc_467b's caller, and the
...
//
// loc_467b's own three instructions (CALL / LD BC,nn / JR) touch NO flag and do
...
class TestMachine {
  ...
  // return. No cycle charge -- a callee's cost is not loc_467b's.
  call(addr) {
...
function checkSpec(res) {
  assert.equal(res.cycles, EXP_CYCLES, "T-state total (39 = loc_467b's own instructions)");
...
test("loc_467b: sound call + BC=0x0010 + tail-jump 0x4689; 39 T", () => {
  checkSpec(run(loc_467b));
});
...
test("mutation (BC=0x0020 not 0x0010) is caught by the register assert", () => {
  const bad = run(loc_467b_incMutant);
...
function loc_467b_cycleMutant(m) {
  const { regs } = m;
  m.push16(0x467e);
  m.step(0x4c8f, 17);
  m.call(0x4c8f);
...
test("mutation (jr mischarged 10 T not 12) is caught by the cycle assertion", () => {
  const bad = run(loc_467b_cycleMutant);
...
// `m.call(); m.ret()` shape. The tail-jump pushed nothing, so loc_4689's ret
...
function loc_467b_doubleRetMutant(m) {
  const { regs } = m;
  m.push16(0x467e);
  m.step(0x4c8f, 17);
  m.call(0x4c8f);
...
test("mutation (tail-jump written as m.call+m.ret, a double-pop) is caught by pc/sp", () => {
  const bad = run(loc_467b_doubleRetMutant);
