...
//
// Drafter test for loc_3e01 (ROM 0x3E01-0x3E12): the column fill that loads a
...
// seam loc_3e01 drives (no m.call in this routine).
function makeMachine(presets = {}) {
  ...
  m.push16(RET); // the return address loc_3e01's `ret` must pop
  return m;
}
...
// Faithful copy of loc_3e01 with the row-advance removed: every write lands on
...
test("the assertions have teeth: the dropped-stride mutation is caught", () => {
  assertColumnFill(loc_3e01); // the real routine passes it
  assert.throws(
    () => assertColumnFill(loc_3e01_mut),
    /row 1 .* filled|185 T/,
