...
/**
 * Drafter test for the translated sub_4b1a (ROM 0x4b1a, The Pit).
 *
 ...
 *
 * sub_4b1a advances the 16-bit PRNG value at 0x800d(low)/0x800e(high) one shift-
 ...
 *
 * Run: node --test games/thepit/translated/test/sub_4b1a.test.js
 */
import test from "node:test";
import assert from "node:assert/strict";

import { Regs, F_C } from "../../../../core/cpu/z80.js";
import { sub_4b1a } from "../sub_4b1a.js";
...
function makeMachine() {
  ...
  return {
    ...
    call(addr) {
      ...
      return undefined; // sub_4b1a makes no calls
    },
  };
}
...
test("sub_4b1a: primary (non-zero) path -- shift, store, 142 T, ret", () => {
  const m = makeMachine();
  run(m, 0x6b, 0xa4);
  assertPrimary(m);
});
...
test("sub_4b1a: seed (all-zero) path -- reseeds C=0x02, 144 T", () => {
  const m = makeMachine();
  run(m, 0x00, 0x00);
  assertSeed(m);
});
...
test("sub_4b1a MUTATION: CB `srl a` mis-charged 4T (not 8T) is caught", () => {
  const m = makeMachine();
...
test("sub_4b1a MUTATION: swapped output stores (0x800d<->0x800e) is caught", () => {
  const m = makeMachine();
