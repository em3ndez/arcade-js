...
/**
 * Drafter test for the translated sub_28ab (ROM 0x28ab, The Pit) -- the object/tile
 ...
 *
 * Run: node --test games/thepit/translated/test/sub_28ab.test.js
 */
import test from "node:test";
import assert from "node:assert/strict";

import { Regs } from "../../../../core/cpu/z80.js";
import { sub_28ab } from "../sub_28ab.js";
...
test("sub_28ab Path A: id 0x9d, no bias, counter non-zero -> arm 0x80b1=0x08", () => {
  const m = makeMachine();
  setupPathA(m);
  sub_28ab(m);
  assertPathAGolden(m);
...
test("sub_28ab Path B: counter==0 -> second-entity commit + 0x2dc3 table remap", () => {
  const m = makeMachine();
  seatCaller(m);
  m.mem.write16(0x806e, 0x8100); // IX pointer
...
test("sub_28ab Path C: D == 0x00 -> immediate `ret nz` after loc_28e4", () => {
  const m = makeMachine();
  seatCaller(m);
...
test("sub_28ab MUTATION: `ld ix,(0x806e)` mis-charged 16T (not 20T) is caught", () => {
  const m = makeMachine();
