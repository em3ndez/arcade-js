...
/**
 * Drafter test for the translated sub_4632 (ROM 0x4632, The Pit).
 *
 ...
 *
 * Run: node --test games/thepit/translated/test/sub_4632.test.js
 */
import test from "node:test";
import assert from "node:assert/strict";

import { Regs } from "../../../../core/cpu/z80.js";
import { sub_4632 } from "../sub_4632.js";

// Minimal machine matching the surface sub_4632 uses: regs, mem, step, ret,
...
function makeMachine() {
  ...
  return {
    ...
    call(addr) {
      ...
      return undefined; // callees stubbed -- sub_4632 makes none anyway
    },
  };
}
...
test("sub_4632 Path Z: (0x8002)==1 -> dest 0x8029, 5-field stride-3 copy", () => {
  const m = makeMachine();
  runPathZ(m);
  assertPathZGolden(m);
});
...
test("sub_4632 Path NZ: (0x8002)!=1 -> inc ix, dest 0x802A", () => {
  const m = makeMachine();
  runPathNZ(m);
...
test("sub_4632 MUTATION: `inc ix` mis-charged 6T (not 10T) is caught", () => {
  const m = makeMachine();
