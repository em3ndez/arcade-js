...
//
// Drafter test for loc_1362 (ROM 0x1362-0x13c8): the actor/level state-block
...
// The exact final memory image loc_1362 leaves. Work RAM powers up to 0 so the
...
// Minimal leaf-routine machine double: exactly the surface loc_1362 touches
...
function makeMachine() {
  ...
  const m = {
    ...
    call() {
      throw new Error("loc_1362 makes no calls");
    },
  };
...
test("loc_1362 seeds the state block with exact values, cycles and control flow", () => {
  const m = makeMachine();
