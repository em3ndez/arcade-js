...
//
// Drafter test for sub_4785 (ROM 0x4785-0x47A0): the border/HUD column drawer.
...
// loads C=0x01 and A=0x1E, then TAIL-jumps into sub_3e1d.
//
...
import { sub_4785 } from "../sub_4785.js";
...
// seam, plus a RECORDING m.call so the tail-jump into sub_3e1d is observed without
// actually running it (this isolates sub_4785's own T-states and effects).
function makeMachine() {
  ...
  const m = {
    ...
    ret(cycles = 10) {
      this.pc = this.pop16();
      this.tstates += cycles;
      this.returned = true; // sub_4785 must NEVER hit this (it tail-jumps)
    },
    call(addr) {
      this.calls.push(addr);
      this.pc = addr;
      return CALL_SENTINEL; // sub_4785 forwards this via `return m.call(...)`
    },
  };
  return m;
}
...
function assertCopyColumn(fn) {
  ...
  assert.equal(m.tstates, TOTAL_T, `sub_4785 costs ${TOTAL_T} T`);
}

test("copies 32 tiles up 0x93FE (stride 0x20), then tail-jumps into sub_3e1d", () => {
  const m = makeMachine();
  const ret = sub_4785(m);
  ...
  assert.equal(m.regs.c, 0x01, "C = 0x01, the fill byte handed to sub_3e1d");
  assert.equal(m.regs.a, 0x1e, "A = 0x1E, the column offset handed to sub_3e1d");

  // control flow: TAIL-jump, not call+ret
  assert.deepEqual(m.calls, [0x3e1d], "tail-jumps into sub_3e1d exactly once");
  assert.equal(m.returned, false, "sub_4785 issues NO ret — sub_3e1d's ret returns to our caller");
...
// Faithful copy of sub_4785 with `inc ix` removed: every read takes SRC[0], so
...
test("the assertions have teeth: the dropped inc-ix mutation is caught", () => {
  assertCopyColumn(sub_4785); // the real routine passes it
  assert.throws(
    () => assertCopyColumn(sub_4785_mut),
