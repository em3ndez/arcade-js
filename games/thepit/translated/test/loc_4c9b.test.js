...
//
// Drafter test for loc_4c9b (ROM 0x4c9b-0x4c9e, The Pit): a sound-request stub that
// loads the command index 0x13 into A and UNCONDITIONALLY tail-jumps into the shared
// enqueue handler loc_4ca5 (0x4ca5). loc_4ca5's own ret unwinds to loc_4c9b's caller,
...
//   * A = 0x13 seeded; exact 19 T total (ld a,n = 7, jr = 12); boundary sequence
//     [0x4c9d, 0x4ca5]; control leaves via return m.call(0x4ca5) (no ret); the
//     caller's seeded flags survive (neither instruction writes F).
//
// TEETH: a faithful twin whose only break is the payload immediate `ld a,0x13`
// swapped for `ld a,0x14` (its reachable neighbour loc_4c9f's value) -- the single
// byte that distinguishes this stub from its ~20 siblings -- must be CAUGHT (A and
// the arg into loc_4ca5 both diverge 0x13 -> 0x14).
//
// Run: node --test games/thepit/translated/test/loc_4c9b.test.js
...
function makeMachine({ f = 0x00 } = {}) {
  ...
  const m = {
    ...
    call(addr) {
      ...
      // machine state at the instant control leaves loc_4c9b, return a sentinel.
...
const EXPECTED_PC_SEQ = [0x4c9d, 0x4ca5];
...
function checkContract(m, ret) {
  // Payload: A = the command index 0x13.
  assert.equal(m.regs.a, 0x13, "A = the sound-command index 0x13");
  ...
  assert.equal(m.tail.a, 0x13, "A into loc_4ca5 = 0x13");
  assert.equal(ret, SENTINEL, "loc_4c9b returns loc_4ca5's result straight through");
...
test("loc_4c9b: seeds A=0x13 and tail-jumps to loc_4ca5, 19 T, no ret", () => {
  const m = makeMachine();
  const ret = loc_4c9b(m);
  checkContract(m, ret);
  console.log("  loc_4c9b: A=0x13, 19 T, [0x4c9d, 0x4ca5], jr -> 0x4ca5 (tail)");
});

test("loc_4c9b: ld a / jr touch no flags -- the caller's F survives unchanged", () => {
...
// -- TEETH -------------------------------------------------------------------
// A faithful copy of loc_4c9b with EXACTLY ONE break: the payload immediate
// `ld a,0x13` (3e 13) swapped for `ld a,0x14` (3e 14) -- its reachable neighbour
// loc_4c9f's value, and the ONLY byte that distinguishes this stub from its ~20
// siblings. The contract rejects it: A and the arg into loc_4ca5 both flip
// 0x13 -> 0x14.
function brokenLoc4c9b(m) {
  const { regs } = m;
  regs.a = 0x14; // BUG: ld a,0x14 instead of ld a,0x13 (neighbour loc_4c9f's index)
  m.step(0x4c9d, 7);
  m.step(0x4ca5, 12);
  return m.call(0x4ca5);
}

test("TEETH: the ld a,0x13 -> ld a,0x14 twin is CAUGHT by the contract", () => {
  // The real routine passes the contract.
  {
    const m = makeMachine();
    checkContract(m, loc_4c9b(m));
  }
  // The mutant fails it -- A (and the arg into loc_4ca5) diverge.
  const m = makeMachine();
  const ret = brokenLoc4c9b(m);
  assert.throws(
    () => checkContract(m, ret),
    /0x13/,
    "the contract FAILED to catch ld a,0x13 -> ld a,0x14 -- it has no teeth",
  );
  // Concretely: the wrong index reaches A and would enqueue the wrong sound.
  assert.equal(m.regs.a, 0x14, "mutant: A = 0x14 (wrong sound-command index)");
  assert.equal(m.tail.a, 0x14, "mutant: 0x14 would be enqueued, not 0x13");
});
