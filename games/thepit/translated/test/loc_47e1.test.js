...
//
// Drafter test for the translated loc_47e1 (ROM 0x47E1-0x4815, The Pit).
//
// loc_47e1 seeds the shared tile-plotter params at 0x8055-0x8059, calls the four
...
// T-state total is loc_47e1's OWN instruction cost. Assertions cover: the exact
...
// loc_47e1's. This mirrors games/thepit/translated/test/loc_4689.test.js.
class TestMachine {
  constructor() {
    this.io = new Io();
...
test("loc_47e1: seeds plot params, calls 4 helpers, tail-jumps to loc_3e01", () => {
  check(run(loc_47e1));
});

// Intermediate 0x8055 values are observable via the stubbed callees: the run
// length is 1 when loc_3dea runs and 7 when loc_3ddb runs. A machine that snoops
// 0x8055 at each call confirms the writes land in order (not just the final 9).
test("loc_47e1: 0x8055 = 1 at loc_3dea, 7 at loc_3ddb, 9 at the tail", () => {
  const m = new TestMachine();
  const seen = {};
  ...
  m.push16(SENTINEL);
  loc_47e1(m);
  assert.equal(seen[0x3dea], 0x01, "run length 1 when loc_3dea runs");
  assert.equal(seen[0x3ddb], 0x07, "run length 7 when loc_3ddb runs");
  assert.equal(seen[0x3e01], 0x09, "run length 9 at the tail into loc_3e01");
});
