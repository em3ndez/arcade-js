...
/**
 * Drafter test for the translated loc_48e5 (ROM 0x48e5, The Pit).
 *
 ...
 * measured T-state total is loc_48e5's OWN 155 T and nothing downstream.
 *
 ...
 *
 * Run: node --test games/thepit/translated/test/loc_48e5.test.js
 */
...
// Minimal machine matching the surface loc_48e5 uses: regs, mem, step, call,
...
// stubbed (recorded, not invoked) to isolate loc_48e5.
...
test("loc_48e5: straight-line column setup, tail-jumps to 0x3e01", () => {
  const m = makeMachine();
  run(m);
  assertGolden(m);
});
...
test("loc_48e5 MUTATION: `ld ix,nn` mis-charged 10T (not 14T) is caught", () => {
  const m = makeMachine();
