...
//
// Drafter test for the translated sub_4b46 (ROM 0x4b46-0x4b54), The Pit.
//
// Runs the routine on a lightweight machine built from the REAL thepit address
// space (boards/thepit/memory.js) + Io + the shared Z80 Regs. sub_4b46 is a
...
// asserted T-state total (74) is sub_4b46's OWN instruction cost and the effects
...
import { sub_4b46 } from "../sub_4b46.js";
...
// enters with CARRY SET so the test can prove sub_4b46 itself writes no flags.
const A_IN = 0xc0;
const F_IN = F_C; // 0x01
...
// sub_4b46 would leave it, so the tail-jump's ret has a target and the stack
...
function checkSpec(res, aIn = A_IN) {
  ...
  assert.equal(res.f, F_IN, "sub_4b46 writes no flags -> F = entry F (carry preserved)");
}

test("sub_4b46: stores A->0x8057, calls 0x4c11/0x4c27/0x4c37, tail-jumps 0x4c1c; 74 T", () => {
  checkSpec(run(sub_4b46));
});

test("sub_4b46: A=0x90 door value also lands in 0x8057 (A is a pure input)", () => {
  checkSpec(run(sub_4b46, 0x90), 0x90);
});
