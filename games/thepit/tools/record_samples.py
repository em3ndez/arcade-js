...
"""Record The Pit's sounds from YOUR MAME + YOUR ROM into local sample files.

WHY THIS TOOL EXISTS (the copyright position)
---------------------------------------------
arcade-js plays audio ABOVE the emulation: the board watches the program's
writes to its sound latch and a SamplePlayer plays a named sample. arcade-js
does NOT emulate The Pit's audio subsystem (the second Z80 + its program ROM +
the two AY-3-8910 PSGs). Instead it records what each sound command actually
PRODUCES and replays that clip when the game sends the same command.

That needs samples -- and The Pit's sounds are the ROM authors' copyrighted
work, exactly like its sprites and program ROM. So we ship NONE of it. Same
posture as the bring-your-own-ROM design already used for `games/thepit/rom/`:
the repository contains the *tooling*, you supply the *content*. This script
drives the MAME you already installed, against the romset you already own, on
your own machine, and writes the resulting audio to a gitignored directory
(`games/thepit/audio/samples/`, matched by `.gitignore`'s `games/*/audio/
samples/`). Nothing copyrighted ever enters the repository. Do not commit or
redistribute anything it produces.

THE PIT'S SOUND HARDWARE (mame-src/src/mame/taito/roundup.cpp, `roundup_state`)
------------------------------------------------------------------------------
MUCH simpler than Donkey Kong's -- one command byte, one write address:

  * The main CPU writes a command byte to 0xB800 (a `generic_latch_8` named
    "soundlatch"). That is the ONLY sound write the main program makes.
  * A SECOND Z80 (`audiocpu`, 2.5 MHz) runs its own program ROM (p30). It is
    interrupted every frame by the VSYNC timer (`vsync_callback` ->
    set_input_line(0, ASSERT_LINE)); its IRQ handler POLLS the soundlatch by
    reading it through AY1 port A, acts on any pending command, then clears the
    latch by writing audio I/O port 0x00.
  * Two AY-3-8910 PSGs (ay1 @ ports 0x8e/0x8f, ay2 @ 0x8c/0x8d) are mixed to
    "mono".

Note the difference from DK, and why it matters here: the soundlatch write does
NOT itself assert the audio IRQ -- the free-running VSYNC IRQ does, every frame.
So triggering a command is simply "leave the value in 0xB800 for at least one
frame"; the audio CPU samples it on its own schedule and clears it. There is no
IRQ timing to get right on our side. Writing 0xB800 from the main CPU's program
space (what this tool does) reaches the soundlatch exactly as the ROM's own
write would.

WHAT THIS IS: A DISCOVERY SWEEP, NOT A SOUND MAP
------------------------------------------------
This tool does NOT assume which command means which sound. It sweeps the command
byte and records what each value actually produced. Peak and RMS are reported
for every clip so it is obvious which commands made sound and which were silent.
The results are EVIDENCE for building the command->sound map; they are not the
map. The default sweep covers 0x00-0x1f and 0x80-0x98 -- the low range plus the
full plausible high range around the values a 2000-frame attract run was
observed to send (130, 134, 137, 138, 143-148 decimal).

TWO PASSES PER COMMAND -- ONE-SHOT vs SUSTAINED
-----------------------------------------------
Each command is swept TWICE, exactly like DK's recorder, so a self-contained
effect can be told apart from a looping tune:

  * PULSE pass  -- trigger the command, record a short window (--gap). A
    self-contained effect finishes inside it; a looping tune is still sounding
    at the window's end (flagged `clipped_at_slot_end`).
  * SUSTAIN pass -- trigger the same command, record a long window
    (--sustain-gap). A looping tune plays for the whole window; the clip written
    is ONE measured LOOP PERIOD (normalised autocorrelation) so the file is a
    whole repeating phrase whose end joins its start.

A command is classified SUSTAINED when its pulse pass is still sounding at the
window boundary AND its sustain pass runs far longer (SUSTAIN_RATIO x). The
classification is reported per clip in index.json, so it can be checked against
-- and disagree with -- a hand-written sound map.

HOW THE ROM IS KEPT OUT OF THE WAY
----------------------------------
The machine must actually boot (the audio CPU has to be alive), but a booted The
Pit writes its own attract-mode sound commands, which would contaminate every
clip. So a write tap on 0xB800 replaces every program-originated write with the
value THIS tool is currently holding (0 = "no command" during the silence gaps).
The program cannot touch the sound latch while the sweep runs. The gaps between
slots being silent is the proof that this works, and it is measured and reported
(--report-gaps).

STOPPING A SOUND BETWEEN SLOTS
------------------------------
Every observed command self-terminates (its sound decays to silence well before
the next slot), so a fixed silence gap is enough to separate clips -- verified
by --report-gaps. If some command instead starts a tune that loops forever, the
audio CPU ignores a latched 0 (0 = "no new command", not "stop"), so it would
bleed into the following gap and --report-gaps would flag it. When that happens,
find the real stop command in the sweep results and pass it as --stop-value; it
is written at the end of every slot to silence a sustained command.

USAGE
-----
  games/thepit/tools/record_samples.py --rompath ~/Downloads
  games/thepit/tools/record_samples.py --commands 0x80-0x98 --report-gaps
  games/thepit/tools/record_samples.py --dry-run     # show the schedule + argv
"""
...
# --- The Pit sound hardware address ---------------------------------------
# Mirrors boards/thepit/hardware.json "writeRanges": sound_latch.
SOUND_LATCH = 0xB800  # generic_latch_8 "soundlatch" -- the command byte

# What the ROM is prevented from writing while a sweep runs: the one sound latch.
# It is write-only from the main Z80's side (a READ of 0xB800 is the watchdog
# reset, a different function), so muting the WRITE cannot change what the
# program computes -- only which sound commands reach the audio CPU.
DEFAULT_MUTE = "0xB800"

DEFAULT_OUT = os.path.join("games", "thepit", "audio", "samples")
DEFAULT_HARDWARE = os.path.join("boards", "thepit", "hardware.json")
...
def center(seg):
    ...
    MEASURED, and the reason a plain mean will not do: The Pit's two AY-3-8910s
    sit at a large idle DC bias that STEPS by ~2000 16-bit LSB between sounds
    (e.g. -4100 while a sound plays, -6140 when silent). A single MEAN removed
...
def parse_ranges(spec):
    """'0xB800' -> [(0xB800,0xB800)]; '0xB000-0xB007,0xB800' -> two ranges."""
    out = []
    for part in spec.split(","):
        part = part.strip()
        if not part:
...
# How many times longer the sustain pass (stop written late) must run than the
# pulse pass (stop written early) before we believe the command drives a
# SUSTAINED (looping) tune -- one whose length tracks the stop time -- rather
# than a one-shot, whose length is the same in both passes.
SUSTAIN_RATIO = 3.0
...
def build_schedule(args):
    ...
    `key` is the command being measured (`cmd_137`) and names the ONE clip file
    it produces. Every slot triggers the command with a single write to 0xB800
    (the audio CPU samples it on the next VSYNC IRQ and clears it), then writes
    `stop` at the end of its hold -- a no-op for the self-terminating commands,
    but the hook by which a real --stop-value silences a sustained one. `dt` in
    `writes` is seconds from the slot's own start.
    """
    slots = []
        ...
        slots.append(
            {
                ...
                "hold": hold, "gap": gap,
                "writes": [(0.0, SOUND_LATCH, v), (hold, SOUND_LATCH, stop)],
            }
        )
        slots.append(
            {
                ...
                "hold": shold, "gap": sgap,
                "writes": [(0.0, SOUND_LATCH, v), (shold, SOUND_LATCH, stop)],
            }
        )
    return slots
...
LUA_TEMPLATE = """\
-- SPDX-License-Identifier: GPL-3.0-only
-- GENERATED by games/thepit/tools/record_samples.py -- do not edit.
--
-- Drives The Pit's sound latch (0xB800) on a fixed schedule and logs the ACTUAL
-- emulated time of every slot, so the recorded wav is split by measured
-- timestamps rather than by assumed ones.
...
def resolve_passes(rows, slots, meta, args, out_dir):
    ...
    The pulse pass and the sustain pass differ ONLY in WHEN the stop value is
    written (--hold early vs --sustain-hold late), and that is exactly what
    tells a looping tune from a self-contained effect:

      SUSTAINED (looping) <=> the sound's length TRACKS the stop time, i.e. the
                              sustain pass (stop late) ran far longer than the
                              pulse pass (stop early): SUSTAIN_RATIO x. A looping
                              tune keeps playing until it is stopped; a one-shot
                              self-terminates at the same length no matter when
                              the stop is written, so the two passes match.
      ...
      one-shot   -> the clip is the pass that captured the whole sound: the
                    sustain pass (its later stop cannot cut a self-terminating
                    effect short), falling back to the pulse pass.
    ...
    for slot in slots:
        ...
        if sustain is not None:
            ...
            sustained = (
                ...
                and sustain["duration_s"] >= SUSTAIN_RATIO * max(pulse["duration_s"], 0.05)
            )
...
def main():
    p = argparse.ArgumentParser(
        ...
        description=(
            "Record The Pit's sounds from YOUR MAME + YOUR ROM into local, "
            "gitignored sample files."
        ),
        epilog=(
            "COPYRIGHT / BRING-YOUR-OWN-ROM\n"
            "  The Pit's audio is the ROM authors' copyrighted work, exactly like its\n"
            "  sprites and program ROM. arcade-js therefore ships NO The Pit audio. This\n"
            ...
            "\n"
            "WHAT IT DOES\n"
            "  Boots The Pit (thepitu1) headless under the project's determinism flags,\n"
            "  mutes the ROM's own writes to the sound latch (0xB800), then sweeps the\n"
            "  sound-command byte -- writing each value to 0xB800, which the audio Z80\n"
            "  polls on its per-frame VSYNC IRQ -- leaving a fixed silence gap between\n"
            "  each. The single -wavwrite recording is split back into one clip per\n"
            "  command, trimmed, and reported with peak/RMS amplitude.\n"
            "\n"
            ...
            "  building the command->sound map in games/thepit/audio/.\n"
            "\n"
            "  Clips are named cmd_<decimal>.wav (e.g. cmd_137.wav) in the output dir.\n"
            "\n"
            "EXAMPLES\n"
            "  record_samples.py --rompath ~/Downloads\n"
            "  record_samples.py --commands 0x80-0x98 --report-gaps\n"
    ...
    p.add_argument(
        ...
        help="MAME rompath holding YOUR thepitu1 romset (default: %(default)s)",
    )
    p.add_argument(
        "--commands",
        default="0x00-0x1f,0x80-0x98",
        metavar="SPEC",
        help="sound-command values to sweep, e.g. '0x80-0x98,0x8a'. The default "
        "covers the low range plus the full plausible high range around the "
        "values a 2000-frame attract run sent (130,134,137,138,143-148). "
        "(default: %(default)s)",
    )
    p.add_argument(
        "--stop-value",
        ...
        default=0,
        metavar="V",
        help="value written to 0xB800 at the end of every slot to silence a "
    ...
    p.add_argument(
        ...
        type=float,
        default=0.1,
        help="seconds the command is held in 0xB800 before --stop-value is "
    ...
    workdir = tempfile.mkdtemp(prefix="record_samples_thepit_")
    ...
    try:
        ...
        # several of The Pit's tunes are longer/through-composed -- not a failed
        ...
        meta.update(
            {
                "generated_by": "games/thepit/tools/record_samples.py",
