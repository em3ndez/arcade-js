 ...
 *
 * Run: node --test games/thepit/test/registry-sync.test.js
 */
import test from "node:test";
import assert from "node:assert/strict";

import { discover } from "../../../tools/gen-thepit-registry.mjs";
import { ORACLE_ROUTINES } from "../translated/_registry.generated.js";

const hx = (a) => "0x" + a.toString(16).padStart(4, "0");

test("generated registry matches translated/ (fix: node tools/gen-thepit-registry.mjs)", async () => {
  ...
  assert.equal(
    missing.length, 0,
    `generated registry is STALE — missing ${missing.map(hx).join(", ")}. Run: node tools/gen-thepit-registry.mjs`,
  );

  const extra = [...ORACLE_ROUTINES.keys()].filter((a) => !expected.has(a)).sort((a, b) => a - b);
  assert.equal(
    extra.length, 0,
    `generated registry has routines no longer in translated/: ${extra.map(hx).join(", ")}. Run: node tools/gen-thepit-registry.mjs`,
  );
