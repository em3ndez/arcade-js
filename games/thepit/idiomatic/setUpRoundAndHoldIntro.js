...
/**
 * setUpRoundAndHoldIntro — one-time round-start setup: make the selected player's saved progress the
 ...
 * The closing tail hand-off to the round-loop setup (initRoundAndEnterMainLoop, 0x031a) is kept as an m.call
 * boundary: initRoundAndEnterMainLoop falls into the never-returning main loop, so it stays a registry
 ...
 *           by one identical hook on both sides so the waits terminate. The tail initRoundAndEnterMainLoop
