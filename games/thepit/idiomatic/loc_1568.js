...
/**
 * loc_1568 — resolve a moving object's step against the terrain directly under it (and, off the
 ...
 * counterpart of the horizontal handler loc_1704. It is handed the tile the object sits ON, the
 ...
 *
 * Kept as loc_1568: the role (the object's terrain-response resolver for the non-loot case) is
 * clear, but its sibling loc_1704 and the wider tile-resolve family are still un-named, and which
 * object it serves plus the ROM tables' exact semantics are not yet pinned — a single effect-verb
 * would over- or under-claim, so the neutral name stays, matching loc_1704.
 *
...
export function loc_1568(m, underTile = m.regs.b, column = m.regs.d, cellPtr = m.regs.ix) {
  const { mem8 } = m;
