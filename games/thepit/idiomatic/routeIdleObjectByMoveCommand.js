...
/**
 * routeIdleObjectByMoveCommand — route an at-rest object to its per-frame handler on its move-command bits.  ROM 0x144c.
 *
...
export function routeIdleObjectByMoveCommand(m) {
  ...
  if (moveCommand & 0x01) return stepObjectRowFlipped(m); // position the object; test the boundary
  if (moveCommand & 0x02) return stepObjectRowUnflipped(m); // derive the object's tile row and dispatch
