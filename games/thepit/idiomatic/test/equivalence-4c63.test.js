...
/**
 * Memory-equivalence gate for requestSound5 (ROM 0x4c63) — the sound-trigger stub that
 * requests sound-command 5 by handing it to the shared sound-ring enqueue.
 *
 ...
 * command 5, so 0x4c63 is never dispatched — the capture/replay harness cannot hook it
 ...
 * faithful state for command 5 too. Crucially 0x4c63 never calls 0x4c57, so cloning that
 ...
 *
 * ONE WRINKLE — the oracle path (0x4c63 -> shared enqueue 0x4ca5) saves and restores two
 ...
 *      0x4c63 is deterministic (oracle vs oracle -> identical whole state). Proves the
 *      capture/clone/diff plumbing reaches a real sound-request state.
 *   1. EQUAL (real sound-request entry) — requestSound5 == oracle over RAM (outside the
 ...
 *   3. TEETH (wrong command) — a twin that requests command 4 instead of 5 is CAUGHT at
 *      the ring slot (this stub's payload is the index, exactly what the mutation attacks).
 *   4. TEETH (missing pending bit) — a twin that queues 5 without the pending high bit is
 *      CAUGHT at the ring slot (an unmarked slot reads as empty to the driver).
 *
 * Run: node --test games/thepit/idiomatic/test/equivalence-4c63.test.js
 */
...
import { loc_4c63 as oracle } from "../../translated/loc_4c63.js";
import { requestSound5 as idiomatic } from "../requestSound5.js";
...
const COMMAND = 5; // the sound-command index this stub requests
const PENDING = COMMAND | 0x80; // 0x85 — the byte queued (high bit marks the slot pending)
...
test("HARNESS: a real 0x4c57 sound-request entry is captured and the oracle run is deterministic", () => {
  ...
  console.log(
    `  HARNESS: captured a real 0x4c57 entry (SP=${hx(entry.regs.sp)}, ` +
      `SOUND_HEAD=${entry.mem.read8(SOUND_HEAD)}); oracle run of 0x4c63 deterministic`,
  );
});

// -- 1. EQUAL on the real captured sound-request entry -----------------------

test("EQUAL (real entry): requestSound5 == oracle over RAM + pc + SP", () => {
  const entry = captureRealSoundRequestEntry(1500);
...
/** Broken twin: requests command 4 (a sibling's index) instead of 5. */
function twinWrongCommand(m) {
  enqueueWrong(m, 4);
}

/** Broken twin: queues command 5 but WITHOUT the pending high bit (slot reads as empty). */
...
test("TEETH (wrong command): a twin that requests command 4 is CAUGHT at the ring slot", () => {
  const entry = captureRealSoundRequestEntry(1500);
