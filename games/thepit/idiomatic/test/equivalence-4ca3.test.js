...
/**
 * Memory-equivalence gate for requestSound21 (ROM 0x4ca3) — the sound-trigger stub that
 * requests sound-command 21 by handing it to the shared sound-ring enqueue.
 *
 ...
 * command 21, so 0x4ca3 is never dispatched — the capture/replay harness cannot hook it
 ...
 * faithful state for command 21 too. Crucially 0x4ca3 never calls 0x4c57, so cloning that
 ...
 *
 * ONE WRINKLE — the oracle path (0x4ca3 -> shared enqueue 0x4ca5) saves and restores two
 ...
 *      0x4ca3 is deterministic (oracle vs oracle -> identical whole state). Proves the
 *      capture/clone/diff plumbing reaches a real sound-request state.
 *   1. EQUAL (real sound-request entry) — requestSound21 == oracle over RAM (outside the
 ...
 *   3. TEETH (wrong command) — a twin that requests command 20 instead of 21 is CAUGHT at
 *      the ring slot (this stub's payload is the index, exactly what the mutation attacks).
 *   4. TEETH (missing pending bit) — a twin that queues 21 without the pending high bit is
 *      CAUGHT at the ring slot (an unmarked slot reads as empty to the driver).
 *
 * Run: node --test games/thepit/idiomatic/test/equivalence-4ca3.test.js
 */
...
import { loc_4ca3 as oracle } from "../../translated/loc_4ca3.js";
import { requestSound21 as idiomatic } from "../requestSound21.js";
...
const COMMAND = 21; // the sound-command index this stub requests (oracle: ld a,0x15)
const PENDING = COMMAND | 0x80; // 0x95 — the byte queued (high bit marks the slot pending)
...
test("HARNESS: a real 0x4c57 sound-request entry is captured and the oracle run is deterministic", () => {
  ...
  console.log(
    `  HARNESS: captured a real 0x4c57 entry (SP=${hx(entry.regs.sp)}, ` +
      `SOUND_HEAD=${entry.mem.read8(SOUND_HEAD)}); oracle run of 0x4ca3 deterministic`,
  );
});

// -- 1. EQUAL on the real captured sound-request entry -----------------------

test("EQUAL (real entry): requestSound21 == oracle over the observable RAM effect", () => {
  const entry = captureRealSoundRequestEntry(1500);
...
/** Broken twin: requests command 20 (the neighbouring stub's index) instead of 21. */
function twinWrongCommand(m) {
  enqueueWrong(m, 20);
}

/** Broken twin: queues command 21 but WITHOUT the pending high bit (slot reads as empty). */
