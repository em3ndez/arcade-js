...
/**
 * Memory-equivalence gate for requestSound20 (ROM 0x4c9f) — the sound-trigger stub that
 * requests sound-command 20 by handing it to the shared sound-ring enqueue.
 *
 ...
 *
 * WHY A CRAFTED ENTRY. Attract requests other sound commands but never command 20, so
 * 0x4c9f is never dispatched — the capture/replay harness cannot hook it directly. Per
 ...
 * command 20 too. Crucially 0x4c9f never calls 0x4c57, so cloning that entry introduces no
 ...
 * pins the 7 -> 0 wrap.
 *
 * ONE WRINKLE — the oracle path (0x4c9f -> shared enqueue 0x4ca5) saves and restores two
 ...
 *      0x4c9f is deterministic (oracle vs oracle -> identical whole state). Proves the
 *      capture/clone/diff plumbing reaches a real sound-request state.
 *   1. EQUAL (real sound-request entry) — requestSound20 == oracle over RAM (outside the
 ...
 *   3. TEETH (wrong command) — a twin that requests command 19 instead of 20 is CAUGHT at
 *      the ring slot (this stub's payload is the index, exactly what the mutation attacks).
 *   4. TEETH (missing pending bit) — a twin that queues 20 without the pending high bit is
 *      CAUGHT at the ring slot (an unmarked slot reads as empty to the driver).
 *
 * Run: node --test games/thepit/idiomatic/test/equivalence-4c9f.test.js
 */
...
import { loc_4c9f as oracle } from "../../translated/loc_4c9f.js";
import { requestSound20 as idiomatic } from "../requestSound20.js";
...
const COMMAND = 20; // the sound-command index this stub requests (oracle: ld a,0x14)
const PENDING = COMMAND | 0x80; // 0x94 — the byte queued (high bit marks the slot pending)
...
test("HARNESS: a real 0x4c57 sound-request entry is captured and the oracle run is deterministic", () => {
  ...
  console.log(
    `  HARNESS: captured a real 0x4c57 entry (SP=${hx(entry.regs.sp)}, ` +
      `SOUND_HEAD=${entry.mem.read8(SOUND_HEAD)}); oracle run of 0x4c9f deterministic`,
  );
});

// -- 1. EQUAL on the real captured sound-request entry -----------------------

test("EQUAL (real entry): requestSound20 == oracle over the observable RAM effect", () => {
  const entry = captureRealSoundRequestEntry(1500);
...
/** Broken twin: requests command 19 (a sibling's index) instead of 20. */
function twinWrongCommand(m) {
  enqueueWrong(m, 19);
}

/** Broken twin: queues command 20 but WITHOUT the pending high bit (slot reads as empty). */
...
test("TEETH (wrong command): a twin that requests command 19 is CAUGHT at the ring slot", () => {
  const entry = captureRealSoundRequestEntry(1500);
