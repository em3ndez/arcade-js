...
/**
 * Memory-equivalence gate for requestSound8 (ROM 0x4c6f) — the sound-trigger stub that
 * requests sound-command 8 by handing it to the shared sound-ring enqueue.
 *
 ...
 * command 8, so 0x4c6f is never dispatched in a boot/attract run — the capture/replay
 * harness cannot hook it directly (a probe over 3000 frames sees 0 dispatches of 0x4c6f
 * and 1 of the sibling 0x4c57). Per the crafted-entry method the gate instead runs the
 * stub from a REAL captured sound-request state: the sibling stub 0x4c57 (command 2) IS
 * reached during attract, and every sibling shares the same call convention, so its entry
 * state is a faithful state for command 8 too. Crucially 0x4c6f never calls 0x4c57, so
 * cloning that entry introduces no registry recursion. The one input that shapes the
 * output — the ring write pointer — is then swept across all of 0..7 by poking SOUND_HEAD
 * identically on both sides, which also pins the 7 -> 0 wrap.
 *
 * ONE WRINKLE — the oracle path (0x4c6f -> shared enqueue 0x4ca5) saves and restores two
 ...
 *      0x4c6f is deterministic (oracle vs oracle -> identical whole state). Proves the
 *      capture/clone/diff plumbing reaches a real sound-request state.
 *   1. EQUAL (real sound-request entry) — requestSound8 == oracle over RAM (outside the
 ...
 *   3. TEETH (wrong command) — a twin that requests command 7 (a sibling's index) instead
 *      of 8 is CAUGHT at the ring slot (this stub's payload is the index, exactly what the
 *      mutation attacks).
 *   4. TEETH (missing pending bit) — a twin that queues 8 without the pending high bit is
 *      CAUGHT at the ring slot (an unmarked slot reads as empty to the driver).
 *
 * Run: node --test games/thepit/idiomatic/test/equivalence-4c6f.test.js
 */
...
import { loc_4c6f as oracle } from "../../translated/loc_4c6f.js";
import { requestSound8 as idiomatic } from "../requestSound8.js";
...
const COMMAND = 8; // the sound-command index this stub requests
const PENDING = COMMAND | 0x80; // 0x88 — the byte queued (high bit marks the slot pending)
...
test("EQUAL (real entry): requestSound8 == oracle over RAM + pc + SP", () => {
  const entry = captureRealSoundRequestEntry(1500);
...
/** Broken twin: requests command 7 (a sibling's index) instead of 8. */
function twinWrongCommand(m) {
  enqueueWrong(m, 7); // BUG: this stub must request 8, not 7
}

/** Broken twin: queues command 8 but WITHOUT the pending high bit (slot reads as empty). */
...
test("TEETH (wrong command): a twin that requests command 7 is CAUGHT at the ring slot", () => {
  const entry = captureRealSoundRequestEntry(1500);
