...
/**
 * Memory-equivalence gate for requestSound3 (ROM 0x4c5b) — the sound-trigger stub that
 * requests sound-command 3 by handing it to the shared sound-ring enqueue.
 *
 ...
 * command 3, so 0x4c5b is never dispatched — the capture/replay harness cannot hook it
 ...
 * faithful state for command 3 too. Crucially 0x4c5b never calls 0x4c57, so cloning that
 ...
 *
 * ONE WRINKLE — the oracle path (0x4c5b -> shared enqueue 0x4ca5) saves and restores two
 ...
 *      0x4c5b is deterministic (oracle vs oracle -> identical whole state). Proves the
 *      capture/clone/diff plumbing reaches a real sound-request state.
 *   1. EQUAL (real sound-request entry) — requestSound3 == oracle over RAM (outside the
 ...
 *   3. TEETH (wrong command) — a twin that requests command 2 instead of 3 is CAUGHT at
 *      the ring slot (this stub's payload is the index, exactly what the mutation attacks).
 *   4. TEETH (missing pending bit) — a twin that queues 3 without the pending high bit is
 *      CAUGHT at the ring slot (an unmarked slot reads as empty to the driver).
 *
 * Run: node --test games/thepit/idiomatic/test/equivalence-4c5b.test.js
 */
...
import { loc_4c5b as oracle } from "../../translated/loc_4c5b.js";
import { requestSound3 as idiomatic } from "../requestSound3.js";
...
const COMMAND = 3; // the sound-command index this stub requests
const PENDING = COMMAND | 0x80; // 0x83 — the byte queued (high bit marks the slot pending)
...
test("EQUAL (real entry): requestSound3 == oracle over RAM + pc + SP", () => {
  const entry = captureRealSoundRequestEntry(1500);
...
/** Broken twin: requests command 2 (a sibling's index) instead of 3. */
function twinWrongCommand(m) {
  enqueueWrong(m, 2);
}

/** Broken twin: queues command 3 but WITHOUT the pending high bit (slot reads as empty). */
...
test("TEETH (wrong command): a twin that requests command 2 is CAUGHT at the ring slot", () => {
  const entry = captureRealSoundRequestEntry(1500);
