...
/**
 * Memory-equivalence gate for loc_30de (ROM 0x30de, The Pit) — seed the second block
 ...
 *
 * loc_30de is entered ONLY by the gameplay round-init tail-jump chain
 * (loc_2f2f → loc_30de → loc_36fe); attract mode never enters gameplay, so it is
 ...
 *      frames (real title/demo RAM), run the oracle and loc_30de on independent clones
 ...
 *   3. NON-VACUOUS + WRITE-COMPLETE (sentinel entry) — pre-set loc_30de's own targets
...
import { loc_30de as idiomatic } from "../loc_30de.js";
...
// loc_30de's OWN parameter-block writes (the tail's writes are the seedActorSpawnState
...
test("EQUAL: loc_30de leaves the same work RAM as the oracle over real captured states", () => {
...
test("NON-VACUOUS: with loc_30de's own targets pre-set to a sentinel, both arms overwrite them and agree", () => {
...
/** Broken twin B: does loc_30de's own writes but drops the seedActorSpawnState tail. */
