...
/**
 * Memory-equivalence gate for requestSound10 (ROM 0x4c77) — the sound-trigger stub
 * that requests sound-command 10 by delegating to the shared sound-ring enqueue.
 *
 * The stub's distinctive job is a single number: it queues command index 10, which
 ...
 *
 * Attract requests command 10 exactly once, and not early — it first fires around
 * frame 1900 — so the capture run needs a wider frame budget than the boot-time
 * stubs. That single real dispatch is the in-distribution entry; a crafted pointer==7
 * entry then pins the ring wrap the one dispatch may not land on.
 *
 * Four checks:
 *   0. IDENTITY (harness) — run the unit gate with both arms = the oracle; EQUAL
 *      proves the capture/clone/replay harness reaches 0x4c77 in a real attract run.
 *   1. EQUAL (real dispatch, full contract) — for the captured entry, oracle vs
 *      requestSound10 leave identical RAM (outside the stack scratch) + pc + SP, and
 *      the ring slot holds 0x8a (10 | pending bit) with the pointer advanced.
 *   2. EQUAL (crafted wrap, pointer==7) — force SOUND_HEAD to 7 on both sides and
 *      confirm both write slot 7 with command 10 and wrap the pointer to 0, identical.
 *   3. TEETH — a twin that requests the WRONG command index (11, a real neighbouring
 *      stub's payload) MUST be caught at the ring slot (0x8b instead of 0x8a).
 *
 * Run: node --test games/thepit/idiomatic/test/equivalence-4c77.test.js
 */
...
import { loc_4c77 as oracle } from "../../translated/loc_4c77.js";
...
const TARGET = 0x4c77;
const COMMAND = 10; // this stub's fixed sound-command index
const PENDING = (COMMAND | 0x80) & 0xff; // 0x8a — the byte the ring slot must end up holding
const MAX_FRAMES = 2200; // attract's one command-10 request first fires ~frame 1900
...
/**
 * Hook 0x4c77 in a real attract run and clone the machine at up to K real dispatches.
...
/** Broken twin: requests the WRONG command index (11, a real neighbouring stub's payload). */
function twinWrongIndex(m) {
  enqueueSoundCommand(m, 11); // BUG: this stub must request 10, not 11
}
...
test("IDENTITY: the unit gate runs on The Pit and reports EQUAL when both arms are the oracle", () => {
  ...
  console.log("  IDENTITY: captured 0x4c77 (a mid-attract sound request), cloned, ran oracle vs oracle -> EQUAL");
});

// -- 1. EQUAL (real dispatch, full contract) ---------------------------------

test("EQUAL (real dispatch): requestSound10 == oracle on the captured 0x4c77 entry", () => {
  const caps = captureDispatches(8, MAX_FRAMES);
  assert.ok(caps.length >= 1, "expected at least one real 0x4c77 dispatch during attract");

  for (const cap of caps) {
    const preHead = cap.mem.read8(SOUND_HEAD);

    // Full-contract equivalence: RAM (outside the stack scratch) + pc + SP.
    const { diffs } = contractDiffs(cap, requestSound10); // fresh clones inside — cap untouched
    assert.equal(diffs.length, 0, diffs.join("; "));

    // Positive checks: slot filled with command 10 (pending bit set), pointer advanced.
    const c = cap.clone();
    requestSound10(c);
    assert.equal(
      c.mem.read8(SOUND_RING + preHead),
      PENDING,
      `ring slot ${preHead} not filled with the pending command 10 on a real dispatch`,
    );
    assert.equal(
  ...
  console.log(
    `  EQUAL/real: ${caps.length} captured dispatch(es) identical over RAM+pc+SP; ` +
      `slot filled with ${hx(PENDING)} (command 10, pending)`,
  );
});

// -- 2. EQUAL (crafted wrap, pointer==7) --------------------------------------

test("EQUAL (crafted wrap): with SOUND_HEAD forced to 7, both write slot 7 (command 10) and wrap to 0", () => {
  ...
  const { diffs } = contractDiffs(entry, requestSound10);
  assert.equal(diffs.length, 0, diffs.join("; "));

  const c = entry.clone();
  requestSound10(c);
  assert.equal(c.mem.read8(SOUND_HEAD), 0, "the pointer must wrap 7 -> 0");
  assert.equal(c.mem.read8(SOUND_RING + 7), PENDING, "slot 7 must be filled with command 10");
  console.log("  EQUAL/crafted: pointer 7 -> 0 wrap, slot 7 filled with command 10, identical to the oracle");
});

// -- 3. TEETH: the wrong-index twin is caught --------------------------------

test("TEETH: a twin that requests the wrong command index (11) is CAUGHT at the ring slot", () => {
