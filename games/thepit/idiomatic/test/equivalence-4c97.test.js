...
/**
 * Memory-equivalence gate for requestSound18 (ROM 0x4c97) — the sound-trigger stub that
 * requests sound-command 18 by handing it to the shared sound-ring enqueue.
 *
 ...
 *
 * WHY A CRAFTED ENTRY. Attract requests other sound commands but never command 18, so
 * 0x4c97 is never dispatched — the capture/replay harness cannot hook it directly. Per
 ...
 * command 18 too. Crucially 0x4c97 never calls 0x4c57, so cloning that entry introduces no
 ...
 * pins the 7 -> 0 wrap.
 *
 * ONE WRINKLE — the oracle path (0x4c97 -> shared enqueue 0x4ca5) saves and restores two
 ...
 *      0x4c97 is deterministic (oracle vs oracle -> identical whole state). Proves the
 *      capture/clone/diff plumbing reaches a real sound-request state.
 *   1. EQUAL (real sound-request entry) — requestSound18 == oracle over RAM (outside the
 ...
 *   3. TEETH (wrong command) — a twin that requests command 17 instead of 18 is CAUGHT at
 *      the ring slot (this stub's payload is the index, exactly what the mutation attacks).
 *   4. TEETH (missing pending bit) — a twin that queues 18 without the pending high bit is
 *      CAUGHT at the ring slot (an unmarked slot reads as empty to the driver).
 *
 * Run: node --test games/thepit/idiomatic/test/equivalence-4c97.test.js
 */
...
import { loc_4c97 as oracle } from "../../translated/loc_4c97.js";
import { requestSound18 as idiomatic } from "../requestSound18.js";
...
const COMMAND = 18; // the sound-command index this stub requests (oracle: ld a,0x12)
const PENDING = COMMAND | 0x80; // 0x92 — the byte queued (high bit marks the slot pending)
...
test("HARNESS: a real 0x4c57 sound-request entry is captured and the oracle run is deterministic", () => {
  ...
  console.log(
    `  HARNESS: captured a real 0x4c57 entry (SP=${hx(entry.regs.sp)}, ` +
      `SOUND_HEAD=${entry.mem.read8(SOUND_HEAD)}); oracle run of 0x4c97 deterministic`,
  );
});

// -- 1. EQUAL on the real captured sound-request entry -----------------------

test("EQUAL (real entry): requestSound18 == oracle over the observable RAM effect", () => {
  const entry = captureRealSoundRequestEntry(1500);
...
/** Broken twin: requests command 17 (a sibling's index) instead of 18. */
function twinWrongCommand(m) {
  enqueueWrong(m, 17);
}

/** Broken twin: queues command 18 but WITHOUT the pending high bit (slot reads as empty). */
...
test("TEETH (wrong command): a twin that requests command 17 is CAUGHT at the ring slot", () => {
  const entry = captureRealSoundRequestEntry(1500);
