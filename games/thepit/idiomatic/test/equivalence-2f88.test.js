 ...
 * background phase clock advanceChamberCreatureAnimation.
 *
 ...
 *
 *   2. revealTerrainColumn tail-delegates to advanceChamberCreatureAnimation, whose own two continuations are still
...
const OSC = 0x2fe3; // the oscillator-body tail reached via advanceChamberCreatureAnimation (still untranslated)
const PUB = 0x3029; // the publish tail reached via advanceChamberCreatureAnimation (still untranslated)
...
const PHASE = 0x80e3; // advanceChamberCreatureAnimation's phase counter, ticked by the hand-off
...
// Stubs standing in for the two untranslated continuations advanceChamberCreatureAnimation reaches.
...
// routing decision through the whole revealTerrainColumn -> advanceChamberCreatureAnimation chain is checkable.
function oscStub(mm) {
...
function brokenHandoff(m) {
  ...
  const gate = (mem.read8(GATE) - 1 + 256) % 256;
  mem.write8(GATE, gate);
  if (gate !== 0) return undefined; // BUG: never delegates to advanceChamberCreatureAnimation
  mem.write8(GATE, mem.read8(PERIOD));
  const cursor = mem.read8(CURSOR) - 6;
...
// The broken twins descend into advanceChamberCreatureAnimation the same way the real routine does — through
