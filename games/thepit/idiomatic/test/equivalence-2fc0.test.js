...
/**
 * Memory-equivalence gate for advanceZonkerAnimation (ROM 0x2fc0, The Pit) — the per-frame phase
 ...
 *
 *   1. advanceZonkerAnimation is NEVER entered through this address in attract. Its subsystem does
 ...
 *      sibling loc_2f71's entry, and advanceZonkerAnimation is invoked on clones of it — the exact
 ...
 *      would run, so its entry state IS a valid advanceZonkerAnimation entry.
 *
 *   2. Two of advanceZonkerAnimation's continuations are still untranslated (0x2fe3 the oscillator
 ...
 *
 *   3. advanceZonkerAnimation is a tail-jumping routine whose caller consumes no register, so its
...
import { advanceZonkerAnimation as idiomatic } from "../advanceZonkerAnimation.js";
...
const TARGET = 0x2fc0; // advanceZonkerAnimation
 ...
 * each carrying the tail stubs in its registry so advanceZonkerAnimation can be run on clones. The
...
// advanceZonkerAnimation is unreached in attract, so a makeMachine wrapper forces a real dispatch:
