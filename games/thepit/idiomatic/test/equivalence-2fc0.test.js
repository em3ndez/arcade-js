...
/**
 * Memory-equivalence gate for advanceChamberCreatureAnimation (ROM 0x2fc0, The Pit) — the per-frame phase
 ...
 *
 *   1. advanceChamberCreatureAnimation is NEVER entered through this address in attract. Its subsystem does
 ...
 *      sibling loc_2f71's entry, and advanceChamberCreatureAnimation is invoked on clones of it — the exact
 ...
 *      would run, so its entry state IS a valid advanceChamberCreatureAnimation entry.
 *
 *   2. Two of advanceChamberCreatureAnimation's continuations are still untranslated (0x2fe3 the oscillator
 ...
 *
 *   3. advanceChamberCreatureAnimation is a tail-jumping routine whose caller consumes no register, so its
...
const TARGET = 0x2fc0; // advanceChamberCreatureAnimation
 ...
 * each carrying the tail stubs in its registry so advanceChamberCreatureAnimation can be run on clones. The
...
// advanceChamberCreatureAnimation is unreached in attract, so a makeMachine wrapper forces a real dispatch:
