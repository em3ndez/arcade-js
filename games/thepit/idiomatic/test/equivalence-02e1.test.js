...
/**
 * Memory-equivalence gate for loc_02e1 (ROM 0x02e1, The Pit) — the round-start intro-hold
 ...
 * setup (loc_031a), which never returns.
 *
 * WHY A CRAFTED ENTRY. loc_02e1 is the loop the round-start setup flows into, on the
 ...
 * first ~100 frames) and cloning the machine the first time it fires. loc_02e1 takes no
 ...
 *
 * HOW THE NEVER-RETURNING TAIL IS BOUNDED. loc_02e1's tail is now the DIRECT idiomatic
 ...
 * stops both arms at the hand-off, the instant loc_031a's board paint is about to begin,
 * BEFORE it repaints. paintScreen (inside loc_031a) waits one frame before its first copy, and
 ...
 * with the loop counter already at 0, throws — freezing both arms at loc_031a's paint boundary
 * with this routine's intro output intact. Both arms reach it identically (oracle via m.call to
 * the registered translated loc_031a, idiomatic via its import), so the hook can only reveal a
 * difference. loc_031a's own correctness is separately gated (equivalence-031a).
 *
...
import { loc_02e1 as idiomatic } from "../loc_02e1.js";
...
const TAIL = 0x031a; // the round-loop setup loc_02e1 tail-jumps to (now a direct idiomatic call)
...
/**
 * Capture one real attract machine state to seed crafted entries. loc_02e1 itself is never
 ...
 * at loc_031a's paint boundary — and diff the observable-RAM contract, excluding the dead stack
...
test("EQUAL (real entry): loc_02e1 == oracle over observable RAM", () => {
...
test("TEETH (corrupted painted cell): a flipped colour cell is CAUGHT in colour RAM", () => {
  // At the paint boundary the playfield-strip's colour cell is intact (loc_031a has not
