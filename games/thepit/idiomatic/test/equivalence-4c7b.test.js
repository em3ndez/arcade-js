...
/**
 * Memory-equivalence gate for requestSound11 (ROM 0x4c7b) — the sound-trigger stub that
 * requests sound-command 11 by handing it to the shared sound-ring enqueue.
 *
 ...
 * command 11, so 0x4c7b is never dispatched in 6000+ frames — the capture/replay harness
 * cannot hook it directly. Per the crafted-entry method the gate instead runs the stub
 * from a REAL captured sound-request state: the sibling stub 0x4c57 (command 2) IS
 * reached during attract (at frame 3), and every sibling shares the same call convention,
 * so its entry state is a faithful state for command 11 too. Crucially 0x4c7b never calls
 * 0x4c57, so cloning that entry introduces no registry recursion. The one input that
 * shapes the output — the ring write pointer — is then swept across all of 0..7 by poking
 * SOUND_HEAD identically on both sides, which also pins the 7 -> 0 wrap.
 *
 * ONE WRINKLE — the oracle path (0x4c7b -> shared enqueue 0x4ca5) saves and restores two
 ...
 *      0x4c7b is deterministic (oracle vs oracle -> identical whole state). Proves the
 *      capture/clone/diff plumbing reaches a real sound-request state.
 *   1. EQUAL (real sound-request entry) — requestSound11 == oracle over RAM (outside the
 ...
 *   3. TEETH (wrong command) — a twin that requests command 10 (a sibling's index) instead
 *      of 11 is CAUGHT at the ring slot (this stub's payload is the index, exactly what the
 *      mutation attacks).
 *   4. TEETH (missing pending bit) — a twin that queues 11 without the pending high bit is
 *      CAUGHT at the ring slot (an unmarked slot reads as empty to the driver).
 *
 * Run: node --test games/thepit/idiomatic/test/equivalence-4c7b.test.js
 */
...
import { loc_4c7b as oracle } from "../../translated/loc_4c7b.js";
import { requestSound11 as idiomatic } from "../requestSound11.js";
...
const COMMAND = 11; // the sound-command index this stub requests
const PENDING = COMMAND | 0x80; // 0x8b — the byte queued (high bit marks the slot pending)
...
test("EQUAL (real entry): requestSound11 == oracle over RAM + pc + SP", () => {
  const entry = captureRealSoundRequestEntry(1500);
...
/** Broken twin: requests command 10 (a sibling's index) instead of 11. */
function twinWrongCommand(m) {
  enqueueWrong(m, 10);
}

/** Broken twin: queues command 11 but WITHOUT the pending high bit (slot reads as empty). */
...
test("TEETH (wrong command): a twin that requests command 10 is CAUGHT at the ring slot", () => {
  const entry = captureRealSoundRequestEntry(1500);
