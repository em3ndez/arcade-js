 ...
 *
 * Runs from the per-frame carve/lift handler whenever the tracked actor lines up on a cell. It reads
 * the tile pair at the actor's cell pointer and proceeds only when the dig system recognises it; on
 * any other pair it does nothing beyond stashing the pointer. For a recognised pair it classifies
 * which dig entity to place, stages the placement into a scratch block, then runs a one-shot debounce:
 * the spawn counter is bumped every pass, but only the pass that finds it idle commits the staged
 * entity (promoting the staging into the live dig-object record and stamping the tilemap); every
 * later pass merely keeps the dig-object timer armed.
 */

import { commitDigEntity } from "./commitDigEntity.js";
import { PLAYER_CELL_PTR, REACTION_OBJ_X, PLAYER_X, REACTION_STATE, DIG_OBJ_SUBTYPE, HAZARD_ACTIVE_COUNT, DIG_OBJ_TIMER, REACTION_PERIOD, STAGED_DIG_SPRITE_ID, STAGED_TARGET_X, STAGED_TARGET_Y, STAGED_DIG_TIMER, STAGED_CELL_PTR } from "./names.js";

// Scratch cells written here, then read back by the commit tail commitDigEntity.
const STAGED_SPRITE_ID = STAGED_DIG_SPRITE_ID;
const STAGED_COLUMN = STAGED_TARGET_X;
const STAGED_ROW = STAGED_TARGET_Y;
const STAGED_ATTR = STAGED_DIG_TIMER;
const SAVED_CELL_PTR = STAGED_CELL_PTR;
