...
/**
 * loc_316f — advance the second object mover one frame and stage its sprite.  ROM 0x316f.
 *
 * The object-2 half of the two-object mover pass: loc_312d runs object 1, then hands
 * here for object 2 (both share one mover, loc_319d, which works out of a scratch
 ...
 * NAMES:    OBJ2_X (0x80f9, the object-2 record base), SPRITE_COORD_BIAS (0x8051),
...
// The shared 17-byte scratch block the mover (loc_319d) reads and updates in place.
...
// The shared per-frame actor update this pass tails into; its return goes to our caller.
const ACTOR_UPDATE = 0x3748;
...
export function loc_316f(m) {
  ...
  for (let i = 0; i < OBJECT_RECORD_BYTES; i++) mem8[MOVER_SCRATCH + i] = mem8[OBJ2_X + i];
  loc_319d(m);
  for (let i = 0; i < OBJECT_RECORD_BYTES; i++) mem8[OBJ2_X + i] = mem8[MOVER_SCRATCH + i];

  // Stage object 2's sprite: position/tile/attribute verbatim, then the fourth byte
  // shifted by the sprite coordinate offset.
  for (let i = 0; i < 3; i++) mem8[OBJ2_SPRITE_RECORD + i] = mem8[OBJ2_X + i];
  mem8[OBJ2_SPRITE_RECORD + 3] = mem8[OBJ2_X + 3] + mem8[SPRITE_COORD_BIAS];

  // Continue into the shared per-frame actor update (still the frozen oracle).
  return m.call(ACTOR_UPDATE);
}
