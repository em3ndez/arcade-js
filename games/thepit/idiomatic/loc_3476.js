...
/**
 * loc_3476 — commit one preset move-step for the tracked mover: step its X down a
 * pixel and, on the movement cadence, republish its travel direction.  ROM 0x3476.
 *
 * The per-object move driver (loc_319d) probes the tiles around the mover each
 ...
 *
 * Two things happen:
 *   - Every call, the object's X position is decreased by one pixel — this preset's
 *     move delta (it wraps within a byte).
 ...
 *           input bytes (cadence countdown, reload period, X position) that also
 *           covers the expire path and the byte wraps, matched against the oracle,
 *           plus teeth.
 * LIVE-OUT: memory only — the cadence countdown (0x808b), the X position (0x8086),
 *           and, on the cadence tick, the travel-direction index (0x8092). The new
 *           X the oracle leaves in the accumulator is dead ABI: the driver reaches
