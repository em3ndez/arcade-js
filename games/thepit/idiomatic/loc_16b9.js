...
/**
 * loc_16b9 — route a moving actor's horizontal step: if it has reached the goal
 * terminator tile, latch the goal crossing and walk it on; otherwise resolve the
 * terrain step it is entering.  ROM 0x16b9.
 *
 * Reached from the per-frame object dispatch (loc_167f) once that has turned the
 ...
 *
 * Kept as loc_16b9: its sibling terrain handler loc_1704 is itself still neutrally
...
export function loc_16b9(m, row = m.regs.h, spriteCode = m.regs.l) {
  const { mem8, mem16 } = m;
  ...
  return loc_1704(m, cellPtr, biasedColumn);
}
