 ...
 * command index and sharing one enqueue tail. This one requests command 10: it hands
 ...
 * from which number they request (9 sits just below it, 11 just above).
 *
 * Which real effect command 10 selects is not yet identified, so the name states the
 ...
 *
 * Memory-equivalent to the frozen oracle — equivalence-4c77.test.js.
 * GATE:     real-dispatch — attract requests command 10 once (fired mid-attract),
 ...
 *           wrong command index is caught at the ring slot.
...
export function requestSound10(m) {
  // Queue sound command 10; the shared enqueue sets the pending bit and files it in
  // the next ring slot.
