...
/**
 * loc_02a1 — sequence the round sub-phase byte and hand off to setup or teardown.  ROM 0x02a1.
 *
 * The round/mode dispatcher jumps here when the mode-count byte is not 1 (see loc_0278).
 ...
 * path (loc_02ca — reload the player, decode the switches, rebuild the setup screen) or
 ...
 *
 * NAME kept loc_02a1: the mechanism (toggle the sub-phase, pick setup vs teardown) is
 ...
 * GATE:     crafted-entry — loc_02a1 is on the round-transition path, never dispatched in
 ...
 *           The two continuations are now idiomatic (loc_02ca setup / submitHighScoresAndReset
...
export function loc_02a1(m) {
  ...
  if (mem8[GAME_STATE2] === 1) {
    mem8[GAME_STATE2] = 2;
    if (mem8[FLAG_AT_ADVANCE] !== 0) return loc_02ca(m);
  }

  // Reset the sub-phase to 1; the first flag alone routes to setup.
  mem8[GAME_STATE2] = 1;
  if (mem8[FLAG_AT_RESET] !== 0) return loc_02ca(m);
