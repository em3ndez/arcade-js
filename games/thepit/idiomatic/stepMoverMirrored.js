...
/**
 * stepMoverMirrored — advance one object-mover step for movement direction 1: on the cadence
 ...
 * ENEMY_WORK_MOVE_PERIOD, publishes direction index 1 in ENEMY_WORK_DIR, and steps ENEMY_WORK_X
 * one pixel, whose low bits pick one of four walk-frame codes stored mirrored (high bit set)
 * in ENEMY_WORK_SPRITE. The screen sign of the step is rotation-ambiguous, so the name stays neutral.
 */

import { ENEMY_WORK_SPRITE, ENEMY_ACTION_TIMER, ENEMY_WORK_DIR, ENEMY_WORK_MOVE_PERIOD, ENEMY_WORK_X } from "./names.js";

// The four walk-frame sprite codes; the stored code is mirrored (high bit set) here.
...
export function stepMoverMirrored(m) {
  ...
  // Step ENEMY_WORK_X one pixel; its low bits choose a walk-frame code, mirrored for the draw.
