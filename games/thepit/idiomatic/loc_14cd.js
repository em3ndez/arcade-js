...
/**
 * loc_14cd — locate the object's tilemap cell, latch a goal crossing if the goal is just ahead,
 ...
 * (loc_1493) with the object's screen row. It first turns the object's row + column coordinates into
 ...
 *
 * Kept as loc_14cd: its role is clear but its resolver siblings (loc_1515 / loc_1568 / loc_1704) and
 * caller (loc_1493) stay neutral too — the object's move axis is contested under the rotated display
 ...
 *           every sub-offset (the delegated loot/push arms are covered by loc_1515's own gate). The
 ...
 * LIVE-OUT: memory-only — the published tile column (OBJ_TILE_COL) and cell pointer (ACTOR_CELL_PTR),
 ...
 * NAMES:    OBJ_Y, OBJ_X, OBJ_TILE_COL, ACTOR_CELL_PTR, NEXT_TILE, GOAL_TILE_LATCH,
 *           GOAL_CROSSING_LATCH from ram.js. The tile classification and its ROM tables live in the
 *           resolver loc_1515.
 */
...
export function loc_14cd(m, row = m.regs.h) {
  const { mem8, mem16 } = m;
  ...
  const column = u8(mem8[OBJ_Y] + COLUMN_BIAS);
  mem8[OBJ_TILE_COL] = column >> 3;
  ...
  const crossAxisAligned = ((mem8[OBJ_X] + ALIGN_BIAS) & 7) === 0;
  if (aheadTile === GOAL_TILE && crossAxisAligned) {
    mem8[GOAL_TILE_LATCH] = 1;
    mem8[GOAL_CROSSING_LATCH] = 1;
  ...
  return loc_1515(m, column, cellPtr);
}
