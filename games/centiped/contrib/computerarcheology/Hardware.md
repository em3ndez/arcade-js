...
Two hardware conventions matter when reading the map. (1) **A read and a write at one
address can be different devices** — most visibly at `0x2000`, where a **read** returns
...
the mushroom field and the on-screen text. **Sprite RAM** at `0x07C0`–`0x07FF` holds **16
objects of four bytes each** — the shooter, the centipede's segments, the spider, the flea,
