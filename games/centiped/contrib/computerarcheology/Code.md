...
>>> binary 0000:roms/136001-307.d1 + roms/136001-308.e1 + roms/136001-309.fh1 + roms/136001-310.j1
...
2019: 8D 00 20        STA     $2000               ; {hard.rom} kick the watchdog with the sampled port value so the board isn't reset mid-frame
201C: AD 00 0C        LDA     $0C00               ; read input port IN0 (vblank edge and service-switch bits)
...
20CD: AD 0A 10        LDA     $100A               ; read the POKEY random register
...
loc_20f4:
20F4: AD 0A 10        LDA     $100A               ; read the POKEY random register
...
213E: AE 00 06        LDX     $0600               ; load a byte from the object page 0x0600
...
21DD: AD 0A 10        LDA     $100A               ; read the POKEY random register
...
2233: AD 0A 10        LDA     $100A               ; timer expired: read the hardware random register
...
2258: 2D 0A 10        AND     $100A               ; gate the result through the hardware random register
...
22FE: AD 0A 10        LDA     $100A               ; timer expired: read the hardware random register
...
23B7: 2C 0A 10        BIT     $100A               ; test bit 7 of the POKEY random register
...
23C1: AD 0A 10        LDA     $100A               ; read the POKEY random register
...
2458: 9D 02 1C        STA     $1C02,X             ; clear the per-slot output latch at $1c02+slot
...
250F: 8D 07 1C        STA     $1C07               ; drive the flip-screen latch from bit 7 of the value
...
2554: 8D 07 1C        STA     $1C07               ; set the flip-screen latch bit 7
...
loc_2561:
2561: AD 01 08        LDA     $0801               ; read the DSW2 config source byte
...
25B1: 8E 03 1C        STX     $1C03               ; drive the per-slot output latch 0x1c03 high
25B4: 8E 04 1C        STX     $1C04               ; drive the per-slot output latch 0x1c04 high
...
25EC: 8D 03 1C        STA     $1C03               ; write it to the per-slot output latch 0x1c03
25EF: E0 02           CPX     #$02                ; is the counter below 2?
25F1: 90 14           BCC     $2607               ; {code.loc_2607} below 2 -- skip ahead
25F3: 8D 04 1C        STA     $1C04               ; write the code to the output latch 0x1c04 as well
25F6: A2 02           LDX     #$02                ; default slot index 2
25F8: AD 01 0C        LDA     $0C01               ; read input port IN1
...
loc_2607:
2607: AD 01 0C        LDA     $0C01               ; read input port IN1
...
2613: 8D 03 1C        STA     $1C03               ; drive the per-slot output latch 0x1c03 high
2616: 8D 04 1C        STA     $1C04               ; drive the per-slot output latch 0x1c04 high
...
2627: 9D 02 1C        STA     $1C02,X             ; clear this slot's output latch at 0x1c02
...
2642: AD 00 0C        LDA     $0C00               ; read input port IN0
...
2663: 8E 0E 14        STX     $140E               ; write b2 into palette cell 0x0E
2666: 8E 06 14        STX     $1406               ; write b2 into palette cell 0x06
2669: 8D 0F 14        STA     $140F               ; write b0 into palette cell 0x0F
266C: 8D 05 14        STA     $1405               ; write b0 into palette cell 0x05
266F: 8C 0D 14        STY     $140D               ; write b1 into palette cell 0x0D
2672: 8C 07 14        STY     $1407               ; write b1 into palette cell 0x07
2675: 60              RTS                         ; return
...
27AF: AD 01 0C        LDA     $0C01               ; read the IN1 control port
...
281C: 8D 89 05        STA     $0589               ; clear the first sprite-shadow cell
281F: 8D A9 05        STA     $05A9               ; clear the second sprite-shadow cell
2822: 8D C9 05        STA     $05C9               ; clear the third sprite-shadow cell
...
2874: 8D 08 10        STA     $1008               ; arm the POKEY audio-control latch for a fresh round
...
288F: 8D 0F 10        STA     $100F               ; silence the POKEY serial/keyboard-control register
...
289E: AD 0A 10        LDA     $100A               ; read the POKEY random register
28A1: 4D 0A 10        EOR     $100A               ; XOR a second read (self-cancelling mix)
...
28AB: 8D 0F 10        STA     $100F               ; bring the POKEY serial-control register back up
...
28C1: 8D 04 14        STA     $1404               ; write the fixed playfield colour value
...
loc_28d1:
28D1: 9D 00 04        STA     $0400,X             ; blank one byte of the first video page
28D4: 9D 00 05        STA     $0500,X             ; blank the matching byte of the second page
28D7: 9D 00 06        STA     $0600,X             ; blank the matching byte of the third page
28DA: 9D 00 07        STA     $0700,X             ; blank the matching byte of the fourth page
...
loc_28ea:
28EA: AD 0A 10        LDA     $100A               ; read the POKEY random register
...
28F3: AD 0A 10        LDA     $100A               ; read the POKEY random register again
...
2C17: AD 0A 10        LDA     $100A               ; read the hardware random register
...
2E31: AD 0A 10        LDA     $100A               ; read the POKEY random register
...
2E48: AD 0A 10        LDA     $100A               ; read the POKEY random register
...
2E51: 2C 0A 10        BIT     $100A               ; test the random register's sign bit
...
2E5C: 2C 0A 10        BIT     $100A               ; test the random register's sign bit
...
2E6B: AD 0A 10        LDA     $100A               ; read the POKEY random register
...
2EE6: AD 01 0C        LDA     $0C01               ; read the IN1 input port
...
2EED: AD 0A 10        LDA     $100A               ; owner negative -- read the POKEY random register instead
...
306E: 8E 01 10        STX     $1001               ; zero the channel-1 volume register
3071: 8E 03 10        STX     $1003               ; zero the channel-2 volume register
3074: 8E 05 10        STX     $1005               ; zero the channel-3 volume register
3077: 8E 07 10        STX     $1007               ; zero the channel-4 volume register
...
3090: 8D 06 10        STA     $1006               ; write it to the channel-4 frequency register
3093: B9 9B 31        LDA     $319B,Y             ; {hard.rom+119B} fetch this step's channel-4 volume from ROM

loc_3096:
3096: 8D 07 10        STA     $1007               ; write the channel-4 volume (or silence)
...
30A3: 8D 04 10        STA     $1004               ; write it to the channel-3 frequency register
30A6: A9 64           LDA     #$64                ; load the fixed channel-3 volume 0x64

loc_30a8:
30A8: 8D 05 10        STA     $1005               ; write the channel-3 volume
...
30BD: 8D 02 10        STA     $1002               ; write it to the channel-2 frequency register
...
30D0: 8D 02 10        STA     $1002               ; write it to the channel-2 frequency register
...
30FA: 8D 02 10        STA     $1002               ; write the noise pitch to the channel-2 frequency register
...
310B: 8D 00 10        STA     $1000               ; write it to the channel-1 frequency register
310E: B9 5B 31        LDA     $315B,Y             ; {hard.rom+115B} fetch this step's channel-1 volume from ROM

loc_3111:
3111: 8D 01 10        STA     $1001               ; write the channel-1 volume (or silence)
...
311F: 8D 02 10        STA     $1002               ; write it to the channel-2 frequency register
3122: B9 75 31        LDA     $3175,Y             ; {hard.rom+1175} fetch this step's channel-2 volume from ROM

loc_3125:
3125: 8D 03 10        STA     $1003               ; write the channel-2 volume (or silence)
...
313A: 8D 00 10        STA     $1000               ; write it to the channel-1 frequency register
...
loc_3145:
3145: 8D 01 10        STA     $1001               ; write the channel-1 priority volume
...
3360: AD 01 0C        LDA     $0C01               ; read input port IN1 for the per-column control bits
...
3386: AD 01 0C        LDA     $0C01               ; read input port IN1 again
...
387D: 8D 02 10        STA     $1002               ; write it to the POKEY channel-2 frequency register
3880: A9 AF           LDA     #$AF                ; control value 0xaf
3882: 8D 03 10        STA     $1003               ; write it to the POKEY channel-2 control register (sound the marching tone)

loc_3885:
3885: 2C 00 0C        BIT     $0C00               ; test the vblank/service input port
...
38BA: AD 03 0C        LDA     $0C03               ; read input port IN3
...
3909: 9D E0 07        STA     $07E0,X             ; copy it into the object's vertical sprite-shadow row
...
loc_391b:
391B: 9D D0 07        STA     $07D0,X             ; store the object's horizontal sprite-shadow coordinate
...
3923: AD 00 0C        LDA     $0C00               ; read input port IN0
...
3956: 9D C0 07        STA     $07C0,X             ; store the object's sprite-shadow picture code
...
3967: 9D F0 07        STA     $07F0,X             ; store the object's sprite-shadow attribute
...
396D: AD 00 0C        LDA     $0C00               ; read input port IN0
...
397A: 2C 00 0C        BIT     $0C00               ; test IN0's vblank bit
...
loc_3989:
3989: 9D 04 14        STA     $1404,X             ; write the ramp value into a diagnostic palette cell
...
399A: 9D 00 1C        STA     $1C00,X             ; mirror it to the output latch
...
loc_39b5:
39B5: BD 00 0C        LDA     $0C00,X             ; read this axis's raw trackball counter
...
39CE: 5D 00 0C        EOR     $0C00,X             ; check it against the raw counter's own sign
...
39E1: 8D 00 18        STA     $1800               ; write the interrupt-acknowledge port to clear the frame IRQ
...
readEaromCell:
3AA7: 9D 00 16        STA     $1600,X             ; latch cell address X into the EAROM
3AAA: A0 08           LDY     #$08                ; read mode with the clock held low
3AAC: 8C 80 16        STY     $1680               ; drive that onto the EAROM control register
3AAF: C8              INY                         ; step to the clock-high value
3AB0: 8C 80 16        STY     $1680               ; pulse the EAROM clock high
3AB3: 88              DEY                         ; step back to the clock-low value
3AB4: 8C 80 16        STY     $1680               ; the falling clock edge latches the cell out
3AB7: A0 00           LDY     #$00                ; control value to release the chip
3AB9: BD 00 17        LDA     $1700,X             ; read the addressed cell from the data-out window
3ABC: 8C 80 16        STY     $1680               ; release EAROM chip-select and clock
...
3AC6: 8D 80 16        STA     $1680               ; release the EAROM control lines before starting
...
3AD3: 8D 80 16        STA     $1680               ; arm the EAROM for a write
3AD6: A9 0A           LDA     #$0A                ; commit-strobe control value
3AD8: 8D 80 16        STA     $1680               ; strobe the freshly-erased cell to commit it
...
3AF3: 8D 80 16        STA     $1680               ; prime the EAROM control latch to accept a write
3AF6: BD 78 01        LDA     $0178,X             ; {hard.workRam+178} read the fresh byte from the RAM high-score mirror
3AF9: 9D 00 16        STA     $1600,X             ; latch it into the EAROM data window at this slot
3AFC: A9 0E           LDA     #$0E                ; load the EAROM write-commit pulse value
3AFE: 8D 80 16        STA     $1680               ; pulse the EAROM control latch to burn the byte in
...
3B0F: 9D 00 04        STA     $0400,X             ; wipe the matching video/object page-4 cell
3B12: 9D 00 05        STA     $0500,X             ; wipe the matching page-5 cell
3B15: 9D 00 06        STA     $0600,X             ; wipe the matching page-6 cell
3B18: 9D 00 07        STA     $0700,X             ; wipe the matching page-7 cell
3B1B: CA              DEX                         ; step to the previous cell
3B1C: D0 EC           BNE     $3B0A               ; {code.loc_3b0a} loop until every plane of work RAM is blank
3B1E: 8D 0F 10        STA     $100F               ; silence the POKEY serial/keyboard control latch
3B21: 8D 08 10        STA     $1008               ; clear the POKEY audio control latch so no sound leaks
3B24: 8D 00 24        STA     $2400               ; {hard.rom+400} dead store the board ignores -- kept so the mirror matches
3B27: 8D 07 1C        STA     $1C07               ; drop the flip-screen latch so the display orientation is defined
3B2A: AD 00 0C        LDA     $0C00               ; read the input port carrying the service switch
...
3B31: AD 00 08        LDA     $0800               ; normal boot -- read the option DIP bank
...
loc_3b4a:
3B4A: 8E 04 14        STX     $1404               ; self-test start -- clear the first playfield color cell
3B4D: 8E 01 10        STX     $1001               ; silence POKEY channel 1
3B50: 8E 03 10        STX     $1003               ; silence POKEY channel 2
3B53: 8E 05 10        STX     $1005               ; silence POKEY channel 3
3B56: 8E 07 10        STX     $1007               ; silence POKEY channel 4
3B59: E8              INX                         ; bump the color value to 1
3B5A: 8E 05 14        STX     $1405               ; paint color 1 into palette cell 05
3B5D: 8E 0D 14        STX     $140D               ; paint color 1 into palette cell 0d
3B60: E8              INX                         ; bump the color value to 2
3B61: 8E 06 14        STX     $1406               ; paint color 2 into palette cell 06
3B64: 8E 0E 14        STX     $140E               ; paint color 2 into palette cell 0e
3B67: E8              INX                         ; bump the color value to 3
3B68: 8E 07 14        STX     $1407               ; paint color 3 into palette cell 07
3B6B: 8E 0F 14        STX     $140F               ; paint color 3 into palette cell 0f
...
3B84: 8D 00 20        STA     $2000               ; {hard.rom} pet the watchdog so it cannot reset the board mid-test
...
3BA4: 8D 00 20        STA     $2000               ; {hard.rom} pet the watchdog between pages
...
3BD3: 8D 00 10        STA     $1000               ; set POKEY channel-1 frequency for the beep
3BD6: A2 03           LDX     #$03                ; load the serial/keyboard control arm value
3BD8: 8E 0F 10        STX     $100F               ; arm the POKEY control latch for the tone

loc_3bdb:
3BDB: A2 10           LDX     #$10                ; load the vblank-edge count for one half-beep
3BDD: A9 AF           LDA     #$AF                ; load the tone-on control/volume value
3BDF: 8D 01 10        STA     $1001               ; turn the beep tone on

loc_3be2:
3BE2: 2C 00 0C        BIT     $0C00               ; read the beam's vblank edge
3BE5: 50 FB           BVC     $3BE2               ; {code.loc_3be2} wait for the vblank edge to arrive

loc_3be7:
3BE7: 2C 00 0C        BIT     $0C00               ; read the beam's vblank edge again
3BEA: 70 FB           BVS     $3BE7               ; {code.loc_3be7} wait for the edge to clear
3BEC: 8D 00 20        STA     $2000               ; {hard.rom} pet the watchdog so the beep can play out
...
3BF2: 8E 01 10        STX     $1001               ; turn the beep tone off
3BF5: A2 10           LDX     #$10                ; load the vblank-edge count for the tone-off half

loc_3bf7:
3BF7: 2C 00 0C        BIT     $0C00               ; read the beam's vblank edge
3BFA: 50 FB           BVC     $3BF7               ; {code.loc_3bf7} wait for the vblank edge to arrive

loc_3bfc:
3BFC: 2C 00 0C        BIT     $0C00               ; read the beam's vblank edge again
3BFF: 70 FB           BVS     $3BFC               ; {code.loc_3bfc} wait for the edge to clear
3C01: 8D 00 20        STA     $2000               ; {hard.rom} pet the watchdog so the silent half can play out
...
loc_3c0a:
3C0A: 8D 00 20        STA     $2000               ; {hard.rom} pet the watchdog while waiting to halt
3C0D: AD 00 0C        LDA     $0C00               ; read the input port carrying the service switch
...
loc_3c16:
3C16: AD 01 0C        LDA     $0C01               ; marches passed -- read the coin/option input port
...
loc_3c53:
3C53: AD 00 0C        LDA     $0C00               ; read the input port carrying the service switch
...
3C5C: 8D 00 20        STA     $2000               ; {hard.rom} pet the watchdog each pass
3C5F: AD 01 0C        LDA     $0C01               ; read the control input port
...
loc_3c6b:
3C6B: 9D 00 04        STA     $0400,X             ; flood video page 4 with the response color
3C6E: 9D 00 05        STA     $0500,X             ; flood video page 5 with the response color
3C71: 9D 00 06        STA     $0600,X             ; flood video page 6 with the response color
3C74: E8              INX                         ; advance to the next cell
3C75: D0 F4           BNE     $3C6B               ; {code.loc_3c6b} flood the whole 256 cells

loc_3c77:
3C77: 9D 00 07        STA     $0700,X             ; flood this cell of page 7 with the response color
...
3C81: 8E 05 14        STX     $1405               ; brighten palette cell 05
3C84: A2 0F           LDX     #$0F                ; load another bright palette value
3C86: 8E 04 14        STX     $1404               ; brighten the first playfield color cell

loc_3c89:
3C89: AD 00 0C        LDA     $0C00               ; read IN0, where the operator service switch lives, to hold the self-test review screen
...
3C90: 8D 00 20        STA     $2000               ; {hard.rom} switch still held -- pet the watchdog so the review screen stays alive
...
3C9A: 9D 00 07        STA     $0700,X             ; paint an ascending tile-code ramp across the sprite/object page at 0x0700
3C9D: A9 00           LDA     #$00                ; load zero for the clears
3C9F: 95 00           STA     $00,X               ; {hard.workRam} clear this zero-page cell
3CA1: 9D 00 04        STA     $0400,X             ; clear the matching page-4 object cell
3CA4: 9D 00 05        STA     $0500,X             ; clear the matching page-5 object cell
3CA7: 9D 00 06        STA     $0600,X             ; clear the matching page-6 object cell
...
3CB2: 8D 03 1C        STA     $1C03               ; clear output latch 3
3CB5: 8D 04 1C        STA     $1C04               ; clear output latch 4
...
3CC4: AD 0A 10        LDA     $100A               ; read the POKEY random register
3CC7: 4D 0A 10        EOR     $100A               ; fold in a second read of the random register (the pair cancels at rest)
...
3CCE: 8D 0F 10        STA     $100F               ; arm the POKEY control latch (SKCTL) so sound can play
...
3CDF: 8E 00 20        STX     $2000               ; {hard.rom} pet the watchdog with the page counter while checksumming
...
3D5B: AD 00 0C        LDA     $0C00               ; read IN0
...
3D62: 8D 00 20        STA     $2000               ; {hard.rom} pet the watchdog
3D65: AD 01 0C        LDA     $0C01               ; read the control port IN1
...
3D7F: 9D 01 10        STA     $1001,X             ; silence the selected POKEY channel volume register
...
3D92: 8E 04 14        STX     $1404               ; write the cycling colour into palette cell 04
3D95: 86 E8           STX     $E8                 ; {hard.workRam+E8} store the palette counter e8

loc_3d97:
3D97: AD 01 0C        LDA     $0C01               ; read the control port IN1
...
3DB1: 99 04 14        STA     $1404,Y             ; write the colour ramp into palette cells 05..07
3DB4: 99 0C 14        STA     $140C,Y             ; write the same into palette cells 0d..0f
...
loc_3dbc:
3DBC: AD 01 0C        LDA     $0C01               ; read the control port IN1
...
3DD5: 8D 07 1C        STA     $1C07               ; drive the flip-screen latch to normal orientation
...
loc_3de1:
3DE1: AD 01 0C        LDA     $0C01               ; read the control port IN1
...
3DFF: 8D 07 1C        STA     $1C07               ; drive the flip-screen latch to flipped orientation
...
3E0A: AD 00 08        LDA     $0800               ; read the option DIP bank DSW1
...
3E30: AD 01 08        LDA     $0801               ; read DIP bank DSW2
...
3E40: AD 01 08        LDA     $0801               ; read DIP bank DSW2 again
...
3E5F: AD 01 08        LDA     $0801               ; read the DSW2 option bank
...
3E8A: 2C 00 08        BIT     $0800               ; test the DSW1 option bank
...
3ECB: AD 01 0C        LDA     $0C01               ; read input port IN1 again
...
3EDC: 99 00 10        STA     $1000,Y             ; set the POKEY tone frequency for the actuated input
3EDF: 8A              TXA                         ; reload the bit index
3EE0: 09 A0           ORA     #$A0                ; set the volume and distortion bits
3EE2: 99 01 10        STA     $1001,Y             ; sound the tone through the POKEY control register
...
3F0E: 99 00 04        STA     $0400,Y             ; write the on/off glyph into the switch-state row
...
; ---- $3FD9-$3FF5: data ----
3FD9: 22 24 24 25 23 20 50 4C 41 59 D3 20 47 41 4D 45
3FE9: 20 54 49 4D C5 48 41 52 C4 45 41 53 D9

spinToSelfHalt:
3FF6: 4C F6 3F        JMP     $3FF6               ; {code.spinToSelfHalt} spin here forever -- the terminal halt after a diagnostic failure

; ---- $3FF9-$3FFF: data ----
3FF9: 0A F6 3F 04 3B 71 38
```
