...
export function advanceSegmentColumns(m, x = m.regs.x) {
  ...
  for (;;) {
    ...
    // bit 5 for column 0.
    const in1 = mem8[loc_0c01];
    const bit = x === 0 ? 5 : x === 1 ? 6 : 7;
    const control = ((in1 >> bit) & 1) !== 0;
    const stepped = mem8[u8(loc_cf + x)] & 0x1f; // the column's low-5-bit step index
    ...
    if (!control) {
      ...
      if (stepped >= 0x1b) {
        ...
        if (sum <= 0xff) {
          ...
          mem8[u8(loc_cf + x)] = 0x1f;
          carry = mem8[u8(loc_cc + x)] !== 0;
          mem8[u8(loc_cc + x)] = 0x78;
        }
      }
    }
    ...
    if (carry) {
      ...
      mem8[loc_ca] = u8(delta + mem8[loc_ca] + 1);
      mem8[loc_c9] = u8(delta + mem8[loc_c9] + 1);
  ...
  const diff = u8(mem8[loc_ca] - mem8[u16(loc_3413 + y)]);
  if (diff & 0x80) return stepPhasedCountersAndWrapCells(m);
  mem8[loc_ca] = diff;
  mem8[loc_cb] = u8(mem8[loc_cb] + 1);
  if (y !== 0x03) return stepPhasedCountersAndWrapCells(m);
  mem8[loc_cb] = u8(mem8[loc_cb] + 1);
  if (mem8[loc_cb] === 0) {
    throw new Error(
      "advanceSegmentColumns: threshold tail fell into the loc_3413 DATA table -- a dead arm, trapped",
    );
  }
