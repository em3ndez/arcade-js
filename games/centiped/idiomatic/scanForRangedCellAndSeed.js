...
import { loc_00, SFX_TIMER_CH1_PRIORITY, loc_da, loc_db, loc_ef, loc_8b, loc_3f, loc_6f, loc_5f, loc_b2 } from "./names.js";
...
export function scanForRangedCellAndSeed(m) {
  ...
  for (;;) {
    if (mem8[loc_db] === 0) return; // high byte exhausted
    if (mem8[loc_db] === 0x07 && mem8[loc_da] >= 0xc0) { mem8[loc_db] = 0x00; return; } // top-of-page fold

    for (;;) {
      const cell = mem8[mem16[loc_da]] & 0x3f;
      if (cell >= 0x38 && cell < 0x3f) { seedRangedHit(m); return; } // in-range cell -> process one and stop

      mem8[loc_da] = u8(mem8[loc_da] + 1); // step to the next cell
      if (mem8[loc_da] !== 0) break; // still in-page -> re-check the boundary guard
      mem8[loc_db] = u8(mem8[loc_db] + 1);
      if (mem8[loc_db] === 0) return plotRecordFieldColumns(m); // pointer fully wrapped -> hand off to the table draw
    }
  }
}
...
function seedRangedHit(m) {
  const { mem8, mem16 } = m;
  mem8[mem16[loc_da]] = 0x3f ^ mem8[loc_ef]; // erase pattern folded with the mask
  ...
  mem8[loc_da] = u8(mem8[loc_da] + 1); // bump the pointer past the processed cell
  if (mem8[loc_da] === 0) mem8[loc_db] = u8(mem8[loc_db] + 1);
