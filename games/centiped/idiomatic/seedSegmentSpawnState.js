...
import {
  loc_88, loc_ab, loc_fd, loc_a9, loc_81, loc_51, loc_f0, loc_71, loc_61,
  loc_41, loc_a1, loc_b5, loc_100a,
} from "./names.js";
...
export function seedSegmentSpawnState(m) {
  ...
  if (m.mem8[(loc_ab + x) & 0xff] === 0) {
    const threshold = (m.mem8[loc_fd] & 0x40) | 0x10; // config-derived threshold
    if (threshold >= m.mem8[(loc_a9 + x) & 0xff]) sel = 0x01; // drop the selector to 1
  }
  m.mem8[loc_81] = sel; // stash the selector

  if (m.mem8[loc_100a] & 0x04) sel = (0x100 - sel) & 0xff; // random bit -> negate
