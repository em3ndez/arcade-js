...
// (loc_c9/loc_cb + loc_c8), the loc_d4 frame tick, and the every-other-frame modulo-0x10 wrap of
...
import { STACK_SCRATCH, loc_c5, loc_c8, loc_c9, loc_cb, loc_d3, loc_d4 } from "../names.js";
...
test("CRAFTED: every phase, accumulator path, and both sweep passes match the oracle", () => {
  const cases = [
    // phase 0: loc_c9 cleared, no loc_c8 bump; odd new-loc_d4 (0x02->0x03) so no sweep.
    ...
    // phase 3 borrow, high byte non-negative: store loc_cb, clear loc_c9, bump loc_c8.
...
test("CRAFTED: targeted cell values on representative paths", () => {
  ...
  assert.equal(c.mem.read8(loc_c9), 0x3f, "loc_c9 -= step(1)");
  assert.equal(c.mem.read8(loc_c8), 0x12, "loc_c8 += 2 for phase 1");
  assert.equal(c.mem.read8(loc_cb), 0x05, "loc_cb unchanged (no borrow)");
  ...
  assert.equal(d.mem.read8(loc_c9), 0x00, "loc_c9 unchanged (discard)");
  assert.equal(d.mem.read8(loc_cb), 0x01, "loc_cb unchanged (discard)");
