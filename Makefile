...
.PHONY: help test serve rom-dkong rom-pooyan rom-galaxian samples trace verify stepcheck pixel golden-mp4
...
help:
	...
	@echo "  make rom-galaxian assemble + sha256-verify the Galaxian ROM from your galaxian.zip"
...
# Galaxian (runbook-built) uses the generic assembler directly (no per-game Makefile).
rom-galaxian:
	node tools/build-rom.mjs galaxian
